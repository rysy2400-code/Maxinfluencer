...
回复要专业、清晰，用中文与客户沟通。`;
  ...
  normalizeHashtags(raw, fallback = "#campaign") {
    const src = (raw || "").trim();
...
只输出这一行标签字符串，不要任何解释。`;
      ...
      const fallback = hasPreviousScript && previousContentScript?.hashtags
        ...
        : [
            ...
            .filter(Boolean)
            .join(" ");
      return this.normalizeHashtags(fallback, "#campaign");
    }
  }
...
Region: ${regionText}
Current hashtags (MUST be used exactly in header Tags line): ${contentScript.hashtags || "#campaign"}
...
1) Header area:
   ...
   - Below that, a line labeled "Tags:" that MUST copy the hashtags VERBATIM from "Current hashtags" above.
     ...
     The "Tags:" line in your returned prompt must be exactly: "Tags: ${contentScript.hashtags || "#campaign"}".
...
**视频标签**: ${contentScript.hashtags || "#campaign"}
