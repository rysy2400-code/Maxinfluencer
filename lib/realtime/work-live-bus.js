...
let redisPublisherDisabled = false;
let redisPublisherErrorWarned = false;

function shouldDisableRedis(error) {
  const message = String(error?.message || error || "");
  return /max requests limit exceeded|ERR max requests limit exceeded/i.test(message);
}

function warnRedisPublisherOnce(message) {
  if (redisPublisherErrorWarned) return;
  redisPublisherErrorWarned = true;
  console.warn(`[work-live] Redis publish disabled: ${message}`);
}

function getRedis() {
  if (redisPublisherDisabled) return null;
  const url = process.env.REDIS_URL;
  if (!url) return null;
  if (!redisPublisher) {
    redisPublisher = new Redis(url, { maxRetriesPerRequest: 2 });
    redisPublisher.on("error", (e) => {
      if (shouldDisableRedis(e)) {
        redisPublisherDisabled = true;
        warnRedisPublisherOnce(e?.message || "redis publish quota exceeded");
      }
    });
  }
