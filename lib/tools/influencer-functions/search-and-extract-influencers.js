...
async function preGateTikTokByEmailAndFollowers(records, videos, options = {}) {
  ...
  const rawConcurrency = Number(process.env.TT_LITE_PREGATE_CONCURRENCY || endpoints.length);
  const concurrency = Math.max(
    1,
    Math.min(endpoints.length, Number.isFinite(rawConcurrency) ? Math.floor(rawConcurrency) : endpoints.length)
  );
  const taskId = Number(options.taskId || 0);

  // 每个端点一个会话（1 tap、1 并发），并发预过滤；每个会话各自保持 1-2s 间隔，
  // 避免把单 IP 请求频率压得太高（限流按调用频率算）。
  const sessions = [];
  for (const endpointKey of endpoints.slice(0, concurrency)) {
    try {
      sessions.push(await acquireTiktokApiSession(null, { endpointKey }));
    } catch (e) {
      console.warn(`[preGateTikTok] 会话失败 ${endpointKey}: ${e.message}`);
    }
  }
  if (!sessions.length) {
    console.warn("[preGateTikTok] 无法获取任何会话，跳过预过滤");
  ...
  try {
    await Promise.all(sessions.map((s) => runSession(s.page)));
