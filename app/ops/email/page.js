...
export default function EmailOpsPage() {
  ...
  return <main style={{ maxWidth: 1680, margin: "0 auto", padding: 20, color: C.text }}>
    ...
    {snapshot?.failedAgentEvents ? <section style={{ marginTop: 12, padding: 12, border: `1px solid ${Number(snapshot.failedAgentEvents.total24h) > 0 ? "#FECACA" : C.border}`, borderRadius: 6, background: Number(snapshot.failedAgentEvents.total24h) > 0 ? C.redBg : C.surface }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        <strong style={{ fontSize: 13 }}>红人 Agent 发信失败</strong>
        <span style={{ fontSize: 12, color: C.secondary }}>近 24h <strong style={{ color: Number(snapshot.failedAgentEvents.total24h) > 0 ? C.red : C.green }}>{snapshot.failedAgentEvents.total24h}</strong> · 近 7 天 {snapshot.failedAgentEvents.total7d}</span>
      </div>
      {Array.isArray(snapshot.failedAgentEvents.recent) && snapshot.failedAgentEvents.recent.length ? <div style={{ marginTop: 8, maxHeight: 220, overflowY: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead><tr style={{ textAlign: "left", color: C.secondary }}>{["事件", "Campaign", "红人", "错误", "时间"].map((label) => <th key={label} style={th}>{label}</th>)}</tr></thead>
          <tbody>{snapshot.failedAgentEvents.recent.map((item) => <tr key={item.id} style={{ borderTop: `1px solid ${C.border}` }}>
            <td style={td}>{item.event_type}</td>
            <td style={td}>{item.campaign_id || "-"}</td>
            <td style={td}>{item.influencer_id || "-"}</td>
            <td style={{ ...td, color: C.red }}>{item.error_message || "-"}</td>
            <td style={td}>{formatTime(item.updated_at)}</td>
          </tr>)}</tbody>
        </table>
      </div> : <div style={{ marginTop: 6, fontSize: 12, color: C.green }}>近 24 小时没有失败事件</div>}
    </section> : null}
