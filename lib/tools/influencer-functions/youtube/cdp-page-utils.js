...
import { isDisposableCdpTabUrl } from "../../../cdp/cdp-tab-utils.js";
...
export async function gotoYoutubeSearchUrl(page, url, opts = {}) {
  ...
  const tryGoto = async (targetPage) => {
    await targetPage.goto(url, { waitUntil: "domcontentloaded", timeout: 90000 });
    return targetPage;
  };

  try {
    return await tryGoto(page);
  } catch (e) {
    console.warn(`${logPrefix} goto 失败 (${e.message})，新开标签重试`);
    try {
      if (!page.isClosed()) {
        const u = page.url();
        if (
          isDisposableCdpTabUrl(u) ||
          isYoutubeSearchTabUrl(u) ||
          isYoutubeEnrichTabUrl(u)
        ) {
          await page.close();
        }
      }
    ...
    const fresh = await openCdpContextPage(context);
    await tryGoto(fresh);
    console.log(`${logPrefix} 新标签 goto 成功`);
    return fresh;
  }
}
