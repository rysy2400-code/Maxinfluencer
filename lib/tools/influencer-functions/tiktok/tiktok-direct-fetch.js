...
export function isVideoInfoDocumentAllowed() {
  if (process.env.TT_LITE_COUNTRY_VIDEO_INFO === "0") return false;
  if (process.env.TT_LITE_COUNTRY_VIDEO_INFO === "1") return true;
  return process.env.TT_LITE_COUNTRY_DISABLE_NAV !== "1";
}
...
/** html fetch 登录 stub 时允许 page.goto 视频页读 DOM（api-only 默认开启） */
export function isCountryStubDocumentFallbackAllowed() {
  if (process.env.TT_LITE_COUNTRY_STUB_DOCUMENT === "0") return false;
  if (process.env.TT_LITE_COUNTRY_STUB_DOCUMENT === "1") return true;
  return isCountryHtmlFirst();
}
