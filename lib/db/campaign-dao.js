...
export async function softDeleteCampaignBySessionId(
  ...
  const rows = await queryTikTok(
    `SELECT id, status FROM tiktok_campaign WHERE session_id = ? ORDER BY updated_at DESC LIMIT 1`,
    [sessionId]
  );
  ...
  if (campaign.status === "deleted") {
    return { success: true, campaignId: campaign.id, message: "campaign 已是删除状态" };
  }
  ...
  await queryTikTok(
    `UPDATE tiktok_campaign
     SET status = 'deleted',
         ...
         updated_at = NOW()
     WHERE id = ?`,
    [deletedBy, deleteReason, campaign.id]
  );

  return { success: true, campaignId: campaign.id, message: "campaign 已软删除" };
}
