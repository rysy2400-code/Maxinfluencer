...
import { resolveLatestDeliverablesFromRow } from "../execution/deliverables-resolution.js";
 ...
 * @param {object|null} [executionRow] 快照交付结果用（取 quote_negotiation 上最新的 deliverables）
 */
export function buildApprovedTermsSnapshot(
  chargeResolved,
  chargedAmount,
  executionRow = null
) {
  const n = Number(chargedAmount);
  const deliverables = resolveLatestDeliverablesFromRow(executionRow);
  return {
    ...
    // 品牌点「同意」那一刻的交付结果快照；合同/邮件优先读这里，避免生成时被后续沟通改写
      ...
      if (currentStage === STAGE_QUOTE_SUBMITTED) {
        ...
        const merged = {
          ...
          ...(isSystemSuggestedQuote
            ...
            : { quoteApprovedAt }),
          approvedTerms: buildApprovedTermsSnapshot(
            chargeResolved,
            ledgerDup[0].amount,
            executionRow
          ),
