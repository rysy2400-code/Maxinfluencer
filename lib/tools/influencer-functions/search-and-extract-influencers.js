...
import { isInstagramPlatform } from '../../influencer/resolve-campaign-platforms.js';
...
function mergeInfluencerData(searchRecord, profileData) {
  ...
  return {
    ...
    video_publish_country: searchRecord.video_publish_country || null,
...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  // 检查是否使用 headless 模式
  const USE_HEADLESS = process.env.PLAYWRIGHT_HEADLESS === 'true';
  const CDP_ENDPOINT = isInstagramEnrich
    ? process.env.CDP_ENDPOINT || 'http://127.0.0.1:9222'
    : process.env.CDP_ENDPOINT_ENRICH || process.env.CDP_ENDPOINT || 'http://127.0.0.1:9223';
  ...
  if (USE_HEADLESS) {
    ...
    if (isInstagramEnrich) {
      console.log(`[enrichInfluencerProfiles] 💡 提示: Instagram 主页提取需已登录，使用 9222`);
    } else {
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  console.log(
    `[searchAndExtractInfluencers] platform=${platformSlug} useInstagram=${useInstagram}`
  );
  ...
  try {
    ...
    const searchResult = useInstagram
    ...
    // 2.5 TikTok：视频发布地 + 国家过滤；Instagram MVP 跳过（国家展示为 -）
    if (!useInstagram && extractionResult.influencerRecords.length > 0) {
      sendStep(
        '采集视频发布地',
    ...
    if (enrichProfileData && recordsToEnrich.length > 0) {
      sendStep(
        ...
        useInstagram
          ...
          : `开始提取 ${recordsToEnrich.length} 位符合国家要求的红人主页（9223）...`
      );
      const enrichEndpoint = useInstagram
        ? process.env.CDP_ENDPOINT || "http://127.0.0.1:9222"
        : process.env.CDP_ENDPOINT_ENRICH || process.env.CDP_ENDPOINT || "http://127.0.0.1:9223";
      if (useInstagram) {
        console.log(
          `[searchAndExtractInfluencers] 💡 Instagram enrich 使用 9222（需已登录）: ${enrichEndpoint}`
        );
      } else {
        console.log(`[searchAndExtractInfluencers] 💡 提示: 主页提取将使用独立的 Chrome 实例（端口 9223），请确保已启动：`);
        console.log(`[searchAndExtractInfluencers]    bash scripts/launch-chrome-remote-debug-enrich.sh`);
      ...
      const enrichedRecords = await enrichInfluencerProfiles(
        recordsToEnrich,
        {
          ...
          campaignInfo,
          enableLiveMatch: !!influencerProfile,
          platform: useInstagram ? "instagram" : "tiktok",
          taskId: tid || null,
          campaignId,
          taskMeta,
        }
    ...
    const result = {
      ...
      influencers: finalInfluencerRecords.map(record => {
        ...
        return {
          ...
          displayName: record.displayName || record.username,
          profileUrl: record.profileUrl,
          platform: record.platform || (useInstagram ? 'Instagram' : 'TikTok'), // UI需要的 platform 字段
