...
function mergeInfluencerData(searchRecord, profileData) {
  ...
  return {
    ...
    userId: userInfo.userId || searchRecord.userId || null,
    tiktokUserId: userInfo.userId || searchRecord.userId || searchRecord.tiktokUserId || null,
...
function recordQualifiesForCandidatePool(inf) {
...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  const {
    ...
    enrichBatchStopOnZeroMatch = process.env.ENRICH_BATCH_STOP_ON_ZERO !== "false",
  ...
  async function ensureYtLitePagePool() {
    ...
    const poolSize = Math.max(1, effectiveConcurrency);
    ...
    if (ytLiteSession?.page && isYtLitePageAlive(ytLiteSession.page)) {
      ytLitePagePool.push(ytLiteSession);
      console.log("[enrichInfluencerProfiles] YouTube Lite: 复用搜索阶段 innertube 会话");
    } else if (ytLiteSession) {
      console.warn(
        "[enrichInfluencerProfiles] YouTube Lite: 搜索会话已失效，将新建 innertube 页"
      );
      ytLiteSession = null;
    }
    ...
    while (ytLitePagePool.length < poolSize) {
      try {
        const session = await acquireYoutubeInnertubeSession(context, {
          persistent: ytLitePagePool.length === 0,
        });
  ...
  async function ensureIgLitePagePool() {
    ...
    if (igLiteSession?.page && isIgLitePageAlive(igLiteSession.page)) {
      ...
      console.log("[enrichInfluencerProfiles] Instagram Lite: 复用搜索阶段 API 会话");
    ...
    while (igLitePagePool.length < poolSize) {
      try {
        const session = await acquireInstagramApiSession(context, {
          forceNewTab: igLitePagePool.length > 0,
          persistent: igLitePagePool.length === 0,
        });
        igLitePagePool.push(session);
        console.log(
          `[enrichInfluencerProfiles] Instagram Lite: 新建 API 页 ${igLitePagePool.length}/${poolSize}`
        );
      } catch (e) {
        console.warn(
  ...
  async function ensureTtLitePagePool() {
    ...
    while (ttLitePagePool.length < poolSize) {
      let session = null;
      let lastErr = null;
      for (const endpointKey of endpointCandidates) {
        try {
          ...
          console.log(
            `[enrichInfluencerProfiles] TikTok Lite: API 页 ${ttLitePagePool.length + 1}/${poolSize} via ${endpointKey}`
          );
          break;
        } catch (e) {
          lastErr = e;
      ...
      if (!session) {
        console.warn(
          `[enrichInfluencerProfiles] TikTok Lite: 创建 API 页失败（已有 ${ttLitePagePool.length}）: ${lastErr?.message || lastErr}`
        );
        break;
      }
  ...
  const processOneRecord = async (record, index, batchTotal) => {
    ...
    const budgetMs = resolveEnrichPerInfluencerBudgetMs();
    ...
    const run = async () => {
      ...
      const recordIsYouTubeNav =
        record.platform === "YouTube" || isYouTubeEnrich;
      const recordIsYouTube =
        record.platform === "YouTube" || isYouTubeEnrich;

      const recordIsInstagram =
        record.platform === "Instagram" || isInstagramEnrich;

      const recordIsTikTok =
        record.platform === "TikTok" || isTikTokEnrich;
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!igLitePage) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] Instagram Lite 国家预筛 @${record.username}...`
          );
          try {
            const igLiteModule = await import("./instagram/extract-instagram-profile-lite.js");
            const countryCtx = await igLiteModule.resolveInstagramLiteCountry(
              igLitePage,
              record.username,
              {
                userId: record.userId || null,
                allowAbout: allowedCountriesIso.length > 0,
              }
            );
            record.video_publish_country = countryCtx.videoPublishCountry || null;

            if (allowedCountriesIso.length > 0) {
              const pubIso = countryCtx.videoPublishCountry || null;
              const pass = pubIso && countryMatchesPublishLocation(pubIso, allowedCountriesIso);
              if (!pubIso) {
                sendStep(
                  "国家预筛",
                  `[${progress}] ⏭️ @${record.username} 未获取国家，跳过 Reels/主页 enrich`
                );
                profileData = {
                  success: true,
                  skippedReason: "country_unknown",
                ...
                sendStep(
                  "国家预筛",
                  `[${progress}] ⏭️ @${record.username} 国家 ${pubIso} 不符合 campaign（${allowedCountriesIso.join(",")}），跳过 Reels/主页 enrich`
                );
                profileData = {
                  success: true,
                ...
                sendStep(
                  "国家预筛",
                  `[${progress}] ✅ @${record.username} 国家 ${pubIso} 符合，开始 Reels enrich`
                );
        ...
        if (!ytLitePage) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] YouTube Lite 国家预筛 @${record.username}...`
          );
          try {
            const ytLiteModule = await import("./youtube/extract-youtube-channel-lite.js");
            const countryCtx = await ytLiteModule.resolveYoutubeLiteCountry(
              ytLitePage,
              record.username,
            ...
            if (allowedCountriesIso.length > 0) {
              ...
              if (!pubIso) {
                ...
                sendStep(
                  "国家预筛",
                  `[${progress}] ⏭️ @${record.username} 国家 ${pubIso} 不符合 campaign（${allowedCountriesIso.join(",")}），跳过视频 enrich`
                );
                profileData = {
                  success: true,
                  skippedReason: "country_mismatch",
                ...
                sendStep(
                  "国家预筛",
                  `[${progress}] ✅ @${record.username} 国家 ${pubIso} 符合，开始视频 enrich`
                );
      ...
      } else if (recordIsTikTok && liteMode && !recordIsInstagram && !recordIsYouTube) {
        ...
        if (!ttLitePage || !isTtLitePageAlive(ttLitePage)) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] TikTok Lite enrich @${record.username}（${ttLiteEnrichEndpoint || "9223"} signed API）...`
          );
          try {
      ...
      if (
        (isInstagramEnrich || isYouTubeEnrich) &&
        allowedCountriesIso.length > 0
      ) {
        const pubIso = mergedRecord.video_publish_country || null;
        const pass = countryMatchesPublishLocation(pubIso, allowedCountriesIso);
        mergedRecord.country_passed = pass && !!pubIso;
        if (!pubIso) {
          mergedRecord.enrich_skipped_reason = "country_unknown";
          skipLiveMatchForCountry = true;
          sendStep(
            "国家过滤",
            `[${progress}] ⏭️ @${record.username} 未获取账户国家，跳过画像分析与候选池`
          );
        } else if (!pass) {
        ...
        try {
          ...
          if (dbSaveResult.success) {
            ...
            if (enrichImportId) {
              ...
              try {
                await bumpSearchTaskEnrichedProgress(enrichTaskId, 1);
              } catch (bumpErr) {
                console.warn(
                  `[enrichInfluencerProfiles] bumpSearchTaskEnrichedProgress:`,
                  bumpErr?.message || bumpErr
                );
              }
            }
      ...
      if (shouldAnalyzeLive && mergedRecord.username && !skipLiveMatchForCountry) {
        try {
      ...
      if (perInfluencerAborted) {
        ...
        return {
          mergedRecord: mergeInfluencerData(record, failPd),
          logEntry: silentLog,
          success: false,
          dbSaveResult: null,
        };
...
async function searchAndExtractInfluencers_v2(params = {}, options = {}) {
  ...
  async function runConcurrencyChunk(chunk, startIndex, batchTotal, moreInPool) {
    ...
    if (processedCount % 5 === 0 && moreInPool) {
    ...
    if (processedCount % 10 === 0 && moreInPool) {
  ...
  if (enrichBatchPolicy) {
    ...
    while (poolCursor < recordsToEnrich.length) {
      ...
      for (let i = 0; i < batchRecords.length; i += effectiveConcurrency) {
        const chunk = batchRecords.slice(i, i + effectiveConcurrency);
        const moreInBatch = i + effectiveConcurrency < batchRecords.length;
        ...
        if (i + effectiveConcurrency < batchRecords.length && effectiveConcurrency > 1) {
    ...
    for (let i = 0; i < recordsToEnrich.length; i += effectiveConcurrency) {
      const chunk = recordsToEnrich.slice(i, i + effectiveConcurrency);
      await runConcurrencyChunk(
        chunk,
        i,
        recordsToEnrich.length,
        i + effectiveConcurrency < recordsToEnrich.length
      );
      if (i + effectiveConcurrency < recordsToEnrich.length && effectiveConcurrency > 1) {
        const delayBetweenInfluencers = liteMode ? 500 : 3000 + Math.floor(Math.random() * 5000);
        sendStep(
          "等待",
          `已处理 ${Math.min(i + effectiveConcurrency, recordsToEnrich.length)}/${recordsToEnrich.length} 个红人，等待 ${(delayBetweenInfluencers / 1000).toFixed(1)} 秒...`
        );
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  if (useInstagram && isLiteScraperMode() && !options.cdpContext) {
    return withCdp9222PreparedSession(
      { platform: "instagram", phase: "lite-pipeline" },
      async ({ context }) =>
        searchAndExtractInfluencers(params, { ...options, cdpContext: context })
    );
  }
  ...
  const countryBatchSize = Math.max(1, Number(process.env.COUNTRY_BATCH_SIZE || 20));
  console.log(
    `[searchAndExtractInfluencers] searchPoolMax=${searchPoolMax} enrichBatchPolicy=${enrichBatchPolicy}` +
      (enrichBatchPolicy
        ? `（每批${Math.max(1, Number(process.env.ENRICH_BATCH_SIZE || countryBatchSize))}人 enrich，LLM≥1符合继续；国家每批${countryBatchSize}人 0明确符合则停）`
  ...
  try {
    ...
    if (!useInstagram && !useYouTube && extractionResult.influencerRecords.length > 0) {
      ...
      countryFilterResult = await resolveVideoPublishCountryForInfluencers({
        influencerRecords: extractionResult.influencerRecords,
        videos: extractionResult.videos,
        maxCount: extractionResult.influencerRecords.length,
        allowedCountries,
        taskId: tid || null,
        onStepUpdate,
        delayBetweenVideos: {
