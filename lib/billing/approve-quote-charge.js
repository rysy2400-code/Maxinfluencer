...
import { resolveLatestInfluencerQuoteFromRow } from "../execution/quote-resolution.js";
...
export function resolveQuoteApproveCharge(campaign, executionRow) {
  const resolvedQuote = resolveLatestInfluencerQuoteFromRow(executionRow);
  const currency = resolvedQuote?.currency ?? executionRow?.currency ?? executionRow?.lastEvent?.currency;
  ...
  const flatFee = parseMoney(resolvedQuote?.amount);
  ...
  // 报价为空仅纯佣金模式允许（无固定费用），其余情况仍按无效报价拦截。
  if (flatFee === 0 || (isCommissionOnly && flatFee == null)) {
    return {
      ok: true,
      chargeAmount: 0,
      influencerAmount: 0,
...
export async function approveQuoteWithCharge(opts) {
  ...
  const nextStage = isSystemSuggestedQuote
    ...
    : needSample ? STAGE_PENDING_SHIPPING_ADDRESS : "pending_draft";
