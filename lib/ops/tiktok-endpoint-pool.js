...
export async function rebuildTiktokEndpointPool({ wanted = 3, verify = true } = {}) {
  ...
  for (const f of [BASE_CONFIG, ...pool.map(enrichConfigPath)]) {
    if (fs.existsSync(f)) fs.copyFileSync(f, `${f}.bak-${ts}`);
  }
  ...
  let details = null;
  if (verify) {
    details = { base: await probeProxyPort(BASE_PROXY_PORT), endpoints: [] };
    for (const f of enrichFiles) {
      details.endpoints.push({
        cdpPort: f.cdpPort,
        proxyPort: f.proxyPort,
        node: f.node,
        ...(await probeProxyPort(f.proxyPort)),
      });
    }
  }
...
export async function preflightTikTokEndpoints({
  ...
  if (anyFail && Date.now() - lastPreflightRebuildAt > rebuildCooldownMs) {
    lastPreflightRebuildAt = Date.now();
    try {
      log("preflight detected unhealthy endpoint; rebuilding pool");
      await rebuildTiktokEndpointPool({ verify: false });
    } catch (e) {
