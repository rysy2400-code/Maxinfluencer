...
export async function getAdvertiserUserById(id) {
  const rows = await queryTikTok(
    `SELECT u.id, u.advertiser_id, u.username, u.is_active, u.is_admin,
...
/**
 * 管理员切换账户：列出可切换的 active 用户（含管理员）
 * @param {{ q?: string, limit?: number, excludeUserId?: number | null }} options
 */
export async function listSwitchableAdvertiserUsers({ q = "", limit = 50, excludeUserId = null } = {}) {
  const safeLimit = Math.min(Math.max(parseInt(String(limit), 10) || 50, 1), 200);
  const term = typeof q === "string" ? q.trim() : "";
  let sql = `
    SELECT u.id, u.username, u.is_admin, a.id AS advertiser_id, a.name AS company_name
    FROM tiktok_advertiser_user u
    INNER JOIN tiktok_advertiser a ON a.id = u.advertiser_id
    WHERE u.is_active = 1
  `;
