...
6. 单位红人报价策略（必填，有默认值）：默认「按 eCPM=$3 给红人报价，单人最高 $1000」；用户也可选「无固定费用，仅佣金」或「纯产品置换（无固定费用 + 0% 佣金）」，或自定义 eCPM / 单人上限
...
6. influencerPricing（单位红人报价策略）：
   - mode: "ecpm_with_cap"（按 eCPM 报固定费+佣金，默认）或 "commission_only"（无固定费；佣金 >0 为仅佣金，佣金 0 为纯产品置换）
   ...
   用户未提及策略 → influencerPricing 各项可为 null（系统将用默认 eCPM=$3、上限 $1000）
...
只返回 JSON 格式，不要其他文字：
{
  ...
  "commission": 15 | null,
  "influencerPricing": {
    "mode": "ecpm_with_cap" | "commission_only" | null,
    "ecpmUsd": 3 | null,
