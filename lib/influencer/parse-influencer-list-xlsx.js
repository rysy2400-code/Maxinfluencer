import { loadXlsx } from "./load-xlsx.js";

function getXlsx() {
  const XLSX = loadXlsx();
  if (!XLSX?.read) {
    throw new Error("xlsx 模块加载失败");
  }
  return XLSX;
}
import { parseProfileUrl, normalizePlatformSlugInput } from "./parse-profile-url.js";

const URL_HEADER_HINTS = [
  "主页链接",
  "主页",
  "达人链接",
  "达人主页",
  "频道主页",
  "红人链接",
  "profile",
  "profileurl",
  "profile url",
  "link",
  "url",
  "tiktok主页链接",
  "频道链接",
  "channel",
];
const EMAIL_HEADER_HINTS = ["email", "邮箱", "e-mail", "mail"];
const PLATFORM_HEADER_HINTS = ["平台", "platform", "channel type"];

function normHeader(h) {
  return String(h || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "");
}

function findColumnIndex(headers, hints) {
  for (let i = 0; i < headers.length; i++) {
    const h = normHeader(headers[i]);
    if (!h) continue;
    for (const hint of hints) {
      const nh = normHeader(hint);
      if (h === nh || h.includes(nh)) return i;
    }
  }
  return -1;
}

/**
 * @param {Buffer} buffer
 * @param {{ maxRows?: number }} [options]
 * @returns {{
 *   rows: Array<{ profileUrl: string, email: string|null, platform: string|null, username: string, platformSlug: string }>,
 *   parseErrors: Array<{ row: number, reason: string }>,
 *   totalRawRows: number
 * }}
 */
export function parseInfluencerListXlsx(buffer, options = {}) {
  const maxRows = Math.min(
    200000,
    Math.max(1, Number(options.maxRows || 10000))
  );
  const XLSX = getXlsx();
  const wb = XLSX.read(buffer, { type: "buffer" });
  const sheetNames = Array.isArray(wb.SheetNames) ? wb.SheetNames : [];
  if (!sheetNames.length) {
    return { rows: [], parseErrors: [{ row: 0, reason: "Excel 无工作表" }], totalRawRows: 0 };
  }

  const rows = [];
  const parseErrors = [];
  const seenHandles = new Set();
  let totalRawRows = 0;
  let bestRows = [];

  // 多 sheet：逐个扫描，优先选择含主页链接列的 sheet 解析（说明/汇总 sheet 自动跳过）
  for (const sheetName of sheetNames) {
    const sheet = wb.Sheets[sheetName];
    if (!sheet) continue;
    let matrix;
    try {
      matrix = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
    } catch {
      continue;
    }
    if (!matrix.length) continue;
    totalRawRows += matrix.length;
    const headers = matrix[0];
    const urlCol = findColumnIndex(headers, URL_HEADER_HINTS);
    if (urlCol < 0) continue;
    const emailCol = findColumnIndex(headers, EMAIL_HEADER_HINTS);
    const platformCol = findColumnIndex(headers, PLATFORM_HEADER_HINTS);
    const sheetRows = [];
    const sheetErrors = [];
    const sheetSeen = new Set();

    for (let r = 1; r < matrix.length && sheetRows.length < maxRows; r++) {
      const line = matrix[r];
      if (!line || !line.length) continue;
      const rawUrl = String(line[urlCol] || "").trim();
      if (!rawUrl) continue;

      const parsed = parseProfileUrl(rawUrl);
      if (!parsed) {
        sheetErrors.push({ row: r + 1, reason: `无法解析主页链接: ${rawUrl.slice(0, 80)}` });
        continue;
      }

      const platformRaw = platformCol >= 0 ? String(line[platformCol] || "").trim() : "";
      const platformSlug = normalizePlatformSlugInput(platformRaw) || parsed.platformSlug;
      const platform =
        platformSlug === "instagram"
          ? "Instagram"
          : platformSlug === "youtube"
            ? "YouTube"
            : "TikTok";

      const emailRaw = emailCol >= 0 ? String(line[emailCol] || "").trim() : "";
      const email = emailRaw && emailRaw.includes("@") ? emailRaw.toLowerCase() : null;

      const key = `${platformSlug}:${parsed.username.toLowerCase()}`;
      if (sheetSeen.has(key) || seenHandles.has(key)) continue;
      sheetSeen.add(key);
      seenHandles.add(key);

      sheetRows.push({
        profileUrl: parsed.profileUrl,
        username: parsed.username,
        platformSlug,
        platform,
        email,
      });
    }
    // 保留解析出红人最多的 sheet 结果；其余 sheet 的错误仅作提示
    if (sheetRows.length > bestRows.length) {
      bestRows = sheetRows;
    }
    if (sheetErrors.length) parseErrors.push(...sheetErrors);
  }

  rows.push(...bestRows);
  if (!rows.length) {
    parseErrors.push({ row: 1, reason: "未找到主页链接列（需包含 link/url/主页链接 等表头）" });
  }

  return {
    rows,
    parseErrors,
    totalRawRows,
  };
}
