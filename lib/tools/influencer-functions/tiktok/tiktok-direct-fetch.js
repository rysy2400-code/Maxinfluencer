...
/**
 * TikTok-Api 风格 signed item_detail API（诊断脚本用，国家预筛不走此路径）
 */
export async function fetchLocationCreatedFromItemDetailApi(page, videoId, username) {
  ...
  for (const [url, params] of endpoints) {
    try {
      const json = await tiktokMakeRequest(page, url, params, {
        referer,
        retries: 3,
      });
      ...
      if (String(e.message || "").includes("preview=<!DOCTYPE")) {
...
/** page.goto 视频页：仅 TT_LITE_COUNTRY_VIDEO_INFO=1 时开启 */
export function isVideoInfoDocumentAllowed() {
  return process.env.TT_LITE_COUNTRY_VIDEO_INFO === "1";
}
...
/**
 * Lite 国家：video_html_fetch →（可选）page.goto DOM
 * goto 仅 TT_LITE_COUNTRY_VIDEO_INFO=1 或 stub+TT_LITE_COUNTRY_STUB_DOCUMENT=1
 * @param {object} page
...
export async function resolveVideoLocationCreated(page, opts = {}) {
  ...
  const stubDocAllowed =
    page?._ttCountryHtmlLoginStub === true &&
    isCountryStubDocumentFallbackAllowed();
...
/**
 * 国家预筛：search_api → 代表视频与搜索 alt 视频逐条 HTML/DOM 探测
 * @param {object} page
 * @param {{ videoId?: string, username?: string, altVideoIds?: string[], searchLocation?: string|null }} opts
 */
