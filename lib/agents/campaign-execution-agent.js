...
5. 向某位红人发起特殊请求（如延后发布时间），并回收红人反馈后同步给广告主
6. 查询 campaign 执行状态
7. 红人执行阶段操作：
...
3. 寒暄、问汇报选项说明书、与状态无关的闲聊 → needTool=false。
...
只返回 JSON，不要其他文字。`;
    ...
    try {
      const raw = await callDeepSeekLLM(
        ...
        "你是指令执行专家，只输出 JSON。用户要求暂停/恢复/完成 campaign 时必须 needTool=true 且使用 set_campaign_status。"
      );
