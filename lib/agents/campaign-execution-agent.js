...
/** 把 PDF 附件转成 ask_influencer_special_request 可用的 attachments 元数据。 */
function pdfAttachmentMetas(attachments) {
...
6. 若最后一条消息 @某位或多位红人 并**询问/协商/回复/同步消息**（如「帮我回复达 @xxx」「把这段话发给 @xxx」「问他能否接受某价格/做几条视频/改时间/给具体报价」「给 @a、@b、@c 分别发送寄样物流链接与内容建议」）→ 必须 needTool=true，toolName=ask_influencer_special_request。
   ...
   - **若用户要求把 PDF 资料（见【附件/名单上下文】中标注为 PDF 资料文件的附件）发给红人**：params 必须携带 attachments 数组（每项 { fileName, storageKey, contentType, sizeBytes }）；单一位红人填 params.attachments，多位红人时每位 request 元素里都填 attachments（同一份 PDF 多位红人共用时每位都带）；requestDetail 中说明「随信附上资料：文件名」，不要丢失文件名。
...
13. 红人名单导入：用户**上传 Excel/CSV 附件**、粘贴**红人主页链接**或 **@用户名 列表** → 属于名单导入场景（见第 14 条两轮确认；注意第 6 条的「回复达人」优先级更高）。**PDF 附件是资料文件，不是名单**，不能作为导入来源。
14. 名单导入两轮确认：
    ...
    - 若用户上传的是 **PDF 附件**并要求导入名单 → 不是有效名单来源，needTool=false，reply 提示「PDF 是资料文件，不能作为红人名单导入，请上传 Excel/CSV 名单或粘贴红人链接」。
...
export async function decideExecutionSchedulerTurn(
  ...
  if (decision.needTool && decision.toolName === "ask_influencer_special_request") {
    const pdfs = pdfAttachmentMetas(input.attachmentsForImport || []);
    if (pdfs.length) {
      if (Array.isArray(decision.params.requests) && decision.params.requests.length) {
        ...
        decision.params.attachments = pdfs;
      }
    }
  }
...
5. 向某位或多位红人发起特殊请求（如延后发布时间、改价、改条数、同步物流单号与内容建议、发送 PDF 资料），并回收红人反馈后同步给广告主；一条消息涉及多位红人时，为每位红人分别发起一次特殊请求（params.requests 数组）
6. 查询 campaign 执行状态
7. 红人执行阶段操作：
...
- 当用户**上传 PDF 资料并要求发给红人**时，同样必须调用 ask_influencer_special_request，并在 params（或 requests 数组中每位红人）携带 attachments：每项 { fileName, storageKey, contentType, sizeBytes }（取自附件上下文，**禁止编造 storageKey**）；requestDetail 中注明「随信附上资料：文件名」。若用户要求「导入名单」但附件是 PDF，则不能导入，应引导上传 Excel/CSV 或粘贴链接。
