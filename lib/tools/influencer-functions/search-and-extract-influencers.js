...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  sendStep(
    ...
    isYouTubeEnrich
      ...
      ? '正在通过 CDP 连接浏览器（TikTok Lite enrich，9222 登录态 API，无 @profile 导航）...'
    ...
    const run = async () => {
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!ttLitePage || !isTtLitePageAlive(ttLitePage)) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] TikTok Lite API enrich @${record.username}（${ttLiteEnrichEndpoint || "9222"}，无 @profile 导航）...`
          );
          try {
        ...
        useYouTube
          ...
          : useInstagram
            ...
            : useTikTok && isLiteScraperMode()
              ? `开始提取 ${recordsToEnrich.length} 位 TikTok 红人主页（9222 Lite API，无 @profile 导航）...`
