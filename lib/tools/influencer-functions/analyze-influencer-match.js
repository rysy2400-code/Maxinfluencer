...
export async function analyzeInfluencerMatch(influencer, influencerProfile, productInfo, campaignInfo, onStreamChunk = null) {
  try {
    ...
    const influencerSummary = {
      ...
      // 搜索视频列表：保留 ID/URL 与标题/时长/发布时间，单行展示压缩 prompt
      ...
      // 主页视频列表：截断到上限，保留 ID/URL 与标题/时长/发布时间
...
- **视频时长要求**: ${influencerProfile.videoDuration != null && String(influencerProfile.videoDuration).trim() !== "" ? String(influencerProfile.videoDuration).trim() : "未指定"}
...
${influencerSummary.searchVideoSamples.length > 0 
  ? influencerSummary.searchVideoSamples.map((v, i) => 
    `${i + 1}. ${v.videoId || '?'} | ${v.videoUrl || '无URL'} | ${v.title || '无标题'} | 时长: ${v.duration || '未知'} | 发布时间: ${v.publishedTimeText || '未知'}`
  ).join('\n')
  : '无搜索视频数据'
}
