...
export async function analyzeSampleWithLLM(payload, options = {}) {
  ...
  // 部分网关（如公网 DeepSeek）不支持关闭思考，模型会先在推理上消耗 token；
  // 额度不够时会返回空 content（finish_reason=length）。因此首次用基础额度，
  // 一旦命中空内容就自动提额重试。
  const baseMaxTokens = Number(
    options.maxTokens || process.env.COMMENT_ANALYSIS_MAX_TOKENS || 8192
  );
  let lastError = null;
  ...
  for (let i = 0; i < attempts; i += 1) {
    try {
      ...
      if (parsed && typeof parsed === "object" && hasUsableAnalysis(parsed)) return parsed;
      // 解析出来是空对象/缺关键字段时不能当成功，否则会写入一行全空的脏数据
      lastError = new Error("llm_shape_invalid");
    } catch (e) {
      lastError = e;
      if (e?.code === "DEEPSEEK_EMPTY_CONTENT") {
...
/**
 * LLM 输出必须至少含「高质量占比」或「摘要」之一，否则视为无效输出。
 * 防止 `{}` 这类空对象被当成功写入，导致主档出现全空的分析行。
 */
export function hasUsableAnalysis(obj) {
  if (!obj || typeof obj !== "object") return false;
  const ratio = Number(obj.qualityCommentRatio);
  const hasRatio = Number.isFinite(ratio);
  const hasSummary = typeof obj.summary === "string" && obj.summary.trim().length > 10;
  return hasRatio || hasSummary;
}
