...
import { promoteSessionToPublished } from "../db/campaign-session-dao.js";
...
工作流程：
1. 检查所有必要信息是否完整（产品信息、Campaign 信息、红人画像）；内容脚本为可选信息
2. 如果有缺失信息，提示用户补充
3. 如果信息完整，生成格式化的汇总报告
4. 与客户确认所有信息
5. 客户确认后，执行发布操作
...
汇总与确认时，投放平台一律展示规范全称：TikTok、Instagram、YouTube、X（多平台用顿号连接，如 YouTube、TikTok、X）。勿在汇总中使用 ytb/tk/ins/x 等缩写。
...
回复要专业、清晰，用中文与客户沟通。`;
  ...
  async confirmAndPublish(messages, context = {}) {
    try {
      ...
      // 2. 检测用户是否确认发布（「下一阶段」仅表示上一环节确认，不等同于发布）
      const isConfirming = await this.resolvePublishConfirmation(messages, context);
      const isPublished = context.published || false;
      const campaignId = context.campaignId || null;
      const sessionId =
        typeof context.sessionId === "string" ? context.sessionId.trim() : "";

      // 库内已有 campaign（权威）：避免重复 INSERT
      if (sessionId) {
        const existingInDb = await getCampaignBySessionId(sessionId);
        if (existingInDb?.id) {
          return {
            reply: `Campaign 已成功发布！\n\n**Campaign ID**: ${existingInDb.id}\n\n你可以使用此 ID 来查询和管理 Campaign。`,
            published: true,
            campaignId: existingInDb.id,
          };
        }
      }
  ...
  checkCompleteness(data) {
    ...
    if (!campaignInfo) {
      ...
      if (campaignInfo.budget === null || campaignInfo.budget === undefined) missing.push("总预算");
      if (campaignInfo.commission === null || campaignInfo.commission === undefined) missing.push("佣金");
      if (!isCampaignInfoComplete(campaignInfo)) {
        const pricing = normalizeInfluencerPricing(campaignInfo.influencerPricing);
        const pv = validateInfluencerPricing(pricing, campaignInfo.commission);
        if (!pv.isValid) missing.push("单位红人报价策略（与佣金组合无效）");
      }
    }
    ...
    // 内容脚本为可选项；内容 Agent 与已有 contentScript 数据仍然保留。
  ...
  /**
   * 规则优先判断是否为「确认发布」；否则再走 LLM。
   * 避免内容阶段点「下一阶段」链式进入本 agent 时被误判为发布。
   */
  async resolvePublishConfirmation(messages, context = {}) {
    const lastMessage = (messages[messages.length - 1]?.content || "").trim();
    const rule = this.classifyPublishConfirmation(lastMessage);
    if (rule !== null) return rule;
    return this.detectConfirmation(messages, context);
  }

  /**
   * @param {string} lastMessage
   * @returns {boolean|null} true=确认发布, false=非发布确认, null=交 LLM
   */
  classifyPublishConfirmation(lastMessage) {
    if (!lastMessage) return false;
    const publishKeywords =
      /确认发布|发布\s*campaign|确认提交|提交发布|发布吧|可以发布|好的[，,]?\s*发布|^发布$/i;
    if (publishKeywords.test(lastMessage)) return true;
    const stageAdvanceOnly =
      /^(下一阶段|下一步|确认无误|确认|继续|行|好的|没问题|可以|就这样|无误|ok)$/i.test(
        lastMessage
      ) ||
      (/下一阶段|下一步/.test(lastMessage) && !publishKeywords.test(lastMessage));
    if (stageAdvanceOnly) return false;
    return null;
  }
...
只返回 JSON 格式：
{
  ...
  formatSummary(data) {
    ...
    if (campaignInfo.platform) {
      const platforms = parseCampaignPlatforms(campaignInfo.platform).join("、");
      if (platforms) parts.push(`- 投放平台：${platforms}`);
    }
    ...
    if (campaignInfo.budget !== null && campaignInfo.budget !== undefined) {
      parts.push(`- 总预算：$${campaignInfo.budget.toLocaleString()} USD`);
    }
    if (campaignInfo.commission !== null && campaignInfo.commission !== undefined) {
      parts.push(`- 佣金：${campaignInfo.commission}%`);
    }
    parts.push(
      `- 单位红人报价策略：${formatInfluencerPricingLabel(campaignInfo.influencerPricing, campaignInfo.commission)}`
    );
    parts.push(
      `- 交付结果：${formatDeliverablesLabel(campaignInfo.deliverables) || formatDeliverablesLabel(getDefaultDeliverables())}`
    );
    parts.push("");
   ...
   * @param {Object} data - 所有收集的信息
   * @param {string} [sessionId] - 关联 tiktok_campaign_sessions.id（必填；缺则发布失败）
  ...
  async publishCampaign(data, sessionId = "") {
    try {
      ...
      if (!sid) {
        return {
          success: false,
          error: "缺少会话 sessionId，无法写入 tiktok_campaign；请从前端带会话发布",
        };
      }

      const existing = await getCampaignBySessionId(sid);
      // 产品链接查重：归一化（去 https://、www.、尾斜杠）后与其他 campaign 比较，
      // 发现重复直接阻止发布（排除本会话已有 campaign，允许同会话更新快照）。
      const productLink = data.productInfo?.productLink || "";
      const normalizedLink = normalizeProductLink(productLink);
      if (normalizedLink) {
        const dupes = await findCampaignsByNormalizedProductLink(normalizedLink, {
      ...
      const newId = `CAMP-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
      const { id: campaignId, reused } = await upsertCampaignForSession({
      ...
      if (reused) {
        console.log(
          `[CampaignPublishAgent] 会话已有 campaign，已更新快照而非新建: ${campaignId}`
        );
      }

      try {
        const needSample = resolveNeedSample(data.productInfo);
      ...
      const mark = await promoteSessionToPublished(sid, {
        campaignId,
        title: sessionTitle,
      });
      if (!mark.success) {
        console.warn(
          "[CampaignPublishAgent] Campaign 已写入但会话未标为 published:",
          mark.message
        );
      }
