    ...
    try {
      ...
      if (binResult.toolCall) {
        ...
        if (toolName === "product_info_agent") {
          ...
          // 发布成功：将会话标记为 published（左侧「已发布」列表依赖 DB status）
          // 注意：服务端不能使用相对路径 fetch('/api/...')，在 Node 中无合法 base，会静默失败
          try {
            if (publishResult.published && context.sessionId) {
              const updates = {
                status: "published",
                // 关键：把执行阶段所需字段持久化到 session.context，
                // 否则用户刷新/重新进入已发布 Campaign 时右侧仍会停留在发布阶段 UI。
                context: {
                  ...context,
                  published: true,
            ...
            } else if (publishResult.published && !context.sessionId) {
              console.warn(
                "[AgentRouter] 发布成功但请求未带 sessionId，无法将会话从草稿移到「已发布」；请检查前端 /api/chat 是否传递 sessionId"
              );
            }

            // 发布后立即触发一次执行心跳（不等待 15 分钟 cron）
            if (publishResult.published) {
              try {
