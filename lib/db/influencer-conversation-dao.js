...
import { syncEmailDeliveryFactForConversation } from "./email-delivery-fact-dao.js";
...
export async function logConversationMessage(opts = {}) {
  ...
  await queryTikTok(
    ...
    INSERT INTO tiktok_influencer_conversation_messages (
      ...
      ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
      /* sent_at：入参是 UTC naive 字符串，按会话时区换算后再落 TIMESTAMP，
         避免 mysql2 本地时区 + 服务器 +08:00 叠加导致 8 小时偏移 */
      CONVERT_TZ(?, '+00:00', @@session.time_zone),
      ?,
      CONVERT_TZ(?, '+00:00', @@session.time_zone),
      ?, ?, ?, ?, ?, ?
    )
  ...
  try {
    await syncEmailDeliveryFactForConversation({
      ...opts,
      influencerId,
      campaignId,
      direction,
    ...
    // 邮件运维事实层不能阻断业务邮件入库；迁移未执行时也保持主链路可用。
    console.error("[InfluencerConversationDAO] 同步邮件运维事实失败:", error?.message || error);
  }
}
