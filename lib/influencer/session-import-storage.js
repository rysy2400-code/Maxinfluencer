...
/** 通用命名空间校验：storageKey 是否属于该 session / campaign 的目录。 */
export function storageKeyBelongsToNamespace(storageKey, namespaceId) {
  if (!storageKey || !namespaceId) return false;
  const safeSession = String(namespaceId).replace(/[^a-zA-Z0-9_-]/g, "_");
  const key = String(storageKey);
  return key.startsWith(`${namespaceId}/`) || key.startsWith(`${safeSession}/`);
}

export function storageKeyBelongsToSession(storageKey, sessionId) {
  return storageKeyBelongsToNamespace(storageKey, sessionId);
}
...
/** 删除会话导入文件（仅允许删除 PENDING- 待确认附件；IMP- 正式文件保留供审计） */
export function removePendingImportFile(storageKey) {
  const key = String(storageKey || "");
  const base = key.split("/").pop() || "";
  if (!base.startsWith("PENDING-")) return false;
  if (key.includes("..")) return false;
  const abs = path.join(ROOT, key);
  const resolvedRoot = path.resolve(ROOT);
  const resolvedAbs = path.resolve(abs);
  if (!resolvedAbs.startsWith(resolvedRoot + path.sep)) return false;
  try {
