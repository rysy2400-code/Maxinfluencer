...
import { normalizeSessionMessagesForStorage } from "../lib/chat/session-messages.js";
...
async function main() {
  ...
  const removed = before.slice(start);
  const normalized = normalizeSessionMessagesForStorage(kept);

  console.log(`[CleanForeignMessages] ${row.title || sessionId}`);
  console.log(`  原 ${before.length} 条 → 保留 ${normalized.length} 条（删除 ${removed.length} 条）`);
  console.log(`  删除起点: 第 ${start + 1} 条`);
  console.log(`  首条删除: ${String(removed[0]?.content || "").slice(0, 80).replace(/\n/g, " ")}`);
