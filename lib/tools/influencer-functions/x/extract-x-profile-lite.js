...
async function fetchAboutAccountCountry(page, collector, handle) {
  try {
    ...
    // 只取 About 响应，不排空可能已到的 timeline（否则推文统计会被误丢）
    const batches = await collector.take("AboutAccountQuery", { timeoutMs: 7000 });
...
/** 从任意拦截批次里抽取 timeline 推文（首屏 UserOriginalsTimeline 等，含 views/likes/comments）。
 * 传入 handleLower 时只保留当前账号自己的推文：SPA 连续换号时队列里可能残留上一个
 * 账号的 timeline，必须过滤，避免播放/互动/样本统计串号。 */
const X_TIMELINE_QUERY_NAMES = new Set(["UserTweets", "UserOriginalsTimeline", "UserVideoTimeline"]);
function collectTimelineFromBatches(target, batches, handleLower = null) {
  for (const b of batches || []) {
    if (!X_TIMELINE_QUERY_NAMES.has(b.name)) continue;
    const tw = extractUserTweetsFromJson(b.json);
    const ownTweets = handleLower
      ? tw.filter(
          (t) =>
            String(t.author?.username || "")
              .replace(/^@/, "")
              .toLowerCase() === handleLower
        )
      : tw;
    if (!ownTweets.length) continue;
    for (const t of ownTweets) {
      if (!target.has(t.tweetId)) target.set(t.tweetId, t);
    }
  }
}
...
export async function extractXProfileLite(session, username, options = {}) {
  ...
  try {
    ...
    // X_LITE_FORCE_FULL_PAGE=1 时（历史数据批量巡检等一次性场景）强制每次整页加载，
    // 绕开连续 pushState 后 React 路由偶发不再触发 UserByScreenName 的问题。
    const forceFullPage =
      String(process.env.X_LITE_FORCE_FULL_PAGE ?? "0").trim() === "1";
    const spaOk = !forceFullPage && (await xSpaNavigate(page, profilePath));
    if (!spaOk) {
    ...
    // 校验 UserByScreenName 是否属于当前账号：SPA 连续跳转时拦截队列会混入
    // 上一个账号的残留响应，必须按 screen_name 精确匹配，避免串号（粉丝/邮箱/国家张冠李戴）
    const handleLower = String(handle).toLowerCase();
    const pickMatchingUbsn = (batchArr) => {
      const list = (batchArr || []).filter((b) => b.name === "UserByScreenName");
      return (
        list.find((b) => {
          const sn = String(
            b?.json?.data?.user?.result?.core?.screen_name ||
              b?.json?.data?.user?.result?.screen_name ||
              ""
          )
            .replace(/^@/, "")
            .toLowerCase();
          return sn === handleLower;
        }) || null
      );
    };

    // 等 UserByScreenName 响应；页面默认只发一次，必要时补一个 UI 等待。
    // 只 take 匹配项：队列里先到/后到的 timeline 不能被排空，否则推文互动会全部丢失。
    const earlyTweets = new Map();
    const collectOwnTimeline = (arr) =>
      collectTimelineFromBatches(earlyTweets, arr || collector.peek(), handleLower);
    let userJsonBatch = null;
    let didFullLoad = !spaOk;
    let batches = await collector.take("UserByScreenName", { timeoutMs: 30000 });
    collectOwnTimeline();
    userJsonBatch = pickMatchingUbsn(batches);
    if (!userJsonBatch) {
      await page.waitForTimeout(1500);
      collectOwnTimeline();
      batches = await collector.take("UserByScreenName", { timeoutMs: 8000 });
      userJsonBatch = pickMatchingUbsn(batches);
    }
    if (!userJsonBatch && spaOk && !didFullLoad) {
      // SPA 路由未触发本账号 UserByScreenName（含旧页残留）：整页回退一次
      console.warn(`[extractXProfileLite] @${handle} SPA 跳转未触发本账号响应，整页回退`);
      await page.goto(profileUrl, { waitUntil: "domcontentloaded", timeout: 60000 }).catch(() => {});
      didFullLoad = true;
      batches = await collector.take("UserByScreenName", { timeoutMs: 20000 });
      collectOwnTimeline();
      userJsonBatch = pickMatchingUbsn(batches);
    }
    ...
    if (isXLiteFetchTweetsEnabled() && userInfo.userId) {
      try {
        ...
        // 以主页加载时最早捕获的 timeline 为起点（take 不再排空），再补滚动/Media 增量；
        // 全程按 handle 过滤，旧账号残留 timeline 不参与统计。
        const tweetMap = new Map(earlyTweets);
        const ingest = (batches) => {
          collectTimelineFromBatches(tweetMap, batches, handleLower);
        };
        // 再消化主页加载/About 往返后新到的 timeline 响应
