...
export async function fetchAffiliateMetricsByUsername(context, username, options = {}) {
  ...
  try {
    ...
    const unitsSold = parseAuthorizedNumber(hit.units_sold);
    const creatorOecuid = unwrapAffiliateField(hit.creator_oecuid);

    const hasCommerceData = gmv != null || unitsSold != null;
...
export function applyAffiliateMetricsToRecord(mergedRecord, affiliate) {
  ...
  if (affiliate.gmv != null) {
    mergedRecord.gmv = affiliate.gmv;
  ...
  if (affiliate.gmvDisplay) {
    mergedRecord.gmvDisplay = affiliate.gmvDisplay;
  }
