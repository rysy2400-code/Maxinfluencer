        ...
        changes: {
          ...
          properties: {
            ...
            platform: {
              type: "string",
              description:
                "投放平台（须为规范全称）。单平台填 \"TikTok\" / \"Instagram\" / \"YouTube\"；多平台用 JSON 数组 [\"YouTube\",\"TikTok\",\"Instagram\"]。勿将 ytb、tk、ins 或「ytb和tk」等缩写/组合写进单个字符串（落库时会规范化，但请优先输出全称数组）。",
            },
            region: {
              type: "string",
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        contactMode: {
          type: "string",
          enum: ["default", "contact_all"],
          description:
            "default=只联系符合投放地区和画像且推荐的红人；contact_all=符合投放地区的红人一律联系（用户明确选择选项2时）",
        },
      },
...
async function askInfluencerSpecialRequest(params, context) {
  ...
  const handle = String(influencerId || "")
    .replace(/^@/, "")
    .trim();
  if (!handle) {
  ...
  const brandMessage =
    (typeof requestDetail === "string" && requestDetail.trim()) ||
  ...
  try {
    ...
    const { specialRequestId, eventId } = enqueued;
    return {
      success: true,
      data: {
        requestId: specialRequestId,
        specialRequestId,
        influencerAgentEventId: eventId,
        campaignId: cid,
        influencerId: handle,
        requestType: requestType || "other",
      ...
      message: `已向红人 @${handle} 发起询问：${brandMessage}\n\n红人回复后会自动同步给您。（specialRequestId: ${specialRequestId}）`,
    };
  } catch (err) {
    return {
      success: false,
...
async function importInfluencerList(params, context = {}) {
  ...
  const result = await executeInfluencerListImport({
    ...
    contactMode: params.contactMode || "default",
  });
  if (!result.success) {
