 ...
 * 支持缩写与组合表述，如 ytb、tk、ins、x/twitter、「ytb和tk」「ytb、tk和ins」。
 */
...
export function platformPayloadSlug(displayName) {
  ...
  if (n === "x" || /twitter/.test(n)) return "x";
  return "tiktok";
}

/** @param {string} slug tiktok | instagram | youtube | x */
...
/** 任务 payload / worker 认领用：统一为 tiktok | instagram | youtube | x */
export function normalizePlatformSlug(slug) {
  const s = String(slug || "").trim().toLowerCase();
  if (!s) return null;
  if (s === "instagram" || s === "ins" || s === "ig") return "instagram";
  if (s === "youtube" || s === "yt" || s === "ytb") return "youtube";
...
export function isXPlatform(platformOrSlug) {
  const s = String(platformOrSlug || "").trim().toLowerCase();
  return s === "x" || s === "twitter" || s === "tw";
}

export function isTikTokPlatform(platformOrSlug) {
  const s = String(platformOrSlug || "").trim().toLowerCase();
  return s === "tiktok" || s === "tt" || s === "tk";
}

/**
 * 单条搜索任务应走的采集链路（仅看任务平台，不看 campaign 全平台列表）。
 * @param {string} platformSlugOrDisplay
 */
export function resolveTaskSearchChannels(platformSlugOrDisplay) {
  const platformSlug =
    normalizePlatformSlug(platformSlugOrDisplay) ||
    platformPayloadSlug(platformSlugOrDisplay) ||
    "tiktok";
  const useInstagram = isInstagramPlatform(platformSlug);
  const useYouTube = isYouTubePlatform(platformSlug);
  const useX = isXPlatform(platformSlug);
  return {
    platformSlug,
    useInstagram,
    useYouTube,
    useX,
    useTikTok: !useInstagram && !useYouTube && !useX,
  };
}
