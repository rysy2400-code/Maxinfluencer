...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  /**
   * item_list 空/失败时：用全新 tab 替换池内 slot，并关闭旧 tab。
   * 轮换 IP 后旧 tab 网络栈可能已断，新 tab 保证后续红人不再踩卡死 tab。
   */
  async function recoverTtLitePoolSlot(slot) {
    if (!ttLitePagePool.length || slot < 0 || slot >= ttLitePagePool.length) {
      return null;
    }
    ...
    try {
      const fresh = await acquireTiktokApiSession(null, {
        endpointKey,
        forceNewTab: true,
      });
      ...
      console.warn(
        `[enrichInfluencerProfiles] TikTok Lite 新 tab 创建失败: ${e.message}`
      );
      return null;
    }
  }
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!ttLitePage || !isTtLitePageAlive(ttLitePage)) {
          ...
          try {
            ...
            // item_list 空：旧 tab 可能已因轮换/会话过期网络栈断裂，
            // 换全新 tab 重试一次；成功则替换池内 tab，后续红人不再踩卡死 tab。
            if (!profileData?.success && profileData?.error === "profile_videos_empty") {
              console.warn(
                `[enrichInfluencerProfiles] [${progress}] @${record.username} item_list 空，换新 tab 重试...`
              );
              const fresh = await recoverTtLitePoolSlot(index % Math.max(ttLitePagePool.length, 1));
              if (fresh) {
                try {
                  const retried = await ttLiteModule.extractTiktokProfileLite(
                    fresh.page,
                    record.username,
                    {
                      onStepUpdate: isLiteScreenshotsDisabled() ? null : onStepUpdate,
                  ...
                  if (retried?.success) {
                    ...
                    profileData = {
                      ...retried,
                      error: `item_list_retry_${retried.error}`,
                    };
                  }
                } catch (retryErr) {
                  console.warn(
                    `[enrichInfluencerProfiles] [${progress}] @${record.username} item_list 新tab重试失败: ${retryErr.message}`
                  );
                }
              }
            }
