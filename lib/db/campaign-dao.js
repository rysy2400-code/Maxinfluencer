...
/**
 * 按 tiktok_campaign.id 软删除（用于 session_id 曾错误写成 campaignId 等历史数据）
 */
export async function softDeleteCampaignById(
  campaignId,
  { deletedBy = "user", deleteReason = "用户删除已发布 campaign" } = {}
) {
  if (!campaignId) {
    return { success: false, message: "campaignId 不能为空" };
  }

  const rows = await queryTikTok(
    `SELECT id, status FROM tiktok_campaign WHERE id = ? LIMIT 1`,
    [campaignId]
  );
  if (!rows || rows.length === 0) {
    return { success: false, message: "未找到该 campaign" };
  }

  const row = rows[0];
  if (row.status === "deleted") {
    return { success: true, campaignId: row.id, message: "campaign 已是删除状态" };
  }
  ...
  await queryTikTok(
    `UPDATE tiktok_campaign
     SET status = 'deleted',
         ...
         updated_at = NOW()
     WHERE id = ?`,
    [deletedBy, deleteReason, row.id]
  );

  return { success: true, campaignId: row.id, message: "campaign 已软删除" };
}
