...
/**
 * campaign 候选池已有「平台 + handle」键（`platform|handle`，均小写）。
 *
 * 候选表的唯一键是 (campaign_id, platform, tiktok_username)：同一个 handle 在不同
 * 平台是两个候选，所以去重、跳过逻辑必须带平台，不能再只看 username。
 */
export async function loadCandidateHandleKeySet(campaignId) {
  if (!campaignId) return new Set();
  const rows = await queryTikTok(
    `
    SELECT LOWER(COALESCE(platform, '')) AS p, LOWER(tiktok_username) AS u
    FROM tiktok_campaign_influencer_candidates
    WHERE campaign_id = ?
  `,
    [campaignId]
...
/** 候选表去重键：platform|handle（小写）。 */
export function candidateHandleKey(platform, username) {
  // 平台缺失时与主档/候选表生成列的默认值保持一致（都是 tiktok），
  // 否则去重键会退化成空串，跳过逻辑失效。
  const p = normalizePlatformSlugInput(platform) || "tiktok";
...
export async function isCandidateInCampaign(campaignId, username, platform = null) {
  if (!campaignId || !username) return false;
  const u = String(username).replace(/^@/, "").trim().toLowerCase();
  if (!u) return false;
  const p = platform ? normalizePlatformSlugInput(platform) : null;
  const rows = await queryTikTok(
    p
      ? `
    SELECT 1 AS ok
    FROM tiktok_campaign_influencer_candidates
    WHERE campaign_id = ? AND LOWER(tiktok_username) = ? AND platform = ?
    LIMIT 1
  `
      : `
    SELECT 1 AS ok
...
export async function pickCandidatesForExecution(campaignId, limit) {
  ...
  return (rows || []).map((r) => ({
    ...
    platform: normalizePlatformSlugInput(r.platform || null),
...
export async function markCandidatePicked(
  campaignId,
  tiktokUsername,
  pickedAt,
  platform = null
  ...
  await queryTikTok(
    ...
    WHERE campaign_id = ? AND tiktok_username = ? AND platform = ?
  `
      : `
    UPDATE tiktok_campaign_influencer_candidates
    SET picked_at = ?, updated_at = CURRENT_TIMESTAMP
  ...
  `,
    p ? [pickedAt, campaignId, tiktokUsername, p] : [pickedAt, campaignId, tiktokUsername]
  );
}
...
/** 从可能的来源里取中位播放量（数值） */
function resolveMedianViews(inf = {}) {
  const candidates = [
    inf.medianViews,
    inf.median_views,
    inf.viewsData?.median,
    inf.views?.median,
    inf.profile_data?.statistics?.medianViews,
    inf.profileData?.statistics?.medianViews,
  ];
...
/** 从可能的来源里取点赞率 / 评论率（0~1 小数） */
function resolveEngagementRate(inf = {}, key) {
  const camel = key === "likeRate" ? "likeRate" : "commentRate";
  const snake = key === "likeRate" ? "like_rate" : "comment_rate";
  const candidates = [
    inf[camel],
    inf[snake],
    inf.engagement?.[camel],
    inf.engagement?.[snake],
    inf.profile_data?.statistics?.[camel],
    inf.profileData?.statistics?.[camel],
  ];
  for (const v of candidates) {
    const n = Number(v);
    if (v != null && v !== "" && Number.isFinite(n)) return n;
...
export function buildNormalizedInfluencerSnapshot(inf = {}, taskMeta = {}) {
  ...
  return {
    ...
    // 中位播放 + 点赞率/评论率（0~1 小数）：卡片「播放(中位数) / 点赞率 / 评论率」直接读这几个键
    medianViews: resolveMedianViews(inf),
    likeRate: resolveEngagementRate(inf, "likeRate"),
    commentRate: resolveEngagementRate(inf, "commentRate"),
    bio:
      inf.bio ??
      inf.profile_data?.userInfo?.bio ??
      inf.profileData?.userInfo?.bio ??
    ...
    accountCountry:
      inf.video_publish_country ??
      inf.videoPublishCountry ??
      inf.account_country ??
      inf.accountCountry ??
      null,
    account_country:
      ...
      null,
    // 居住国家：红人在邮件里本人确认的常住地
    residenceCountry: inf.residence_country ?? inf.residenceCountry ?? null,
    residence_country: inf.residence_country ?? inf.residenceCountry ?? null,
    // 展示/导出用「生效国家」：账号国家优先，居住国家兜底
    videoPublishCountry:
      inf.video_publish_country ??
      inf.videoPublishCountry ??
      inf.account_country ??
      inf.accountCountry ??
      inf.residence_country ??
      inf.residenceCountry ??
      null,
    videoPublishCountrySource:
      inf.video_publish_country_source ?? inf.videoPublishCountrySource ?? null,
    // 红人主页简介语言（卡片展示；发信语言的兜底依据）
    bioLanguage: inf.bioLanguage ?? inf.bio_language ?? null,
    bioLanguageConfidence: inf.bioLanguageConfidence ?? inf.bio_language_confidence ?? null,
    bioLanguageSource: inf.bioLanguageSource ?? inf.bio_language_source ?? null,
    gmv,
    gmvDisplay,
    gmvCurrency: "USD",
    gmvPeriodDays: 30,
    ...
    excluded: inf.doNotContact === true ? true : null,
    exclusionSource: inf.exclusionSource ?? null,
  };
}
...
export async function upsertCandidatesForCampaign(campaignId, influencers, taskMeta = {}) {
  ...
  // 去重键 = platform|handle：同 handle 跨平台是两个候选，不能互相跳过
  ...
  for (const inf of influencers) {
    ...
    const infPlatformSlug = normalizePlatformSlugInput(
      inf?.platform || inf?.platformSlug
    );
    if (isHandleExcluded(exclusionMaps, infPlatformSlug, tiktokUsername)) {
      // 命中不联系名单：无论搜索/导入如何推荐，都强制不联系
      inf.isRecommended = false;
      inf.doNotContact = true;
      inf.exclusionSource = "global_contact_exclusion";
    }
    if (inf?.isRecommended !== true) continue;
    const handleKey = candidateHandleKey(infPlatformSlug, tiktokUsername);
    if (handleKey && existingHandles.has(handleKey)) continue;
  ...
  for (const inf of influencers) {
    ...
    const isExcluded = isHandleExcluded(exclusionMaps, infPlatformSlug, tiktokUsername);
    if (isExcluded) {
      inf.isRecommended = false;
      inf.doNotContact = true;
      inf.exclusionSource = inf.exclusionSource || "global_contact_exclusion";
    }
    const infForSnapshot = applyGlobalEmailFallback(
      inf,
      tiktokUsername,
    ...
    const shouldContact =
      infForSnapshot.doNotContact === true
        ? 0
        : typeof infForSnapshot.isRecommended === "boolean"
          ? infForSnapshot.isRecommended
            ? 1
            : 0
          : 0;
    const doNotContact = infForSnapshot.doNotContact === true ? 1 : 0;
...
/** 任务收尾：一次性写入最终指标（worker-influencer-search 收尾调用，绝对覆盖非累加） */
export async function setSearchTaskFinalMetrics(taskId, metrics = {}) {
  const id = Number(taskId || 0);
  if (!id) return;
  const m = {
    searchFoundCount: Number(metrics.searchFoundCount || 0),
    profileBrowsedCount: Number(metrics.profileBrowsedCount || 0),
    analyzedCount: Number(metrics.analyzedCount || 0),
    recommendedCount: Number(metrics.recommendedCount || 0),
    contactableCount: Number(metrics.contactableCount || 0),
    skipCountryUnknownCount: Number(metrics.skipCountryUnknownCount || 0),
    skipCountryMismatchCount: Number(metrics.skipCountryMismatchCount || 0),
    newRecommendedInsertCount: Number(metrics.newRecommendedInsertCount || 0),
  };
  await queryTikTok(
    `
    UPDATE tiktok_influencer_search_task
  ...
  `,
    [
      m.searchFoundCount,
      m.profileBrowsedCount,
      m.analyzedCount,
      m.recommendedCount,
      m.contactableCount,
      m.skipCountryUnknownCount,
      m.skipCountryMismatchCount,
      m.newRecommendedInsertCount,
      id,
    ]
  );
}
