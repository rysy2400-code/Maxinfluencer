...
- upstreamProfileDelta (if present) lists facts the upstream step already extracted from this same email. Treat it as a hint only: the email text stays authoritative, and never invent facts it does not contain.
- Return JSON only.
  ...
  const raw = await callDeepSeekLLM(
    [{
      ...
      content: JSON.stringify({
        ...
        upstreamProfileDelta: profileDelta || null,
      }),
    }],
    systemPrompt,
