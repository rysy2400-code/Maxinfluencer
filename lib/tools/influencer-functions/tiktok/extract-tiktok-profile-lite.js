...
export async function extractTiktokProfileLite(page, username, options = {}) {
  ...
  // 导入任务由调用方显式关闭邮箱门禁（bio 门禁通过即拉视频+LLM，邮箱只用于联系判断）；
  // 搜索任务沿用 env（TT_LITE_REQUIRE_EMAIL_FOR_ANALYSIS）默认开关。
  const emailGateEnabled =
    options.emailGateEnabled ?? isTiktokLiteEmailGateEnabled();
  if (emailGateEnabled && !String(userInfo.email || "").trim()) {
