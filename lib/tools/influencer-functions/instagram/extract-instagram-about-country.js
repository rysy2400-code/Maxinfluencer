...
export function parseWbloksAboutCountry(text) {
  ...
  try {
    ...
    const readCountry = (item) => {
      ...
      if (key !== ABOUT_COUNTRY_BLOKS_KEY && !key.startsWith(`${ABOUT_COUNTRY_BLOKS_KEY}:`)) {
        return null;
      }
      const v =
        item?.data?.initial ??
    ...
    if (Array.isArray(arr)) {
      for (const item of arr) {
        const country = readCountry(item);
        if (country) return country;
      }
    }
    let found = null;
    const walk = (obj, depth = 0) => {
      if (found || depth > 18 || !obj || typeof obj !== "object") return;
      if (Array.isArray(obj)) {
        for (const x of obj) walk(x, depth + 1);
        return;
      }
      found = readCountry(obj);
      if (found) return;
