...
- 重点指标：${JSON.stringify(reportConfig?.includeMetrics || [])}`;
    ...
    if (shouldExtractImport) {
      try {
        ...
        if (importTurn.phase === "status_tool" && importTurn.statusAction) {
          return {
            reply: importTurn.reply || "正在更新 Campaign 运行状态…",
            toolCall: {
              toolName: "set_campaign_status",
              params: {
                campaignId,
                action: importTurn.statusAction,
              },
            },
          };
        }

        if (importTurn.phase === "defer_tool_router") {
          if (importTurn.reply) {
            return { reply: importTurn.reply, toolCall: null };
          }
          // 无 reply 时交给下方工具调度 prompt
        } else if (importTurn.phase === "confirm") {
          const warn =
...
8. 寒暄、问汇报选项说明书、问「有哪些 Campaign 状态可以修改」等说明类问题 → needTool=false，但**说明当前状态时必须引用【当前 Campaign 数据库状态】**，不得声称已是名单模式除非 status=running_passive。
9. **禁止** needTool=false 时声称已向红人发送询问、已收到红人反馈，或声称已修改执行节奏/汇报/Campaign 配置/寻源模式。
