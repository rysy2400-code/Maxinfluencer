...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  // Lite：9222 UNIVERSAL 读 locationCreated（HTML fetch 优先，SSR 导航回退）；9223 仅 enrich
  if (isLiteScraperMode()) {
    ...
    const concurrency = Math.max(
      1,
      Number(process.env.TT_LITE_COUNTRY_CONCURRENCY || 1)
    );
    ...
    sendStep(
      "采集视频发布地",
      `Lite：9222 UNIVERSAL 读发布地（HTML fetch → SSR 回退，每批 ${countryBatchSize} 人，0 明确符合国家则停）...`
    );
