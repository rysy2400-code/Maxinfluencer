  ...
  const [advRows, aggRows] = await Promise.all([
    ...
    queryTikTok(
      ...
      SELECT
        ...
        COALESCE(SUM(CASE WHEN type = 'quote_approve' THEN ABS(influencer_amount) ELSE 0 END), 0) AS total_influencer_spend,
        COALESCE(SUM(CASE WHEN type = 'quote_approve' THEN ABS(platform_fee_amount) ELSE 0 END), 0) AS total_platform_fee,
        COALESCE(SUM(CASE WHEN type = 'quote_approve' THEN ABS(amount) ELSE 0 END), 0) AS total_consumption
  ...
  const [countRows, rows] = await Promise.all([
    ...
    queryTikTok(
      ...
      SELECT id, amount, balance_after, currency, type, campaign_id, influencer_id,
             influencer_amount, platform_fee_amount, campaign_name, influencer_display_name,
             note, created_at
      FROM tiktok_advertiser_balance_ledger
      WHERE ${whereSql}
