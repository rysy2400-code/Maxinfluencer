...
} from "../influencer/extract-keyword-signals.js";
...
export async function ingestKeywordSignalsFromRecommendedInfluencer(
  ...
  let ingested = 0;
  const signalFilter = filterKeywordSignalsForSearch([
    ...hashtags.map((tag) => ({
      signal_type: "hashtag",
      signal_value: tag,
    })),
    ...mentions.map((mention) => ({
      signal_type: "mention",
      signal_value: mention,
    })),
  ]);

  for (const signal of signalFilter.kept) {
  ...
  return { ingested, filtered: signalFilter.dropped.length };
}
