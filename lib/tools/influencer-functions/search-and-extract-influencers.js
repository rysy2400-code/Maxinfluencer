...
async function preGateTikTokByEmailAndFollowers(records, videos, options = {}) {
  ...
  const { extractUserInfoFromUserDetailAPI } = await import(
    "./tiktok/extract-tiktok-profile-lite.js"
  );
  const endpoints = String(
    process.env.TT_LITE_ENRICH_CDP_ENDPOINTS ||
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  const endpoint = endpoints[0] || "http://127.0.0.1:9223";
  ...
  try {
    ...
    console.warn(`[preGateTikTok] 无法获取会话，跳过预过滤: ${e.message}`);
  ...
  try {
    ...
    for (let i = 0; i < records.length; i += 1) {
      ...
      try {
        ...
        console.warn(`[preGateTikTok] user/detail @${rec.username} 失败: ${e.message}`);
      }
      ...
      if (skipReason) {
        ...
        if (videos && Array.isArray(videos)) keptVideos.push(videos[i]);
      }
