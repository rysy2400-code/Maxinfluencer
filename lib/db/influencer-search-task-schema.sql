...
CREATE TABLE IF NOT EXISTS tiktok_influencer_search_task (
  ...
  progress_analyzed_count INT NOT NULL DEFAULT 0 COMMENT 'LLM 分析完成并写入候选池',
  progress_recommended_count INT NOT NULL DEFAULT 0 COMMENT '符合红人画像（isRecommended=true）',
...
CREATE TABLE IF NOT EXISTS tiktok_keyword_run_result (
  ...
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uk_campaign_run_keyword_platform (campaign_id, run_id, keyword, platform),
  INDEX idx_campaign_run_keyword (campaign_id, run_id, keyword, platform),
