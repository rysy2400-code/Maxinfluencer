...
} from "../influencer/import-intent-extractor.js";
...
8. 红人名单导入：用户 📎 上传附件或粘贴链接/@用户名 → import_influencer_list（分析后按心跳联系；不得跳过分析直接发邮件）
...
- 若用户要暂停但当前已是 paused，或要恢复但当前已是 running：仍调用 set_campaign_status 对应 action（工具会返回幂等说明）。`;
    ...
    const shouldExtractImport =
      ...
      (hasFileAttachments || shouldRunImportIntentExtractor(lastMessage));

    if (shouldExtractImport) {
      try {
        const extraction = await extractImportIntent({
        ...
        if (extraction.needsConfirmation) {
          const warn =
            Array.isArray(extraction.warnings) && extraction.warnings.length
              ...
              : "";
          return {
            reply: `${extraction.confirmationQuestion || "请确认您的意图。"}${warn}`,
            toolCall: null,
          };
        }

        if (extraction.intent === "import_and_contact") {
          let attachmentPlan = extraction.attachmentPlan || null;
          const fileAtt = lastAttachments.filter((a) => a?.storageKey);
          if (fileAtt.length === 1) {
            attachmentPlan = {
              ...(attachmentPlan || {}),
              storageKey: fileAtt[0].storageKey,
              fileName: fileAtt[0].name || attachmentPlan?.fileName,
            };
          } else if (fileAtt.length > 1 && attachmentPlan?.storageKey) {
            const matched = fileAtt.find((a) => a.storageKey === attachmentPlan.storageKey);
            if (matched) {
              attachmentPlan.fileName = matched.name || attachmentPlan.fileName;
            }
          }
          return {
            reply: "正在处理红人名单…",
            toolCall: {
              ...
              params: {
                ...
                userMessage: lastMessage,
                attachmentPlan,
                textItems: extraction.textItems || [],
              },
            },
          };
        }

        if (extraction.intent === "chat_only" && hasFileAttachments) {
        ...
        console.warn(
          "[CampaignExecutionAgent] 导入意图分析失败，回退常规模型:",
