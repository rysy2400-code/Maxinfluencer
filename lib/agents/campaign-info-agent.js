...
function normalizePlatformList(platform) {
...
function isValidPlatform(platform) {
  ...
  if (Array.isArray(platform)) {
    const items = platform.filter((p) => p != null && String(p).trim());
    return items.every((p) => parseCampaignPlatforms(p).length > 0);
  }
  const raw = String(platform).trim();
  if (!raw) return false;
  const lower = raw.toLowerCase();
  if (parseCampaignPlatforms(lower).length > 0) return true;
  return parseCampaignPlatforms(raw).length > 0;
}
...
Campaign 基本信息包括：
1. 投放平台（必填）：TikTok、Instagram、YouTube 或 X（Twitter）（可单选或多选）。客户口语中可能出现 ytb/tk/ins/x/twitter 等缩写，你理解即可；写入与确认时一律使用规范全称
2. 投放地区（必填）：TikTok、Instagram、YouTube、X 支持的国家/地区，可单选或多选（如美国、德国、法国、英国、日本等；也接受 ISO 两位代码如 US、DE、FR）
3. 发布时间段（必填）：campaign 的发布时间范围，例如 "2024年3月1日-3月31日"
4. 总预算（必填）：整个 campaign 的总预算金额，单位：美元（USD）
5. 佣金（必填）：给红人的佣金比例，单位：百分比（%）
6. 单位红人报价策略（必填，有默认值）：默认「不主动报价，询问红人合作价格」；用户也可选「按 eCPM=$3 给红人报价，单人最高 $1000」（可自定义 eCPM / 单人上限）、「无固定费用，仅佣金」或「纯产品置换（无固定费用 + 0% 佣金）」
7. 交付结果（有默认值，独立于内容脚本）：默认「1条专属视频；发布前需分享草稿供确认；个人简介链接（保留14天）；广告投放代码（有效期3个月）；原始素材授权（有效期3个月）」；用户可自定义条数、时长等

工作流程：
1. 主动询问客户以上信息（如果缺少任何一项）
2. 从客户的消息中提取已提供的信息
3. 验证信息的有效性：
   - 平台规范名称为 "TikTok"、"Instagram"、"YouTube"、"X"（多平台用数组，如 ["YouTube","TikTok"]；X 即 Twitter，规范全称用 "X"）
   - 地区须为可识别的国家/地区（中文常用名、英文国名或 ISO 两位代码均可）
   - 总预算必须是正数
   - 佣金必须是 0-100 之间的数字
   - 若策略为「无固定费用，仅佣金」，佣金可为 0–100%；佣金为 0% 时代表纯产品置换合作（送产品置换内容，无现金报酬）
   - 若策略为按 eCPM 报价，eCPM 与单人上限须为正数
   - 默认策略「不主动报价，询问红人合作价格」对佣金无额外要求，佣金为 0% 也合法
4. 整理已收集的信息，与客户确认
5. 如果信息不完整，继续询问缺失的信息
...
回复要专业、清晰，用中文与客户沟通。`;
   ...
   */
  async collectCampaignInfo(messages, context = {}, options = {}) {
    try {
...
需要提取的信息：
1. platform（平台）：必须是 "TikTok"、"Instagram"、"YouTube" 或 "X" 中的一个或多个，用数组表示，例如 ["TikTok"] 或 ["TikTok", "X", "YouTube"]
2. region（地区）：国家/地区的一个或多个，用数组表示，例如 ["美国"]、["法国", "德国"] 或 ["US", "JP"]；使用规范中文国名优先（如「法国」而非「法兰西」）
3. publishTimeRange（发布时间段）：字符串，例如 "2024年3月1日-3月31日"
4. budget（总预算）：数字，单位：美元
5. commission（佣金）：数字，单位：百分比（0-100）
6. influencerPricing（单位红人报价策略）：
   - mode: "ask_creator_quote"（不主动报价，询问红人提供合作价格，默认）或 "ecpm_with_cap"（按 eCPM 报固定费）或 "commission_only"（无固定费；佣金 >0 为仅佣金，佣金 0 为纯产品置换）
   - ecpmUsd: 数字，美元，仅 mode=ecpm_with_cap 时有效；用户说「ecpm 5」则填 5
   - maxFlatFeeUsd: 数字，美元，单人固定费上限；用户说「最高 500」则填 500
   用户说「不主动报价」「不报价」「询问红人报价/价格」「让红人/博主报价」「请红人/博主提供报价」「不要按 eCPM 报价」「由红人报价」「红人自己报价」→ mode=ask_creator_quote（可带 ecpmUsd/maxFlatFeeUsd 为 null）
   用户说「只要佣金」「纯 CPS」「无固定费用」→ mode=commission_only
   用户说「纯产品置换」「产品置换」「置换」→ mode=commission_only 且 commission=0（即使未单独写 0%，纯产品置换即无现金报酬）
   用户明确说 0% 或「无佣金」「不付佣金」时填 commission=0
   用户未提及策略 → influencerPricing 各项可为 null（系统将用默认「不主动报价，询问红人合作价格」）
7. deliverables（交付结果）：**必须是字符串**（禁止 JSON 对象）。描述红人需交付的内容（视频条数、草稿审核、Bio 链接、投流码、素材授权等）。多平台时用换行分段，例如「YouTube：1条专属视频；Bio 14天\\nInstagram：1条 Reel；Ad code 60天」
   用户未提及 → deliverables 可为 null（系统将用上述默认交付结果）

如果用户消息中没有提到某项信息，该项为 null。
如果用户提到多个平台或地区，提取所有提到的。
用户若用缩写（ytb、tk、ins）或组合（ytb和tk），须展开为规范全称数组，勿把整段组合写进单个字符串。
只接受用户明确表达且含义唯一的值；金额单位、时间、字段归属不明确时必须返回 null，禁止猜测。

只返回 JSON 格式，不要其他文字：
{
  "platform": ["TikTok"] | ["Instagram"] | ["YouTube"] | ["X"] | ["TikTok", "X", "YouTube"] | null,
  "region": ["美国"] | ["法国", "德国"] | ["US", "GB"] | null,
  "publishTimeRange": "2024年3月1日-3月31日" | null,
  "budget": 10000 | null,
  "commission": 15 | null,
  "influencerPricing": {
    "mode": "ask_creator_quote" | "ecpm_with_cap" | "commission_only" | null,
    "ecpmUsd": 3 | null,
    "maxFlatFeeUsd": 1000 | null
  } | null,
  "deliverables": "1条专属视频\\n发布前需分享草稿供确认\\n..." | "YouTube：...\\nInstagram：..." | null
}`;
      ...
      const mergedInfo = {
        ...
        commission: extractedInfo.commission !== null && extractedInfo.commission !== undefined 
          ...
          : (existingCampaignInfo.commission !== null && existingCampaignInfo.commission !== undefined 
              ...
              : null),
        influencerPricing: mergeInfluencerPricingExtracted(
          extractedInfo.influencerPricing,
          {
            ...existingCampaignInfo,
            influencerPricing:
              existingCampaignInfo.influencerPricing ?? getDefaultInfluencerPricing(),
          }
        ),
        deliverables: mergeDeliverablesExtracted(
          extractedInfo.deliverables,
          existingCampaignInfo
        ),
      };

      if (mergedInfo.platform) {
        mergedInfo.platform = normalizePlatformList(mergedInfo.platform);
      }

      if (mergedInfo.region) {
        const regionZh = normalizeRegionToZhLabels(mergedInfo.region);
        if (regionZh.length) {
          mergedInfo.region = regionZh.length === 1 ? regionZh[0] : regionZh;
        }
      }
      ...
      if (!validationResult.isValid) {
        ...
        if (validationResult.invalidPlatforms && validationResult.invalidPlatforms.length > 0) {
          ...
          if (Array.isArray(filteredInfo.platform)) {
            filteredInfo.platform = normalizePlatformList(
              filteredInfo.platform.filter((p) => isValidPlatform(p))
            );
          } else if (!isValidPlatform(filteredInfo.platform)) {
            filteredInfo.platform = null;
          } else {
            filteredInfo.platform = normalizePlatformList(filteredInfo.platform);
          }
        }
        
        if (validationResult.invalidRegions && validationResult.invalidRegions.length > 0) {
          if (Array.isArray(filteredInfo.region)) {
            const kept = filteredInfo.region.filter((r) =>
              isRecognizedCountryRegion(r)
            );
            const keptZh = normalizeRegionToZhLabels(kept);
            filteredInfo.region = keptZh.length ? keptZh : null;
          } else if (!isRecognizedCountryRegion(filteredInfo.region)) {
            filteredInfo.region = null;
          } else {
            const keptZh = normalizeRegionToZhLabels(filteredInfo.region);
            filteredInfo.region = keptZh.length ? keptZh[0] : null;
          }
        }
      ...
      if (missingFields.length > 0) {
        ...
        const ambiguityNotes = (context.briefIntake?.ambiguities || [])
          .filter((item) => item.block === "campaignInfo" && missingFields.includes(item.field))
          .slice(-3)
          .map((item) => `- “${item.quote}”：${item.reason}`);
      ...
      if (!justCompleted && !options.enteringStage) {
...
只返回 JSON 格式：
{
  ...
  validateCampaignInfo(info) {
    ...
    if (info.region) {
      ...
      for (const region of regions) {
        if (!isRecognizedCountryRegion(region)) {
    ...
    if (invalidPlatforms.length > 0 || invalidRegions.length > 0) {
      ...
      if (invalidPlatforms.length > 0) {
        ...
        errorMessages.push(`目前仅支持 TikTok、Instagram、YouTube 和 X（Twitter），${invalidPlatformsText}会尽快开放，咱们是否考虑在这些平台投放呀？`);
      }
      
      if (invalidRegions.length > 0) {
        const invalidRegionsText = invalidRegions.join("、");
        errorMessages.push(
          `暂未识别「${invalidRegionsText}」对应的国家/地区。请使用常用中文国名（如美国、法国、日本）、英文国名或 ISO 两位代码（如 US、FR、JP），也可说明「全球」若需多国组合。`
        );
      }
    ...
    if (info.budget !== null && info.budget !== undefined) {
      ...
      if (isNaN(budget) || budget <= 0) {
        return {
          isValid: false,
          errorMessage: `总预算 "${info.budget}" 无效。总预算必须是大于 0 的数字。`,
        };
      }
    }
    ...
    const pricing = normalizeInfluencerPricing(info.influencerPricing);
    const pricingValidation = validateInfluencerPricing(pricing, info.commission);
    if (!pricingValidation.isValid) {
      return {
        isValid: false,
        errorMessage: pricingValidation.errorMessage,
      };
    }
  ...
  formatCampaignInfo(info) {
    ...
    if (info.platform) {
      const platforms = Array.isArray(info.platform) ? info.platform : [info.platform];
      // 标准化平台名称
      const normalizedPlatforms = parseCampaignPlatforms(platforms);
    ...
    if (info.budget !== null && info.budget !== undefined) {
      parts.push(`**总预算**: $${info.budget.toLocaleString()} USD`);
    }
    ...
    parts.push(
      `**单位红人报价策略**: ${formatInfluencerPricingLabel(info.influencerPricing, info.commission)}`
    );

    parts.push(
      `**交付结果**: ${formatDeliverablesLabel(info.deliverables) || formatDeliverablesLabel(getDefaultDeliverables())}`
    );
