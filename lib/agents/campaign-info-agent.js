...
Campaign 基本信息包括：
1. 投放平台（必填）：TikTok、Instagram 或 YouTube（可单选或多选）。客户口语中可能出现 ytb/tk/ins 等缩写，你理解即可；写入与确认时一律使用规范全称
2. 投放地区（必填）：TikTok、Instagram、YouTube 支持的国家/地区，可单选或多选（如美国、德国、法国、英国、日本等；也接受 ISO 两位代码如 US、DE、FR）
...
2. 从客户的消息中提取已提供的信息
3. 验证信息的有效性：
   - 平台规范名称为 "TikTok"、"Instagram"、"YouTube"（多平台用数组，如 ["YouTube","TikTok"]）
...
需要提取的信息：
1. platform（平台）：必须是 "TikTok"、"Instagram" 或 "YouTube" 中的一个或多个，用数组表示，例如 ["TikTok"] 或 ["TikTok", "Instagram", "YouTube"]
...
只返回 JSON 格式，不要其他文字：
{
  "platform": ["TikTok"] | ["Instagram"] | ["YouTube"] | ["TikTok", "Instagram", "YouTube"] | null,
  ...
  validateCampaignInfo(info) {
    ...
    if (invalidPlatforms.length > 0 || invalidRegions.length > 0) {
      ...
      if (invalidPlatforms.length > 0) {
        ...
        errorMessages.push(`目前仅支持 TikTok、Instagram 和 YouTube，${invalidPlatformsText}会尽快开放，咱们是否考虑在 TikTok、Instagram 或 YouTube 投放呀？`);
      }
