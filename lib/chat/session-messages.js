...
/** Bin（assistant）且在聊天框可见的消息条数 —— 未读计数的基准 */
export function countChatAssistantMessages(messages) {
  if (!Array.isArray(messages)) return 0;
  return messages.filter((m) => m?.role === "assistant" && isChatVisibleMessage(m))
    .length;
}

/**
 * 相对上一版新增的 Bin 消息条数。
 * 用合并键去重，容忍入库前的排序 / 裁剪差异；只增不减，负数夹到 0。
 */
export function countNewChatAssistantMessages(prevMessages, nextMessages) {
  const prevKeys = new Set(
    (Array.isArray(prevMessages) ? prevMessages : [])
      .filter((m) => m?.role === "assistant")
      .map((m) => sessionMessageMergeKey(m))
      .filter(Boolean)
  );
  let added = 0;
  for (const m of Array.isArray(nextMessages) ? nextMessages : []) {
    if (m?.role !== "assistant" || !isChatVisibleMessage(m)) continue;
    const key = sessionMessageMergeKey(m);
