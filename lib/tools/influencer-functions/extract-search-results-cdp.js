...
export function extractVideosFromSearchAPI(apiData) {
  ...
  for (const item of itemList) {
    ...
    const video = {
      ...
      userId: author.id || author.userId || author.uid || null,
...
export function extractInfluencersFromVideos(videos) {
  ...
  for (const video of videos) {
    ...
    if (influencerMap.has(username)) {
      ...
      if (!existing.secUid && (video.secUid || video.creator.secUid)) {
        existing.secUid = video.secUid || video.creator.secUid;
        existing.tiktokSecUid = existing.secUid;
      }
      if (!existing.userId && (video.userId || video.creator.userId)) {
        existing.userId = video.userId || video.creator.userId;
        existing.tiktokUserId = existing.userId;
      }
    } else {
      // 创建新的红人记录
      const secUid = video.secUid || video.creator?.secUid || null;
      const userId = video.userId || video.creator?.userId || null;
