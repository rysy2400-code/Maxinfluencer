...
/** mysql2 把 DATETIME 解析成 Date；游标里存 UTC 字符串，与 CONVERT_TZ(列, 会话时区, '+00:00') 对齐 */
function toCursorTime(value) {
...
function encodeListCursor({ handoverRank, sortTime, influencerId }) {
  const time = toCursorTime(sortTime);
  if (!time || !influencerId) return null;
  return Buffer.from(
    JSON.stringify({
...
export async function listInfluencerConversations({
  ...
  if (decoded) {
    cursorSql = `AND (
      ...
      OR (
        ${handoverRankExpr} = ?
        AND (
          CONVERT_TZ(last_event_time, @@session.time_zone, '+00:00') < ?
          OR (
            CONVERT_TZ(last_event_time, @@session.time_zone, '+00:00') = ?
            AND l.influencer_id < ?
          )
  ...
  const sql = `
    ...
    SELECT
      ...
      LEFT(m.body_text, 120) AS last_body_text,
    ...
    FROM latest l
    JOIN tiktok_influencer_conversation_messages m ON m.id = l.last_message_id
