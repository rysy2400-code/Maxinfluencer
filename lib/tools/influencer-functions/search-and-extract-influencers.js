...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  function describeLitePage(page) {
    ...
    return {
      targetId: page?._igCdpTargetId || page?._cdpTargetId || null,
      url,
    };
  }
    ...
    const run = async () => {
      ...
      if (
        ...
        const matchTask = {
          ...
          checkAbort: igLiteAnalysisAsync ? () => false : () => perInfluencerAborted,
        };
  ...
  if (enrichBatchPolicy) {
    ...
    while (poolCursor < recordsToEnrich.length) {
      ...
      if (igLiteAnalysisAsync) {
        ...
        batchRecommended = activeBatchStats.recommended;
        totalAnalyzedCount += activeBatchStats.completed;
        sendStep(
          "分析队列",
          `本批异步分析完成 ${activeBatchStats.completed}/${batchRecords.length} 位（推荐 ${activeBatchStats.recommended}，失败 ${activeBatchStats.failed}）`
        );
      } else {
    ...
    if (igLiteAnalysisAsync) {
      await awaitIgAnalysisDrain();
      totalAnalyzedCount = igAnalysisCompletedTotal;
    } else {
