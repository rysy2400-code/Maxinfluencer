...
export async function fetchPublishedVideoMetricsViaCdp(page, videoUrl, options = {}) {
  ...
  switch (parsed.platform) {
    ...
    case "youtube":
      // API 优先：innertube player + next，不加载播放页；失败再回退页面拦截
      try {
        const viaApi = await fetchYoutubeMetricsViaApi(page, parsed, { gotoTimeout });
        if (viaApi) return viaApi;
      } catch (e) {
        console.warn(
          `[published-video-metrics] YouTube API-only 失败，回退页面加载: ${String(e?.message || e).slice(0, 120)}`
        );
      }
...
/**
 * YouTube API-only：不打开播放页，直接调 innertube。
 *   POST /youtubei/v1/player  -> videoDetails.viewCount（播放量）
 *   POST /youtubei/v1/next    -> 点赞数 / 评论数
 * 相比 page.goto 播放页，省掉页面 HTML + 播放器 JS + 缩略图，流量下降一个量级。
 */
async function fetchYoutubeMetricsViaApi(page, parsed, { gotoTimeout } = {}) {
  const videoId = parsed?.videoId;
  if (!page || !videoId) return null;

  // 需要一个 youtube.com 上下文来读取 ytcfg（API key + context）
  if (!/youtube\.com/.test(String(page.url?.() || ""))) {
    await page.goto("https://www.youtube.com/", {
      waitUntil: "domcontentloaded",
      timeout: gotoTimeout || DEFAULT_GOTO_TIMEOUT_MS,
    });
    await page.waitForTimeout(1500);
  }
  ...
  const callInnertube = () =>
    ...
    async ({ vid }) => {
      ...
      const get = (k) => ytcfg[k] ?? (typeof window.ytcfg?.get === "function" ? window.ytcfg.get(k) : null);
      const apiKey = get("INNERTUBE_API_KEY");
      const context = get("INNERTUBE_CONTEXT");
      if (!apiKey || !context) return { error: "missing_ytcfg" };
      ...
      const player = await call("player", { context, videoId: vid });
      const next = await call("next", { context, videoId: vid });
      return {
        player: player.json,
        next: next.json,
        bytes: player.bytes + next.bytes,
        statuses: [player.status, next.status],
      };
    },
    { vid: videoId }
    );

  let out = await callInnertube();
  // 页面在 youtube.com 但 ytcfg 尚未就绪（SPA 状态）时，回首页重建上下文再试一次
  if (!out || out.error === "missing_ytcfg") {
    try {
      await page.goto("https://www.youtube.com/", {
        waitUntil: "domcontentloaded",
        timeout: gotoTimeout || DEFAULT_GOTO_TIMEOUT_MS,
  ...
  walkExtractYoutubeCounts(out.player, counts);
  walkExtractYoutubeCounts(out.next, counts);
  const normalized = normalizeMetricsPayload(counts);
  if (!normalized.views && !normalized.likes && !normalized.comments) return null;
  return {
    platform: "youtube",
    ...normalized,
    source: "youtube_innertube_api",
    trafficBytes: out.bytes || 0,
  };
}
