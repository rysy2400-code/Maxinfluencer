...
/**
 * 把历史/跨平台写入的 pdf_storage_key 归一化成项目内相对 POSIX 路径。
 *
 * 历史上 Web 端在 Windows 上运行时用 path.join 生成 key，落库为
 * "storage\\invoices\\4\\GCG-C-xxx.pdf"；在 macOS/Linux 上 path.join 会把整串
 * 当成单个文件名，导致永远找不到 PDF。这里统一还原成
 * "storage/invoices/4/GCG-C-xxx.pdf"。
 *
 * @param {string} storageKey
 * @returns {string | null} 相对项目根的 POSIX 路径；非法或越界时返回 null
 */
export function normalizeInvoicePdfStorageKey(storageKey) {
  if (!storageKey) return null;
  let key = String(storageKey).trim().replace(/\\/g, "/");
  if (!key) return null;

  // 去掉 Windows 盘符与绝对路径前缀，只保留 storage/invoices/ 之后的部分
  key = key.replace(/^[A-Za-z]:\/+/, "").replace(/^\/+/, "");
  const marker = "storage/invoices/";
  const idx = key.toLowerCase().indexOf(marker);
  if (idx > 0) key = key.slice(idx);
  key = key.replace(/\/{2,}/g, "/");

  if (!key.startsWith(marker)) return null;
  if (key.slice(marker.length).split("/").includes("..")) return null;
  return key;
}

/**
 * 按统一规则生成发票 PDF 的 storage key（始终使用 "/"，与运行平台无关）。
 *
 * @param {number} advertiserId
 * @param {string} invoiceNo
 */
export function buildInvoicePdfStorageKey(advertiserId, invoiceNo) {
  return path.posix.join("storage", "invoices", String(advertiserId), `${invoiceNo}.pdf`);
}

/** @param {string} storageKey */
export function resolveInvoicePdfPath(storageKey) {
  const key = normalizeInvoicePdfStorageKey(storageKey);
  if (!key) return null;
  return path.join(projectRoot, key);
}

/**
 * 解析发票 PDF 的候选绝对路径：先按库里存的 key，再按
 * storage/invoices/<advertiserId>/<invoiceNo>.pdf 兜底。
 *
 * @param {{ pdf_storage_key?: string, advertiser_id: number, invoice_no: string }} row
 * @returns {string[]}
 */
export function resolveInvoicePdfPathCandidates(row) {
  const candidates = [];
  const fromKey = resolveInvoicePdfPath(row?.pdf_storage_key);
  if (fromKey) candidates.push(fromKey);
  if (row?.advertiser_id && row?.invoice_no) {
    candidates.push(
      path.join(projectRoot, buildInvoicePdfStorageKey(row.advertiser_id, row.invoice_no))
    );
  }
...
/**
 * @param {number} advertiserId
 * @param {'recharge' | 'monthly_consumption' | 'influencer_campaign'} invoiceType
...
/**
 * 已按“红人 × 活动”开过发票的 ledger 明细 id（仅限 influencer_campaign 类型，
 * 月度/充值发票不影响红人维度去重）。
 *
 * @param {number} advertiserId
 */
export async function getInvoicedInfluencerLedgerIds(advertiserId) {
  const rows = await queryTikTok(
    `SELECT related_ledger_ids FROM tiktok_advertiser_invoice
     WHERE advertiser_id = ? AND invoice_type = 'influencer_campaign'`,
    [advertiserId]
  );
  const ids = new Set();
...
/**
 * 回填/修正发票 PDF 的 storage key（如把历史 Windows 反斜杠路径归一化）。
 *
 * @param {number} invoiceId
 * @param {string} storageKey
 */
export async function updateInvoicePdfStorageKey(invoiceId, storageKey) {
  await queryTikTok(`UPDATE tiktok_advertiser_invoice SET pdf_storage_key = ? WHERE id = ?`, [
    storageKey,
    invoiceId,
  ]);
}
