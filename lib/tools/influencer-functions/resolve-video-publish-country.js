...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  // Lite：9223 API-only 只读代表视频/alt 视频 HTML 中的 ISO-2 locationCreated；不使用 search_api 国家。
  if (isLiteScraperMode()) {
    ...
    sendStep(
      "采集视频发布地",
      `Lite：9223 API-only 读视频发布地（全池 ${queue.length} 人，并发 ${concurrency}，${tabPoolSize} tab${stopOnZeroBatchMatch ? "，0 明确符合则停" : ""}，不 goto）...`
    );
    ...
    for (let i = 0; i < tabPoolSize; i += 1) {
      try {
        ...
        console.warn(
          `[resolveVideoPublishCountry] 9223 country session ${i + 1} failed: ${e.message}`
        );
      }
    }
    if (!pool.length) {
    ...
    sendStep(
      "采集视频发布地",
      `Lite 9223 API-only 发布地完成：检查 ${checked.length} 人，进入 enrich ${passed.length} 人（仅明确符合国家），unknown ${unknown} mismatch ${mismatch}${stoppedEarly ? "（提前停止）" : ""}`
    );
