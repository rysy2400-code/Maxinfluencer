...
export const CAMPAIGN_EXECUTION_TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    description:
      "修改已发布 campaign 的内容。可针对整体或单个红人修改。整体修改支持：品牌/产品（brandName/productName）、投放与预算（platform/region/publishTimeRange/budget/commission）、单位红人报价策略（influencerPricing 或 pricingMode/pricingEcpmUsd/pricingMaxFlatFeeUsd）、红人画像 screeningConditions（与 influencer_profile 对齐：followerRange 粉丝量、viewRange 播放量、accountType 帐号类型/内容方向）、内容要求 contentRequirements、关键词策略 keywordStrategy（供搜索关键词生成参考）。报价策略变更仅影响尚未发出首封邀约的红人。修改后需同步给红人经纪人 agent。",
    parameters: {
      type: "object",
      properties: {
        campaignId: {
...
async function modifyCampaign(params, context) {
  ...
  const pricingNote = pricingChanged
    ? " 单位红人报价策略变更仅影响尚未发出首封邀约的红人，已联系红人不受影响。"
    : "";
  ...
  return {
    ...
    message:
      effectiveScope === "whole"
        ? `已修改整体 campaign 配置，${pricingSummary}并通知红人经纪人同步给相关红人。${pricingNote}`
