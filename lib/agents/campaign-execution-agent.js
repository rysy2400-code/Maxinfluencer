...
- **回复风格**：简洁，只确认用户刚改的那一项；不要罗列工作笔记全文、不要复述未修改字段；未成功调用工具前不得声称已改库。
...
- 若用户要暂停但当前已是 paused，或要恢复但当前已是 running：仍调用 set_campaign_status 对应 action（工具会返回幂等说明）。`;
      ...
      executionConfigHint = `
【当前执行配置（用户只改其中一项时，其他项勿动）】
- 执行节奏：每天联系 ${campaign?.influencersPerDay ?? "未设置"} 位红人
- 汇报：每 ${reportConfig?.intervalHours ?? 24} 小时，时间 ${reportConfig?.reportTime ?? "09:00"}，形式 ${reportConfig?.contentPreference ?? "brief"}
- 重点指标：${JSON.stringify(reportConfig?.includeMetrics || [])}`;
    } catch {
      /* ignore */
    }
...
5. 若最后一条消息要求**修改 Campaign 信息或执行筛选**（品牌、产品、平台、地区、预算、佣金、报价策略、红人画像、关键词策略等）→ 必须 needTool=true，toolName=modify_campaign，scope=whole；**禁止** needTool=false 口头声称已改 Campaign。
6. 若最后一条消息 @某红人 并询问/协商（能否接受某价格、做几条视频、改时间等）→ 必须 needTool=true，toolName=ask_influencer_special_request，params.influencerId=handle（无@），params.requestDetail=用户原意，requestType 选 adjust_price / other 等。
7. 若用户问「红人回了没」且上下文有 specialRequestId → needTool=true，toolName=get_influencer_special_request_feedback。
8. 寒暄、问汇报选项说明书、与状态无关的闲聊 → needTool=false。
9. **禁止** needTool=false 时声称已向红人发送询问、已收到红人反馈，或声称已修改执行节奏/汇报/Campaign 配置。
