/**
 * TikTok Lite 主页 enrich：9223 无登录 + API 直调，不打开 @profile 页面
 */
...
export async function extractTiktokProfileLite(page, username, options = {}) {
  ...
  reportStep(
    ...
    STEP_STATUS.RUNNING,
    `TikTok Lite enrich @${handle}（API 直调，9223）`
  );
  ...
  if (!secUid) {
    const hasPartial =
      userInfo.followers || userInfo.bio || userInfo.userId || options.userId;
    return {
      success: false,
  ...
  const itemBatches = await fetchPostItemListAll(page, {
    secUid,
    maxPages: Math.min(Math.max(Number(process.env.TT_LITE_ITEM_LIST_MAX_PAGES || 3), 1), 8),
  });
  ...
  return {
    ...
    statistics,
    extractMode: "lite",
    enrichEndpoint: process.env.CDP_ENDPOINT_ENRICH || "http://127.0.0.1:9223",
  };
}
