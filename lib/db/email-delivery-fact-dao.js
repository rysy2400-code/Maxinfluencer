...
async function findAttributableFact(message) {
  ...
  if (inReplyTo) {
    const exact = await queryTikTok(
      `SELECT id FROM email_outreach_delivery_fact
       WHERE outreach_message_id = ? AND sent_at <= CONVERT_TZ(?, '+00:00', @@session.time_zone)
       LIMIT 1`,
  ...
  if (influencerId && campaignId && receivedAt) {
    const candidates = await queryTikTok(
      `SELECT id FROM email_outreach_delivery_fact
       WHERE influencer_id = ? AND campaign_id = ? AND sent_at <= CONVERT_TZ(?, '+00:00', @@session.time_zone)
...
export async function recordInboundAttribution(message) {
  ...
  const assignment = bounce
    ? `bounce_message_id = COALESCE(bounce_message_id, ?), bounce_at = LEAST(COALESCE(bounce_at, CONVERT_TZ(?, '+00:00', @@session.time_zone)), CONVERT_TZ(?, '+00:00', @@session.time_zone))`
    : `first_reply_message_id = COALESCE(first_reply_message_id, ?), first_reply_at = LEAST(COALESCE(first_reply_at, CONVERT_TZ(?, '+00:00', @@session.time_zone)), CONVERT_TZ(?, '+00:00', @@session.time_zone))`;
  await queryTikTok(
