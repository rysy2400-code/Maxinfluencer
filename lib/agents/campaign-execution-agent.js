...
import {
  ...
  detectCampaignStatusIntentFromMessage,
...
- 当用户说「暂停 campaign」「先停一下」「别联系了」「pause campaign」「stop the campaign」等，必须调用 set_campaign_status，params.action 为 pause。
- 当用户说「继续 campaign」「恢复」「重新开始」「resume campaign」「continue the campaign」等，必须调用 set_campaign_status，params.action 为 resume。
- 当用户说「campaign 完成了」「可以结束了」「结项」「mark complete」「end campaign」「campaign is done」等，必须调用 set_campaign_status，params.action 为 complete。
...
当不需要调用工具时，你必须直接输出中文自然语言回复（不要输出 JSON）。`;
  ...
  async processWithTools(messages, context = {}) {
    ...
    const statusAction = detectCampaignStatusIntentFromMessage(lastMessage);
    if (statusAction) {
      return {
        reply: "正在更新 Campaign 运行状态…",
        toolCall: {
          toolName: "set_campaign_status",
          params: { action: statusAction, campaignId },
        },
      };
    }
...
【modify_campaign 必填形状示例】用户要改「红人搜索关键词策略」时，必须返回（changes 内必须有 keywordStrategy 字符串，勿传空对象 {}）：
{ "needTool": true, "toolName": "modify_campaign", "params": { "scope": "whole", "changes": { "keywordStrategy": "（把用户说的策略原文写进这里）" } } }
不要把 keywordStrategy 放在 params 顶层；不要用 single_influencer 改整单策略。
