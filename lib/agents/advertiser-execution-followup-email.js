...
export async function generateAdvertiserExecutionFollowupEmailBody({
  ...
  const languageRule =
    languageCode === "en"
      ...
      : `- **必须用${languageLabel}（${languageCode}）写邮件正文**（纯文本，不要 markdown，不要 JSON）；这是该红人的沟通语言。品牌名、产品名、链接、地址、日期等保留原文，不要在正文里解释语言选择。`;
...
${languageRule}${actionExtraRules}${extraInstruction ? `\n${extraInstruction}` : ""}`;
  ...
  const userContent = `请根据以下 JSON 上下文撰写跟进邮件正文，正文语言必须是 ${languageLabel}（${languageCode}）：\n${JSON.stringify(payloadForLLM, null, 2)}`;
