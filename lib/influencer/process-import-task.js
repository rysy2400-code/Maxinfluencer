...
function buildImportRegionText(campaignInfo, allowedCountries) {
  const ci = campaignInfo || {};
  const fromInfo =
    Array.isArray(ci.countries) && ci.countries.length
      ? ci.countries.join("/")
      : Array.isArray(ci.region) && ci.region.length
        ? ci.region.join("/")
        : "";
  const fromAllowed = Array.isArray(allowedCountries) && allowedCountries.length
    ? allowedCountries.join("/")
    : "";
  return fromInfo || fromAllowed || "未指定";
}

function importPlatformLabel(platform) {
  const p = String(platform || "").toLowerCase();
  if (p === "tiktok") return "TikTok";
  if (p === "instagram") return "Instagram";
  if (p === "youtube") return "YouTube";
  return String(platform || "未知");
}
...
function buildCompletionSummary({
  ...
  const modeNote = contactAll
    ? "（直接联系模式：符合 campaign 投放地区且有邮箱的红人一律联系）"
    : "（推荐模式：分析后仅推荐且符合投放地区的红人进入待联系）";
  const totalN = Number(totalRows) || 0;
  const analyzedN = Number(analyzedCount) || 0;
  const recommendedN = Number(recommendedCount) || 0;
  const willContactN = contactAll
    ? Number(willContactCount) || 0
    : recommendedN;
  const head = `【名单导入完成】共 ${totalN} 位红人，已分析 ${analyzedN} · 推荐 ${recommendedN} · 将联系 ${willContactN}${modeNote}`;

  return [
    head,
    "",
    `- ${platformLabel}（${totalN} 位）：已分析 ${analyzedN} · 推荐 ${recommendedN} · 将联系 ${willContactN}`,
    "",
    "可在执行总览查看详情。",
  ].join("\n");
}
...
async function resolveLiteTiktokImportRecordsForEnrich(
  ...
  try {
    ...
    // Phase 2 国家门禁：首视频 locationCreated 明确 mismatch 跳过，未知走 bio（已语言匹配）保留
    if (
      tiktokForEnrich.length > 0 &&
      String(process.env.TT_IMPORT_COUNTRY_GATE ?? "1").trim() !== "0"
    ) {
      const { resolveImportTiktokCountryGate } = await import(
        "../tools/influencer-functions/tiktok/resolve-import-tiktok-country-gate.js"
      );
      const countryResult = await resolveImportTiktokCountryGate({
        records: tiktokForEnrich,
        allowedCountries,
        importTaskId: taskId,
        onStepUpdate: options.onStepUpdate,
      });
      tiktokForEnrich = countryResult.records || [];
      console.log(
        `[process-import-task] Lite 国家门禁: 检查 ${countryResult.stats.total}，` +
          `保留 ${tiktokForEnrich.length}（mismatch ${countryResult.stats.mismatch}，unknown ${countryResult.stats.unknown}，失败 ${countryResult.stats.failed}）`
      );
    }
  } catch (phase1Err) {
    console.warn(
...
export async function processInfluencerImportTask(task, options = {}) {
  ...
  // 前置候选查重（recommended_only）：候选表已存在的红人不花 user/detail 流量
  if (!contactAll && tiktokRecords.length > 0) {
    const handles = [
      ...new Set(
        tiktokRecords
    ...
    if (handles.length > 0) {
      ...
      const existing = new Set(
        (existingRows || []).map((r) => String(r.u || ""))
      );
      const before = tiktokRecords.length;
      const keep = new Set(
        tiktokRecords
          .map((r) => normalizeHandle(r.username))
          .filter((u) => !existing.has(u))
      );
      tiktokRecords = tiktokRecords.filter((r) =>
        keep.has(normalizeHandle(r.username))
      );
      console.log(
        `[process-import-task] task=${taskId} 候选表前置查重: 已有 ${before - tiktokRecords.length} 跳过，剩余 ${tiktokRecords.length}`
      );
    }
  }
  ...
  const summary = buildCompletionSummary({
    ...
    contactAll,
    totalRows: task.total_rows || rows.length,
    platformLabel: importPlatformLabel(task.platform || payload.platform),
  });
