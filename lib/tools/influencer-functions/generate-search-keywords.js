...
const PLATFORM_KEYWORD_HINTS = Object.freeze({
  ...
  x: [
    "X 搜索支持短语与高级运算符（如 lang:en、filter:media、-filter:replies、from:），关键词宜为真实搜索意图短语。",
    "常用 product review、best X 2026、X vs Y、deal、promo code 等创作者推文常带表达。",
    "优先能命中创作者推文正文/简介的检索词；避免纯 hashtag（X 上 hashtag 覆盖弱）。",
  ],
});

const KEYWORD_LLM_TIMEOUT_MS = Math.max(
  1000,
  Number(process.env.EXECUTION_KEYWORD_LLM_TIMEOUT_MS || 90000) || 90000
);
...
【红人搜索关键词策略（最高优先级，必须严格遵循）】
${keywordStrategy || "(未设置)"}
${keywordStrategy ? "\n若本策略非空：\n- 生成的每一条 keyword 都必须符合该策略。\n- 产品词、竞品词、探索词、bucket 配比都不能覆盖或稀释该策略。\n- 如果策略限定某类人群、场景或内容方向，所有关键词都必须直接服务于该人群、场景或方向。\n- 可以使用与策略强相关的场景词、行为词、生活方式词，不要求每条都机械包含策略原词。\n" : ""}
...
10. 所有 search_queries 中的 keyword 必须逐条符合【红人搜索关键词策略】；若策略与其他生成目标冲突，以红人搜索关键词策略为准。
11. 若用户关键词策略要求「竞品/竞争对手」相关检索，competitor bucket 的关键词必须体现竞品品牌或同类对标产品检索意图，不得全部落在 product。${platformOutputExtra}
12. 只返回 JSON，格式示例：
{
  ...
  "search_queries": [
    ...
    const llmResult = await callDeepSeekLLM(
      ...
      systemPrompt,
      { returnFullResponse: true, timeoutMs: KEYWORD_LLM_TIMEOUT_MS }
    );
