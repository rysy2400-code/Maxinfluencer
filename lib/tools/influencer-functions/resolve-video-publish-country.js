...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  if (isLiteScraperMode()) {
    ...
    sendStep(
      "采集视频发布地",
      `Lite：9223 API 读取发布地（每批 ${countryBatchSize} 人，0 符合国家则停，无页面导航）...`
    );
    ...
    sendStep(
      "采集视频发布地",
      `Lite 9223 发布地完成：检查 ${checked.length} 人，进入 enrich ${passed.length} 人，明确符合国家 ${explicitMatches} 人${stoppedEarly ? "（提前停止）" : ""}`
    );
