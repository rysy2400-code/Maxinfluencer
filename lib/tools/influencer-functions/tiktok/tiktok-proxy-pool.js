...
const PORT_RESTART_AT = new Map(); // cdpPort -> timestamp

/** 请求重启某 CDP 端口的 Chrome（写信号文件，guard 负责拉起），带节流 */
export async function requestTiktokPortRestart(cdpPort, reason = "") {
  if (!cdpPort) return false;
  const minIntervalMs = Math.max(
    60 * 1000,
    Number(process.env.TT_LITE_CHROME_RESTART_MIN_INTERVAL_MS || 10 * 60 * 1000)
  );
  const lastAt = PORT_RESTART_AT.get(cdpPort) || 0;
  if (Date.now() - lastAt < minIntervalMs) return false;
  PORT_RESTART_AT.set(cdpPort, Date.now());
  try {
    const fs = await import("node:fs/promises");
    const path = await import("node:path");
    const signalPath = path.join(
      String(process.env.TT_LITE_SIGNALS_DIR || "C:\\maxinfluencer\\signals"),
      `restart-chrome-${cdpPort}.flag`
    );
    await fs.mkdir(path.dirname(signalPath), { recursive: true });
    await fs.writeFile(signalPath, `${new Date().toISOString()} ${reason}\n`, "utf8");
    console.warn(`[tt-proxy-pool] requested Chrome restart port=${cdpPort} (${reason})`);
    return true;
  } catch (e) {
    console.warn(`[tt-proxy-pool] restart signal failed port=${cdpPort}: ${e.message}`);
    return false;
  }
}
...
/** 按 TT_LITE_PROXY_NODE_PRIORITY 排序节点（优先级在前 = 默认/首选） */
export function sortPoolNodesByPriority(nodes) {
  const raw = String(process.env.TT_LITE_PROXY_NODE_PRIORITY || "");
  const priority = raw
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
export async function readProxyGroup(controlUrl = resolveControlUrl(), groupName = resolveGroupName()) {
  ...
  const nodes = sortPoolNodesByPriority(all.filter((n) => {
    const p = proxies[n];
...
export async function fetchWithPoolRetry({
  ...
  let lastErr = null;
  let pageHangCount = 0;
  const hangThreshold = Math.max(
    2,
    Number(process.env.TT_LITE_PAGE_HANG_RESTART_THRESHOLD || 3)
  );
  for (let round = 0; round <= maxNodeSwitches; round += 1) {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        if (/CDP timeout|Failed to fetch|Runtime\.evaluate|Target closed/i.test(msg)) {
          pageHangCount += 1;
          if (pageHangCount >= hangThreshold) {
            const cdpPort = resolveCdpPortFromPage(page);
            await requestTiktokPortRestart(cdpPort, `hang:${label}`).catch(() => false);
            throw lastErr; // 页面已挂：本轮放弃，下次获取会用新 Chrome
          }
        }
        if (attempt === 1) {
    ...
    if (typeof onBeforeSwitch === "function") {
      try {
        await onBeforeSwitch(currentNode);
      } catch (e) {
        console.warn(`[tt-proxy-pool] onBeforeSwitch failed: ${e.message}`);
      }
    }
