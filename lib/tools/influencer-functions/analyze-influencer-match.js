...
- **GMV（TikTok Shop Affiliate，近 ${influencerSummary.gmvPeriodDays} 天）**: ${influencerSummary.gmv != null ? (influencerSummary.gmvDisplay || `$${influencerSummary.gmv.toLocaleString()}`) : '未知'}
- **Units sold（成交件数，近 ${influencerSummary.gmvPeriodDays} 天）**: ${influencerSummary.unitsSold != null ? influencerSummary.unitsSold.toLocaleString() : '未知'}` : ''}
...
- 使用 **4 个二级标题** 组织（顺序固定、标题字面一致，便于阅读）：
  - \`## 基础数据评估\`：粉丝量、播放、点赞、评论${influencerSummary.hasAffiliateCommerce ? "、Affiliate GMV、Units sold（若有）" : ""}及数据小结（含一条 **加粗** 小结句）${influencerSummary.hasAffiliateCommerce ? "；若提供了 GMV / Units sold，需结合带货规模评估该红人的商业化能力与预算匹配度" : ""}
