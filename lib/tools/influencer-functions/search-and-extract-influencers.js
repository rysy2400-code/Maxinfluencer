...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  async function touchEnrichProgress() {
    if (!taskIdNumForTouch && !importTaskIdNum) return;
    const now = Date.now();
    if (now - lastEnrichTouchAt < 15_000) return;
    lastEnrichTouchAt = now;
    try {
      if (taskIdNumForTouch) {
        await touchSearchTaskLastProgressAt(taskIdNumForTouch);
      } else if (importTaskIdNum) {
        const { touchImportTaskLastProgressAt } = await import(
          "../../db/influencer-import-task-dao.js"
        );
        await touchImportTaskLastProgressAt(importTaskIdNum);
      }
    } catch (e) {
  ...
  // 导入任务不套用搜索任务的邮箱门禁：bio 门禁已过滤语言，enrich 直接拉视频 + LLM 异步。
  const importEmailGateOff = importTaskIdNum > 0;
    ...
    for (let desiredSlot = 0; desiredSlot < poolSize; desiredSlot += 1) {
      ...
      for (const endpointKey of candidates) {
        try {
          ...
          lastErr = e;
          // 轮换后旧 tab 的 cookie/acrawler 已失效，strict api-only 下无法重 bootstrap：
          // 回退开一个新 tab（与准入探测/国家检查一致），避免 enrich 整批因复用旧 tab 失败。
          console.warn(
            `[enrichInfluencerProfiles] TikTok Lite: ${endpointKey} 复用失败，尝试开新 tab: ${e?.message || e}`
          );
          try {
            session = await acquireTiktokApiSession(null, {
              forceNewTab: true,
            ...
            ttLiteEnrichEndpoint = endpointKey;
            console.log(
              `[enrichInfluencerProfiles] TikTok Lite: API 页 ${ttLitePagePool.length + 1}/${poolSize} via ${endpointKey} (fresh tab)（slot ${desiredSlot + 1}, enrich 并发 ${effectiveConcurrency}）`
            );
            break;
          } catch (e2) {
            lastErr = e2;
            console.warn(
              `[enrichInfluencerProfiles] TikTok Lite: ${endpointKey} 新 tab 也失败: ${e2?.message || e2}`
            );
          }
        }
      }
  ...
  /** 页面卡死/网络类失败时，请求 guard 重启对应端口的 Chrome（写 flag） */
  async function requestTiktokChromeRestart(endpointKey, reason) {
    const PORT_MAP = {
    ...
    const m = String(endpointKey || "").match(/:(\d+)/);
    const port = m ? m[1] : "9223";
    const cfg = PORT_MAP[port] || PORT_MAP["9223"];
    try {
      const { mkdir, writeFile } = await import("node:fs/promises");
      const dir = "C:\\maxinfluencer\\signals";
      await mkdir(dir, { recursive: true });
      const flag = `${dir}\\restart-chrome-${port}.flag`;
      await writeFile(flag, `${new Date().toISOString()} ${reason}\n`, "utf8");
      console.warn(`[enrichInfluencerProfiles] 请求重启 Chrome ${port}（${reason}）via ${flag}`);
    } catch (e) {
      console.warn(`[enrichInfluencerProfiles] 写 Chrome 重启信号失败: ${e.message}`);
    }
    // 直接重启该端口 Chrome（guard 若在运行也会消费 flag；不依赖 guard）
    try {
      const { execFile } = await import("node:child_process");
      const dirEsc = String(cfg.dir).replace(/'/g, "''");
      const script =
        "$p=@(Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'chrome.exe' -and $_.CommandLine -match [regex]::Escape('" +
        dirEsc +
        "') }); foreach($x in $p){ Stop-Process -Id $x.ProcessId -Force -ErrorAction SilentlyContinue }; " +
        "Start-Sleep -Seconds 2; " +
        "Remove-Item -LiteralPath '" +
        String(cfg.dir).replace(/'/g, "''") +
        "' -Recurse -Force -ErrorAction SilentlyContinue; " +
        "Start-Process -FilePath 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe' -ArgumentList " +
        "'--disable-gpu','--disable-quic','--remote-debugging-address=127.0.0.1','--remote-debugging-port=" +
        port +
        "','--user-data-dir=" +
        cfg.dir +
        "','--proxy-server=http://127.0.0.1:" +
        cfg.proxy +
        "','--no-first-run','--no-default-browser-check','--blink-settings=imagesEnabled=false','--autoplay-policy=user-gesture-required','about:blank' -WindowStyle Hidden";
      execFile("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], {
        windowsHide: true,
        timeout: 60000,
      });
      console.warn(`[enrichInfluencerProfiles] 已触发 Chrome ${port} 直接重启`);
    } catch (e) {
      console.warn(`[enrichInfluencerProfiles] Chrome ${port} 直接重启失败: ${e.message}`);
    }
  }
    ...
    try {
      retried = await ttLiteModule.extractTiktokProfileLite(
        ...
        String(record.username || "").replace(/^@/, ""),
        {
          ...
          emailGateEnabled: importEmailGateOff ? false : undefined,
        }
      );
    } catch (e) {
      console.warn(
  ...
  /**
   * item_list 空结果时的首选恢复：轮换 tk-ip IP（0 数据流量）但保留旧 tab 直接重试。
   * 轮换重启 mihomo 后旧 tab 连接栈可能断（Failed to fetch）——
   * 重试结果会带 itemListError（network/timeout），上层据此再决定是否重建 tab。
   */
  async function retryTiktokEnrichRotateKeepTab(record, slot, ttLiteModule, {
    deadlineMs = 90000,
  } = {}) {
    if (!ttLitePagePool.length || slot < 0 || slot >= ttLitePagePool.length) return null;
    const old = ttLitePagePool[slot];
    if (!old?.page) return null;
    const endpointKey =
      old?.page?._ttApiSessionKey?.split("#")[0] ||
      old?.endpointKey ||
      ttLiteEnrichEndpoint ||
    ...
    try {
      ...
      if (!rot?.ok || rot?.skipped) {
        console.warn(
          `[enrichInfluencerProfiles] 任务中途轮换 IP（保留 tab）未生效: ${rot?.error || "skipped"}`
        );
        return null;
      }
      console.log(
        `[enrichInfluencerProfiles] 任务中途轮换 IP（保留 tab）ok sid=${rot.sid || "-"} ip=${rot.ip || "-"}`
      );
      try {
        const st = getTkIpSessionState(endpointKey);
        st.healthy = false;
    ...
    await sleep(3000); // mihomo 重启后连接栈稳定
    let retried = null;
    const tRetry = Date.now();
    try {
      retried = await ttLiteModule.extractTiktokProfileLite(
      ...
      console.warn(
        `[enrichInfluencerProfiles] 轮换保留 tab 重试异常: ${e.message}`
      );
      retried = null;
    }
    const retryMs = Date.now() - tRetry;
    if (retryMs > deadlineMs) {
      console.warn(
        `[enrichInfluencerProfiles] 轮换保留 tab 重试超预算 ${retryMs}ms > ${deadlineMs}ms，标记失败`
      );
      retried = null;
    }
    return retried;
  ...
  // 单任务中途轮换预算：搜索任务默认 ≤2 次、间隔 ≥60s；
  // 导入任务放宽（默认 ≤20 次、间隔 ≥30s，可 env 覆盖）——导入恢复链用户已确认不设硬上限。
  const isImportEnrich = importTaskIdNum > 0;
  const midTaskRotationMax = Math.max(
    2,
    Number(
      process.env.TT_ENRICH_MID_ROTATION_MAX ??
        (isImportEnrich ? 20 : 2)
    ) || 2
  );
  const midTaskRotationIntervalMs = Math.max(
    10000,
    Number(
      process.env.TT_ENRICH_MID_ROTATION_INTERVAL_MS ??
        (isImportEnrich ? 30000 : 60000)
    ) || 60000
  );
  let midTaskIpRotations = 0;
  let lastMidTaskIpRotationAt = 0;
  const canRotateMidTask = () =>
    midTaskIpRotations < midTaskRotationMax &&
    Date.now() - lastMidTaskIpRotationAt >= midTaskRotationIntervalMs;
  const markMidTaskRotation = () => {
    midTaskIpRotations += 1;
    lastMidTaskIpRotationAt = Date.now();
  };
  ...
  const processOneRecord = async (record, index, batchTotal) => {
    ...
    // 协作式轮换：每红人之间检查并换 IP（keep_alive=3min），不中断任务、不重跑
    try {
      const { cooperativeRotateIfStale } = await import(
        "../../ops/tiktok-session-manager.js"
      );
      await cooperativeRotateIfStale({ label: "enrich" });
    } catch {
      /* ignore */
    }
    const budgetMs =
    ...
    const run = async () => {
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!ttLitePage || !isTtLitePageAlive(ttLitePage)) {
          ...
          try {
            ...
            // - EMPTY/403（数据层被拦）→ 1) 轮换 IP 保留 tab 重试（0 额外流量）；
            //   2) tab 断/仍空 → 重建 tab 再试；3) 还不行 → 该红人标记未完成。
            // - timeout/network/tab_closed（卡死/断连）→ 1) 重建 tab（同 IP）重试；
            //   2) 仍失败且轮换预算允许 → 轮换 IP + 新 tab；3) 还不行 → 标记未完成。
            // 单任务中途轮换 ≤2 次，两次间隔 ≥60s。
            if (!profileData?.success && profileData?.error === "profile_videos_empty") {
              ...
              const errType = String(profileData.itemListError || "empty");
              if (!perInfluencerAborted && remaining > 25_000) {
                if (errType === "empty" && canRotateMidTask()) {
                  // 数据层空结果（IP 风控）：直接轮换 IP + 新建 tab。
                  // 旧 tab 的 cookie/acrawler 绑定旧 IP，保留 tab 换 IP 后仍被风控，
                  // 跨 IP 实测 85% 空结果在换 IP + 新会话后能正常拉取视频。
                  console.warn(
                    `[enrichInfluencerProfiles] [${progress}] @${record.username} item_list 空，轮换 IP + 新 tab 重试（剩余预算 ${Math.round(remaining / 1000)}s）...`
                  );
                  markMidTaskRotation();
                  const retried = await retryTiktokEnrichOnFreshTab(
                    ...
                    slot,
                    ttLiteModule,
                    { rotateIp: true, deadlineMs: remaining }
                  );
                  if (retried?.success) {
                    profileData = retried;
                  } else if (retried?.error) {
                    profileData = { ...retried, error: `item_list_ip_tab_retry_${retried.error}` };
                  }
                } else {
                  // 卡死/超时/网络类，或轮换预算/冷却不足：先重建 tab（同 IP）重试
                  console.warn(
                    `[enrichInfluencerProfiles] [${progress}] @${record.username} ${errType === "empty" ? "轮换预算/冷却限制" : "item_list " + errType}，换新 tab 重试（剩余预算 ${Math.round(remaining / 1000)}s）...`
                  );
                  const retried = await retryTiktokEnrichOnFreshTab(
                    record,
                    slot,
                  ...
                  if (retried?.success) {
                    ...
                    console.warn(
                      `[enrichInfluencerProfiles] [${progress}] @${record.username} 新 tab 仍失败，轮换 IP + 新 tab 重试...`
                    );
                    markMidTaskRotation();
      ...
      if (profileData.success) {
        ...
        failedCount++;
        // 页面卡死/网络类失败：请求 guard 重启该端口 Chrome，避免后续红人持续失败
        if (
          /timeout|network|tab_closed|unresponsive|Runtime\.evaluate/i.test(
            String(profileData?.itemListError || profileData?.error || "")
          )
        ) {
          const slot0 = index % Math.max(ttLitePagePool.length, 1);
          const ep =
            ttLitePagePool[slot0]?.page?._ttApiSessionKey?.split("#")[0] ||
            ttLitePagePool[slot0]?.endpointKey ||
            ttLiteEnrichEndpoint ||
            process.env.CDP_ENDPOINT_ENRICH ||
            process.env.CDP_ENDPOINT ||
            "http://127.0.0.1:9223";
          await requestTiktokChromeRestart(
            ep,
            `enrich_${String(profileData?.itemListError || profileData?.error || "").slice(0, 60)}`
          );
        }
      ...
      if (profileData.success && mergedRecord.username) {
        try {
          ...
          if (dbSaveResult.success) {
            ...
            // 画像语言写旁路小表（tiktok_influencer_language）：不动千万级大表，失败也不影响主流程。
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  try {
    ...
    // 协作式轮换：搜索完成后、预过滤前换一次 IP（长任务每阶段边界刷新，不打断搜索）
    try {
      const { cooperativeRotateIfStale } = await import(
        "../../ops/tiktok-session-manager.js"
      );
      await cooperativeRotateIfStale({ label: "搜索后" });
    } catch {
      /* ignore */
    }
