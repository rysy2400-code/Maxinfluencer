            ...
            if (!profileData?.success && profileData?.error === "profile_videos_empty") {
              ...
              if (!perInfluencerAborted && remaining > 25_000) {
                if (errType === "empty" && canRotateMidTask()) {
                  // 数据层空结果：先轮换 IP 保留 tab（0 额外流量）
                  console.warn(
                    `[enrichInfluencerProfiles] [${progress}] @${record.username} item_list 空，轮换 IP 保留 tab 重试（剩余预算 ${Math.round(remaining / 1000)}s）...`
                  );
                  markMidTaskRotation();
                  const retried = await retryTiktokEnrichRotateKeepTab(
                    record,
                    slot,
                    ttLiteModule,
                  ...
                  if (retried?.success) {
                    profileData = retried;
                  } else if (retried?.itemListError === "timeout" || retried?.itemListError === "network" || retried?.itemListError === "tab_closed") {
                    // 轮换后旧 tab 已断 → 重建 tab（新 IP 上）重试
                    console.warn(
                      `[enrichInfluencerProfiles] [${progress}] @${record.username} 轮换后 tab 已断（${retried.itemListError}），重建 tab 重试...`
                    );
                    ...
                    if (retried2?.success) {
                      ...
                      profileData = { ...retried2, error: `item_list_tab_retry_${retried2.error}` };
                    }
                  } else if (retried?.error) {
                    // 轮换保留 tab 仍空 → 重建 tab 再试
                    console.warn(
                      `[enrichInfluencerProfiles] [${progress}] @${record.username} 轮换保留 tab 仍失败，重建 tab 再试...`
                    );
                    const retried2 = await retryTiktokEnrichOnFreshTab(
                      record,
                      slot,
                    ...
                    // 轮换本身失败 → 降级为同 IP 重建 tab 重试
                    console.warn(
                      `[enrichInfluencerProfiles] [${progress}] @${record.username} 轮换未生效，降级重建 tab 重试...`
                    );
                    const retried2 = await retryTiktokEnrichOnFreshTab(
                      record,
                      slot,
