...
function withPageEvaluateLock(page, fn) {
  if (resolveIgLiteDisableEvaluateLock()) {
    return fn();
  }
...
export async function igApiFetch(page, pathWithQuery, options = {}) {
  ...
  try {
    const json = await withPageEvaluateLock(page, () =>
      page.evaluate(
        async ({ pathWithQuery, options, creds, referer }) => {
        ...
        { pathWithQuery, options, creds, referer }
      )
    );
    if (!json || json.__error) {
      console.warn(
...
export async function fetchIgAboutCountryBloks(page, userId, options = {}) {
  ...
  try {
      ...
      page.evaluate(
        async ({ url, body, templateHeaders, referer }) => {
          const headers = {
            ...(templateHeaders || {}),
            "Content-Type":
              templateHeaders?.["content-type"] ||
              templateHeaders?.["Content-Type"] ||
              "application/x-www-form-urlencoded;charset=UTF-8",
            Accept: templateHeaders?.accept || templateHeaders?.Accept || "*/*",
            Referer: referer,
          };
          delete headers.host;
          delete headers.Host;
    ...
    return {
      ...
      error: accountCountryIso ? null : "about_bloks_country_not_found",
      templateAvailable: true,
    };
  } catch (e) {
    return {
...
export async function fetchUserClipsPage(page, userId, options = {}) {
  ...
  if (!isLiteScraperMode() && !options.skipCapture && handle) {
    try {
    ...
    console.warn(
      `[instagram-direct] Lite Reels GraphQL 失败，不打开 /reels/ 页: userId=${uid}`
    );
  }
