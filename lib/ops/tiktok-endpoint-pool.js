...
export function renderBaseConfig(nodes) {
  ...
  const priorityRaw = String(process.env.TT_LITE_PROXY_NODE_PRIORITY || "");
  ...
  const groupEntries = ordered.map((n) => `      - "${n.name}"`).join("\n");
...
export function renderEnrichConfig(baseYaml, proxyPort, controllerPort, nodeName) {
  return baseYaml
    ...
    // 规则走组（TikTokProxy），失败时可经组选择切换节点；nodeName 仅作为默认首选。
...
export async function rebuildTiktokEndpointPool({ wanted = 3, verify = true } = {}) {
  ...
  // 端点尽量分散到不同主机/IP，避免全部端点共用同一节点 IP 被 TikTok 限流。
  const pinned = diversifyByHost(healthy, wanted);
  ...
  // 端口 LISTENING 不代表代理已就绪，先给 mihomo 预热再探测，失败重试（容忍瞬时抖动）。
  await new Promise((r) => setTimeout(r, 8000));
  const probeResults = {};
  for (const port of ports) {
    probeResults[port] = await probeProxyPort(port);
    for (let retry = 0; retry < 3 && !probeResults[port].ok; retry += 1) {
...
/** 按主机轮转挑选 pinned 节点，让端点尽量落在不同 IP 上 */
function diversifyByHost(nodes, wanted) {
  const byHost = new Map();
  for (const n of nodes) {
    const key = String(n.server || "").toLowerCase();
    if (!byHost.has(key)) byHost.set(key, []);
    byHost.get(key).push(n);
  }
  const hosts = [...byHost.keys()];
  const pinned = [];
  let idx = 0;
  while (pinned.length < wanted && pinned.length < nodes.length && hosts.length) {
    const host = hosts[idx % hosts.length];
    const list = byHost.get(host);
    if (list && list.length) {
      pinned.push(list.shift());
      idx += 1;
    } else {
      hosts.splice(idx % hosts.length, 1);
    }
  }
  return pinned;
}
