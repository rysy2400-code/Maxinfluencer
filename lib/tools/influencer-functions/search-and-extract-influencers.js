...
async function checkTikTokLoginStatus(page) {
  try {
    console.log('[checkTikTokLoginStatus] 开始检测登录状态...');
    ...
    // 检查 URL 是否包含登录相关路径
    const url = page.url();
    console.log(`[checkTikTokLoginStatus] 当前 URL: ${url}`);
    if (url.includes('/login') || url.includes('/signup')) {
      console.log('[checkTikTokLoginStatus] ❌ 检测到登录/注册页面 URL');
      return false;
    }
    ...
    for (const selector of loginButtonSelectors) {
      try {
        ...
        if (loginButton) {
          console.log(`[checkTikTokLoginStatus] ⚠️  检测到登录按钮: ${selector}`);
          break;
        }
      } catch (e) {
        // 忽略选择器错误
    ...
    for (const selector of loginPromptSelectors) {
      try {
        loginPrompt = await page.$(selector);
        if (loginPrompt) {
          console.log(`[checkTikTokLoginStatus] ⚠️  检测到登录提示: ${selector}`);
          break;
        }
      } catch (e) {
        // 忽略选择器错误
    ...
    // 3. 检查是否存在用户头像或用户菜单（已登录的标识）
    const loggedInSelectors = [
      '[data-e2e="top-nav-avatar"]',
      '[data-e2e="profile-icon"]',
      '[data-e2e="user-menu"]',
      'a[href*="/@"]:has([data-e2e="top-nav-avatar"])',
      '[data-e2e="user-menu-container"]',
      'div[data-e2e="user-menu"]',
      'a[href^="/@"]' // 用户主页链接（但需要排除搜索结果中的链接）
    ];
    ...
    for (const selector of loggedInSelectors) {
      try {
        ...
        if (element) {
          // 检查元素是否在顶部导航栏（排除搜索结果中的用户链接）
          const boundingBox = await element.boundingBox();
          if (boundingBox && boundingBox.y < 100) { // 顶部导航栏通常在页面顶部
            if (selector.includes('avatar') || selector.includes('profile-icon')) {
              userAvatar = element;
              console.log(`[checkTikTokLoginStatus] ✅ 检测到用户头像: ${selector}`);
            } else if (selector.includes('menu')) {
              userMenu = element;
              console.log(`[checkTikTokLoginStatus] ✅ 检测到用户菜单: ${selector}`);
            }
            break;
          }
        }
    ...
    const hasLoginText = /log\s*in|sign\s*in|登录|登\s*录/i.test(pageContent);
    const hasUserContent = /@[\w.]+|profile|following|followers/i.test(pageContent);
    
    console.log(`[checkTikTokLoginStatus] 页面内容检查: hasLoginText=${hasLoginText}, hasUserContent=${hasUserContent}`);
    
    // 判断逻辑：更严格
    // 1. 如果存在登录按钮或登录提示，且不存在用户头像/菜单，则未登录
    if ((loginButton || loginPrompt) && !userAvatar && !userMenu) {
      console.log('[checkTikTokLoginStatus] ❌ 检测结果: 未登录（存在登录按钮/提示，且无用户头像/菜单）');
      return false;
    }
    
    // 2. 如果存在用户头像或用户菜单，则已登录
    if (userAvatar || userMenu) {
      console.log('[checkTikTokLoginStatus] ✅ 检测结果: 已登录（存在用户头像/菜单）');
      return true;
    }
    
    // 3. 如果页面有登录文本但没有用户相关内容，可能未登录
    if (hasLoginText && !hasUserContent) {
      console.log('[checkTikTokLoginStatus] ❌ 检测结果: 未登录（页面有登录文本但无用户内容）');
      return false;
    }
    
    // 4. 如果无法确定，默认认为未登录（更严格，避免误判）
    console.log('[checkTikTokLoginStatus] ⚠️  无法确定登录状态，默认返回: 未登录');
    return false;
  } catch (error) {
    console.warn('[checkTikTokLoginStatus] 检测登录状态失败:', error.message);
    console.warn('[checkTikTokLoginStatus] 默认返回: 未登录（避免误判）');
    // 如果检测失败，默认返回未登录（更严格）
    return false;
  }
}
 ...
 */
async function reportScreenshot(onStepUpdate, stepId, label, base64Image) {
  if (!onStepUpdate || !base64Image) return;
...
export async function searchInfluencersByKeyword(params = {}, options = {}) {
  ...
  if (USE_HEADLESS) {
    ...
    // 为了避免“第一页不是 TikTok 页面”导致误判：遍历全部 TikTok Tab 检测登录态
    const pages = context.pages();
    if (pages.length > 0) {
      console.log(`[searchInfluencersByKeyword] ✅ 检测 context 页面数: ${pages.length}`);
      const tiktokPages = pages.filter((p) => {
        try {
          const u = p.url();
          return typeof u === "string" && u.includes("tiktok.com");
        } catch {
          return false;
        }
      });
      ...
      if (tiktokPages.length === 0) {
        console.warn(`[searchInfluencersByKeyword] ⚠️ 未发现 TikTok tab，创建临时 TikTok 页面做登录检测`);
        const probePage = await context.newPage();
        try {
          await probePage.goto("https://www.tiktok.com/", { waitUntil: "domcontentloaded", timeout: 30000 });
          const loggedIn = await checkTikTokLoginStatus(probePage);
          if (!loggedIn) {
            throw new Error("检测到 TikTok 未登录（请在手动启动的 Chrome 中先登录 TikTok）");
          }
        } finally {
        ...
        for (let i = 0; i < tiktokPages.length; i++) {
          const p = tiktokPages[i];
          const u = p.url();
          console.log(`[searchInfluencersByKeyword] 🔎 检测 TikTok tab[${i + 1}/${tiktokPages.length}]: ${u}`);
          try {
            const loggedIn = await checkTikTokLoginStatus(p);
            if (loggedIn) {
              anyLoggedIn = true;
              console.log(`[searchInfluencersByKeyword] ✅ TikTok tab[${i + 1}] 登录有效`);
              break;
            }
          } catch (e) {
            console.warn(`[searchInfluencersByKeyword] ⚠️ TikTok tab[${i + 1}] 检测异常: ${e.message}`);
          }
        }
        if (!anyLoggedIn) {
          throw new Error("检测到 TikTok 未登录（请在手动启动的 Chrome 中先登录 TikTok）");
      ...
      console.warn(`[searchInfluencersByKeyword] ⚠️ 当前 context 无现有页面，创建临时 TikTok 页面检测登录态`);
      const probePage = await context.newPage();
      try {
        await probePage.goto("https://www.tiktok.com/", { waitUntil: "domcontentloaded", timeout: 30000 });
        const loggedIn = await checkTikTokLoginStatus(probePage);
      ...
      if (shouldAnalyzeLive && mergedRecord.username) {
        try {
          ...
          const onStreamChunk = (chunk) => {
            ...
            // 实时更新浏览器步骤的 detail，显示流式分析文本
            if (onStepUpdate) {
              reportStep(
                onStepUpdate,
                BROWSER_STEP_IDS.ANALYZE_MATCH,
                STEP_STATUS.RUNNING,
                streamingAnalysisText, // 使用累积的流式文本作为 detail
                {
                  current: index + 1,
                  total: recordsToEnrich.length,
                  analyzing: mergedRecord.username,
                  streamingAnalysis: streamingAnalysisText // 新增：流式分析文本
                }
              );
            }
          };
...
async function searchAndExtractInfluencers_v2(params = {}, options = {}) {
  ...
  try {
    ...
    if (enrichProfileData && finalInfluencerRecords.length > 0) {
      ...
      const enrichEndpoint = process.env.CDP_ENDPOINT_ENRICH || process.env.CDP_ENDPOINT || 'http://localhost:9223';
    ...
    if (campaignId && finalInfluencerRecords.length > 0) {
      try {
        const { upsertCandidatesForCampaign } = await import("../../db/campaign-candidates-dao.js");
        ...
        sendStep('写入候选池失败', `⚠️ 写入候选池失败：${e.message || e}`);
      }
    }
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  const { keywords = {}, platforms = [], countries = [], productInfo = {}, campaignInfo = {}, influencerProfile = null } = params;
  const { 
    maxResults = 20, 
    onStepUpdate = null,
    enrichProfileData = true,  // 是否提取主页数据（默认启用）
