...
粉丝量要求：
播放量要求
视频时长要求（如「长视频教程，30-60 分钟」）
红人账户类型要求：

请告诉我你对红人类型、粉丝量、播放量和视频时长的具体要求，或者说「没有特别要求，按你建议来」，我会据此整理出一版完整的红人画像供你确认。`;
        return {
          reply,
          influencerProfile,
          influencers: [],
        ...
        if (!profileComplete) {
          return {
            reply: `当前红人画像还有信息未明确：\n\n${this.formatProfileSummary(
              existingProfile
            )}\n\n请补充粉丝量、播放量和视频时长要求；不设限制可直接说“无要求”。`,
            influencerProfile: existingProfile,
            influencers: [],
            isConfirmed: false,
            meta: { influencerStep: "profile_only" },
...
{ "intent": "confirm" | "provide_or_update" | "unclear" }`;
  ...
  async buildProfileFromConversation(
    ...
    const prompt = `根据对话整理完整红人画像，输出 followerRange、viewRange、videoDuration、accountType 四个字段。
...
只返回 JSON：
{
  ...
  "videoDuration": "无要求" | "长视频教程 30-60 分钟" 等,
  "accountType": "账户类型描述"
}`;
    ...
    try {
      ...
      return {
        ...
        videoDuration: this.normalizeProfileField(
          parsed.videoDuration,
          existingProfile?.videoDuration
        ),
        accountType:
      ...
      return {
        ...
        videoDuration: existingProfile?.videoDuration ?? null,
