...
import { closeYoutubeEnrichTabIfNeeded } from "./cdp-page-utils.js";
import {
...
export async function extractYoutubeChannelFromPageCDP(page, username, options = {}) {
  ...
  if (needAbout) {
    ...
    let aboutPage = null;
    let openedAboutTab = false;
    try {
      aboutPage = await page.context().newPage();
      openedAboutTab = true;
      await aboutPage.goto(aboutUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
      await aboutPage.waitForTimeout(2500);
      const aboutData = await readYtInitialDataFromPage(aboutPage);
      if (aboutData) {
        let aboutMeta = null;
        ...
        if (aboutMeta) {
          ...
          console.log(
            `[extractYoutubeChannel] /about country=${countryText || "(空)"} subs=${subs?.display || "(无)"} email=${emailFromAbout || "(无)"}`
          );
        }
      }
    } catch (e) {
      ...
      if (openedAboutTab) {
        await closeYoutubeEnrichTabIfNeeded(aboutPage, { force: true });
      }
    }
  }
