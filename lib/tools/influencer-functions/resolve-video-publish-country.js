...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  // Lite：搜索 API 已带 locationCreated，无需再打开视频详情页（省流量）
  if (isLiteScraperMode()) {
    ...
    sendStep(
      "采集视频发布地",
      `Lite：从搜索 API 读取 ${queue.length} 位红人视频发布地（无需打开视频页）...`
    );
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const locationCreated = srcVideo?.locationCreated || null;
      ...
      await saveVideoPublishCountry({
        ...
        representativeVideoId: record.representativeVideoId || srcVideo?.videoId || null,
        locationSource: "search_api",
      });
      ...
      const outcome = {
        username,
        video_publish_country: locationCreated,
        representative_video_id: record.representativeVideoId || srcVideo?.videoId || null,
        enrich_skipped_reason: enrichSkippedReason,
      ...
      console.log(
        `[resolveVideoPublishCountry] [${progress}] ${flag} @${username} locationCreated=${locationCreated ?? "null"} (lite search_api)${enrichSkippedReason ? ` (${enrichSkippedReason})` : ""}`
      );
    }

    sendStep(
      "采集视频发布地",
      `Lite 发布地完成：检查 ${checked.length} 人，符合 campaign 国家 ${passed.length} 人`
    );
