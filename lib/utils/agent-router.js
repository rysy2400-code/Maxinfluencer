...
import { prefillCampaignBrief } from "../agents/campaign-brief-intake.js";
...
import { getCampaignBySessionId } from "../db/campaign-dao.js";
import { runExecutionHeartbeatTick } from "../heartbeat/execution-heartbeat.js";
import { isCampaignInfoComplete } from "../campaign/influencer-pricing.js";
...
export class AgentRouter {
  ...
  async runNextStageAgent(toolName, messages, context, thinking, sendThinkingUpdate) {
    ...
    if (toolName === "influencer_profile_agent") {
      ...
      const influencerResult = await this.influencerProfileAgent.recommendInfluencers(
        messages,
        context,
        influencerStepUpdate,
        { enteringStage: true }
      );
    ...
    if (toolName === "content_requirement_agent") {
      const contentResult = await this.contentRequirementAgent.generateContent(messages, context, (progress) => {
        sendThinkingUpdate({
          steps: [...thinking.steps],
          contentRequirementProgress: {
    ...
    if (toolName === "campaign_publish_agent") {
      ...
      const nextWorkflowState = publishResult.published
        ? "published"
        : WORKFLOW_STATES.STEP_5_PUBLISH_CONFIRM;
      const newContext = {
        ...context,
      ...
      if (publishResult.published) {
        await this.persistPublishedSession(context, publishResult, newContext);
      }
  ...
  /**
   * 发布成功后将 tiktok_campaign_sessions 标为 published，并触发一次执行心跳。
   * 主流程与链式调用（如内容确认后自动进入发布）均需调用，否则侧栏仍显示在草稿区。
   */
  async persistPublishedSession(context, publishResult, newContext) {
    try {
      const sessionId = context.sessionId || newContext?.sessionId;
      if (!publishResult?.published || !sessionId) {
        if (publishResult?.published && !sessionId) {
          console.warn(
            "[AgentRouter] 发布成功但请求未带 sessionId，无法将会话从草稿移到「已发布」；请检查前端 /api/chat 是否传递 sessionId"
          );
        }
        return;
      }
      const upd = await promoteSessionToPublished(sessionId, {
        context: newContext,
        title: publishResult.sessionTitle || undefined,
        campaignId: publishResult.campaignId || newContext.campaignId,
      });
      if (!upd.success) {
        console.warn("[AgentRouter] 会话标记 published 失败:", upd.message);
      }
      try {
        await runExecutionHeartbeatTick(new Date());
      } catch (hbErr) {
        console.warn(
          "[AgentRouter] 发布后触发 execution heartbeat 失败:",
          hbErr?.message || hbErr
        );
      }
    } catch (e) {
      console.warn("[AgentRouter] 更新会话状态异常:", e?.message || e);
    }
  }
  ...
  async process(messages, context = {}, onThinkingUpdate = null) {
    try {
      if (!context.published && !context.campaignId) {
        context = await prefillCampaignBrief(messages, context);
      }
      // 初始化思考过程
      const thinking = {
        steps: [],
      ...
      // 须与前端「执行阶段」判定一致：workflowState===published 或 published===true（避免仅其一缺失时误走 BinAgent、工具不落库）
      const isExecutionMode = !!(
        context.campaignId &&
        (context.published === true || context.workflowState === "published")
      );
      const intentAgentName = isExecutionMode ? "CampaignExecutionAgent" : "BinAgent";

      // 感知提速：LLM 意图识别可能耗时数秒到数十秒，先推一条 thinking，
      // 避免用户对着空白气泡等待；调度器流式返回时会持续更新该步骤。
      const receivingStep = {
        agent: "AgentRouter",
        action: "意图识别",
        result: "正在分析你的请求…",
      ...
      thinking.steps.push(receivingStep);
      sendThinkingUpdate({ steps: [...thinking.steps] });

      let binResult;
      if (isExecutionMode) {
        binResult = await this.campaignExecutionAgent.processWithTools(messages, context, {
          onSchedulerProgress: (text) => {
            const last = thinking.steps[thinking.steps.length - 1];
            // 仅更新「意图识别」进度步，避免覆盖后续步骤
            if (last && last.action === "意图识别") {
        ...
        binResult = await this.binAgent.processWithTools(messages, context);
      }
      ...
      if (binResult.toolCall) {
        ...
        if (CAMPAIGN_EXECUTION_TOOLS.has(toolName)) {
          ...
          if (execSessionId) {
            try {
              const bySession = await getCampaignBySessionId(execSessionId);
              if (bySession?.id) execCampaignId = bySession.id;
            } catch (e) {
              console.warn("[AgentRouter] 按 session 解析 campaignId 失败:", e?.message || e);
            }
          }

          const execContext = {
            campaignId: execCampaignId || params.campaignId,
            sessionId: execSessionId || context.sessionId || null,
            advertiserAuth: context.advertiserAuth || null,
            influencerAgentClient: context.influencerAgentClient || null,
            /** 供 ask_influencer_special_request 等工具补全 requestDetail */
            lastUserMessage: messages[messages.length - 1]?.content || "",
            campaignSnapshotHint: binResult.campaignSnapshotHint || null,
          };
          ...
          return {
            ...
            context: {
              ...context,
              ...(execCampaignId ? { campaignId: execCampaignId } : {}),
            },
            thinking,
          };
        }
        ...
        if (toolName === "product_info_agent") {
          ...
          if (context.campaignInfo && currentWorkflowState === WORKFLOW_STATES.STEP_2_CAMPAIGN_INFO) {
            ...
            const hasAllFields = isCampaignInfoComplete(campaignInfo);
          ...
          if (campaignResult.campaignInfo) {
            const campaignInfo = campaignResult.campaignInfo;
            hasAllFields = isCampaignInfoComplete(campaignInfo);
          }
          ...
          if (currentWorkflowState === WORKFLOW_STATES.STEP_3_INFLUENCER_PROFILE) {
            // 如果已经在步骤 3
            if (isConfirming && influencerResult.influencerProfile) {
              // 用户确认红人画像，直接进入发布确认；内容阶段保留但不再是必经步骤。
              nextWorkflowState = WORKFLOW_STATES.STEP_5_PUBLISH_CONFIRM;
