...
6. 若最后一条消息 @某红人 并**询问/协商/回复/同步消息**（如「帮我回复达 @xxx」「把这段话发给 @xxx」「问他能否接受某价格/做几条视频/改时间/给具体报价」）→ 必须 needTool=true，toolName=ask_influencer_special_request，params.influencerId=handle（无 @），params.requestDetail=用户要求的完整文案（原样保留，不要改写），requestType 按语义选 adjust_price / delay_publish / change_content / other。
 ...
 * @param {Object} deps - 可注入 { callLlm } 便于测试
...
export async function decideExecutionSchedulerTurn(
  ...
  const raw = await callLlm(
    ...
    "你是指令执行专家，只输出 JSON。用户要求暂停/恢复/完成 campaign 时必须 needTool=true 且使用 set_campaign_status；@红人回复/询问/协商时必须 ask_influencer_special_request；红人名单确认后必须 import_influencer_list。"
  );
...
- 若用户要暂停但当前已是 paused，或要恢复但当前已是 running/running_passive：仍调用 set_campaign_status 对应 action（工具会返回幂等说明）。`;
    ...
    try {
      ...
      if (decision.needTool && decision.toolName && TOOL_NAMES.includes(decision.toolName)) {
        ...
        if (
          decision.toolName === "ask_influencer_special_request" &&
          typeof params.influencerId === "string"
        ) {
