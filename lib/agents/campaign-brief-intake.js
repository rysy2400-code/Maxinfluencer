...
const PROFILE_FIELDS = ["followerRange", "viewRange", "accountType"];
...
function parseJsonObject(value) {
  ...
  try {
    ...
    return parseFirstJsonObject(value);
  }
}
...
5. region 可多选；budget 为美元数字；commission 为百分比数字。用户说「不主动报价」「不报价」「询问红人报价/价格」「让红人/博主报价」「请红人/博主提供报价」「不要按 eCPM 报价」时 mode=ask_creator_quote；用户说「按 eCPM/ecpm」时 mode=ecpm_with_cap（ecpmUsd/maxFlatFeeUsd 按用户说的数字填）；用户说「纯产品置换/置换」时 commission=0 且 mode=commission_only（无需额外出现 0%）。
6. 红人粉丝量、播放量、账户类型保持用户原意。用户明确说“不限/无要求”时填“无要求”。
6b) 红人画像只有 followerRange、viewRange、accountType 三个字段：用户提到的互动率、点赞率、评论率、完播率等其它筛选要求，必须原样并入 accountType（与同句中的基地/城市/垂类要求合并成一段，不要拆句、不要丢字），**禁止**新增 engagementRate / interactionRate 等字段，也不要丢弃该要求。用户只单独说互动率时，accountType 填用户原话即可，由后续画像阶段合并。
...
只返回 JSON：
{
  "campaignInfo": {
    ...
    "budget": 10000 | null,
    "commission": 15 | null,
    "influencerPricing": { "mode": "ask_creator_quote" | "ecpm_with_cap" | "commission_only", "ecpmUsd": 3 | null, "maxFlatFeeUsd": 1000 | null } | null,
    "deliverables": "明确交付要求" | null
  },
  "influencerProfile": {
    "followerRange": "用户原意" | null,
    "viewRange": "用户原意" | null,
    "accountType": "用户原意（账户类型/内容方向；互动率、点赞率等其它筛选要求也原样并入此字段）" | null
  },
  "ambiguities": [
  ...
  // 用户说「不主动报价/询问红人价格/让红人报价」→ mode=ask_creator_quote；
  // 「纯产品置换/置换/无固定费用+无佣金」→ mode=commission_only、commission=0。
