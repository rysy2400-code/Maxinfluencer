...
function needsInboundAttachmentEnrich(item) {
  if (!item || item.eventType !== "email_inbound") return false;
  if (item.sourceType !== "influencer_email_event" || !item.sourceEventId) return false;
  const items = item.payloadSafe?.attachments?.items;
  return !Array.isArray(items) || items.length === 0;
}

async function enrichTimelineItemsWithInboundAttachments(items) {
  if (!Array.isArray(items) || !items.length) return items;

  const eventIds = [
    ...new Set(
      items
        .filter(needsInboundAttachmentEnrich)
        .map((item) => Number(item.sourceEventId))
        .filter((id) => id > 0)
    ),
  ];
  if (!eventIds.length) return items;

  const attMap = await listInboundAttachmentsByEmailEventIds(eventIds);
  return items.map((item) => {
    if (!needsInboundAttachmentEnrich(item)) return item;
    const atts = attMap.get(Number(item.sourceEventId)) || [];
    if (!atts.length) return item;
    return {
      ...item,
      payloadSafe: {
        ...(item.payloadSafe || {}),
...
export async function listTimelineEvents({
  ...
  items = await enrichTimelineItemsWithInboundAttachments(items);
