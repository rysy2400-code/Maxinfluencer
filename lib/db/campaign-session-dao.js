...
export async function getAllCampaignSessions(options = {}) {
  try {
    ...
    let sql = `
      SELECT id, title, status${includeMessages ? ', messages, context' : ''}, published_user_hidden_at, created_at, updated_at
      FROM ${SESSION_TABLE}
    `;
    const params = [];
...
export async function getCampaignSessionById(sessionId) {
  try {
    ...
    const sql = `
      SELECT id, title, status, messages, context, published_user_hidden_at, created_at, updated_at
      FROM ${SESSION_TABLE}
      WHERE id = ?
    `;
