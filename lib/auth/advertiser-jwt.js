import * as jose from "jose";

const COOKIE_NAME = "maxin_advertiser_auth";
const INBOX_COOKIE_NAME = "maxin_influencer_inbox_auth";
const MAX_AGE_SEC = 30 * 24 * 60 * 60; // 30 天

function getSecretKey() {
  const s = process.env.AUTH_JWT_SECRET;
  if (!s || s.length < 16) {
    if (process.env.NODE_ENV === "production") {
      throw new Error("AUTH_JWT_SECRET 未设置或过短（生产环境必填）");
    }
    return new TextEncoder().encode("dev-only-insecure-secret-change-me");
  }
  return new TextEncoder().encode(s);
}

export { COOKIE_NAME, INBOX_COOKIE_NAME, MAX_AGE_SEC };

/**
 * @param {{ advertiserUserId: number, advertiserId: number, username: string, companyName: string, isAdmin: boolean, actingAsUserId?: number | null }} claims
 */
export async function signAdvertiserToken(claims) {
  const key = getSecretKey();
  const payload = {
    aid: claims.advertiserId,
    un: claims.username,
    cn: claims.companyName,
    adm: claims.isAdmin ? 1 : 0,
  };
  const act = claims.actingAsUserId;
  if (act != null && Number.isFinite(Number(act))) {
    payload.act = Number(act);
  }
  return new jose.SignJWT(payload)
    .setProtectedHeader({ alg: "HS256" })
    .setSubject(String(claims.advertiserUserId))
    .setIssuedAt()
    .setExpirationTime(`${MAX_AGE_SEC}s`)
    .sign(key);
}

/** @returns {Promise<{ advertiserUserId: number, advertiserId: number, username: string, companyName: string, isAdmin: boolean, actingAsUserId: number | null } | null>} */
export async function verifyAdvertiserToken(token) {
  if (!token || typeof token !== "string") return null;
  try {
    const key = getSecretKey();
    const { payload } = await jose.jwtVerify(token, key);
    const advertiserUserId = parseInt(String(payload.sub), 10);
    const rawAid = payload.aid;
    const advertiserId =
      typeof rawAid === "number" && Number.isFinite(rawAid)
        ? rawAid
        : parseInt(String(rawAid ?? ""), 10);
    if (!Number.isFinite(advertiserUserId) || !Number.isFinite(advertiserId)) return null;
    const rawAct = payload.act;
    const actingAsUserId =
      rawAct != null && Number.isFinite(Number(rawAct)) ? Number(rawAct) : null;
    return {
      advertiserUserId,
      advertiserId,
      username: typeof payload.un === "string" ? payload.un : "",
      companyName: typeof payload.cn === "string" ? payload.cn : "",
      isAdmin: payload.adm === 1 || payload.adm === true,
      actingAsUserId,
    };
  } catch {
    return null;
  }
}

/**
 * 登录 Cookie 是否加 Secure。纯 HTTP（如 http://公网IP）若为 true，浏览器不会发送 Cookie，所有 API 会表现为未登录。
 * 优先读 AUTH_COOKIE_SECURE=0|1；否则若能从 request 解析出协议，则与访问方式一致；最后才按 NODE_ENV。
 * @param {Request} [request] 登录/登出等 Route Handler 的 req，用于解析 url 或 x-forwarded-proto
 */
export function cookieIsSecure(request) {
  if (process.env.AUTH_COOKIE_SECURE === "0") return false;
  if (process.env.AUTH_COOKIE_SECURE === "1") return true;

  if (request && typeof request === "object") {
    try {
      const raw = request.headers?.get?.("x-forwarded-proto");
      if (raw) {
        const first = raw.split(",")[0].trim().toLowerCase();
        if (first === "http") return false;
        if (first === "https") return true;
      }
      const u = request.url;
      if (typeof u === "string" && u.startsWith("http")) {
        return u.startsWith("https:");
      }
    } catch {
      /* ignore */
    }
  }

  return process.env.NODE_ENV === "production";
}
