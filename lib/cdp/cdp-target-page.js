...
import { requestChrome9222Restart } from "./cdp-chrome-restart.js";
...
function recordCdpRpcSuccess() {
  consecutiveRpcTimeouts = 0;
}

function recordCdpRpcTimeout(method) {
  const threshold = Math.floor(Number(process.env.CDP_RPC_TIMEOUTS_BEFORE_RESTART) || 0);
  if (threshold < 1) return;
  consecutiveRpcTimeouts += 1;
  if (consecutiveRpcTimeouts < threshold) return;
  consecutiveRpcTimeouts = 0;
  void requestChrome9222Restart(`rpc_timeout_streak:${method}`).catch(() => {});
}
...
export async function verifyTiktokCdpTargetHealthy(target) {
  const session = new CdpSession(target.webSocketDebuggerUrl);
  try {
    const timeoutMs = Number(process.env.TT_CDP_TARGET_HEALTH_TIMEOUT_MS || 5_000);
    await Promise.race([
      session.connect(),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("TikTok target connect timeout")), timeoutMs)
      ),
    ]);
    const href = await Promise.race([
      session.evaluate(() => location.href),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("TikTok target evaluate timeout")), timeoutMs)
      ),
    ]);
...
export async function pickTiktokCdpTarget(cdpEndpoint) {
  ...
  const ranked = pages
    .filter(
      (t) =>
        String(t.url || "").includes("tiktok.com") &&
        !String(t.url || "").includes("/api/")
    )
...
export async function connectCdpTargetPage(target, options = {}) {
  ...
  const page = {
    ...
    evaluate: (fn, arg) => {
      ...
      return session.evaluate(fn, arg);
    },
    ...
    goBack: async (opts = {}) => {
      ...
      const idx = Number(history.currentIndex || 0);
      const target = entries[idx - 1];
      if (!target) return null;
      await session.send("Page.navigateToHistoryEntry", { entryId: target.id });
      const waitMs = opts.waitUntil === "commit" ? 1000 : 2500;
      await new Promise((r) => setTimeout(r, waitMs));
      try {
        const href = await session.evaluate(() => location.href);
        if (href) currentUrl = href;
      } catch {
        currentUrl = target.url || currentUrl;
      }
      return null;
    },
    reload: async (opts = {}) => {
