...
function resolveCandidateEmail(inf) {
  return (
    ...
    extractEmailFromBio(inf?.bio) ||
    extractEmailFromBio(inf?.profile_data?.userInfo?.bio) ||
    extractEmailFromBio(inf?.profileData?.userInfo?.bio) ||
    null
  );
}
...
export async function bumpSearchTaskEnrichedProgress(taskId, enrichedDelta = 1) {
  const id = Number(taskId || 0);
  const delta = Number(enrichedDelta || 0);
  if (!id || delta <= 0) return;
  await queryTikTok(
    `
    UPDATE tiktok_influencer_search_task
