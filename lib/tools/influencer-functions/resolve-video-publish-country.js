...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  // Lite：9223 signed API 读取代表视频 locationCreated（不打开视频页）
  if (isLiteScraperMode()) {
    const { acquireTiktokApiSession, fetchVideoItemDetail } = await import(
    ...
    const concurrency = Math.max(
      1,
      Number(process.env.TT_LITE_COUNTRY_CONCURRENCY || 2)
    );

    sendStep(
      "采集视频发布地",
      `Lite：9223 API 并发读取 ${queue.length} 位红人视频发布地（无页面导航）...`
    );
    ...
    for (let i = 0; i < concurrency; i += 1) {
      try {
        ...
        console.warn(`[resolveVideoPublishCountry] 9223 session ${i + 1} failed: ${e.message}`);
      }
    }
    if (!pool.length) {
      throw new Error("TikTok Lite 国家检测：9223 API 会话不可用");
    }
    ...
    async function probeLocation(record, sessionIdx) {
      ...
      const videoId = record.representativeVideoId || extractVideoId(record.representativeVideoUrl);
      const srcVideo = videoByUser.get(username);
      if (!videoId) {
        const fromSearch = srcVideo?.locationCreated || null;
        return {
          locationCreated: fromSearch != null ? String(fromSearch) : null,
          source: fromSearch ? "search_api_fallback" : null,
          error: fromSearch ? null : "missing_video_id",
        };
      }
      const page = pool[sessionIdx % pool.length].page;
      try {
        const json = await fetchVideoItemDetail(page, videoId, { username });
        const item =
          ...
          json?.itemList?.[0] ||
          null;
        let locationCreated = item?.locationCreated ?? null;
        if (locationCreated == null && srcVideo?.locationCreated != null) {
          locationCreated = srcVideo.locationCreated;
        }
        return {
          locationCreated: locationCreated != null ? String(locationCreated) : null,
          source: item?.locationCreated != null ? "item_detail_api" : srcVideo?.locationCreated ? "search_api_fallback" : "item_detail_api",
        };
      } catch (e) {
        const fromSearch = srcVideo?.locationCreated || null;
        return {
    ...
    for (let batchStart = 0; batchStart < queue.length; batchStart += concurrency) {
      ...
      const probes = await Promise.all(
        batch.map((record, j) => probeLocation(record, batchStart + j))
      );

      for (let j = 0; j < batch.length; j += 1) {
        const record = batch[j];
        const username = normalizeUsername(record.username);
        const progress = `${batchStart + j + 1}/${queue.length}`;
        ...
        await saveVideoPublishCountry({
          ...
          locationSource: probe.source || "item_detail_api",
        });
        ...
        console.log(
          `[resolveVideoPublishCountry] [${progress}] ${flag} @${username} locationCreated=${locationCreated ?? "null"} (lite item_detail_api)${enrichSkippedReason ? ` (${enrichSkippedReason})` : ""}${probe.error ? ` err=${probe.error}` : ""}`
        );
      }
    }
    ...
    sendStep(
      "采集视频发布地",
      `Lite 9223 发布地完成：检查 ${checked.length} 人，符合 campaign 国家 ${passed.length} 人`
    );
