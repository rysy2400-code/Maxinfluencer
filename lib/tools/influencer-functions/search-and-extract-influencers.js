...
function mergeInfluencerData(searchRecord, profileData) {
  ...
  // 只要存在样本（sampleCount>0），0 也是真实值，不能与“缺失”混为一谈
  const hasTweetSample =
    !!statistics &&
    Number.isFinite(Number(statistics.sampleCount)) &&
    Number(statistics.sampleCount) > 0;
  const statNumber = (v) =>
    Number.isFinite(Number(v)) ? Number(v) : null;
  const avgViewsNum = hasTweetSample ? statNumber(statistics.avgViews) : null;
  const avgLikesNum = hasTweetSample ? statNumber(statistics.avgLikes) : null;
  const avgCommentsNum = hasTweetSample
