...
} from "../../campaign/parse-modify-campaign-message.js";
...
async function modifyCampaign(params, context) {
  ...
  if (changeKeys.length === 0 && ctxLast) {
    const parsedFromMsg = parseModifyCampaignChangesFromUserMessage(ctxLast);
    if (parsedFromMsg) {
      ch = { ...parsedFromMsg };
  ...
  if (ctxLast && !changesIncludeInfluencerPricing(ch)) {
    const pricingPatch = parsePricingChangesFromUserMessage(ctxLast);
    if (pricingPatch) {
      ch = { ...ch, ...pricingPatch };
      changeKeys = Object.keys(ch);
    }
