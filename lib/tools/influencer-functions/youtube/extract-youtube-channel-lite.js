...
const YOUTUBE_ABOUT_EMAIL_SOURCES = new Set([
  "about_description",
  "about_mailto_link",
  "about_external_link",
]);

export function isYoutubeLiteEmailGateEnabled(
  raw = process.env.YT_LITE_REQUIRE_EMAIL_FOR_ANALYSIS
) {
  return String(raw ?? "").trim() !== "0";
}

export function hasYoutubeAboutEmail(userInfo) {
  const email = String(userInfo?.email || "").trim();
  const source = String(userInfo?.aboutEmailSource || "").trim();
  return !!email && YOUTUBE_ABOUT_EMAIL_SOURCES.has(source);
}
...
function buildLowFollowersSkipLiteResult(aboutCtx, userInfo = aboutCtx.userInfo) {
  const resolvedChannelId =
    userInfo?.channelId || userInfo?.userId || aboutCtx.browseId || null;
  const profileUrl =
    aboutCtx.profileUrl ||
    buildChannelPublicUrl(aboutCtx.handle, resolvedChannelId) ||
    userInfo?.profileUrl ||
    null;
  return {
    ...buildCountrySkipLiteResult(
      { ...aboutCtx, userInfo, profileUrl },
      "followers_below_500"
    ),
...
function buildMissingAboutEmailSkipLiteResult(aboutCtx) {
  const resolvedChannelId = aboutCtx.browseId || aboutCtx.channelId || null;
  const userInfo = aboutCtx.userInfo || {
    username: aboutCtx.handle || resolvedChannelId,
    displayName: aboutCtx.handle || resolvedChannelId,
    channelId: resolvedChannelId,
    userId: resolvedChannelId,
    profileUrl: aboutCtx.profileUrl || null,
    bio: "",
    email: null,
...
export async function extractYoutubeChannelLite(page, username, options = {}) {
  ...
  if (
    ...
    !hasYoutubeAboutEmail(aboutCtx.userInfo)
  ) {
    console.log(
      `[extractYoutubeChannelLite] @${handle || channelIdOpt} About 未识别到邮箱，跳过视频 enrich 和 LLM 分析`
    );
    reportStep(
      onStepUpdate,
      BROWSER_STEP_IDS.ENRICH_PROFILES,
      STEP_STATUS.COMPLETED,
      `YouTube Lite @${handle || channelIdOpt} About 无邮箱，跳过后续分析`
    );
    return buildMissingAboutEmailSkipLiteResult(aboutCtx);
  }
  ...
  let firstPageUserInfo = mergeUserInfo(
    aboutCtx.userInfo,
    extractChannelHeaderFromInnertubeJson(first, handle)
  );
  if (!(firstPageUserInfo?.followers?.count >= 0)) {
    const firstPageFollowers = extractSubscriberCountFromInnertubeJson(first);
    if (firstPageFollowers?.count >= 0) {
      firstPageUserInfo = mergeUserInfo(firstPageUserInfo, {
        followers: firstPageFollowers,
      });
    }
  }
  if (shouldSkipYoutubeLiteLowFollowers(firstPageUserInfo)) {
    const followerCount = Number(firstPageUserInfo.followers.count);
    console.log(
      `[extractYoutubeChannelLite] @${handle || channelIdOpt} first-page followers=${followerCount} < ${YOUTUBE_LITE_MIN_FOLLOWERS_FOR_ANALYSIS}, skip pagination, engagement and LLM analysis`
    );
    reportStep(
      onStepUpdate,
      BROWSER_STEP_IDS.ENRICH_PROFILES,
    ...
    return buildLowFollowersSkipLiteResult(aboutCtx, firstPageUserInfo);
  }
