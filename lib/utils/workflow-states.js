...
/**
 * 按当前工作流状态选择 agent。主创建路径为产品→Campaign→红人画像→发布；
 * 内容状态保留，供历史会话和用户明确要求生成内容时使用。
...
export function getNextWorkflowState(currentState) {
  const stateMap = {
    ...
    [WORKFLOW_STATES.STEP_3_INFLUENCER_PROFILE]: WORKFLOW_STATES.STEP_5_PUBLISH_CONFIRM,
...
export function getWorkflowStateDescription(state) {
  const descriptions = {
    ...
    [WORKFLOW_STATES.STEP_4_CONTENT_REQUIREMENT]: "可选 - 确认内容要求",
    [WORKFLOW_STATES.STEP_5_PUBLISH_CONFIRM]: "步骤 4 - 确认发布",
...
export function detectStepKeywords(message) {
  const stepKeywords = {
    ...
    step_2_campaign_info: ["平台", "tiktok", "instagram", "ins", "youtube", "ytb", "x", "twitter", "tw", "地区", "国家", "投放地区", "美国", "德国", "法国", "英国", "预算", "总预算", "佣金", "报价", "ecpm", "固定费", "发布时间", "时间段", "时间范围", "日期", "美元", "usd", "$"],
