...
export function resolveSnapshotCountry(snapshot = {}) {
  ...
  const candidates = [
    snapshot.residenceCountry,
    snapshot.residence_country,
 ...
 *
 * 口径（2026-09 确认）：
 * - 账号国家（平台抓取）已知也要问——首封邮件只补「居住国家」；
 * - 只要居住国家已落库（含跨 campaign 的历史回复）就不再问；
 * - 账号国家/平台国家不作为「已问过」的依据。
 */
export function shouldAskCountryInOutreach({ influencer, executionSnapshot } = {}) {
  const knownResidence =
    normalizeInfluencerCountryToIso(influencer?.residenceCountry) ||
    normalizeInfluencerCountryToIso(influencer?.residence_country) ||
    normalizeInfluencerCountryToIso(executionSnapshot?.residenceCountry) ||
    normalizeInfluencerCountryToIso(executionSnapshot?.residence_country) ||
    null;
...
function withResidenceCountry(snapshot, countryInfo, event) {
  ...
  return {
    ...base,
    // 只写「本人确认常住地」，不覆盖平台抓取的 videoPublishCountry
    residenceCountry: countryInfo.iso,
    residence_country: countryInfo.iso,
    residenceCountrySource: RESIDENCE_COUNTRY_SOURCE,
    residenceCountryRelation: countryInfo.relation,
    residenceCountryEvidence: countryInfo.evidence,
    residenceCountryConfidence: countryInfo.confidence,
    residenceCountryRaw: countryInfo.raw,
    residenceCountryUpdatedAt: now,
    residenceCountryReplySourceMessageId: event?.message_id || null,
  };
}
...
async function writeResidenceCountryToDb({
  ...
  await queryTikTok(
    ...
    [
      countryInfo.iso,
      RESIDENCE_COUNTRY_SOURCE,
      countryInfo.relation,
      countryInfo.evidence,
      countryInfo.confidence,
      countryInfo.iso,
      influencerId,
    ]
  );
  ...
  try {
    ...
    console.warn(
      "[CountryReplySync] 更新 TikTok_influencer.residence_country 失败:",
      err?.message || err
    );
  }
