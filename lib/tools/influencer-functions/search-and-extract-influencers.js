        ...
        try {
          ...
          if (campaignId && recordQualifiesForCandidatePool(mergedRecord)) {
            try {
              ...
              if (taskIdNum) await bumpSearchTaskProgress(taskIdNum, 1);
              sendStep(
                "写入候选池",
    ...
    try {
      ...
      if (tid) {
        try {
          ...
          console.warn(`[enrichInfluencerProfiles] touchSearchTaskLastProgressAt: ${err2.message}`);
        }
      }
    }
  };
  ...
  if (enrichBatchPolicy) {
    ...
    while (poolCursor < recordsToEnrich.length) {
      ...
      for (let i = 0; i < batchRecords.length; i += concurrency) {
        const chunk = batchRecords.slice(i, i + concurrency);
        const moreInBatch = i + concurrency < batchRecords.length;
        ...
        if (i + concurrency < batchRecords.length && concurrency > 1) {
          const delay = randomDelay(
            delayBetweenBatches.min,
            delayBetweenBatches.max
          );
    ...
    for (let i = 0; i < recordsToEnrich.length; i += concurrency) {
      ...
      await runConcurrencyChunk(
        ...
        i + concurrency < recordsToEnrich.length
      );
      if (i + concurrency < recordsToEnrich.length && concurrency > 1) {
        const delayBetweenInfluencers = 3000 + Math.floor(Math.random() * 5000);
        sendStep(
          "等待",
          `已处理 ${Math.min(i + concurrency, recordsToEnrich.length)}/${recordsToEnrich.length} 个红人，等待 ${(delayBetweenInfluencers / 1000).toFixed(1)} 秒...`
        );
  ...
  try {
    ...
    const searchResult = useYouTube
      ...
      : useInstagram
        ? await searchInstagramByKeyword(
            { keywords, campaignInfo },
            { onStepUpdate, searchOptions: sharedSearchOptions }
          )
        : await searchInfluencersByKeyword(
            { keywords, campaignInfo },
            { onStepUpdate, searchOptions: sharedSearchOptions }
          );
    const extractionResult = {
    ...
    if (enrichProfileData && recordsToEnrich.length > 0) {
      ...
      const enrichEndpoint =
        ...
        "http://127.0.0.1:9222";
      console.log(
        `[searchAndExtractInfluencers] 💡 enrich 使用 9222: ${enrichEndpoint}` +
          (useInstagram || useYouTube ? "（需已登录）" : "（TikTok 可不登录）")
      );
