...
/** 新红人首封邀约：所有发件邮箱均在 60s 冷却中 */
export class OutboundCooldownError extends Error {
  constructor(message = "所有发件邮箱均在首封邀约冷却中（60s），下一轮 Worker 重试") {
    super(message);
    this.name = "OutboundCooldownError";
  }
}

const NEW_OUTREACH_COOLDOWN_MS = 60_000;
...
async function fetchLastFirstOutreachTimeByEmail() {
  ...
  for (const row of rows || []) {
    ...
    if (email) map.set(email, row.last_at);
  }
  return map;
}

/**
 * 为新红人首封邀约选发件账号：动态池 + 60s 冷却 + LRU（最久未用于首封的优先）。
...
export async function pickOutboundAccountForNewOutreach() {
  ...
  for (const acc of accounts) {
    ...
    if (Number.isFinite(lastMs) && now - lastMs >= NEW_OUTREACH_COOLDOWN_MS) {
