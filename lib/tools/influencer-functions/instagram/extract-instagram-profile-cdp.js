...
export async function extractInstagramProfileFromPageCDP(
  ...
  if (shouldCollectAboutCountry(options)) {
    reportStep(
      onStepUpdate,
      BROWSER_STEP_IDS.ENRICH_PROFILES,
      STEP_STATUS.RUNNING,
      `正在读取 @${handle} 账户所在地（账户简介）`
    );
    try {
      ...
      if (aboutCountryResult?.videoPublishCountry) {
        console.log(
          `[extractInstagramProfile] @${handle} account_country=${aboutCountryResult.accountCountryRaw} ` +
            `iso=${aboutCountryResult.videoPublishCountry} source=${aboutCountryResult.source}`
        );
      } else {
        console.warn(
          `[extractInstagramProfile] @${handle} 未获取账户所在地: ${aboutCountryResult?.error || "unknown"}`
        );
      }
    } catch (e) {
      console.warn(`[extractInstagramProfile] @${handle} about country: ${e.message}`);
  ...
  return {
    ...
    profileUrl,
    reelsUrl,
    videoPublishCountry: aboutCountryResult?.videoPublishCountry || null,
    accountCountryRaw: aboutCountryResult?.accountCountryRaw || null,
    accountCountrySource: aboutCountryResult?.source || null,
