...
import {
  isImageFileName,
  isImportListFileName,
  isPdfFileName,
  isSendableAttachmentFileName,
} from "./attachment-file-types.js";

export const PROFILE_URL_IN_TEXT =
...
const DEFAULT_ROW_RULES = [
  ...
  { priority: 1, kind: "profile_url", column: "博主链接" },
  {
    priority: 2,
...
/** 名单导入只认 Excel/CSV；PDF/图片/文档属于资料附件，不能作为红人名单。 */
export function isImportListAttachment(att) {
  if (!att?.storageKey) return false;
  return isImportListFileName(att?.name);
}

/** PDF 资料附件（可用于特殊请求随信发送）。 */
export function isPdfAttachment(att) {
  return Boolean(att?.storageKey) && isPdfFileName(att?.name);
}

/** 可随特殊请求邮件发送的资料附件（PDF / Word / PPT / 图片）。 */
export function isSendableAttachment(att) {
  return Boolean(att?.storageKey) && isSendableAttachmentFileName(att?.name);
}
...
export function buildAttachmentContextForPrompt(fileAttachments) {
  ...
  for (const att of fileAttachments) {
    ...
    // 资料附件（PDF / Word / PPT / 图片）不解析内容，只告诉模型可用于随信发送。
    if (isSendableAttachment(att)) {
      const kindLabel = isImageFileName(att.name) ? "图片资料" : "资料文件";
      attachmentContext += `\n附件「${att.name}」（storageKey: ${att.storageKey}，${kindLabel}，仅供随信发送给红人，不能作为红人名单导入）\n`;
      continue;
    }
    if (!isImportListFileName(att.name)) continue;
