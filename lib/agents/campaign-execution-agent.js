...
import { parseModifyCampaignChangesFromUserMessage } from "../campaign/parse-modify-campaign-message.js";
import { formatInfluencerPricingLabel } from "../campaign/influencer-pricing.js";
...
当不需要调用工具时，你必须直接输出中文自然语言回复（不要输出 JSON）。`;
  ...
  async processWithTools(messages, context = {}) {
    ...
    const ruleBasedChanges = parseModifyCampaignChangesFromUserMessage(lastMessage);
    if (ruleBasedChanges) {
      return {
        reply: "正在修改 campaign 并通知红人经纪人…",
        toolCall: {
          toolName: "modify_campaign",
          params: { scope: "whole", changes: ruleBasedChanges, campaignId },
        },
      };
    }
    ...
    if (toolName === "modify_campaign" && toolResult.success) {
      ...
      if (cid) {
        try {
          ...
          const parts = [];
          if (ci.budget != null) parts.push(`总预算 $${Number(ci.budget).toLocaleString("en-US")} USD`);
          if (ci.commission != null) parts.push(`佣金 ${ci.commission}%`);
          const pricingLabel = formatInfluencerPricingLabel(ci.influencerPricing);
          if (pricingLabel) parts.push(`单位红人报价策略：${pricingLabel}`);
