...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  const {
    ...
    enrichBatchSize = Math.max(1, Number(process.env.ENRICH_BATCH_SIZE || 10)),
    ...
    enrichBatchZeroStreakLimit = Math.max(
      1,
      Number(process.env.ENRICH_BATCH_ZERO_STREAK || 2)
    ),
  ...
  sendStep(
    ...
    isYouTubeEnrich
      ...
      ? '正在通过 CDP 连接浏览器（TikTok Lite enrich，9223，不登录）...'
  ...
  if (USE_HEADLESS) {
    ...
    if (liteMode && isTikTokEnrich && !isInstagramEnrich && !isYouTubeEnrich) {
      const { withCdp9223PreparedSession } = await import("../../cdp/connect-cdp-9223.js");
      return withCdp9223PreparedSession(
        { platform: "tiktok", phase: "enrich-lite" },
  ...
  async function ensureTtLitePagePool() {
    ...
    while (ttLitePagePool.length < poolSize) {
      try {
        const session = await acquireTiktokApiSession(context, {
          forceNewTab: ttLitePagePool.length > 0,
          endpointKey,
        });
        ttLitePagePool.push(session);
        console.log(
          `[enrichInfluencerProfiles] TikTok Lite: 新建 API 页 ${ttLitePagePool.length}/${poolSize}（9223）`
        );
      } catch (e) {
        console.warn(
          `[enrichInfluencerProfiles] TikTok Lite: 创建 API 页失败（已有 ${ttLitePagePool.length}）: ${e.message}`
        );
        break;
      }
    }
    ...
    if (!ttLitePagePool.length) {
      throw new Error("TikTok Lite API 会话不可用（9223 CDP 可能未就绪）");
    }
  }
    ...
    const run = async () => {
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!igLitePage) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] Instagram Lite API enrich @${record.username}（无页面导航）...`
          );
          try {
        ...
        sendStep(
          "提取数据",
          `[${progress}] YouTube Lite API enrich @${record.username}（无页面导航）...`
        );
        try {
        ...
        if (!ttLitePage || !isTtLitePageAlive(ttLitePage)) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] TikTok Lite API enrich @${record.username}（9223，无页面导航）...`
          );
          try {
...
async function searchAndExtractInfluencers_v2(params = {}, options = {}) {
    ...
    while (poolCursor < recordsToEnrich.length) {
      ...
      if (enrichBatchStopOnZeroMatch && batchRecommended === 0) {
        ...
        if (zeroMatchStreak >= enrichBatchZeroStreakLimit) {
          sendStep(
            "批次停止",
            `连续 ${zeroMatchStreak} 批（每批 ${batchRecords.length} 人）analyze 后 0 人符合要求，结束本关键词 enrich`
          );
        ...
        sendStep(
          "批次继续",
          `本批 0 人符合要求（连续 ${zeroMatchStreak}/${enrichBatchZeroStreakLimit}），继续下一批`
        );
  ...
  if (ytLitePagePool.length) {
    ...
    console.log(
      `[enrichInfluencerProfiles] YouTube Lite innertube 会话已关闭（${ytLitePagePool.length} 页）`
    );
  }
  ...
  if (igLitePagePool.length) {
    ...
    console.log(
      `[enrichInfluencerProfiles] Instagram Lite API 会话已关闭（${igLitePagePool.length} 页）`
    );
  }
  ...
  if (ttLitePagePool.length) {
    ...
    console.log(
      `[enrichInfluencerProfiles] TikTok Lite API 会话已关闭（${ttLitePagePool.length} 页，9223）`
    );
  }
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  if (useTikTok && isLiteScraperMode() && !options.cdpContext) {
    return withCdp9222PreparedSession(
      { platform: "tiktok", phase: "lite-pipeline" },
      async ({ context }) =>
        searchAndExtractInfluencers(params, { ...options, cdpContext: context })
    );
  }
  ...
  try {
    ...
    if (enrichProfileData && recordsToEnrich.length > 0) {
        ...
        useYouTube
          ...
          : useInstagram
            ...
            : useTikTok && isLiteScraperMode()
              ? `开始提取 ${recordsToEnrich.length} 位 TikTok 红人主页（9223 Lite API，不登录）...`
      ...
      const enrichEndpoint =
        useTikTok && isLiteScraperMode()
          ...
          : process.env.CDP_ENDPOINT_ENRICH ||
            ...
            "http://127.0.0.1:9222";
      console.log(
        `[searchAndExtractInfluencers] 💡 enrich 使用 ${useTikTok && isLiteScraperMode() ? "9223" : "9222"}: ${enrichEndpoint}` +
          (useInstagram || useYouTube
            ? "（需已登录）"
            : useTikTok && isLiteScraperMode()
              ? "（TikTok Lite enrich，无登录）"
              : "（TikTok 可不登录）")
      );
      ...
      const enrichedRecords = await enrichInfluencerProfiles(
        recordsToEnrich,
        {
          onStepUpdate: liteMode ? null : onStepUpdate,
