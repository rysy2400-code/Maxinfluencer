...
function normalizeRowRules(rowRules) {
  ...
  if (rowRules && typeof rowRules === "object") {
    ...
    const out = [];
    if (rowRules.profile_url) {
      out.push({
        priority: 1,
        kind: "profile_url",
        column: rowRules.profile_url.column || rowRules.profile_url.urlColumn,
      });
    }
    if (rowRules.username_platform) {
      out.push({
        priority: 2,
        kind: "username_platform",
        usernameColumn: rowRules.username_platform.usernameColumn,
        platformColumn: rowRules.username_platform.platformColumn,
      });
    }
...
export function rowsFromTextItems(textItems, options = {}) {
  const maxItems = Math.min(200, Math.max(1, Number(options.maxItems || 200)));
  const rows = [];
  const parseErrors = [];
  const seen = new Set();
