...
/**
 * 把资料附件（PDF / Word / PPT / 图片）转成 ask_influencer_special_request 可用的
...
6. 若最后一条消息 @某位或多位红人 并**询问/协商/回复/同步消息**（如「帮我回复达 @xxx」「把这段话发给 @xxx」「问他能否接受某价格/做几条视频/改时间/给具体报价」「给 @a、@b、@c 分别发送寄样物流链接与内容建议」）→ 必须 needTool=true，toolName=ask_influencer_special_request。
   ...
   - **若用户要求把资料附件（见【附件/名单上下文】中标注为资料文件/图片资料的附件，支持 PDF / Word / PPT / 图片）发给红人**：params 必须携带 attachments 数组（每项 { fileName, storageKey, contentType, sizeBytes }）；单一位红人填 params.attachments，多位红人时每位 request 元素里都填 attachments（同一份资料多位红人共用时每位都带）；requestDetail 中说明「随信附上资料：文件名」，不要丢失文件名。单封最多 5 个附件。
   - **视频不作为附件发送**：若用户想给红人发视频，不要放进 attachments，而是让用户提供可公开访问的链接（YouTube / Google Drive / Dropbox / Loom 等），并把链接自然写进 requestDetail 正文。
...
13. 红人名单导入：用户**上传 Excel/CSV 附件**、粘贴**红人主页链接**或 **@用户名 列表** → 属于名单导入场景（见第 14 条两轮确认；注意第 6 条的「回复达人」优先级更高）。**PDF / Word / PPT / 图片都是资料文件，不是名单**，不能作为导入来源。
14. 名单导入两轮确认：
    ...
    - 若用户上传的是 **PDF / Word / PPT / 图片**并要求导入名单 → 不是有效名单来源，needTool=false，reply 提示「该附件是资料文件，不能作为红人名单导入，请上传 Excel/CSV 名单或粘贴红人链接」。
...
5. 向某位或多位红人发起特殊请求（如延后发布时间、改价、改条数、同步物流单号与内容建议、发送资料文件：PDF / Word / PPT / 图片），并回收红人反馈后同步给广告主；一条消息涉及多位红人时，为每位红人分别发起一次特殊请求（params.requests 数组）
6. 查询 campaign 执行状态
7. 红人执行阶段操作：
...
- 当用户**上传资料附件（PDF / Word / PPT / 图片，见【附件/名单上下文】）并要求发给红人**时，同样必须调用 ask_influencer_special_request，并在 params（或 requests 数组中每位红人）携带 attachments：每项 { fileName, storageKey, contentType, sizeBytes }（取自附件上下文，**禁止编造 storageKey**）；requestDetail 中注明「随信附上资料：文件名」；单封最多 5 个附件。若用户要求「导入名单」但附件是 PDF/Word/PPT/图片，则不能导入，应引导上传 Excel/CSV 或粘贴链接。
- **视频不走附件**：用户想给红人发视频时，不要调用附件参数，直接请用户提供可公开访问的链接（YouTube / Google Drive / Dropbox / Loom 等），再用 ask_influencer_special_request 把链接写进 requestDetail 发给红人。
