...
- confirm：用户提供了名单/链接/Excel，但尚未明确同意导入并联系。reply 须描述看到的名单，并给出两种处理方式让用户选择：
  「1) 只联系符合campaign投放地区和红人画像要求的红人（默认）；2) 符合campaign投放地区的红人一律联系」
  用户后续回复选 2 → phase=import 且 contactMode="contact_all"；选 1 或未明确 → contactMode="default"。
...
【结构化字段 — 导入相关】
- contactMode："default"（只联系符合投放地区和画像且推荐的红人）| "contact_all"（符合投放地区的红人一律联系，仅当用户明确选 2 时用 contact_all，否则 default）
- textItems：从对话中的主页链接提取，每项含 profileUrl；不得编造
- attachmentPlan：Excel 列映射；含 storageKey、fileName、headerRow、rowRules（rowRules.kind 仅用 profile_url 或 username_platform，勿填 textItems 代替 Excel 解析）
- 有 Excel 附件时：textItems 留空 []，由后端读取完整文件；勿把样本前 15 行抄进 textItems
...
只输出 JSON：
{
  ...
  "contactMode": "default" | "contact_all",
  "reply": "给用户的中文回复",
...
输出 JSON。`;
  ...
  try {
    ...
    const statusAction = VALID_STATUS_ACTIONS.has(statusActionRaw)
      ...
      : null;
    const contactMode =
      parsed.contactMode === "contact_all" ? "contact_all" : "default";
