...
import {
  fetchWithPoolRetry,
  resolveCdpPortFromPage,
  resolveEnrichControllerForCdp,
} from "../influencer-functions/tiktok/tiktok-proxy-pool.js";
  ...
  if (isLiteScraperMode()) {
      ...
      String(process.env.TT_LITE_SESSION_REFRESH_ON_START ?? "1").trim() !== "0";
    {
      ...
      try {
        ...
        console.warn(
          `[resolveVideoPublishCountry] country setup touch progress failed: ${e.message}`
        );
      }
    }
    ...
    // 周期会话刷新：单会话累计 N 次探测后清 cookies 重建匿名指纹（默认 0=关闭，任务级刷新已覆盖）。
    const sessionRefreshInterval = Math.max(
      0,
      Math.floor(Number(process.env.TT_LITE_COUNTRY_SESSION_REFRESH_INTERVAL || 0))
    );
    ...
    async function probeLocation(record, sessionIdx) {
      ...
      if (
        ...
        sessionProbeCounts[poolIdx] % sessionRefreshInterval === 0
      ) {
        try {
      ...
      try {
        return await fetchWithPoolRetry({
          fn: async () => {
            const r = await resolveVideoLocationCreatedForInfluencer(page, {
              videoId: primaryVid,
              altVideoIds,
              username,
              secUid,
            });
            if (!r.locationCreated) {
              const stub = !!page._ttCountryHtmlLoginStub;
              page._ttCountryHtmlLoginStub = false;
              if (stub) {
                throw new Error(`country stub @${username}`);
              }
              return r; // 正常无位置数据：不视为节点失败，不换节点
            }
            return r;
          },
          page,
          controlUrl: resolveEnrichControllerForCdp(cdpPort) || undefined,
    ...
    // 国家阶段 setup（会话刷新/健康采样）可能耗时较长，先 touch 进度避免被 stuck_reclaim 回收。
    {
      const importTid = Number(importTaskId || 0);
      const tid = Number(taskId || 0);
      try {
        ...
        console.warn(
          `[resolveVideoPublishCountry] country-start touch progress failed: ${e.message}`
        );
      }
    }
