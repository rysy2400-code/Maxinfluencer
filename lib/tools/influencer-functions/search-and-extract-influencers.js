...
import {
  resolveUnknownCountryBioGate,
  detectBioLanguageProfile,
} from '../../influencer/infer-bio-language.js';
import { upsertBioLanguage } from '../../influencer/influencer-language-store.js';
...
import { searchXByKeyword } from './x/search-x-by-keyword.js';
import {
  isLiteScraperMode,
  ...
  resolveEnrichStopOnZeroMatch,
  resolveLiteCdpTabPoolSize,
  resolveYtLiteTabPoolSize,
  resolveYtLiteTabRecycleProfiles,
  resolveIgLiteTabPoolSize,
  resolveXLiteTabPoolSize,
...
function mergeInfluencerData(searchRecord, profileData) {
  ...
  // X 的 computeXTweetStatistics 用 sampleCount 表达样本数（0 是真实值，不能当缺失）；
  // YT/IG/TikTok 不返回 sampleCount，而是 avgViews/videosWithPlayCount/totalVideos/videoCount，
  // 因此不能用“有 sampleCount 字段”作为唯一样本判定，否则非 X 平台会丢掉已抓到的均播。
  const hasExplicitSampleCount =
    !!statistics &&
    Object.prototype.hasOwnProperty.call(statistics, "sampleCount") &&
    Number.isFinite(Number(statistics.sampleCount)) &&
    Number(statistics.sampleCount) > 0;
  const hasLegacyVideoSample =
    !!statistics &&
    !Object.prototype.hasOwnProperty.call(statistics, "sampleCount") &&
    (statistics.avgViews != null ||
      Number(statistics.videosWithPlayCount || 0) > 0 ||
      Number(statistics.totalVideos || 0) > 0 ||
      Number(statistics.videoCount || 0) > 0);
  const hasVideoSample = hasExplicitSampleCount || hasLegacyVideoSample;
  const statNumber = (v) =>
    v == null || v === ""
      ? null
      : Number.isFinite(Number(v))
        ? Number(v)
        : null;
  const avgViewsNum = hasVideoSample ? statNumber(statistics.avgViews) : null;
  const avgLikesNum = hasVideoSample ? statNumber(statistics.avgLikes) : null;
  const avgCommentsNum = hasVideoSample
    ? statNumber(statistics.avgComments)
    : null;
  // 中位播放 + 点赞率/评论率（0~1 小数）：与均播同一批样本
  const medianViewsNum = hasVideoSample
    ? statNumber(statistics.medianViews)
    : null;
  const likeRateNum = hasVideoSample ? statNumber(statistics.likeRate) : null;
  const commentRateNum = hasVideoSample
    ? statNumber(statistics.commentRate)
    : null;
  ...
  return {
    ...
    location: userInfo.location || searchRecord.location || null,
    locationRaw: userInfo.locationRaw || searchRecord.locationRaw || null,
    accountBasedIn: userInfo.accountBasedIn || searchRecord.accountBasedIn || null,
    ...
    views:
      avgViewsNum != null || medianViewsNum != null
        ? {
            ...(avgViewsNum != null
              ? { avg: avgViewsNum, display: formatNumber(avgViewsNum) }
              : {}),
            ...(medianViewsNum != null
              ? {
                  median: medianViewsNum,
                  medianDisplay: formatNumber(medianViewsNum),
                }
              : {}),
          }
        : null,
    ...
    engagement: {
      ...
      avgLikes:
        avgLikesNum ??
        searchRecord.engagement?.avgLikes ??
        null,
      avgComments:
        avgCommentsNum ??
        searchRecord.engagement?.avgComments ??
        null,
      likeRate:
        likeRateNum ?? searchRecord.engagement?.likeRate ?? null,
      commentRate:
        commentRateNum ?? searchRecord.engagement?.commentRate ?? null,
    },
    // 顶层冗余一份，供候选快照/卡片直接读取
    medianViews: medianViewsNum,
    likeRate: likeRateNum,
    commentRate: commentRateNum,
    
    // 视频/帖数量（主页数据优先）：兼容 YT/IG 的 {count} 对象与 X 的数字形态
    postsCount: (() => {
      const raw = userInfo.postsCount ?? searchRecord.postsCount ?? null;
      if (raw && typeof raw === "object" && raw.count != null) return raw.count;
 ...
 */
/**
 * 把「红人主页简介语言」挂到记录上（卡片展示 + 发信语言兜底）。
 * 识别不出语言时保留已有值，不覆盖，也不写空。
 */
export function attachBioLanguageToRecord(mergedRecord, profileData = null) {
  if (!mergedRecord || typeof mergedRecord !== "object") return mergedRecord;
  const bio =
    mergedRecord.bio ||
    profileData?.userInfo?.bio ||
    profileData?.profileData?.userInfo?.bio ||
    "";
  const detected = detectBioLanguageProfile(bio);
  if (!detected.language) return mergedRecord;

  mergedRecord.bioLanguage = detected.language;
  mergedRecord.bioLanguageConfidence = detected.confidence;
  mergedRecord.bioLanguageSource = detected.source;
  return mergedRecord;
}
...
export function applyEnrichObservabilityFields(
  ...
  // 红人主页简介语言：仅用于卡片展示与发信语言兜底，不参与国家门禁。
  attachBioLanguageToRecord(mergedRecord, profileData);
 ...
 * @returns {"youtube"|"instagram"|"tiktok"|"x"}
 */
export function resolveRecordEnrichPlatform(record, enrichPlatformOption = null) {
  ...
  if (raw === "x" || raw === "twitter" || raw === "tw") return "x";
  ...
  if (opt === "youtube" || opt === "instagram" || opt === "x" || opt === "tiktok") return opt;
  return "tiktok";
}
...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  const {
    ...
    enrichBatchStopOnZeroMatch = resolveEnrichStopOnZeroMatch(),
    /** 直接联系模式：即使候选表已存在也重新 enrich（不跳过） */
    enrichExistingCandidates = false,
    enrichBatchZeroStreakLimit = Math.max(
      1,
      Number(process.env.ENRICH_BATCH_ZERO_STREAK || 1)
    ),
    ...
    /** X Lite：搜索阶段 x.com API 会话 */
    xLiteSession: xLiteSessionOption = null,
  ...
  // 每个任务开始时重置 IG Lite 资料端点熔断与早探 clips 预算（generation 计数）
  try {
    const { resetInstagramLiteProfileCircuits } = await import(
      "./instagram/extract-instagram-profile-lite.js"
    );
    resetInstagramLiteProfileCircuits();
  } catch {
    /* ignore */
  }
  ...
  const litePlatformForConcurrency =
    ...
    (influencerRecords || []).some((r) => r.platform === "YouTube")
      ...
      : String(enrichPlatformOption || "").toLowerCase() === "instagram" ||
        ...
        ? "instagram"
        : String(enrichPlatformOption || "").toLowerCase() === "x" ||
            (influencerRecords || []).some((r) => r.platform === "X")
          ? "x"
  ...
  if (USE_HEADLESS) {
    ...
    if (liteMode && isYouTubeEnrich && importTaskIdNum > 0) {
      // 名单导入 YouTube Lite：与 Instagram 一样走 raw CDP target attach，
      // 不再用 Playwright connect+prune 整批占住 9222 全局锁，
      // 避免上一任务超时/泄漏锁导致整批导入等待 300s 后失败。
    ...
    const cdpLockPlatform =
      enrichPlatformSlug ||
      (isYouTubeEnrich ? "youtube" : isInstagramEnrich ? "instagram" : isXEnrich ? "x" : "tiktok");
    return await withCdp9222PreparedSession(
      { platform: cdpLockPlatform, phase: "enrich" },
  ...
  async function ensureYtLitePagePool() {
    ...
    const recycleEvery = resolveYtLiteTabRecycleProfiles();
    console.log(
      `[enrichInfluencerProfiles] YouTube Lite: tab池=${poolSize} enrich并发=${effectiveConcurrency} 回收阈值=${recycleEvery}（单 tab 多并发 API，对齐 TikTok Lite）`
    );

    while (ytLitePagePool.length < poolSize) {
      try {
        const session = await acquireYoutubeInnertubeSession(context, {
          persistent: poolSize === 1 && recycleEvery <= 0,
          forceNewTab:
            poolSize > 1
              ? ytLitePagePool.length > 0
              : recycleEvery > 0,
        });
  ...
  function ytLiteSlotForIndex(index) {
    if (!ytLitePagePool.length) return null;
    return index % ytLitePagePool.length;
  }

  async function ytLitePageForIndex(index) {
    if (!ytLitePagePool.length) return null;
    const slot = ytLiteSlotForIndex(index);
    const meta = ytLiteTapMeta[slot];
    if (meta) {
      meta.inflight += 1;
      const recycleEvery = resolveYtLiteTabRecycleProfiles();
      if (
        recycleEvery > 0 &&
        meta.assigned >= meta.nextRecycleAt &&
        meta.inflight === 1 &&
        !meta.recyclePending
      ) {
        // 该 slot 当前无其它在途请求，就地回收重建
        meta.nextRecycleAt = meta.assigned + recycleEvery;
        meta.recyclePending = recycleYtLitePageSlot(slot)
          .catch((e) => {
            console.warn(
              `[enrichInfluencerProfiles] YT tap ${slot + 1} 回收异常:`,
              e?.message || e
            );
          })
          .finally(() => {
            meta.recyclePending = null;
          });
      }
      if (meta.recyclePending) {
        try {
          await meta.recyclePending;
        } catch {
          /* 回收失败继续用旧页 */
        }
      }
    }
    return ytLitePagePool[slot]?.page || null;
  }

  function noteYtLiteTapResult(index, ok) {
    if (!ytLitePagePool.length) return;
    const slot = ytLiteSlotForIndex(index);
    if (slot == null || !ytLiteTapMeta[slot]) return;
    const meta = ytLiteTapMeta[slot];
    meta.assigned += 1;
    if (ok) meta.success += 1;
    else meta.failed += 1;
    const recycleEvery = resolveYtLiteTabRecycleProfiles();
    if (
      recycleEvery > 0 &&
  ...
  async function recycleYtLitePageSlot(slot) {
    if (slot == null || !ytLitePagePool[slot]) return null;
    const oldSession = ytLitePagePool[slot];
    const oldTargetId =
      oldSession?.page?._ytCdpTargetId ||
      describeLitePage(oldSession.page).targetId ||
      null;
    const oldUrl = describeLitePage(oldSession.page).url;
    console.warn(
      `[enrichInfluencerProfiles] 🔄 回收 YT tap ${slot + 1}（已处理 ${ytLiteTapMeta[slot]?.assigned || 0} 个红人，旧页 ${oldTargetId || "(playwright)"} ${oldUrl}）`
    );
    // 关闭旧标签页（CDP 目标走浏览器级 /json/close，不受渲染器挂起影响）
    try {
      if (oldTargetId) {
        const { closeCdpTarget } = await import("../../cdp/cdp-target-page.js");
        await closeCdpTarget(
          process.env.CDP_ENDPOINT || "http://127.0.0.1:9222",
          oldTargetId
        );
      } else if (
        oldSession?.page &&
        typeof oldSession.page.isClosed === "function" &&
        !oldSession.page.isClosed()
      ) {
        await oldSession.page.close();
      }
    } catch (e) {
      console.warn(
        `[enrichInfluencerProfiles] 关闭旧 YT 页面失败 tap=${slot + 1}:`,
        e?.message || e
      );
    }
    try {
      ...
      console.warn(
        `[enrichInfluencerProfiles] 释放旧 YT 会话失败 tap=${slot + 1}:`,
        e?.message || e
      );
    }
    try {
      ...
      console.log(
        `[enrichInfluencerProfiles] ✅ YT tap ${slot + 1} 已回收重建 targetId=${info.targetId || "(unknown)"}`
      );
      return session;
    } catch (e) {
      console.warn(
        `[enrichInfluencerProfiles] ❌ 回收重建 YT tap ${slot + 1} 失败:`,
        e?.message || e
      );
      return null;
    }
  ...
  let xLiteSession = xLiteSessionOption || null;
  const xLitePagePool = [];
  const xLiteTapStats = [];
  ...
  function resolveIgLiteEnrichEndpoints() {
    const explicit = String(process.env.IG_LITE_ENRICH_CDP_ENDPOINTS || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  function describeLitePage(page) {
    ...
    return {
      targetId:
        page?._igCdpTargetId || page?._ytCdpTargetId || page?._cdpTargetId || null,
      url,
    };
  }
  ...
  async function ensureIgLitePagePool() {
    ...
    const endpointCandidates = resolveIgLiteEnrichEndpoints();
    const explicitEndpointPool =
      String(process.env.IG_LITE_ENRICH_CDP_ENDPOINTS || "")
        .split(",")
        .map((s) => s.trim())
    ...
    console.log(
      `[enrichInfluencerProfiles] Instagram Lite endpoint pool: ${endpointCandidates.join(", ")} explicit=${explicitEndpointPool ? "yes" : "no"} targetPool=${poolSize} plannedConcurrency=${effectiveConcurrency}`
    );

    if (!explicitEndpointPool && igLiteSession?.page && isIgLitePageAlive(igLiteSession.page)) {
    ...
    while (igLitePagePool.length < poolSize) {
      try {
        ...
        const endpointKey =
          endpointCandidates[slot % endpointCandidates.length] ||
        ...
        const info = describeLitePage(session.page);
        console.log(
          `[enrichInfluencerProfiles] Instagram Lite tap=${igLitePagePool.length}/${poolSize}: API 页 via ${endpointKey} targetId=${info.targetId || "(unknown)"} url=${info.url || "(unknown)"}（enrich 并发 ${effectiveConcurrency}）`
        );
      } catch (e) {
        console.warn(
    ...
    igLitePagePool.forEach((session, idx) => {
      ...
      igLiteTapStats[idx] = {
        tap: idx + 1,
        endpoint: session.page?._igRelaySessionKey || null,
      ...
      console.log(
        `[enrichInfluencerProfiles] Instagram Lite target binding tap=${idx + 1} endpoint=${session.page?._igRelaySessionKey || "(unknown)"} targetId=${info.targetId || "(unknown)"} url=${info.url || "(unknown)"}`
      );
    });
  }
  ...
  // ============ X Lite 会话池（风控敏感：1 tab 串行，可复用搜索会话） ============
  function isXLiteSessionAlive(session) {
    try {
      return (
        !!session?.page &&
  ...
  async function ensureXLitePagePool() {
    if (!isXEnrich || !liteMode) return;
    const poolSize = resolveXLiteTabPoolSize();
    const { acquireXSession } = await import("./x/x-session.js");
    console.log(
      `[enrichInfluencerProfiles] X Lite: tab池=${poolSize} enrich并发=${effectiveConcurrency}（单 tab 串行 API，对齐 Instagram Lite 保守策略）`
    );
    if (xLiteSession && isXLiteSessionAlive(xLiteSession)) {
      xLitePagePool.push(xLiteSession);
      xLiteSession = null; // 会话已移交池内
      console.log(
        `[enrichInfluencerProfiles] X Lite tap=1/${poolSize}: 复用搜索阶段 x.com API 会话`
      );
    }
    while (xLitePagePool.length < poolSize) {
      try {
        const session = await acquireXSession(context, { logPrefix: "[enrich-x]" });
        xLitePagePool.push(session);
        console.log(
          `[enrichInfluencerProfiles] X Lite: 新建 x.com API 会话 ${xLitePagePool.length}/${poolSize}`
        );
      } catch (e) {
        console.warn(
          `[enrichInfluencerProfiles] X Lite: 创建会话失败（已有 ${xLitePagePool.length}）: ${e.message}`
        );
        break;
      }
    }
    if (!xLitePagePool.length) {
      throw new Error("X Lite x.com API 会话不可用（9222 CDP 需已登录 X）");
    }
    xLitePagePool.forEach((session, idx) => {
      xLiteTapStats[idx] = {
        tap: idx + 1,
        assigned: 0,
        success: 0,
  ...
  await ensureXLitePagePool();

  function xLiteSessionForIndex(index) {
    if (!xLitePagePool.length) return null;
    return xLitePagePool[index % xLitePagePool.length] || null;
  }

  function noteXLiteTapResult(index, ok) {
    if (!xLitePagePool.length) return;
    const tapIndex = index % xLitePagePool.length;
    const stat = xLiteTapStats[tapIndex];
    if (!stat) return;
    stat.assigned += 1;
  ...
  // ============ IG 页面自愈（僵尸页检测后重建标签页） ============
  function igLiteSlotForIndex(index) {
    if (!igLitePagePool.length) return null;
    return index % igLitePagePool.length;
  }

  async function rebuildIgLitePageSlot(slot) {
    if (slot == null || !igLitePagePool[slot]) return null;
    const oldSession = igLitePagePool[slot];
    const endpointKey =
      oldSession?.page?._igRelaySessionKey ||
      process.env.IG_LITE_ENRICH_CDP ||
      process.env.CDP_ENDPOINT ||
      "http://127.0.0.1:9222";
    const oldTargetId = oldSession?.page?._igCdpTargetId || null;
    const oldUrl = describeLitePage(oldSession.page).url;
    console.warn(
      `[enrichInfluencerProfiles] 🔧 重建 IG tap ${slot + 1}（旧页 ${oldTargetId || "(playwright)"} ${oldUrl}）`
    );
    // 关闭僵尸标签页（CDP 目标走浏览器级 /json/close，不受渲染器挂起影响）
    try {
      if (oldTargetId) {
        const { closeCdpTarget } = await import("../../cdp/cdp-target-page.js");
        await closeCdpTarget(endpointKey, oldTargetId);
      } else if (
        oldSession?.page &&
        typeof oldSession.page.isClosed === "function" &&
        !oldSession.page.isClosed()
      ...
      console.warn(
        `[enrichInfluencerProfiles] 关闭旧 IG 页面失败 tap=${slot + 1}:`,
        e?.message || e
      );
    }
    try {
      ...
      console.warn(
        `[enrichInfluencerProfiles] 释放旧 IG 会话失败 tap=${slot + 1}:`,
        e?.message || e
      );
    }
    // 重建不依赖 Playwright context：直接走 CDP 端点开新标签。
    // 挂起时 CDP 可能整体不可用（Chrome 需由 guard 重启），带退避重试等待恢复。
    const { acquireInstagramApiSession } = await import(
      "./instagram/instagram-direct-fetch.js"
    );
    let lastErr = null;
    for (let attempt = 1; attempt <= 4; attempt++) {
      try {
        const session = await acquireInstagramApiSession(null, {
          forceNewTab: true,
          persistent: slot === 0,
          endpointKey,
          targetId: null,
        });
        igLitePagePool[slot] = session;
        const info = describeLitePage(session.page);
        igLiteTapStats[slot] = {
          tap: slot + 1,
          endpoint: session.page?._igRelaySessionKey || null,
        ...
        sendStep(
          "IG 页面自愈",
          `IG tap ${slot + 1} 页面已重建（${info.url || "instagram.com"}），登录态保留，继续任务`
        );
        console.log(
          `[enrichInfluencerProfiles] ✅ IG tap ${slot + 1} 页面已重建 targetId=${info.targetId || "(unknown)"}`
        );
        return session;
      } catch (e) {
        lastErr = e;
        ...
        console.warn(
          `[enrichInfluencerProfiles] 🔧 重建 IG tap ${slot + 1} 第 ${attempt}/4 次失败: ${e?.message || e}，${waitMs / 1000}s 后重试`
        );
        if (attempt < 4) {
    ...
    console.warn(
      `[enrichInfluencerProfiles] ❌ 重建 IG tap ${slot + 1} 失败（已重试 4 次）:`,
      lastErr?.message || lastErr
    );
    return null;
  }

  async function ensureHealthyIgLitePage(index) {
    const slot = igLiteSlotForIndex(index);
    if (slot == null) return null;
    const session = igLitePagePool[slot];
    if (!session) return null;
    const { isIgPageUnhealthy } = await import(
      "./instagram/instagram-direct-fetch.js"
    );
    if (isIgPageUnhealthy(session.page)) {
      sendStep(
        "IG 页面自愈",
        `检测到 IG tap ${slot + 1} 页面挂起，重建浏览器页面后继续...`
      );
      console.warn(
        `[enrichInfluencerProfiles] IG tap ${slot + 1} 页面不健康，触发自愈重建`
      );
      await rebuildIgLitePageSlot(slot);
    }
    return igLitePagePool[slot]?.page || null;
  }

  async function rebuildIgPageIfUnhealthyAfterRecord(index) {
    const slot = igLiteSlotForIndex(index);
    if (slot == null) return;
    const session = igLitePagePool[slot];
    if (!session) return;
    ...
    if (isIgPageUnhealthy(session.page)) {
      console.warn(
        `[enrichInfluencerProfiles] 本红人处理期间 IG tap ${slot + 1} 挂起，任务后重建页面`
      );
      await rebuildIgLitePageSlot(slot);
    }
  }
  ...
  const runtimeEffectiveConcurrency =
    isInstagramEnrich && liteMode && igLitePagePool.length
      ? Math.min(effectiveConcurrency, igLitePagePool.length)
  ...
  if (runtimeEffectiveConcurrency !== effectiveConcurrency) {
    console.warn(
      `[enrichInfluencerProfiles] ${isInstagramEnrich ? "Instagram" : "TikTok"} Lite: planned concurrency ${effectiveConcurrency} -> runtime concurrency ${runtimeEffectiveConcurrency}`
    );
  }
  ...
  const candidateHandleKeySet = new Set();
  if (campaignId && !enrichExistingCandidates) {
    try {
      const loaded = await loadCandidateHandleKeySet(campaignId);
      for (const k of loaded) candidateHandleKeySet.add(k);
      console.log(
        `[enrichInfluencerProfiles] 候选表已有 ${candidateHandleKeySet.size} 个平台账号，enrich 前将跳过`
      );
    } catch (e) {
      console.warn(
  ...
  // 不联系名单（全局 + 本 campaign do_not_contact）：enrich 前一律跳过
  let exclusionKeyed = new Set();
  let exclusionHandles = new Set();
  let campaignDoNotContactSet = new Set();
  if (campaignId) {
    try {
      const maps = await loadGlobalContactExclusionMaps();
      exclusionKeyed = maps.keyed;
      exclusionHandles = maps.handles;
      campaignDoNotContactSet = await loadCampaignDoNotContactSet(campaignId);
    } catch (e) {
      console.warn(
        `[enrichInfluencerProfiles] 加载不联系名单失败，将不按名单跳过:`,
        e?.message || e
      );
    }
  }
  ...
  async function shouldSkipExistingCandidate(record) {
    ...
    const recordPlatformSlug = normalizePlatformSlugInput(
      record?.platform || record?.platformSlug
    );
    // 命中不联系名单（全局或本 campaign do_not_contact）永远跳过，即使直接联系模式
    if (
      campaignDoNotContactSet.has(normUser) ||
      isHandleExcluded(
        { keyed: exclusionKeyed, handles: exclusionHandles },
        recordPlatformSlug,
        normUser
      )
    ) {
      lastSkipReason = "excluded";
      return true;
    }
    if (enrichExistingCandidates || !campaignId) return false;
    const handleKey = candidateHandleKey(recordPlatformSlug, normUser);
    if (handleKey && candidateHandleKeySet.has(handleKey)) {
      lastSkipReason = "candidate";
      return true;
    }
    try {
      const exists = await isCandidateInCampaign(campaignId, normUser, recordPlatformSlug);
      if (exists && handleKey) candidateHandleKeySet.add(handleKey);
      lastSkipReason = exists ? "candidate" : null;
      return exists;
    } catch (e) {
      console.warn(
  ...
  // ============ IG Lite 异步画像分析队列 ============
  // LLM（DeepSeek）分析与 IG 数据拉取解耦：IG 拉取完成即入队返回，
  // 由后台队列按 IG_LITE_ANALYSIS_CONCURRENCY 并发分析，批次边界/任务结束前排空。
  const igLiteAnalysisAsync =
    isInstagramEnrich &&
    liteMode &&
    String(process.env.IG_LITE_ANALYSIS_ASYNC ?? "1").trim() !== "0";
  const IG_ANALYSIS_QUEUE_CONCURRENCY = Math.min(
    Math.max(Number(process.env.IG_LITE_ANALYSIS_CONCURRENCY || 10), 1),
    50
  );
  const igAnalysisQueue = [];
  let igAnalysisRunning = 0;
  let igAnalysisQueuedTotal = 0;
  let igAnalysisCompletedTotal = 0;
  let igAnalysisRecommendedTotal = 0;
  let igAnalysisContactableTotal = 0;
  let igAnalysisFailedTotal = 0;
  let igAnalysisDrainWaiters = [];
  // 当前批次分析统计（批次边界排空后重置；队列 worker 并发写入）
  let activeBatchStats = { completed: 0, recommended: 0, contactable: 0, failed: 0 };

  function wakeIgAnalysisDrainWaiters() {
    if (igAnalysisQueue.length === 0 && igAnalysisRunning === 0) {
      const waiters = igAnalysisDrainWaiters;
      igAnalysisDrainWaiters = [];
      for (const resolve of waiters) resolve();
    }
  }

  function kickIgAnalysisWorkers() {
    while (
      igAnalysisRunning < IG_ANALYSIS_QUEUE_CONCURRENCY &&
      igAnalysisQueue.length > 0
    ) {
      const item = igAnalysisQueue.shift();
      igAnalysisRunning += 1;
      runIgAnalysisItem(item)
        .catch((err) => {
          igAnalysisFailedTotal += 1;
          activeBatchStats.failed += 1;
          console.warn(
            `[enrichInfluencerProfiles] 异步画像分析异常 @${item?.mergedRecord?.username || "?"}:`,
            err?.message || err
          );
        })
        .finally(() => {
          igAnalysisRunning -= 1;
          wakeIgAnalysisDrainWaiters();
        });
    }
  }

  function enqueueIgAnalysis(item) {
    igAnalysisQueue.push(item);
    igAnalysisQueuedTotal += 1;
    kickIgAnalysisWorkers();
  }

  async function runIgAnalysisItem(item) {
    try {
      const outcome = await runInfluencerMatch(item);
      if (outcome?.failed) activeBatchStats.failed += 1;
      activeBatchStats.completed += 1;
      igAnalysisCompletedTotal += 1;
      if (item.mergedRecord?.isRecommended === true) {
        activeBatchStats.recommended += 1;
        igAnalysisRecommendedTotal += 1;
      }
      if (
        item.mergedRecord?.isRecommended === true &&
        resolveCandidateEmail(item.mergedRecord)
      ) {
        activeBatchStats.contactable += 1;
        igAnalysisContactableTotal += 1;
      }
    } catch (err) {
      activeBatchStats.failed += 1;
      igAnalysisFailedTotal += 1;
      console.warn(
        `[enrichInfluencerProfiles] 异步画像分析失败 @${item?.mergedRecord?.username || "?"}:`,
        err?.message || err
      );
    }
  }

  async function awaitIgAnalysisDrain() {
    while (igAnalysisQueue.length > 0 || igAnalysisRunning > 0) {
      await new Promise((resolve) => igAnalysisDrainWaiters.push(resolve));
    }
  }

  // 任务中途限流中止：enrich 过程中检测到当前 IG 账户被标记冷却时置位，
  // 提前结束本任务（标记 failed/account_throttled），下个任务自动切账户。
  let igAccountThrottledAbort = null;

  async function checkIgAccountThrottledMidTask() {
    if (!isInstagramEnrich || !liteMode) return false;
    const endpoint =
      process.env.IG_LITE_ENRICH_CDP_ENDPOINTS ||
      process.env.CDP_ENDPOINT ||
      "http://127.0.0.1:9222";
    try {
      const { isIgAccountThrottled } = await import("../../ig-account-rotation.js");
      return isIgAccountThrottled(endpoint);
    } catch (e) {
      console.warn(
        "[enrichInfluencerProfiles] 检测 IG 账户限流状态失败:",
        e?.message || e
      );
      return false;
    }
  ...
  // ============ YT Lite 异步画像分析队列 ============
  // 与 IG 一致：LLM（DeepSeek）分析与 YT 数据拉取解耦，拉取完成即入队返回，
  // 由后台队列按 YT_LITE_ANALYSIS_CONCURRENCY 并发分析，批次边界/任务结束前排空。
  const ytLiteAnalysisAsync =
    isYouTubeEnrich &&
    liteMode &&
    String(process.env.YT_LITE_ANALYSIS_ASYNC ?? "1").trim() !== "0";
  const YT_ANALYSIS_QUEUE_CONCURRENCY = Math.min(
    Math.max(Number(process.env.YT_LITE_ANALYSIS_CONCURRENCY || 10), 1),
    50
  );
  const ytAnalysisQueue = [];
  let ytAnalysisRunning = 0;
  let ytAnalysisQueuedTotal = 0;
  let ytAnalysisCompletedTotal = 0;
  let ytAnalysisRecommendedTotal = 0;
  let ytAnalysisContactableTotal = 0;
  let ytAnalysisFailedTotal = 0;
  let ytAnalysisDrainWaiters = [];
  // 当前批次分析统计（批次边界排空后重置；队列 worker 并发写入）
  let ytActiveBatchStats = { completed: 0, recommended: 0, contactable: 0, failed: 0 };

  function wakeYtAnalysisDrainWaiters() {
    if (ytAnalysisQueue.length === 0 && ytAnalysisRunning === 0) {
      const waiters = ytAnalysisDrainWaiters;
      ytAnalysisDrainWaiters = [];
      for (const resolve of waiters) resolve();
    }
  }

  function kickYtAnalysisWorkers() {
    while (
      ytAnalysisRunning < YT_ANALYSIS_QUEUE_CONCURRENCY &&
      ytAnalysisQueue.length > 0
    ) {
      const item = ytAnalysisQueue.shift();
      ytAnalysisRunning += 1;
      runYtAnalysisItem(item)
        .catch((err) => {
          ytAnalysisFailedTotal += 1;
          ytActiveBatchStats.failed += 1;
          console.warn(
            `[enrichInfluencerProfiles] 异步画像分析异常 @${item?.mergedRecord?.username || "?"}:`,
            err?.message || err
          );
        ...
        .finally(() => {
          ytAnalysisRunning -= 1;
          wakeYtAnalysisDrainWaiters();
        });
    }
  }

  function enqueueYtAnalysis(item) {
    ytAnalysisQueue.push(item);
    ytAnalysisQueuedTotal += 1;
    kickYtAnalysisWorkers();
  }

  async function runYtAnalysisItem(item) {
    try {
      const outcome = await runInfluencerMatch(item);
      if (outcome?.failed) ytActiveBatchStats.failed += 1;
      ytActiveBatchStats.completed += 1;
      ytAnalysisCompletedTotal += 1;
      if (item.mergedRecord?.isRecommended === true) {
        ytActiveBatchStats.recommended += 1;
        ytAnalysisRecommendedTotal += 1;
      }
      if (
        item.mergedRecord?.isRecommended === true &&
        resolveCandidateEmail(item.mergedRecord)
      ) {
        ytActiveBatchStats.contactable += 1;
        ytAnalysisContactableTotal += 1;
      }
    } catch (err) {
      ytActiveBatchStats.failed += 1;
      ytAnalysisFailedTotal += 1;
  ...
  async function awaitYtAnalysisDrain() {
    while (ytAnalysisQueue.length > 0 || ytAnalysisRunning > 0) {
      await new Promise((resolve) => ytAnalysisDrainWaiters.push(resolve));
    }
  }

  // ============ 单红人画像匹配分析（同步/异步共用） ============
  // item: { mergedRecord, influencerProfile, productInfo, campaignInfo, progress, index, onStepUpdate, checkAbort }
  // checkAbort：同步路径传入 () => perInfluencerAborted，预算超时后丢弃分析结果；
  // 异步路径传入 () => false（IG 拉取已完成，LLM 分析结果正常落库）。
  const runInfluencerMatch = async (item) => {
    const {
      mergedRecord,
    ...
    const username = mergedRecord?.username || "?";
    try {
      console.log(`[enrichInfluencerProfiles] [${progress}] 🧠 开始分析红人匹配度: @${username}`);
      // 懒加载分析函数
      ...
      const analysisResult = await analyzeInfluencerMatchFn(
        {
          ...mergedRecord,
          // 确保包含分析函数期望的字段
          profile_data: mergedRecord.profile_data || mergedRecord.profileData || null,
          search_video_data:
            mergedRecord.search_video_data || mergedRecord.searchVideoData || null,
        },
        _influencerProfile,
        _productInfo || {},
        _campaignInfo || {},
      ...
      if (checkAbort()) {
        console.warn(
          `[enrichInfluencerProfiles] [${progress}] 单红人预算已超时，丢弃画像分析结果且不写候选池 @${username}`
        );
        return { aborted: true };
      }

      console.log(
        `[enrichInfluencerProfiles] [${progress}] ✅ 匹配度分析完成: @${username} ` +
      ...
      reportStep(
        ...
        BROWSER_STEP_IDS.ANALYZE_MATCH,
        STEP_STATUS.RUNNING,
        `[${progress}] ${statusText} @${username} - ${analysisResult.reason}`,
        {
          current: index + 1,
          total: recordsToEnrich.length,
          analyzing: username,
      ...
      if (_onStepUpdate) {
        const followersDisplay =
          typeof mergedRecord.followers === 'object' && mergedRecord.followers?.display
            ? mergedRecord.followers.display
            : typeof mergedRecord.followers === 'string'
              ? mergedRecord.followers
              : formatNumber(mergedRecord.followers?.count || 0);
        const viewsDisplay =
          typeof mergedRecord.views === 'object' && mergedRecord.views?.display
            ? mergedRecord.views.display
            : typeof mergedRecord.views === 'object' && mergedRecord.views?.avg
              ? formatNumber(mergedRecord.views.avg)
              : typeof mergedRecord.views === 'string'
                ? mergedRecord.views
                : formatNumber(mergedRecord.views?.count || 0);
        try {
          console.log(`[enrichInfluencerProfiles] [${progress}] 📤 推送 influencerAnalysis 到前端: @${username}`);
          _onStepUpdate({
              ...
              platform:
                ...
                (mergedRecord.profileUrl?.includes('instagram.com')
                  ...
                  : mergedRecord.profileUrl?.includes('youtube.com')
                    ...
                    : 'TikTok'),
              id: username || '',
              name: mergedRecord.displayName || username || '',
      ...
      if (campaignId && recordQualifiesForCandidatePool(mergedRecord)) {
        try {
          ...
          const normWritten = String(mergedRecord.username || '')
            ...
            .trim()
            .toLowerCase();
          const writtenKey = candidateHandleKey(
            mergedRecord.platform || mergedRecord.platformSlug,
            normWritten
          );
          if (writtenKey) candidateHandleKeySet.add(writtenKey);
          ...
          const recRecommended =
            ...
            mergedRecord.isRecommended
              ? 1
              : 0;
          const recContactable =
            recRecommended && resolveCandidateEmail(mergedRecord) ? 1 : 0;
          if (importIdNum) {
            await bumpImportTaskAnalyzedProgress(importIdNum, recDelta, recRecommended);
          } else if (taskIdNum) {
          ...
          sendStep(
            '写入候选池',
            `[${progress}] ✅ 已写入候选池（INSERT IGNORE）@${username}`
          );
        } catch (candErr) {
          console.warn(
            `[enrichInfluencerProfiles] 逐条写入候选失败 @${username}:`,
      ...
      console.warn(
        `[enrichInfluencerProfiles] 分析红人匹配度失败 (@${username}):`,
        analysisError
      );
  ...
  const processOneRecord = async (record, index, batchTotal) => {
    ...
    const run = async () => {
      try {
        const skipReason = await shouldSkipExistingCandidate(record);
        if (skipReason) {
          const isExcludedSkip = skipReason === "excluded";
          if (isExcludedSkip) exclusionSkipCount++;
          ...
          const skipLog = {
            ...
            success: true,
            skippedReason: isExcludedSkip
              ? "in_contact_exclusion"
          ...
          sendStep(
            "跳过 enrich",
            isExcludedSkip
              ? `[${progress}] @${record.username} 命中不联系名单，跳过`
      ...
      const recordIsX = recordPlatformSlug === "x";
      ...
      if (recordIsInstagram && liteMode) {
        const igLitePage = await ensureHealthyIgLitePage(index);
        if (!igLitePage) {
          profileData = {
            success: false,
          ...
          // 若本红人处理期间页面挂起（ig_page_hang），立即重建，下一位红人使用新页面
          await rebuildIgPageIfUnhealthyAfterRecord(index);
        }
      } else if (recordIsYouTube && liteMode) {
        const ytLitePage = await ytLitePageForIndex(index);
        const ytSlot = ytLiteSlotForIndex(index);
        if (!ytLitePage) {
          profileData = {
            success: false,
          ...
          if (ytSlot != null && ytLiteTapMeta[ytSlot]) {
            ytLiteTapMeta[ytSlot].inflight = Math.max(
              0,
              ytLiteTapMeta[ytSlot].inflight - 1
            );
          }
          noteYtLiteTapResult(index, false);
        } else {
          sendStep(
            "提取数据",
          ...
          try {
            ...
            // 搜索命中的 channelId 是唯一可靠身份。若富化打开的 @handle 实际落到
            // 另一个频道（历史脏数据里曾出现 @AI/@M 截断 handle 指向别的频道），
            // 必须丢弃这份 profileData，避免把别的频道的粉丝/邮箱写进本记录。
            {
              const expectedChannelId =
                record.userId || record.channelId || null;
              const enrichedChannelId =
                profileData?.userInfo?.userId || null;
              if (
                profileData?.success &&
                expectedChannelId &&
                enrichedChannelId &&
                String(expectedChannelId).trim() !== String(enrichedChannelId).trim()
              ) {
                const mismatchMsg = `youtube_channel_id_mismatch:${expectedChannelId}!=${enrichedChannelId}`;
                sendStep(
                  "提取数据",
                  `[${progress}] ⚠️ YouTube 富化频道不匹配 @${record.username}（${mismatchMsg}），已丢弃该次富化`
                );
                console.warn(
                  `[enrichInfluencerProfiles] [${progress}] ${mismatchMsg} @${record.username}`
                );
                profileData = {
                  success: false,
                  error: mismatchMsg,
          ...
          noteYtLiteTapResult(
            index,
            !!profileData?.success && !profileData?.skippedReason
          );
        }
      } else if (recordIsX && liteMode) {
        const xSession = xLiteSessionForIndex(index);
        if (!xSession || !isXLiteSessionAlive(xSession)) {
          profileData = {
            success: false,
            error: "x_api_session_unavailable",
            userInfo: null,
            videos: [],
          };
          noteXLiteTapResult(index, false);
        } else {
          sendStep(
            "提取数据",
            `[${progress}] X Lite @${record.username}（GraphQL API → 国家门禁 → 近期推文）...`
          );
          try {
            const xLiteModule = await import("./x/extract-x-profile-lite.js");
            profileData = await xLiteModule.extractXProfileLite(xSession, record.username, {
              onStepUpdate: isLiteEnrichScreenshotsEnabled() ? onStepUpdate : null,
              allowedCountriesIso:
                allowedCountriesIso.length > 0 ? allowedCountriesIso : undefined,
            });
            ...
            if (profileData.skippedReason === "x_email_not_found") {
              sendStep(
                "邮箱门槛",
                `[${progress}] ⏭️ @${record.username} 未识别到邮箱，跳过推文 enrich 与 LLM 分析`
              );
            } else if (profileData.skippedReason === "bio_language_mismatch") {
              sendStep(
                "国家预筛",
                `[${progress}] ⏭️ @${record.username} bio 语言不符合 campaign（${allowedCountriesIso.join(",")}），跳过推文 enrich`
              );
              await reportSearchTaskCountrySkip(taskId, "bio_language_mismatch", onTaskProgress);
            } else if (profileData.skippedReason === "country_unknown") {
              sendStep(
                "国家预筛",
                `[${progress}] ⏭️ @${record.username} 未获取国家，跳过推文 enrich`
              );
              await reportSearchTaskCountrySkip(taskId, "country_unknown", onTaskProgress);
            } else if (profileData.skippedReason === "country_mismatch") {
              sendStep(
                "国家预筛",
                `[${progress}] ⏭️ @${record.username} 国家 ${profileData.videoPublishCountry} 不符合 campaign（${allowedCountriesIso.join(",")}），跳过推文 enrich`
              );
              await reportSearchTaskCountrySkip(taskId, "country_mismatch", onTaskProgress);
            } else if (
              allowedCountriesIso.length > 0 &&
              !profileData.videoPublishCountry &&
              (profileData.countrySource === "bio_language_maybe" ||
                profileData.countrySource === "location")
            ) {
              sendStep(
                "国家预筛",
                `[${progress}] ✅ @${record.username} 国家未知，bio 语言可能符合，继续推文 enrich`
              );
            } else if (
              allowedCountriesIso.length > 0 &&
              profileData.videoPublishCountry
              ...
              sendStep(
                "国家预筛",
                `[${progress}] ✅ @${record.username} 国家 ${profileData.videoPublishCountry} 符合，推文 enrich 完成`
              );
            }
            const elapsedTime = ((Date.now() - startTime) / 1000).toFixed(1);
            console.log(
              `[enrichInfluencerProfiles] [${progress}] ✅ X Lite 完成（${elapsedTime}s）`
            );
          } catch (error) {
            profileData = {
              success: false,
            ...
            console.warn(
              `[enrichInfluencerProfiles] [${progress}] ❌ X Lite 异常: ${error.message}`
            );
          }
          noteXLiteTapResult(
            index,
      ...
      if (skipLiveMatchForLowFollowers) {
        ...
        sendStep(
          "粉丝过滤",
          `[${progress}] ⏭️ @${record.username} 粉丝 ${Number.isFinite(followerCount) ? followerCount : "低于500"}，跳过视频 enrich、画像分析与候选池`
        );
      }
      ...
      if (skipLiveMatchForMissingProfileEmail) {
        ...
        sendStep(
          "邮箱过滤",
          `[${progress}] ⏭️ @${record.username} bio/profile 未识别到邮箱，跳过视频 enrich、画像分析与候选池`
        );
      }
      ...
      if (
        (recordIsInstagram || recordIsYouTube) &&
        allowedCountriesIso.length > 0
      ) {
      ...
      if (profileData.success && mergedRecord.username) {
        try {
          ...
          if (dbSaveResult.success) {
            ...
            // 画像语言写旁路小表：不动千万级大表，失败也不影响主流程。
            if (mergedRecord.bioLanguage && dbPlatformInfluencerId) {
              try {
                const langResult = await upsertBioLanguage({
                  influencerId: dbPlatformInfluencerId,
                  bioLanguage: mergedRecord.bioLanguage,
                  confidence: mergedRecord.bioLanguageConfidence,
                  source: mergedRecord.bioLanguageSource,
                });
                if (langResult?.written) {
                  logEntry.bioLanguage = {
                ...
                console.warn(
                  `[enrichInfluencerProfiles] [${progress}] ⚠️ 画像语言写入失败: @${record.username} - ${langErr?.message || langErr}`
                );
              }
            }
      ...
      if (
        ...
        const matchTask = {
          ...
          index,
          onStepUpdate,
          // 异步路径：IG 拉取已结束，LLM 分析结果正常落库，不因预算丢弃
          // YT 异步路径同理：拉取已结束，分析结果正常落库
          checkAbort:
            igLiteAnalysisAsync || ytLiteAnalysisAsync
              ? () => false
              : () => perInfluencerAborted,
        };
        if (igLiteAnalysisAsync) {
          enqueueIgAnalysis(matchTask);
          console.log(
            `[enrichInfluencerProfiles] [${progress}] 📥 @${mergedRecord.username} 已入异步分析队列，继续下一位红人`
          );
        } else if (ytLiteAnalysisAsync) {
          enqueueYtAnalysis(matchTask);
          console.log(
            `[enrichInfluencerProfiles] [${progress}] 📥 @${mergedRecord.username} 已入 YT 异步分析队列，继续下一位红人`
          );
        } else {
          const outcome = await runInfluencerMatch(matchTask);
          if (outcome?.aborted) {
            return { mergedRecord, logEntry, success: false, dbSaveResult };
          }
        }
...
async function searchAndExtractInfluencers_v2(params = {}, options = {}) {
  ...
  async function runConcurrencyChunk(chunk, startIndex, batchTotal, moreInPool) {
    ...
    // 任务中途账户限流：命中即中止整个 enrich（上层把任务标记 failed/account_throttled）
    if (isInstagramEnrich && liteMode) {
      const { consumeIgAccountThrottleFlag } = await import(
        "../../ig-account-rotation.js"
      );
      const flag = consumeIgAccountThrottleFlag(
        process.env.IG_LITE_ENRICH_CDP_ENDPOINTS ||
          process.env.CDP_ENDPOINT ||
          "http://127.0.0.1:9222"
      );
      if (flag) {
        const err = new Error(
          `account_throttled_mid_task: ${flag.reason || "unknown"}`
        );
        err.code = "IG_ACCOUNT_THROTTLED_MID_TASK";
        throw err;
      }
    }
    ...
    for (const { mergedRecord, logEntry, success, skippedExisting } of batchResults) {
      ...
      if (mergedRecord?.isRecommended === true) chunkRecommended++;
    }
  ...
  if (enrichBatchPolicy) {
    ...
    while (poolCursor < recordsToEnrich.length) {
      ...
      while (
        ...
        const skipReason = await shouldSkipExistingCandidate(rec);
        if (skipReason) {
          const isExcludedSkip = skipReason === "excluded";
          if (isExcludedSkip) exclusionSkipCount++;
          else skippedExistingCount++;
      ...
      sendStep(
        "enrich 批次",
        `第 ${batchIndex} 批：enrich+分析 ${batchRecords.length} 位红人（每批 ${enrichBatchSize} 人，连续 ${enrichBatchZeroStreakLimit} 批 0 符合才停）`
      );

      // 受限流水线：并发池（在途上限 = runtimeEffectiveConcurrency）。
      // 每处理完一条红人立即取下一条，拉取与分析自然重叠，
      // 不再等整块（含 LLM 分析）全部完成才拉下一块。
      // IG Lite 异步模式：拉取完即入分析队列，批次边界排空后再做 0 推荐判定。
      activeBatchStats = { completed: 0, recommended: 0, contactable: 0, failed: 0 };
      ytActiveBatchStats = { completed: 0, recommended: 0, contactable: 0, failed: 0 };
      let batchRecommended = 0;
      let poolIdx = 0;
      let processedInBatch = 0;
      let lastShortRest = 0;
      let lastLongRest = 0;
      const workerCount = Math.min(runtimeEffectiveConcurrency, batchRecords.length);
      const moreRemaining = () =>
        poolIdx < batchRecords.length || poolCursor < recordsToEnrich.length;
      const runPoolWorker = async () => {
        while (poolIdx < batchRecords.length) {
          const idx = poolIdx++;
          const rec = batchRecords[idx];
          const { mergedRecord, logEntry, success, skippedExisting } =
            await processOneRecord(
              rec,
              totalAnalyzedCount + idx,
              batchRecords.length
            );
          // 任务中途账户限流：命中即中止整个 enrich（上层把任务标记 failed/account_throttled）
          if (isInstagramEnrich && liteMode) {
          ...
          const randomRestEnabled =
            !liteMode ||
            String(process.env.LITE_ENRICH_RANDOM_REST || "").trim() === "1";
          if (randomRestEnabled && moreRemaining()) {
            if (processedInBatch % 5 === 0 && processedInBatch !== lastShortRest) {
              lastShortRest = processedInBatch;
              const extraDelay = 10000 + Math.floor(Math.random() * 10000);
              sendStep(
                "休息",
                `已处理 ${processedInBatch} 个红人，额外休息 ${(extraDelay / 1000).toFixed(1)} 秒...`
              );
              await new Promise((resolve) => setTimeout(resolve, extraDelay));
            }
            if (processedInBatch % 10 === 0 && processedInBatch !== lastLongRest) {
              lastLongRest = processedInBatch;
              const longRest = 30000 + Math.floor(Math.random() * 30000);
              sendStep(
                "长时间休息",
                `已处理 ${processedInBatch} 个红人，长时间休息 ${(longRest / 1000).toFixed(1)} 秒...`
              );
              await new Promise((resolve) => setTimeout(resolve, longRest));
            }
          }
      ...
      await Promise.all(
        Array.from({ length: workerCount }, () => runPoolWorker())
      );
      if (igLiteAnalysisAsync || ytLiteAnalysisAsync) {
        // 等待本批异步分析全部完成，再基于真实推荐数做停止判定
        await awaitIgAnalysisDrain();
        if (ytLiteAnalysisAsync) await awaitYtAnalysisDrain();
        batchRecommended =
          activeBatchStats.recommended + ytActiveBatchStats.recommended;
        totalAnalyzedCount +=
          activeBatchStats.completed + ytActiveBatchStats.completed;
        sendStep(
          "分析队列",
          `本批异步分析完成 ${activeBatchStats.completed + ytActiveBatchStats.completed}/${batchRecords.length} 位（推荐 ${batchRecommended}，失败 ${activeBatchStats.failed + ytActiveBatchStats.failed}）`
        );
      } else {
        totalAnalyzedCount += batchRecords.length;
      }

      if (enrichBatchStopOnZeroMatch && batchRecommended === 0) {
        zeroMatchStreak += 1;
        if (zeroMatchStreak >= enrichBatchZeroStreakLimit) {
          sendStep(
            "批次停止",
            `本批 ${batchRecords.length} 人 analyze 后 0 人推荐，结束本关键词任务（已分析 ${totalAnalyzedCount} 人）`
          );
          recordsToSkip = recordsToEnrich.slice(poolCursor);
          break;
        }
        sendStep(
          "批次继续",
          `本批 0 人推荐（连续 ${zeroMatchStreak}/${enrichBatchZeroStreakLimit}），继续下一批 ${enrichBatchSize} 人`
        );
      } else if (batchRecommended > 0) {
        zeroMatchStreak = 0;
        sendStep(
          "批次继续",
    ...
    if (igLiteAnalysisAsync || ytLiteAnalysisAsync) {
      await awaitIgAnalysisDrain();
      if (ytLiteAnalysisAsync) await awaitYtAnalysisDrain();
      totalAnalyzedCount = igAnalysisCompletedTotal + ytAnalysisCompletedTotal;
    } else {
  ...
  try {
    ...
    const logData = {
      summary: {
        ...
        skippedExclusion: exclusionSkipCount,
    ...
    if (exclusionSkipCount > 0) {
      console.log(`   不联系名单跳过: ${exclusionSkipCount} 个`);
    }
  ...
  sendStep(
    '完成',
    `✅ 主页数据提取完成：成功 ${successCount}，失败 ${failedCount}，跳过 ${recordsToSkip.length}（候选表 ${skippedExistingCount}，不联系名单 ${exclusionSkipCount}）`
  );
  ...
  if (xLitePagePool.length) {
    for (const session of xLitePagePool) {
      try {
        await session.dispose();
      } catch (e) {
        console.warn(
          "[enrichInfluencerProfiles] 关闭 X Lite 会话失败:",
          e?.message || e
        );
      }
    }
    console.log(
      `[enrichInfluencerProfiles] X Lite API 会话已释放（${xLitePagePool.length} 页）`
    );
  }
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  const { platformSlug, useInstagram, useYouTube, useX } = resolveTaskSearchChannels(
    platformParam || platformOption || platforms[0] || "tiktok"
  );

  console.log(
    `[searchAndExtractInfluencers] platform=${platformSlug} useInstagram=${useInstagram} useYouTube=${useYouTube} useX=${useX} (task-only routing)`
  );

  if ((useYouTube || useX) && isLiteScraperMode() && !options.cdpContext) {
    return withCdp9222PreparedSession(
      { platform: useX ? "x" : "youtube", phase: "lite-pipeline" },
      async ({ context }) =>
        searchAndExtractInfluencers(params, { ...options, cdpContext: context })
    );
  }

  const useTikTok = !useInstagram && !useYouTube && !useX;
  ...
  const enrichStopOnZero = resolveEnrichStopOnZeroMatch();
  console.log(
    `[searchAndExtractInfluencers] searchPoolMax=${searchPoolMax} enrichBatchPolicy=${enrichBatchPolicy}` +
      (enrichBatchPolicy
        ? `（每批${Math.max(1, Number(process.env.ENRICH_BATCH_SIZE || 20))}人 enrich${enrichStopOnZero ? "，0推荐则停" : "，跑完全池"}；国家${isLiteScraperMode() ? "全池" : `每批${Math.max(1, Number(process.env.COUNTRY_BATCH_SIZE || 20))}人`}${countryStopOnZero ? " 0明确符合则停" : ""}）`
  ...
  try {
    ...
    sendStep(
      ...
      useYouTube
        ...
        : useInstagram
          ...
          : useX
            ? '正在 X 关键词搜索（GraphQL Lite，9222，需已登录 X）...'
    ...
    if (!useInstagram && !useYouTube && !useX && extractionResult.influencerRecords.length > 0) {
      sendStep(
        '采集视频发布地',
      ...
      sendStep(
        "采集视频发布地",
        "X 国家将在主页 enrich 时从 location/bio 推断（不做国家预筛）"
      );
    }
    ...
    let xLiteSession = searchResult.xLiteSession || null;
    ...
    if (enrichProfileData && recordsToEnrich.length > 0) {
      sendStep(
        ...
        useYouTube
          ...
          : useInstagram
            ...
            : useX
              ? `开始提取 ${recordsToEnrich.length} 位 X 红人主页（9222 Lite GraphQL）...`
      ...
      const enrichedRecords = await enrichInfluencerProfiles(
        recordsToEnrich,
        {
          ...
          concurrency:
            liteMode && useYouTube
              ...
              : liteMode && useInstagram
                ...
                : liteMode && useX
                  ? resolveLiteEnrichConcurrency("x")
                  : liteMode && useTikTok
                    ? resolveLiteEnrichConcurrency("tiktok")
                    : 1,
          ...
          productInfo,
          campaignInfo,
          platform: useYouTube ? "youtube" : useInstagram ? "instagram" : useX ? "x" : "tiktok",
          taskId: tid || null,
          campaignId,
          taskMeta,
          ...
          enrichBatchStopOnZeroMatch: enrichStopOnZero,
          enableLiveMatch:
            !!influencerProfile &&
            process.env.SKIP_LLM_ANALYSIS !== "1" &&
            process.env.SKIP_LLM !== "1",
          ...
          xLiteSession: liteMode && useX ? xLiteSession : null,
          skipCdpWrap: !!(
            liteMode &&
            (useYouTube || useInstagram || useX) &&
            pipelineCdpContext
          ),
          cdpContext: liteMode && (useYouTube || useInstagram || useX) ? pipelineCdpContext : null,
    ...
    if (xLiteSession) {
      try {
        await xLiteSession.dispose();
      } catch (e) {
        console.warn("[searchAndExtractInfluencers] 清理 X Lite 会话失败:", e?.message || e);
      }
      xLiteSession = null;
    }
    ...
    const result = {
      ...
      influencers: finalInfluencerRecords.map(record => {
        ...
        return {
          ...
          platform:
            record.platform ||
            (useYouTube ? 'YouTube' : useInstagram ? 'Instagram' : useX ? 'X' : 'TikTok'),
