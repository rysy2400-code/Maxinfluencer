...
import { attachXGraphqlCollector, waitForUiElements, xSpaNavigate } from "./x-browser-io.js";
...
export async function extractXSearchLite(context, keyword, options = {}) {
  ...
  // 默认 Media：A/B 实测（WorkBuddy 5 关键词对比）Media 每词红人量约 Top 1.7x、
  // 进入分析/候选池约 3x，更适合"找可合作内容创作者"；找头部大 V 可切 top。
  const product = process.env.X_LITE_SEARCH_PRODUCT || "media";
  ...
  try {
    ...
    const searchPath = `/search?q=${encodeURIComponent(keyword)}&f=${searchF}`;
    const searchUrl = `https://x.com${searchPath}`;
    console.log(`[extractXSearchLite] 打开搜索页 ${searchUrl} product=${product}`);
    // 优先 SPA 路由跳转（脚本不重载，仅 API 流量）；失败回退整页加载
    const spaOk = await xSpaNavigate(page, searchPath);
    if (!spaOk) {
    ...
    if (firstBatches.length === 0) {
      ...
      if (spaOk) {
        // SPA 路由可能未生效：整页回退一次
        console.warn("[extractXSearchLite] SPA 跳转未触发结果，回退整页加载");
