...
import { forceCloseCdpPage } from '../../cdp/cdp-tab-utils.js';
...
export async function searchInfluencersByKeyword(params = {}, options = {}) {
  ...
  if (USE_HEADLESS) {
    ...
    // 采集时创建新页面，减少其它 Tab/页面状态对 API 拦截的影响（登记到 loop 以便兜底关闭）
...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  if (USE_HEADLESS) {
    ...
    if (isInstagramEnrich || isYouTubeEnrich) {
      console.log(`[enrichInfluencerProfiles] 💡 提示: ${isYouTubeEnrich ? "YouTube" : "Instagram"} 主页提取需已登录，使用 9222`);
    } else {
      console.log(`[enrichInfluencerProfiles] 💡 提示: TikTok 主页提取使用 9222（与搜索同一 Chrome）`);
    }
    ...
    while (retryCount < maxRetries) {
      try {
        browser = await connectCdp9222({
          platform: enrichPlatformSlug || "tiktok",
          phase: "enrich",
        });
  ...
  const processOneRecord = async (record, index) => {
    ...
    const run = async () => {
      try {
        profilePage = await openCdpContextPage(context);
        try {
...
async function searchAndExtractInfluencers_v2(params = {}, options = {}) {
  ...
  // 关闭 CDP 连接（不关闭用户手动启动的 Chrome）
  if (browser) {
    try {
      if (!USE_HEADLESS && typeof browser.disconnect === "function") {
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  const useInstagram =
    isInstagramPlatform(platformSlug) ||
    platforms.some((p) => isInstagramPlatform(p));
  const useYouTube =
    !useInstagram &&
    (isYouTubePlatform(platformSlug) ||
      platforms.some((p) => isYouTubePlatform(p)));

  console.log(
    `[searchAndExtractInfluencers] platform=${platformSlug} useInstagram=${useInstagram} useYouTube=${useYouTube}`
  );
  ...
  try {
    ...
    const searchResult = useYouTube
          ...
          { keywords, campaignInfo },
          {
            ...
            searchOptions: {
              maxChannels: Math.min(
                Number(process.env.YT_SEARCH_MAX_CHANNELS || 20),
                maxResults
              ),
            },
          }
      ...
      : useInstagram
        ? await searchInstagramByKeyword(
            { keywords, campaignInfo },
            { onStepUpdate, searchOptions: { maxInfluencers: maxResults } }
          )
        : await searchInfluencersByKeyword({ keywords, campaignInfo }, { onStepUpdate });
    const extractionResult = {
      influencerRecords: searchResult.influencerRecords,
      videos: searchResult.videos,
      stats: searchResult.stats
    ...
    if (!useInstagram && !useYouTube && extractionResult.influencerRecords.length > 0) {
      sendStep(
        '采集视频发布地',
        `正在采集前 ${maxEnrichCount} 位红人的视频发布地并过滤国家...`
      );
