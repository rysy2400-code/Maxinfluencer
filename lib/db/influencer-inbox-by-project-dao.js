...
export async function listInfluencerInboxByProject({
  ...
  const execSql = `
    WITH latest AS (
      ...
      WHERE m.influencer_id IN (
        SELECT COALESCE(ri.influencer_id, ru.influencer_id)
        ...
        WHERE s.advertiser_user_id IS NOT NULL
          ...
          AND COALESCE(ri.influencer_id, ru.influencer_id) IS NOT NULL
      )
    )
    ...
    LEFT JOIN latest lm ON lm.rn = 1 AND lm.influencer_id = COALESCE(i.influencer_id, iu.influencer_id)
    WHERE s.advertiser_user_id IS NOT NULL
      AND s.advertiser_user_id IN (${inPlaceholders})
    ORDER BY c.created_at ASC, c.id ASC
  `;

  // execSql 中 CTE 缩窄子查询与外部 WHERE 各出现一次 ${inPlaceholders}
  const execRows = await queryTikTok(execSql, [...advIds, ...advIds]);
  ...
  `;
  if (like) {
    emptyParams.push(like, like, like, like, like, like, like, like, like, like, like);
  }
  const emptyRows = await queryTikTok(emptySql, emptyParams);

  let orphans = [];
  try {
    orphans = await loadOrphanInfluencers({ like });
  } catch (error) {
    // 孤儿列表只是补充视图；超时/失败时降级为空，避免阻塞整个收件箱接口
...
function assembleAccounts(pageAccounts, execRows, emptyRows) {
  ...
  for (const row of execRows) {
    ...
    c.influencers.push({
      ...
      handoverMode: row.handover_mode || "auto",
