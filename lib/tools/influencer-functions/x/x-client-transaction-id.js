...
/** 拉取 x.com 首页 HTML（登录态无关，供 txid 初始化数据兜底） */
export async function fetchHomeHtmlForTxInit() {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 25000);
  try {
    const resp = await fetch("https://x.com/home", {
      headers: {
        "User-Agent": HOME_HTML_UA,
        "Accept-Language": "en-US,en;q=0.9",
      },
      redirect: "follow",
      signal: controller.signal,
    });
    if (!resp.ok) throw new Error(`x.com home HTTP ${resp.status}`);
...
/** 从首页 HTML 解析 txid 初始化数据（对齐 elongator：meta key + ondemand hash + loading-x-anim 动画帧） */
export function parseTxInitFromHtml(html) {
  const $ = cheerio.load(html);
  const siteVerification =
    $('meta[name="twitter-site-verification"]').first().attr("content") || "";
  ...
  const animFramePaths = $('[id^="loading-x-anim"]')
    .toArray()
    .map((el) => {
      const $el = $(el);
      const g = $el.children().first();
      const pathEl = g.children().eq(1);
      return pathEl.attr("d") || "";
    });
  return { siteVerification, ondemandHash, animFramePaths };
}

/** 网络兜底 harvest：无需页面 DOM 状态（登录后页面无 loading-x-anim） */
export async function harvestTransactionInitFromNetwork() {
  const html = await fetchHomeHtmlForTxInit();
  const parsed = parseTxInitFromHtml(html);
  if (
    !parsed.siteVerification ||
    !parsed.ondemandHash ||
    parsed.animFramePaths.length < 4
  ) {
    throw new Error(
      `X txid 网络 harvest 不完整: sv=${!!parsed.siteVerification} hash=${parsed.ondemandHash || "?"} anim=${parsed.animFramePaths.length}`
    );
  }
  return parsed;
}
