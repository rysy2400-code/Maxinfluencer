...
function mapInfluencerRow(r) {
  return {
    ...
    // 红人本人确认的常住地（跨 campaign 复用；首封邮件据此决定是否再问）
    residenceCountry: r.residence_country || null,
    residenceCountrySource: r.residence_country_source || null,
    residenceCountryEvidence: r.residence_country_evidence || null,
    // 账号国家（平台抓取）
    accountCountry: r.video_publish_country || null,
...
/**
 * 同一 influencer_id 可能对应多个平台账号（platform + username 唯一）。
 * 商务档案追问护栏用它把已知平台身份补进 Platform Profiles，避免重复问主页链接。
 */
export async function listInfluencerPlatformIdentities(influencerId) {
  const id = String(influencerId || "").trim();
  if (!id) return [];
  const rows = await queryTikTok(
