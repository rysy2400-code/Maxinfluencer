...
// YouTube handle 规则：3-30 位 ASCII（字母/数字/._-）。
// 必须校验长度并要求以分隔符收尾，否则 /@AI%EB%A1%9C...（百分号编码的频道名路径）
// 会被旧正则截断成 "AI"，进而把多个不同频道折叠成同一个账号。
...
function handleFromCanonicalUrl(url) {
  ...
  if (!m) return null;
  const handle = m[1];
  return YT_HANDLE_RE.test(handle) ? handle : null;
}
...
export function channelFromVideoRenderer(vr) {
  ...
  for (const run of ownerRuns) {
    ...
    if (t.startsWith("@")) {
      ...
      if (YT_HANDLE_RE.test(candidate)) handle = candidate;
    }
...
export function computeYtVideoStatistics(videos) {
  // 口径统一：剔除播放量为 0/缺失的视频，均值分母同步用剔除后的条数
  ...
  return {
    ...
    avgLikes: avgFrom(withViews, (v) => v.likes?.count),
    avgComments: avgFrom(withViews, (v) => v.comments?.count),
    // 中位播放 + 点赞率/评论率：与均播同一批样本（views > 0）
    ...computeViewMedianAndRates(withViews),
