...
function pickLocationFromItem(item) {
  ...
  if (poi?.regionCode != null && poi.regionCode !== "") return String(poi.regionCode);
  if (poi?.countryCode != null && poi.countryCode !== "") return String(poi.countryCode);
  if (item.region != null && item.region !== "") return String(item.region);
  return null;
}

/**
 * 不打开视频页：在 tiktok.com 上下文中 fetch 视频 HTML，解析 UNIVERSAL 中的 locationCreated
 * （9222 登录态有效；9223 guest 通常只返回精简 HTML）
 */
...
/**
 * 不打开视频页：search → item_detail → post/item_list 解析 locationCreated
 * @param {object} page
...
export async function resolveVideoLocationCreated(page, opts = {}) {
  ...
  let secUid = opts.secUid ? String(opts.secUid) : "";
  ...
  if (!secUid && username) {
    try {
      ...
      secUid = user?.secUid || user?.sec_uid || secUid;
    } catch {
      /* ignore */
    }
  }
  ...
  if (videoId) {
    const attempts = [
      ["https://www.tiktok.com/api/post/item_detail/", { itemId: videoId }],
      ["https://www.tiktok.com/api/post/item_detail/", { item_id: videoId }],
      ["https://www.tiktok.com/api/item/detail/", { itemId: videoId }],
      ["https://www.tiktok.com/api/item/detail/", { item_id: videoId }],
    ];
    ...
    for (const [apiUrl, params] of attempts) {
      try {
        const json = await tiktokMakeRequest(page, apiUrl, params, { referer });
        const loc = parseVideoLocationFromDetailJson(json);
        if (loc) return { locationCreated: loc, source: "item_detail_api" };
      } catch {
        /* try next */
      }
    }
  ...
  if (secUid) {
    try {
      const json = await fetchPostItemList(page, {
        secUid,
        count: Math.min(Number(process.env.TT_LITE_COUNTRY_ITEM_LIST_COUNT || 20), 35),
      ...
      const hit = videoId
        ...
        : list[0];
      const loc = pickLocationFromItem(hit);
      if (loc) {
        return {
          locationCreated: loc,
          source: videoId ? "post_item_list_api" : "post_item_list_api_first",
        };
      }
      for (const v of list) {
        const scanned = pickLocationFromItem(v);
        if (scanned) {
          return { locationCreated: scanned, source: "post_item_list_api_scan" };
        }
      }
    } catch (e) {
  ...
  if (videoId && username) {
    const htmlLoc = await fetchLocationCreatedFromVideoHtmlRequest(page, username, videoId);
    if (htmlLoc) {
      return { locationCreated: htmlLoc, source: "video_html_fetch" };
    }
  }

  return { locationCreated: null, source: null, error: secUid ? "no_location_in_api" : "missing_sec_uid" };
}
