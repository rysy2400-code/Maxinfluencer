...
function connectTimeoutMs() {
  // 默认 20s：Chrome 忙碌（IG 页面/代理加载）时 CDP 握手偶发超过 10s，
  // 过短会导致搜索阶段整任务失败。不影响 IG API 调用间隔。
...
export async function withCdp9222PreparedSession(meta, fn) {
  const run = async () => {
    ...
    try {
      ...
      const skipPrune = String(process.env.CDP_9222_SKIP_PRUNE || "")
        .trim()
        .toLowerCase();
      if (skipPrune !== "1" && skipPrune !== "true" && skipPrune !== "yes") {
