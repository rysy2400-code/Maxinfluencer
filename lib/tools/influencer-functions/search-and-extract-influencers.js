    ...
    (influencerRecords || []).some((r) => r.platform === "YouTube")
      ...
      : String(enrichPlatformOption || "").toLowerCase() === "instagram" ||
        ...
        : String(enrichPlatformOption || "").toLowerCase() === "tiktok" ||
          ...
          ? "tiktok"
          : null;
  const concurrencyOverride = Number(concurrency);
  const effectiveConcurrency =
    liteMode && litePlatformForConcurrency
      ? Number.isFinite(concurrencyOverride) && concurrencyOverride > 0
        ...
        : resolveLiteEnrichConcurrency(litePlatformForConcurrency)
      : Math.max(1, Number.isFinite(concurrencyOverride) ? concurrencyOverride : 1);
    ...
    while (ttLitePagePool.length < poolSize) {
      ...
      for (const endpointKey of endpointCandidates) {
        try {
          ...
          console.log(
            `[enrichInfluencerProfiles] TikTok Lite: 复用 API 页 ${ttLitePagePool.length + 1}/${poolSize} via ${endpointKey}（enrich 并发 ${effectiveConcurrency}）`
          );
          break;
        } catch (e) {
          lastErr = e;
    ...
    const run = async () => {
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!ytLitePage) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] YouTube Lite v2 @${record.username}（/about → 国家门禁 → API 视频）...`
          );
          try {
        ...
        useYouTube
          ...
          : useInstagram
            ...
            : useTikTok && isLiteScraperMode()
              ? `开始提取 ${recordsToEnrich.length} 位 TikTok 红人主页（9222 Lite signed API）...`
