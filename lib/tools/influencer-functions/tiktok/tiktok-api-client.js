...
export async function recoverTiktokPageFromAccessDenied(page) {
  ...
  if (process.env.TT_LITE_STRICT_API_ONLY_NO_GOTO === "1") return false;
  ...
  if (!ready) {
    ...
    if (needsHome && typeof page.goto === "function") {
      ...
      await page
        ...
        .catch(() => {});
    } else if (current.includes("tiktok.com") && process.env.TT_LITE_STRICT_API_ONLY_NO_GOTO === "1") {
      console.log(`[tiktok-api] refresh homepage context via reload ${key}`);
    ...
    try {
      ...
      if (result.isHtml || items === 0) {
        ...
        const cooldownMs = Math.max(
          0,
          Number(process.env.TT_LITE_EMPTY_ITEMS_COOLDOWN_MS || 15000)
        );
        if (cooldownMs > 0) {
