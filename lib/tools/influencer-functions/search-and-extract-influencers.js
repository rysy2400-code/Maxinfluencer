...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  async function ensureTtLitePagePool() {
    ...
    // 任务级会话刷新：enrich 阶段开始时清 cookies 重建匿名指纹（默认开，TT_LITE_SESSION_REFRESH_ON_START=0 关）。
    if (
      String(process.env.TT_LITE_SESSION_REFRESH_ON_START ?? "1").trim() !== "0" &&
      ttLitePagePool.length
    ) {
      const { refreshTiktokApiSession } = await import(
        "./tiktok/tiktok-api-client.js"
      );
      for (const session of ttLitePagePool) {
        try {
          await refreshTiktokApiSession(session.page);
          console.log(
            `[enrichInfluencerProfiles] TikTok Lite task-start session refreshed ${session.endpointKey || "-"}`
          );
        } catch (e) {
          console.warn(
            `[enrichInfluencerProfiles] TikTok Lite task-start session refresh failed ${session.endpointKey || "-"}: ${e.message}`
          );
        }
      }
    }
  ...
  /** TikTok Lite enrich 周期会话刷新：单 tab 累计 N 条后清 cookies 重建匿名指纹（默认 0=关闭，任务级刷新已覆盖） */
  async function maybeRefreshTtLiteSession(page, index) {
    if (!page || !ttLitePagePool.length) return;
    const poolIdx = index % ttLitePagePool.length;
    ttLiteRefreshCounters[poolIdx] = (ttLiteRefreshCounters[poolIdx] || 0) + 1;
    const interval = Math.max(
      0,
      Math.floor(Number(process.env.TT_LITE_ENRICH_SESSION_REFRESH_INTERVAL || 0))
    );
    if (interval <= 0 || ttLiteRefreshCounters[poolIdx] % interval !== 0) return;
    try {
      const { refreshTiktokApiSession } = await import(
        "./tiktok/tiktok-api-client.js"
      );
      ...
      console.log(
        `[enrichInfluencerProfiles] TikTok Lite periodic session refreshed slot ${poolIdx} count=${ttLiteRefreshCounters[poolIdx]}`
      );
    } catch (e) {
      console.warn(
        `[enrichInfluencerProfiles] TikTok Lite periodic session refresh failed slot ${poolIdx}: ${e.message}`
      );
    }
  }
    ...
    const run = async () => {
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!ttLitePage || !isTtLitePageAlive(ttLitePage)) {
          ...
          await maybeRefreshTtLiteSession(ttLitePage, index);
          sendStep(
            "提取数据",
