...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  if (isLiteScraperMode()) {
    ...
    async function probeLocation(record, sessionIdx) {
      // bio 语言门禁前置：明确不符者不抓视频页（省 40KB+/人）
      const bioGate = resolveUnknownCountryBioGate(record.bio, allowedIso);
      if (!bioGate.proceed) {
        return {
          ...
          error: null,
          bioGateSkipped: true,
          bioGateReason: bioGate.skippedReason,
        };
      }
      ...
      const page = activePool[sessionIdx % activePool.length].page;
      try {
        const result = await resolveVideoLocationCreatedForInfluencer(page, {
          videoId: primaryVid,
          altVideoIds,
          username,
          secUid,
        });
        return { ...result, bioCountrySource: bioGate.countrySource || null };
      } catch (e) {
        return {
          locationCreated: null,
          source: null,
          error: e.message,
          bioCountrySource: bioGate.countrySource || null,
        };
      }
    }
    ...
    async function processOneRecord(record, globalIndex, probe) {
      ...
      let bioCountrySource = null;
      let bioPass = false;
      if (probe.bioGateSkipped) {
        // bio 语言前置已判不符，未抓视频页
        enrichSkippedReason = probe.bioGateReason || "bio_language_mismatch";
        bioMismatch += 1;
        ...
        bioPass = true;
        bioPassed += 1;
        if (probe.bioCountrySource) {
          bioCountrySource = probe.bioCountrySource;
        }
      } else if (!pass) {
      ...
      if (bioCountrySource) {
        enrichedRecord.video_publish_country_source = bioCountrySource;
      }
      checked.push(enrichedRecord);

      // Lite：明确符合国家，或国家未知但 bio 语言符合/不可识别者，进入 enrich
      if (explicitMatch || bioPass) {
      ...
      const outcome = {
        ...
        representative_video_id:
          probe.representativeVideoId || record.representativeVideoId || null,
        enrich_skipped_reason: enrichSkippedReason,
        country_passed: explicitMatch || bioPass,
      };
        ...
        try {
          if (importTid) {
            await bumpImportTaskCountryProgress(importTid, {
              checkedDelta: 1,
              passedDelta: explicitMatch || bioPass ? 1 : 0,
            });
    ...
    sendStep(
      "采集视频发布地",
      `Lite endpoint pool API-only 发布地完成：检查 ${checked.length} 人，进入 enrich ${passed.length} 人（明确符合 ${explicitMatches} + bio 语言 ${bioPassed}），国家 unknown ${unknown} mismatch ${mismatch} bio 语言不符 ${bioMismatch}${stoppedEarly ? "（提前停止）" : ""}`
    );
  ...
  let bioMismatch2 = 0;
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      if (!locationCreated) {
        ...
        const bioGate = resolveUnknownCountryBioGate(record.bio, allowedIso);
        if (!bioGate.proceed) {
          enrichSkippedReason = bioGate.skippedReason || "country_unknown";
          bioMismatch2 += 1;
        } else {
          bioPass = true;
          bioPassed2 += 1;
          if (bioGate.countrySource) {
            bioCountrySource = bioGate.countrySource;
          }
        }
      } else if (!pass) {
        enrichSkippedReason = "country_mismatch";
      ...
      const outcome = {
        ...
        enrich_skipped_reason: enrichSkippedReason,
        country_passed: (pass && !!locationCreated) || bioPass,
      };
      outcomes.push(outcome);
      ...
      if (importTid || tid) {
        try {
          if (importTid) {
            await bumpImportTaskCountryProgress(importTid, {
              checkedDelta: 1,
              passedDelta: pass && locationCreated || bioPass ? 1 : 0,
            });
            await touchImportTaskLastProgressAt(importTid);
          } else {
            await bumpSearchTaskCountryProgress(tid, {
      ...
      if ((pass && locationCreated) || bioPass) {
        passed.push(enrichedRecord);
      }
  ...
  sendStep(
    "采集视频发布地",
    `发布地采集完成：检查 ${checked.length} 人，进入 enrich ${passed.length} 人（明确符合 + bio 语言 ${bioPassed2}），unknown ${unknown} mismatch ${mismatch} bio 语言不符 ${bioMismatch2}`
  );
  ...
  return {
    ...
    stats: {
      ...
      unknown,
      mismatch,
      bioPassed: bioPassed2,
      bioMismatch: bioMismatch2,
    },
  };
    }
  );
