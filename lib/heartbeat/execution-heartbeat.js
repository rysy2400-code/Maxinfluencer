...
// LLM 并发默认不设上限（用户确认）；如确有需要，可显式设置 EXECUTION_MAX_PARALLEL_WORKERS=N。
function resolveMaxParallelFromEnv() {
  const raw = process.env.EXECUTION_MAX_PARALLEL_WORKERS;
  if (raw == null || String(raw).trim() === "") return null;
  const n = Number(raw);
  return Number.isFinite(n) && n > 0 ? n : null;
}
const DEFAULT_MAX_PARALLEL = resolveMaxParallelFromEnv();
const DEFAULT_TASK_BATCH_SIZE = Math.max(
  1,
  Number(process.env.EXECUTION_TASK_BATCH_SIZE || 20) || 20
);
// 单次心跳 tick 的整体超时看门狗：正常情况下一次 tick 约 2~4 分钟即可完成；
// 超过该时长说明有 LLM/DB 调用挂死（如 LLM 无超时、Promise 永不结束）。
// 超时后本次 tick 以错误结束，调度进程退出，等待下一轮计划任务接管
// （计划任务为 IgnoreNew 策略，只有当前实例退出后下一轮才会真正执行）。
const EXECUTION_HEARTBEAT_TIMEOUT_MS = Math.max(
  60_000,
  Number(process.env.EXECUTION_HEARTBEAT_TIMEOUT_MS || 15 * 60 * 1000) || 15 * 60 * 1000
);
...
async function fillExecutionFromCandidates(campaign, needCount, now) {
  ...
  // 商务档案评估（含 LLM 调用）并发执行；写库环节仍按序串行，保证计数与日志顺序稳定。
  const assessments = await Promise.allSettled(
    candidates.map(async (cand) => {
      ...
      if (!platformInfluencerId) {
        return {
          cand,
          profileDecision: { decision: "normal_outreach", reason: "missing_platform_id" },
        };
      }
      try {
        const influencer = await getInfluencerById(platformInfluencerId);
        const profileDecision = influencer
          ? await assessBusinessProfileForCampaign({ influencer, campaign, now })
          : { decision: "normal_outreach", reason: "missing_platform_id" };
        return { cand, profileDecision };
      } catch (err) {
        console.warn(
          `[ExecutionHeartbeat] 商务档案判断失败，降级为正常邀约 (campaign=${campaignId}, influencer=${cand.id}):`,
          err?.message || err
        );
        return { cand, profileDecision: { decision: "normal_outreach", reason: "assessment_error" } };
      }
    })
  );
  ...
  for (const item of assessments) {
    if (item.status === "rejected") {
      console.warn(
        `[ExecutionHeartbeat] 商务档案评估异常，跳过该候选 (campaign=${campaignId}):`,
        item.reason?.message || item.reason
      );
      continue;
    }

    const { cand, profileDecision } = item.value;
    ...
    if (profileDecision.decision === "skip") {
      await markCandidatePicked(campaignId, tiktokUsername, now, cand.platform);
      console.log(
...
async function runExecutionForCampaign(campaign, now) {
  ...
  // 不再阻塞本平台。已清空平台的关键词规划改为并行生成（默认不设并发上限，可用
  // EXECUTION_MAX_PARALLEL_WORKERS 显式限流），每个平台内部的 refill 轮次仍保持串行
  // （依赖上一轮结果去重）；规划完成后统一入队，由 DB 唯一键
  // uk_campaign_run_keyword_platform + INSERT IGNORE 兜底防重。
  const planTargets = [];
 ...
 * - maxParallel 默认不设上限；传入正数时用并发限制器兜底。
 *
...
export async function dispatchPlatformKeywordTasks({
  ...
  // 惰性包装：只有真正开始执行时才调用 planPlatform，否则并发限制器无法控制启动时机。
  const planTasks = planTargets.map((platformSlug) => () =>
    planPlatform(campaign, new Set(existingKeywords || []), platformSlug).then(
      (plans) => ({ platformSlug, plans })
    )
  );
  let settled;
  if (maxParallel == null) {
    settled = await Promise.allSettled(planTasks.map((task) => task()));
  } else {
    const limit = createConcurrencyLimiter(Math.max(1, Number(maxParallel) || 1));
    settled = await Promise.allSettled(planTasks.map((task) => limit(task)));
  }
...
async function runExecutionHeartbeatTickInner(now = new Date()) {
  ...
  // campaign 之间并发执行（默认不设上限），单个 campaign 出错不影响其它 campaign。
 ...
 * 执行心跳 tick（带整体超时看门狗）。
 *
 * - 正常情况：内部 tick 完成后立即返回；
 * - 挂死情况：超过 EXECUTION_HEARTBEAT_TIMEOUT_MS（默认 15 分钟，可用
 *   EXECUTION_HEARTBEAT_TIMEOUT_MS 环境变量调整）后 reject，调用方（计划任务入口
 *   scripts/run-execution-heartbeat.js）会退出进程，让下一轮计划任务重新拉起；
 *   网页端调用方（agent-router / campaign-execution-tools）已 try/catch，仅告警。
 */
export async function runExecutionHeartbeatTick(now = new Date()) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      const err = new Error(
        `[ExecutionHeartbeat] 心跳 tick 整体超时（${EXECUTION_HEARTBEAT_TIMEOUT_MS}ms），本次派单未完成，将退出交由下一轮计划任务接管`
      );
      err.code = "EXECUTION_HEARTBEAT_TIMEOUT";
      reject(err);
    }, EXECUTION_HEARTBEAT_TIMEOUT_MS);
  });

  try {
    return await Promise.race([runExecutionHeartbeatTickInner(now), timeoutPromise]);
  } finally {
