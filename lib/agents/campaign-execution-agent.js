...
/**
 * 把资料附件（PDF / Word / PPT / 图片 / 视频）转成 ask_influencer_special_request 可用的
 * attachments 元数据；单封最多 MAX_ATTACHMENTS_PER_MESSAGE 个。
 */
function sendableAttachmentMetas(attachments) {
  return (Array.isArray(attachments) ? attachments : [])
    ...
    .map((a) => ({
      ...
      contentType: normalizeAttachmentContentType(a?.name, a?.contentType),
...
5. 若最后一条消息要求**修改 Campaign 信息或执行筛选**（品牌、产品、平台、地区、预算、佣金、报价策略、交付结果、红人画像、关键词策略等，含「改成」「设置为」「更新为」）→ 必须 needTool=true，toolName=modify_campaign，scope=whole；**禁止** needTool=false 口头声称已改 Campaign。
   - **红人画像只有 followerRange（粉丝量）/ viewRange（播放量）/ accountType（帐号类型与内容方向）三个字段**。用户提到的互动率、点赞率、评论率、完播率等其它筛选要求，**一律原样并入 accountType 字符串**：与同一句里的基地/城市/垂类要求合并成一段完整文字，不要拆句、不要丢字。**禁止**新增 engagementRate / interactionRate 等字段（这些字段不会进入红人筛选，等于要求丢失）。
   - 若用户只单独补充互动率等要求（同句没有账户类型内容），必须把新要求**合并进快照中现有的「红人帐号类型要求」**再整段写回 accountType，**不得覆盖或丢弃原有内容**（如原有「base在巴西圣保罗加戈亚尼亚的美食探店类或母婴类红人都可以」+ 新增「互动率要求5%以上」→ accountType 为「互动率要求5%以上，base在巴西圣保罗加戈亚尼亚的美食探店类或母婴类红人都可以」）。
6. 若最后一条消息 @某位或多位红人 并**询问/协商/回复/同步消息**（如「帮我回复达 @xxx」「把这段话发给 @xxx」「问他能否接受某价格/做几条视频/改时间/给具体报价」「给 @a、@b、@c 分别发送寄样物流链接与内容建议」）→ 必须 needTool=true，toolName=ask_influencer_special_request。
   ...
   - **若用户要求把资料附件（见【附件/名单上下文】中标注为资料文件/图片资料/视频资料的附件，支持 PDF / Word / PPT / 图片 / 视频，单个≤25MB）发给红人**：params 必须携带 attachments 数组（每项 { fileName, storageKey, contentType, sizeBytes }）；单一位红人填 params.attachments，多位红人时每位 request 元素里都填 attachments（同一份资料多位红人共用时每位都带）；requestDetail 中说明「随信附上资料：文件名」，不要丢失文件名。单封最多 5 个附件，且单个≤25MB、**单封合计≤25MB**（合计超限邮件会被发信服务器拒收）。
   - 视频同样走附件通道（单文件≤25MB）；**只有超过 25MB 的视频**才不能上传，需让用户提供可公开访问的链接（YouTube / Google Drive / Dropbox / Loom 等）并写进 requestDetail 正文。
...
13. 红人名单导入：用户**上传 Excel/CSV 附件**、粘贴**红人主页链接**或 **@用户名 列表** → 属于名单导入场景（见第 14 条两轮确认；注意第 6 条的「回复达人」优先级更高）。**PDF / Word / PPT / 图片 / 视频都是资料文件，不是名单**，不能作为导入来源。
14. 名单导入两轮确认：
    - 若用户**仅提供名单/附件、尚未明确同意导入** → needTool=false，并在 reply 中用自然语言描述看到的名单/附件，同时询问导入方式：「是否导入？按哪种方式处理：1) 只联系符合campaign投放地区和红人画像要求的红人（默认）；2) 联系符合campaign投放地区的所有红人；3) 仅排重/不联系（不分析不联系，用于避免重复触达已联系过的红人）。」；**不要**调用工具。
    - 若用户已同意导入并明确选择模式（如回复「直接联系」「按分析结果来」「仅排重/不联系」），或**上一轮 Bin 刚问过导入方式、用户回复确认** → 必须 needTool=true，toolName=import_influencer_list，params.contactMode=recommended_only|contact_all|do_not_contact；params.textItems / params.attachmentPlan **可留空**，系统会从最近对话历史与附件自动补全，**不要编造 profileUrl 或 attachmentPlan.storageKey**。
    - **即使**用户消息里已经明确说「直接联系/全部联系/不用筛选」，也必须先追问一次确认模式，再调用工具，不得跳过确认。
    - 若用户**明确只想解读表格/链接内容、不要导入联系**（如「帮我看看这份表格里有哪些红人」「这些链接能分析吗」）→ needTool=false，并在 reply 中结合附件样本/链接直接解读，**不要**调用工具。
    - 若用户上传的是 **PDF / Word / PPT / 图片 / 视频**并要求导入名单 → 不是有效名单来源，needTool=false，reply 提示「该附件是资料文件，不能作为红人名单导入，请上传 Excel/CSV 名单或粘贴红人链接」。
...
说明：红人画像**只有这三个字段**。用户提的互动率 / 点赞率 / 评论率等其它指标**不是独立字段**，必须原样并入 accountType（保留全部限定词、不要拆句），例如：
...
{ "needTool": true, "toolName": "modify_campaign", "params": { "scope": "whole", "changes": { "influencerPricing": { "mode": "ask_creator_quote" } } } }
...
第一轮（仅名单，未同意）：
{ "needTool": false, "toolName": null, "params": null, "reply": "我看到你提供了 2 个红人主页链接，是否导入？按哪种方式处理：1) 只联系符合campaign投放地区和红人画像要求的红人（默认）；2) 联系符合campaign投放地区的所有红人；3) 仅排重/不联系（不分析不联系，用于避免重复触达）。" }
第二轮（用户选择直接联系）：
{ "needTool": true, "toolName": "import_influencer_list", "params": { "contactMode": "contact_all" } }
第二轮（用户选择按分析结果）：
{ "needTool": true, "toolName": "import_influencer_list", "params": { "contactMode": "recommended_only" } }
{ "needTool": true, "toolName": "import_influencer_list", "params": { "contactMode": "do_not_contact" } }
...
/**
 * 解析调度器 LLM 输出（兼容 markdown 代码块包裹、前后夹带说明文字）。
 * 只取第一个括号平衡的 JSON 对象：模型偶尔会把同一段 JSON 输出两遍，
 * 贪婪正则 `/\{[\s\S]*\}/` 会把两段一起截下来导致 JSON.parse 抛错。
 * 解析不出对象时返回空决策（needTool=false），不抛错。
 */
...
export function hydrateImportParams(
  ...
  const currentMessage = String(messages[messages.length - 1]?.content || "");
  const currentHasLinks = PROFILE_URL_IN_TEXT.test(currentMessage);
  // 文本优先：当前轮用户消息含真实主页链接时，放弃历史附件映射（避免 PENDING 附件劫持文本名单）
  if (currentHasLinks) params.attachmentPlan = null;
  const effectiveAttachmentImport =
    Boolean(params.attachmentPlan?.storageKey) && !currentHasLinks;
  if (!effectiveAttachmentImport) {
  ...
  if (!["recommended_only", "contact_all", "do_not_contact"].includes(params.contactMode)) {
    const recentText = (messages || [])
      .slice(-6)
      .map((m) => String(m?.content || ""))
      .join("\n");
    params.contactMode = /排重|不联系|触达记录|黑名单|已联系|不要联系/i.test(recentText)
      ? "do_not_contact"
      : /直接联系|全部联系|都联系|不筛选|不用筛选|直接导入|跳过筛选/i.test(recentText)
        ? "contact_all"
        : "recommended_only";
  }
  return params;
...
export async function decideExecutionSchedulerTurn(
  ...
  if (decision.needTool && decision.toolName === "import_influencer_list") {
    const importableAttachments = (input.attachmentsForImport || []).filter(
      isImportListAttachment
    );
    const params = hydrateImportParams(decision, {
      messages: input.messages || [],
      attachmentsForImport: importableAttachments,
    });
    ...
    if (!hasImportSource) {
      ...
      decision.reply =
        decision.reply ||
        "我这边没有识别到可导入的红人链接或名单附件（仅支持 Excel/CSV），请重新提供红人主页链接或上传 Excel 名单。";
    }
  }
  if (decision.needTool && decision.toolName === "ask_influencer_special_request") {
    const sendableAttachments = sendableAttachmentMetas(
      input.attachmentsForImport || []
    );
    if (sendableAttachments.length) {
      if (Array.isArray(decision.params.requests) && decision.params.requests.length) {
        decision.params.requests = decision.params.requests.map((r) => ({
          ...(r || {}),
          attachments:
        ...
        !Array.isArray(decision.params.attachments) ||
        !decision.params.attachments.length
      ) {
        decision.params.attachments = sendableAttachments;
      }
    }
  }
  if (decision.needTool && decision.toolName === "approve_quote") {
    const mode = decision.params?.contentBriefMode;
    const provided = Array.isArray(decision.params?.attachments)
      ? decision.params.attachments
      : [];
    // 严格参考脚本模式下，用户刚上传的脚本 / 资料附件随确认邮件发给红人；
    // 只取「最后一条消息」的附件，避免把历史附件误带到本次合作确认。
    if (mode === "reference_script" && !provided.length) {
      const lastMsg = Array.isArray(input.messages)
        ? input.messages[input.messages.length - 1]
        : null;
      const lastAttachments = Array.isArray(lastMsg?.attachments)
        ? lastMsg.attachments.filter((a) => a?.storageKey)
        : [];
      const sendableAttachments = sendableAttachmentMetas(lastAttachments);
      if (sendableAttachments.length) {
        decision.params.attachments = sendableAttachments;
      }
    }
...
5. 向某位或多位红人发起特殊请求（如延后发布时间、改价、改条数、同步物流单号与内容建议、发送资料文件：PDF / Word / PPT / 图片 / 视频），并回收红人反馈后同步给广告主；一条消息涉及多位红人时，为每位红人分别发起一次特殊请求（params.requests 数组）
6. 查询 campaign 执行状态
7. 红人执行阶段操作：
   - 同意/通过某红人报价 → approve_quote（**必须先确认** contentBriefMode：reference_script 需 https scriptLink 或脚本附件（两者至少一个）；free_creative 为自由发挥；scriptNotes 可选。用户未说明模式时先追问，批量同意多位红人时**分别**询问每位红人的模式与脚本来源/备注，再逐个调用）
...
8. 红人名单导入：粘贴红人主页链接、@用户名或 📎 Excel 时，由统一工具调度器按「先确认导入方式再导入」流程处理；**必须**先和用户确认「只联系符合投放地区和画像要求的红人（默认）」「联系符合投放地区的所有红人」还是「仅排重/不联系（不分析不联系，用于避免重复触达）」，即使用户已说直接联系也要追问确认；确认后调用 import_influencer_list 并填 params.contactMode（textItems / attachmentPlan 由系统自动补全，勿编造）。用户只想解读表格/链接时直接解读，不导入。用语自然，勿固定说「已收到附件」。
...
- 当用户修改红人画像（粉丝量、播放量、帐号类型/内容方向）时，必须调用 modify_campaign，**params.scope 必须精确为字符串 whole**（不要用 single_influencer），changes.screeningConditions 填入 followerRange、viewRange、accountType（可只填要改的键；**值必须是字符串**，如 "5万以上"，不要用 JSON 对象）。**互动率 / 点赞率 / 评论率等其它筛选要求没有独立字段，必须原样并入 accountType**（例如「互动率要求5%以上，base在巴西圣保罗加戈亚尼亚的美食探店类或母婴类红人都可以」整句写入 accountType）；**禁止**把它们拆成 engagementRate / interactionRate 等额外键，也不要因为 accountType 已存在就丢掉该要求。若用户单独补充互动率等要求，写入前先把【当前 Campaign 配置快照】里的「红人帐号类型要求」读出来合并，整段写回，**禁止覆盖原有内容**。
...
- 当用户**上传资料附件（PDF / Word / PPT / 图片 / 视频，见【附件/名单上下文】）并要求发给红人**时，同样必须调用 ask_influencer_special_request，并在 params（或 requests 数组中每位红人）携带 attachments：每项 { fileName, storageKey, contentType, sizeBytes }（取自附件上下文，**禁止编造 storageKey**）；requestDetail 中注明「随信附上资料：文件名」；单封最多 5 个附件，单个附件≤25MB 且**单封合计≤25MB**。若用户要求「导入名单」但附件是 PDF/Word/PPT/图片/视频，则不能导入，应引导上传 Excel/CSV 或粘贴链接。
- 视频也走 attachments（单文件≤25MB）；**超过 25MB 的视频无法上传**，此时才请用户提供可公开访问的链接（YouTube / Google Drive / Dropbox / Loom 等），再用 ask_influencer_special_request 把链接写进 requestDetail 发给红人。
...
- 若用户要暂停但当前已是 paused，或要恢复但当前已是 running/running_passive：仍调用 set_campaign_status 对应 action（工具会返回幂等说明）。`;
    ...
    try {
      ...
      if (decision.needTool && decision.toolName && TOOL_NAMES.includes(decision.toolName)) {
        ...
        else if (decision.toolName === "finalize_contract_terms") reply = "正在按协商结果生成并发送新版合同…";
