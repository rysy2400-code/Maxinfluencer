...
function stringifyJsonForMysql(value) {
  if (value == null) return null;
  return JSON.stringify(sanitizeJsonValueForMysql(value));
}
...
function buildTikTokInfluencerSavePayload(influencer) {
  ...
  const accountTypes =
    influencer.accountTypes && Array.isArray(influencer.accountTypes)
      ? stringifyJsonForMysql(influencer.accountTypes)
      : null;
  ...
  const profileData = influencer.profile_data
    ? stringifyJsonForMysql(influencer.profile_data)
    : null;
  ...
  const searchVideoData =
    ...
    influencer.search_video_data.length > 0
      ? stringifyJsonForMysql(influencer.search_video_data)
      : null;
...
export async function saveVideoPublishCountry({
  ...
  try {
    const rows = await queryTikTok(
      `SELECT id, search_video_data FROM TikTok_influencer WHERE username = ? LIMIT 1`,
      [u]
    );
    ...
    if (searchVideoDataJson) {
      await queryTikTok(
        ...
        WHERE username = ?
      `,
        [countryValue, searchVideoDataJson, u]
      );
    } else {
      await queryTikTok(
        `
