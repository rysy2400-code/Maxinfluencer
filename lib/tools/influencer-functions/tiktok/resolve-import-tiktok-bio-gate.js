...
export async function resolveImportTiktokBioGate({
  ...
  const stats = { total: queue.length, passed: 0, skipped: 0, mismatch: 0, unknown: 0, failed: 0, email_skipped: 0 };
  const records = [];
    ...
    for (let i = 0; i < queue.length; i += 1) {
      ...
      if (!userInfo) {
        ...
        if (!gate.mayMatch) {
          ...
          // 邮箱预过滤：bio 语言匹配但无邮箱 → 跳过（省视频/LLM 流量，无邮箱无法联系）
          const emailGate =
            String(process.env.TT_IMPORT_BIO_EMAIL_GATE ?? "1").trim() !== "0";
          if (emailGate && !userInfo.email) {
            stats.skipped += 1;
            stats.email_skipped += 1;
            console.log(
              `[resolveImportTiktokBioGate] [${progress}] ⏭️ @${handle} bio=${gate.bioLanguage} (${gate.source}) no_email_skip`
            );
          } else {
            stats.passed += 1;
            records.push({
            ...
            console.log(
              `[resolveImportTiktokBioGate] [${progress}] ✅ @${handle} bio=${gate.bioLanguage} (${gate.source})${emailGate ? " has_email" : ""}`
            );
          }
        }
      }
