...
async function runExecutionForCampaign(campaign, now) {
  ...
  const dispatchPlatform = await pickNextDispatchPlatform(
    campaignId,
    campaignPlatforms,
    queryTikTok
  );
  ...
  for (const plan of plansToDispatch) {
    ...
    console.log(
      `[ExecutionHeartbeat] Campaign ${campaignId} 派发任务: platform=${dispatchPlatform} keyword=${plan.keyword} type=${plan.keywordType}`
    );
  }
}
