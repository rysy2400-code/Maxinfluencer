...
import {
  buildInstagramKeywordSearchUrl,
} from "./normalize-instagram-search-keyword.js";

function buildKeywordSearchUrl(keyword) {
  return buildInstagramKeywordSearchUrl(keyword);
}
...
export async function extractInstagramSearchResultsFromPageCDP(
  ...
  const maxInfluencers = Math.min(
    Math.max(
      Number(options.maxInfluencers || process.env.IG_SEARCH_MAX_INFLUENCERS || 500),
      1
    ),
    2000
  );
  ...
  if (scrollUntilStuck && !(typeof options.scrollRounds === "number" && options.scrollRounds > 0)) {
    await runScrollUntilStuck(page, {
      scrollOnce: async () => {
        await page.evaluate(() => window.scrollBy(0, 600));
      },
      getProgress: () => captured.length,
      waitMs: 2000,
    });
  } else {
  ...
  if (success) {
    await reportIgScreenshot(
      onStepUpdate,
      BROWSER_STEP_IDS.SEARCH_VIDEOS,
      `Instagram 搜索完成（${influencerRecords.length} 红人）`,
      page
    );
  }
