...
export async function resolveImportTiktokFirstVideos({
  ...
  try {
    ...
    const isEmptyishError = (msg) => {
      const m = String(msg || "");
      // 注意：fetchFirstRepresentativeVideoForUser 对空 item_list 返回 error="no_posts"，
      // 无视频 id 返回 "no_video_id"——都归为“数据层空结果”，触发换 IP 而非仅重建 tab。
      return /empty|status_code.?403|items=0|signed fetch empty|no_representative_video|no_posts|no_video_id|missing_sec_uid/i.test(m);
    };
    ...
    for (let start = 0; start < queue.length; start += runtimeConcurrency) {
      ...
      // 首视频请求间隔 1.5-2s：降低 item_list 频率型限流触发概率，
      // 让每个 IP 在触发限流前能拉更多红人（enrich 阶段有 3-5s 冷却即受益于此）。
      if (start + runtimeConcurrency < queue.length) {
        const delayMin = Math.max(0, Number(process.env.TT_LITE_FIRST_VIDEO_DELAY_MIN ?? 1500) || 0);
        const delayMax = Math.max(delayMin, Number(process.env.TT_LITE_FIRST_VIDEO_DELAY_MAX ?? 2000) || delayMin);
