...
export async function createImportTask({
  ...
  const result = await queryTikTok(
    ...
    INSERT INTO tiktok_influencer_import_task (
      campaign_id, session_id, import_batch_id, batch_group_id, platform, contact_mode, priority, payload,
  ...
  `,
    [
      ...
      batchGroupId || null,
      platform,
      ["recommended_only", "contact_all", "do_not_contact"].includes(contactMode)
        ? contactMode
        : "recommended_only",
      priority,
...
/**
 * 按附件原始 storageKey 查找导入任务（会话内，最近优先）。
 * 用于聊天附件（PENDING-*.xlsx）在导入后被清理时，下载接口从任务
 * payload / IMP- 审计副本兜底恢复文件。
 */
export async function getImportTasksBySourceFileStorageKey(sessionId, storageKey) {
  const rows = await queryTikTok(
    `
    SELECT id, session_id, import_batch_id, batch_group_id, contact_mode,
           source_file_name, source_file_storage_key, payload
    FROM tiktok_influencer_import_task
    WHERE session_id = ? AND source_file_storage_key = ?
