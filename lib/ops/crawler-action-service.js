...
function buildRemotePowerShell(action, machine, deployment) {
  ...
  if (action === "restart-worker") {
    return [
      ...
      "$ok=@(Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and $_.CommandLine -match 'worker-influencer-search\\.js' }).Count -ge 1",
  ...
  if (action === "restart-chrome") {
    return [
      ...
      "& node --experimental-default-type=module (Join-Path $root 'scripts\\probe-cdp-rpc.mjs') 'http://127.0.0.1:9222' 8000",
      "if($LASTEXITCODE -ne 0){ throw 'cdp_rpc_not_recovered' }",
      "Write-Output '[crawler-action] cdp_rpc_ok=true'",
    ].join("; ");
  }
  return [
    ...common,
