 ...
 *   advertiserId: number,
 *   userId: number | null,
 *   type: 'recharge' | 'monthly_consumption' | 'influencer_campaign',
 *   ledgerId?: number,
 *   periodYyyymm?: string,
 * }} opts
 */
export async function requestInvoice(opts) {
  ...
  if (invoiceType === "recharge") {
  ...
  } else if (invoiceType === "influencer_campaign") {
    prefix = "C";
    const ledgerId = Number(opts.ledgerId);
    if (!Number.isFinite(ledgerId) || ledgerId <= 0) {
      return { ok: false, error: "请选择一条红人合作记录" };
    }
    const invoiced = await getInvoicedInfluencerLedgerIds(opts.advertiserId);
    if (invoiced.has(ledgerId)) {
      return { ok: false, error: "该红人合作记录已开过发票" };
    }
    const row = await getQuoteLedgerRow(opts.advertiserId, ledgerId);
    if (!row) {
      return { ok: false, error: "红人合作记录不存在" };
    }
    const influencerFee = Math.abs(num(row.influencer_amount));
    const platformFee = Math.abs(num(row.platform_fee_amount));
    const total = Math.abs(num(row.amount)) || influencerFee + platformFee;
    ...
    amountUsd = total;
    relatedLedgerIds = [Number(row.id)];
    periodYyyymm = periodYyyymmFromDate(new Date(row.created_at));
    periodStart = formatInvoiceDateHkt(new Date(row.created_at));
    periodEnd = periodStart;
  } else {
  ...
  // 始终写 POSIX 相对 key：Windows 上 path.join 会产生反斜杠，跨平台读取会失效
  const storageKey = buildInvoicePdfStorageKey(opts.advertiserId, invoiceNo);
  const absPath = path.join(getInvoiceStorageDir(opts.advertiserId), `${invoiceNo}.pdf`);
