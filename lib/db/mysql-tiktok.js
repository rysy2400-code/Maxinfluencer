...
const tiktokDbConfig = {
  ...
  connectionLimit: Math.max(Number(process.env.TIKTOK_DB_CONNECTION_LIMIT || 10), 1),
  maxPreparedStatements: Math.max(
    Number(process.env.TIKTOK_DB_MAX_PREPARED_STATEMENTS || 100),
    1
  ),
  queueLimit: 0,
...
export async function queryTikTok(sql, params = []) {
  const maxAttempts = Math.max(Number(process.env.TIKTOK_DB_QUERY_RETRIES || 2), 1);
  let lastError = null;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      // Most application SQL is assembled dynamically (batch sizes, filters,
      // update fields). Using execute() here caches every distinct SQL shape as
      // a server-side prepared statement on every pool connection and can
      // exhaust MySQL's global max_prepared_stmt_count.
      const [rows] = await tiktokPool.query(sql, params);
      return rows;
    } catch (error) {
      lastError = error;
      if (attempt < maxAttempts && isTransientMysqlError(error)) {
