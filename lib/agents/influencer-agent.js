...
import { createOutreachCooldown } from "../email/outreach-cooldown.js";
...
import { getInfluencerHandoverMode } from "../db/influencer-handover-dao.js";
import { logDraftOutboundMessage } from "../db/influencer-draft-dao.js";
...
import { shouldAskCountryInOutreach } from "../influencer/country-reply-sync.js";
import {
  resolveCommunicationLanguage,
  languageEnglishName,
} from "../influencer/infer-bio-language.js";
import { languageDisplayName } from "../influencer/country-primary-language.js";
import { getInfluencerLanguage } from "../influencer/influencer-language-store.js";
...
function getPlatformLabel(campaign) {
  ...
  const fallback = parseCampaignPlatforms(campaign?.campaignInfo?.platform);
  if (fallback.length === 1) return fallback[0];
  if (fallback.length > 1) return fallback.join(" / ");
  return "social media";
}

/** 首封邀约：红人账户平台（YouTube / Instagram / TikTok / X），用于 deliverables 筛选与称呼创作者 */
function resolveInfluencerPlatformLabel(influencer, executionSnapshot) {
  const raw =
    influencer?.platform ??
    executionSnapshot?.platform ??
    executionSnapshot?.Platform ??
    null;
  ...
  const profileUrl =
    ...
    executionSnapshot?.profileUrl ||
    executionSnapshot?.profile_url ||
    "";
  const url = String(profileUrl).toLowerCase();
...
/** 红人邮件上下文：剥离 campaign_info.budget，避免 LLM 误当单人报价或泄露整案总预算 */
export function stripBudgetFromCampaignInfo(campaignInfo) {
...
export async function generateOutreachBodyWithLLM({
  campaign,
  influencer,
  conversationHistory,
  executionSnapshot,
  ...
  const influencerPlatformLabel = resolveInfluencerPlatformLabel(
    influencer,
    executionSnapshot
  );
  // 交付要求的发布平台：campaign 投放平台优先；红人主平台不在投放范围内时，
  // 不能把交付要求写成「只发红人主平台」（例如 campaign 只做 YouTube/Instagram，
  // 红人账号是 TikTok 时，Deliverables 应写 YouTube/Instagram）。
  const campaignPlatformList = parseCampaignPlatforms(
    campaign?.campaignInfo?.platform ?? campaign?.campaignInfo?.platforms
  );
  const influencerPlatformInCampaign =
    !campaignPlatformList.length ||
    campaignPlatformList.includes(influencerPlatformLabel);
  const publishingPlatformLabel = influencerPlatformInCampaign
    ? influencerPlatformLabel
    : campaignPlatformList.join(" / ");

  const cid = campaign?.id || null;
  const campaignInfoSafe = stripBudgetFromCampaignInfo(campaign?.campaignInfo || null);
  const askCountry = shouldAskCountryInOutreach({ influencer, executionSnapshot });

  // 发信语言：红人最近一次回复语言 > bio 语言（置信度过线）> 英语
  const languageInfo = resolveCommunicationLanguage({
    replyLanguage: influencer?.communicationLanguage || null,
    bioLanguage: influencer?.bioLanguage || null,
    bioLanguageConfidence: influencer?.bioLanguageConfidence ?? null,
  });
  const outboundLanguage = languageInfo.language;
  const outboundLanguageName = languageDisplayName(outboundLanguage);
  const outboundLanguageEn = languageEnglishName(outboundLanguage);
  ...
  const pricingRules = (() => {
    ...
    const mode = outreachPricing?.pricingMode;
    const ecpmUsd = outreachPricing?.ecpmUsd;
    const maxFlat = outreachPricing?.maxFlatFeeUsd;
    const isProductExchange =
      ...
      Number(pct) === 0;
    const lines = [
      `- **金额纪律（极其重要）**：正文**禁止**提及 campaign 总预算、program budget、overall campaign budget 或任何整案/项目级预算数字。`,
      `- 仅可使用 outreachPricing 中的 quotedFlatFeeUsd（若有）作为**该创作者个人**的 fixed fee；**禁止**自造其他美元数字。`,
      `- 佣金百分比仅使用 outreachPricing.commissionPercent；无则不要捏造百分比。`,
    ];

    if (mode === PRICING_MODE_ASK_CREATOR_QUOTE) {
      lines.push(
        `- **报价策略为 ask_creator_quote（不主动报价，询问红人合作价格）**：正文**禁止**提及 fixed fee、flat fee、upfront payment、commission、affiliate 或任何具体美元金额；**禁止**给出建议价格或价格区间。`
      );
      lines.push(
        `- 表述要求：用发信语言，在介绍产品与交付范围后，自然、礼貌地请创作者提供其合作报价（rate / fee for these deliverables），例如 "Could you share your rate for this collaboration?" 或 "Please let us know your fee based on the deliverables below."；不要主动给出任何建议价格。`
      );
    } else if (mode === PRICING_MODE_COMMISSION_ONLY) {
      if (isProductExchange) {
        lines.push(
          `- **报价策略为纯产品置换（commission_only + 0% commission）**：正文说明以免费赠送产品作为唯一报酬（free product / gifted product in exchange for content），**禁止**提及 commission、affiliate、fixed fee、flat fee、upfront payment 或任何具体美元金额。`
        );
      } else {
        lines.push(
          `- **报价策略为 commission_only**：正文**只**说明佣金结构（commission / affiliate），**禁止**提及 fixed fee、flat fee、upfront payment 等固定费表述。`
        );
      }
      if (
        !isProductExchange &&
        ...
        lines.push(
          `- 表述要求：用发信语言说明 **${Number(pct)}% commission**（或 affiliate program），不要出现具体固定费美元数字。`
        );
      }
    } else {
      lines.push(
        `- **报价策略为 ecpm_with_cap**：固定费由系统按 snapshot 平均播放量、eCPM=$${ecpmUsd ?? 3} 计算并四舍五入到 $10，且不超过 $${maxFlat ?? 1000}。`
      );
      lines.push(
        `- 若 quotedFlatFeeUsd 有数值：正文用发信语言说明该固定费与（若有）佣金；**禁止**自造其他美元数字。`
      );
      lines.push(
        `- 若 quotedFlatFeeUsd 为 null（如无播放量数据）：不要编造具体固定费；若 commissionPercent 有值可提佣金；可表示愿在了解内容表现后讨论 fixed compensation。`
      );
      if (q != null && Number.isFinite(Number(q))) {
        const commHint =
          pct != null && Number.isFinite(Number(pct)) && Number(pct) > 0
        ...
        lines.push(`- 表述要求：用发信语言表达——${commHint}`);
      }
    }
  ...
  const deliverablesEn = formatDeliverablesForOutreach(
    campaign?.campaignInfo?.deliverables
  );
  const deliverablesRules = deliverablesEn
    ? (() => {
        const isDefaultEn = deliverablesEn === DEFAULT_DELIVERABLES_EN;
        const pct = outreachPricing?.commissionPercent;
        const isProductExchange =
          outreachPricing?.pricingMode === PRICING_MODE_COMMISSION_ONLY &&
          pct != null &&
          Number.isFinite(Number(pct)) &&
          Number(pct) === 0;
        const isAskQuote =
          outreachPricing?.pricingMode === PRICING_MODE_ASK_CREATOR_QUOTE;
        return [
          isProductExchange
            ? `- **Deliverables（交付结果）**：正文必须用发信语言自然说明本次赠送产品（gifted product）是针对以下 Deliverables 的回报；**不要**写 fixed fee / commission 或任何金额。`
            : isAskQuote
              ? `- **Deliverables（交付结果）**：正文用发信语言自然列出以下交付要求，并请创作者**根据这些交付要求提供合作报价**；不要写 fixed fee / commission 或任何金额。`
              : `- **Deliverables（交付结果）**：正文必须用发信语言自然说明本次 fixed fee / commission 是针对以下 Deliverables 的报价。`,
          `- **平台优先级（极其重要）**：本封邮件的交付要求以「本次要发布的平台」为准，当前为 **${publishingPlatformLabel}**（campaign 投放平台：${platformLabel}；红人账号平台：${influencerPlatformLabel}${
            influencerPlatformInCampaign
              ? ""
              : "，注意红人主平台不在本次投放范围内，**不要**把交付要求写成只发红人主平台"
          }）。`,
          `- 阅读 **campaignInfo.deliverables** 与 payload.deliverables（通常为同一全文）。若文本按平台分段（如「YouTube：…」「Instagram：…」），**Deliverables 段落只写与 ${publishingPlatformLabel} 匹配的那段**；若无平台分段（通用条目），可对全文使用。`,
          `- 在正文中以 "Deliverables:" 引出，用自然段落或逗号分隔列出（不要 markdown bullet、不要编号列表）。`,
          isDefaultEn
            ? `- 交付内容（默认模板，各平台通用）：${DEFAULT_DELIVERABLES_EN}`
            : `- 交付内容：从上述全文筛选 influencer.platform 对应条目后，译成发信语言自然写出（不得遗漏该段条目，不得加入其他平台条目）。原文参考：${deliverablesEn}`,
          `- **禁止**自行添加未出现在匹配平台交付内容中的项目；**禁止**在 Deliverables 中提及本次发布范围之外的平台。`,
        ].join("\n");
      })()
    : `- 当前未配置 deliverables，无需单独列出 Deliverables 段落。`;

  const fitRules = creatorFitContext
    ? `- **合作动机**：请阅读 creatorFitContext（来自执行表 influencer_snapshot 的分析与匹配说明）。用 1–2 段自然（发信语言）说明**为什么**这位创作者适合该品牌/产品：可引用 analysisSummary、matchAnalysis 中的要点（受众、内容风格、匹配度），避免空泛客套，让红人感到「有被认真看过」；不要逐条抄字段名。`
    : `- 若无 creatorFitContext，用真诚、具体的语气说明你欣赏其内容风格与受众，但仍避免夸张或与事实不符的陈述。`;

  const countryQuestionRule = askCountry
    ? `- **居住国家补全（必需）**：系统尚未记录该红人的居住国家（账号国家已知也要问）。请在邮件结尾附近用**发信语言**自然询问对方目前常驻哪个国家，英文参考表述："Also, could you let us know which country you're currently based in? It helps us coordinate campaign details and shipping if needed."；若发信语言不是英语，请用同一意思的当地自然说法，不要照抄英文。不要把它写成表单，不要影响主体邀约。`
    : `- 系统已记录该红人的居住国家，不要额外询问对方所在国家。`;

  const languageRule =
    outboundLanguage === "en"
      ? `- 用英文写邮件正文（纯文本，不要 markdown，不要 JSON）。`
      : `- **必须用${outboundLanguageName}（${outboundLanguage}）写邮件正文**（纯文本，不要 markdown，不要 JSON）：这是该红人的沟通语言（来源：${languageInfo.source === "reply" ? "红人最近一次回复语言" : "红人主页简介语言"}）。用当地母语者的自然表达，不要中英夹杂，也不要在正文里解释语言选择。品牌名、产品名、链接、专有名词保留原文。`;
...
${languageRule}
- 下文规则中出现的英文示例句只作**语义参考**，正文一律用上面的发信语言书写（品牌名、产品名、链接、专有名词保留原文）。
- 语气：专业、友好、简洁，不要太推销感。
...
- 适当根据 conversationHistory 判断：
  ...
  - 但不要重复上一封几乎一模一样的句子。
- 主体人设一定是 Bin（代表 Binfluencer 与品牌沟通；Bin 是 Binfluencer 创始人开发的 AI talent manager）。
- 公司身份口径：公司/产品名 Binfluencer，唯一官网 https://www.binfluencer.xyz，没有 LinkedIn 主页。正文如需提及公司背景或官网，一律使用该网址；**禁止**编造其他域名、LinkedIn 主页或机构隶属关系。
- 称呼创作者及其内容时，以 **influencer.platform**（${influencerPlatformLabel}）为准（如 your YouTube channel），不要用 campaign.platformLabel 的多平台拼接来描述「这位创作者」的平台身份。
...
`;
  ...
  const payload = {
    influencer: {
      ...
      platform: influencerPlatformLabel,
      countryMissingForOutreach: askCountry,
      outboundLanguage,
      outboundLanguageSource: languageInfo.source,
    },
    campaign: {
      id: campaign?.id || null,
      brand,
    ...
    deliverables: deliverablesEn || null,
...
Please output ONLY the email body in ${outboundLanguageEn} (${outboundLanguage}), no greeting explanation, no JSON, no extra commentary.
`;
 ...
 */
/**
 * 解析首封邀约对应的主档行（canonical identity）。
 *
 * 历史脏数据里，候选/执行行存的是"搜索命中的平台账号 id"，而主档可能只存了同一
 * handle 在另一个平台的 id，于是按 id 查不到。这里按 id → (platform, handle) →
 * handle 三级回退，命中后以主档行的 id 作为最终身份（对话、商务档案都挂在它上面）。
 *
 * @returns {Promise<{influencer: object|null, influencerId: string|null, matchedBy: 'id'|'platform_handle'|'handle'|null}>}
 */
export async function resolveInfluencerForOutreach({
  ...
  const handle = String(tiktokUsername || "")
    .replace(/^@/, "")
    .trim();
  if (!handle) return { influencer: null, influencerId: pid || null, matchedBy: null };

  const platformHint = normalizePlatformSlug(
    platform || snapshot?.platform || snapshot?.platformSlug || null
  );
  ...
  if (byPlatformHandle) {
    console.warn(
      `[InfluencerAgent] 主档按 id 未命中，回退按 (${platformHint}, @${handle}) 命中：event_id=${sourceEventId || "-"} ${pid || "(no id)"} -> ${byPlatformHandle.influencerId}`
    );
    return {
  ...
  if (byHandle) {
    console.warn(
      `[InfluencerAgent] 主档按 id / (platform,handle) 均未命中，回退按 @${handle} 命中：event_id=${sourceEventId || "-"} ${pid || "(no id)"} -> ${byHandle.influencerId} (${byHandle.platform})`
    );
    return {
      influencer: byHandle,
...
export async function sendOutreach({
  ...
  const resolved = await resolveInfluencerForOutreach({
    platformInfluencerId: initialPid,
    tiktokUsername,
    platform,
    snapshot,
    sourceEventId,
  });
  const influencer = resolved.influencer;
  const pid = resolved.influencerId || initialPid;
  if (!influencer) {
    throw new Error(
  ...
  if (influencer.contactStatus === "do_not_contact") {
    throw new Error(
      `[InfluencerAgent] 红人 influencer_id=${pid} 已要求停止联系`
    );
  }

  // 沟通语言（红人回复语言优先，其次 bio 语言）取自语言旁路表，缺失时按英语兜底。
  try {
    const languageRecord = await getInfluencerLanguage(pid);
    if (languageRecord) {
      if (influencer.bioLanguage == null) {
        influencer.bioLanguage = languageRecord.bioLanguage;
      }
      if (influencer.bioLanguageConfidence == null) {
        influencer.bioLanguageConfidence = languageRecord.bioLanguageConfidence;
      }
      if (influencer.communicationLanguage == null) {
        influencer.communicationLanguage = languageRecord.communicationLanguage;
      }
    }
  } catch (langErr) {
    console.warn(
      `[InfluencerAgent] 读取红人沟通语言失败（按英语兜底）: ${langErr?.message || langErr}`
    );
  }
  ...
  const influencerPricing = normalizeInfluencerPricing(
    campaign.campaignInfo?.influencerPricing
  );
  const quotedFlatFeeUsd = computeQuotedFlatFeeUsd(
    avgViewsForQuote,
    influencerPricing
  );
  const commissionPercent =
    campaign.commissionPercent != null
      ...
      : campaign.campaignInfo?.commission != null
        ? Number(campaign.campaignInfo.commission)
        : null;
  const outreachPricing = {
    ...
    rule:
      influencerPricing.mode === PRICING_MODE_ASK_CREATOR_QUOTE
        ...
        : influencerPricing.mode === PRICING_MODE_COMMISSION_ONLY
          ...
          : `ecpm_with_cap: flat_fee = min(round10(avg_views/1000*${influencerPricing.ecpmUsd}), ${influencerPricing.maxFlatFeeUsd})`,
  };
  ...
  const handoverMode = (await getInfluencerHandoverMode(pid)) || "auto";
  if (handoverMode === "assist") {
    const traceId = `first_outreach:${sourceEventId || campaignId}`;
    await logDraftOutboundMessage({
      ...
      bodyText: text,
      sourceType: "seed_outreach",
      sourceEventTable: sourceEventId ? "tiktok_influencer_agent_event" : null,
      sourceEventId,
      triggerType: "first_outreach",
      traceId,
      payload: {
        email: {
          to: toEmail,
        ...
        source: {
          eventTable: sourceEventId ? "tiktok_influencer_agent_event" : null,
    ...
    try {
      ...
      let flatFeeClause = "";
      if (
        quotedFlatFeeUsd != null &&
        Number.isFinite(Number(quotedFlatFeeUsd)) &&
        influencerPricing.mode === PRICING_MODE_ECPM_WITH_CAP
      ) {
        flatFeeClause = ", flat_fee = ?, currency = COALESCE(currency, 'USD')";
        flatFeeParams.push(Number(quotedFlatFeeUsd));
      }
      ...
      console.error(
        "[InfluencerAgent] 更新 tiktok_campaign_execution.outreachDraft 失败:",
        err
      );
    }
  ...
  const outreachCooldown = createOutreachCooldown({ sentAt });

  // 记录到执行表的 last_event，并在有首封报价时写入 flat_fee
  try {
    const flatFeeParams = [];
    let flatFeeClause = "";
    if (
    ...
    `,
      [
        ...
        quotedFlatFeeUsd,
        influencerPricing.mode,
        outreachCooldown.nextEligibleAt,
        ...flatFeeParams,
        campaignId,
        ...paramsExecutionCreatorMatch(tiktokUsername || pid),
      ]
