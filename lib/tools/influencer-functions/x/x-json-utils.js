...
export function normalizeXUser(userResult) {
  ...
  // 2026 版响应 legacy 已消失，粉丝/关注数移到 relationship_counts，帖数在 tweet_counts
  const followersRaw = pickFirst(
    userResult.relationship_counts?.followers,
    userResult.followers_count,
    legacy.followers_count
  );
  const followingRaw = pickFirst(
    userResult.relationship_counts?.following,
    userResult.friends_count,
    legacy.friends_count
  );
  const postsRaw = pickFirst(
    userResult.tweet_counts?.statuses,
  ...
  return {
    ...
    followingCount: Number.isFinite(Number(followingRaw)) ? Number(followingRaw) : 0,
    postsCount: Number.isFinite(Number(postsRaw)) ? Number(postsRaw) : 0,
...
export function computeXTweetStatistics(tweets = []) {
  ...
  // 口径统一：剔除播放量为 0/缺失的推文，均值分母同步用剔除后的条数
  const viewTweets = tweets.filter((t) => (Number(t.views) || 0) > 0);
  const sumOver = (list, fn) =>
    list.reduce((acc, t) => acc + (Number(fn(t)) || 0), 0);
  const avgOver = (list, fn) =>
    list.length > 0 ? Math.round(sumOver(list, fn) / list.length) : 0;
  return {
    avgViews: avgOver(viewTweets, (t) => t.views),
    avgLikes: avgOver(viewTweets, (t) => t.likes),
    avgComments: avgOver(viewTweets, (t) => t.replies),
    avgRetweets: avgOver(viewTweets, (t) => t.retweets),
    tweetsWithPlayCount: viewTweets.length,
    // 中位播放 + 点赞率/评论率：与均播同一批样本（views > 0）
    ...computeViewMedianAndRates(viewTweets, {
      viewsOf: (t) => t.views,
      likesOf: (t) => t.likes,
      commentsOf: (t) => t.replies,
    }),
