...
/**
 * 规则识别暂停/恢复/结项（避免 LLM 只口头回复却不调用 set_campaign_status）。
 * @returns {"pause"|"resume"|"complete"|null}
 */
export function detectCampaignStatusIntentFromMessage(text) {
  const raw = String(text || "").trim();
  if (!raw) return null;
  const compact = raw.toLowerCase().replace(/\s+/g, "");

  const hasCampaignWord = /campaign|活动|项目/.test(compact);

  if (
    /campaign.{0,8}(完成|结束|结项)|完成.{0,8}campaign|可以结束|结项了|markcomplete|endcampaign|campaignisdone|campaigndone/.test(
      compact
    ) ||
    ((/完成了|可以结束了|campaign完成/.test(compact) || /完成\s*campaign/.test(raw)) &&
      !/暂停|pause|stop/.test(compact))
  ) {
    return "complete";
  }

  if (
    /暂停campaign|campaign暂停|pausecampaign|campaignpause|stopcampaign|先停|别联系/.test(
      compact
    ) ||
    (/暂停|pause|stop/.test(compact) && hasCampaignWord)
  ) {
    return "pause";
  }

  if (
    /继续campaign|恢复campaign|resumecampaign|continuecampaign|campaign继续|campaign恢复|campaignresume|campaigncontinue|重新开始/.test(
      compact
    ) ||
    ((/继续|恢复|resume|continue/.test(compact) || /^(继续|恢复)/.test(raw)) &&
      (hasCampaignWord || /^(继续|恢复)/.test(raw)))
  ) {
    return "resume";
  }
