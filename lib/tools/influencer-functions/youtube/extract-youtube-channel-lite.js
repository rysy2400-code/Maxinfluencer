/**
 * YouTube Lite 频道 enrich v2：goto /about → 国家门禁 → innertube 拉视频 → /videos 截图
 */
...
export async function loadYoutubeChannelAboutFromPage(page, username, channelIdOpt = null) {
  ...
  if (videoPublishCountry) {
    console.log(
      `[loadYoutubeChannelAboutFromPage] @${handle} country=${videoPublishCountry} source=${countrySource}`
    );
  } else {
    console.log(`[loadYoutubeChannelAboutFromPage] @${handle} country=(空)`);
  }
...
export async function extractYoutubeChannelLite(page, username, options = {}) {
  ...
  if (options.countryOnly) {
    const aboutCtx = await loadYoutubeChannelAboutFromPage(page, handle, channelIdOpt);
    return {
  ...
  if (gateCountries) {
    ...
    if (!pubIso) {
      return buildCountrySkipLiteResult(aboutCtx, "country_unknown");
    }
    if (!countryMatchesPublishLocation(pubIso, gateCountries)) {
