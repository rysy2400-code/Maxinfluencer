...
import { updateCampaignSession } from "../db/campaign-session-dao.js";
...
export class AgentRouter {
  ...
  async persistPublishedSession(context, publishResult, newContext) {
    try {
      if (!publishResult?.published || !context.sessionId) {
        if (publishResult?.published && !context.sessionId) {
          console.warn(
      ...
      if (publishResult.sessionTitle) {
        updates.title = publishResult.sessionTitle;
      }
      const upd = await updateCampaignSession(context.sessionId, updates);
      if (!upd.success) {
