...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  // Lite：9222 登录态读视频页 UNIVERSAL locationCreated；9223 enrich 未登录无法读发布地
  if (isLiteScraperMode()) {
    const { acquireTiktokApiSession, resolveVideoLocationCreatedForInfluencer } =
    ...
    const rawConcurrency = Number(process.env.TT_LITE_COUNTRY_CONCURRENCY || 10);
    const concurrency = Math.max(
      1,
      Math.min(
        Number.isFinite(rawConcurrency) ? Math.floor(rawConcurrency) : 10,
        countryHardMax
      )
    );
    ...
    sendStep(
      "采集视频发布地",
      `Lite：9222 读视频发布地（全池 ${queue.length} 人，并发 ${concurrency}，${tabPoolSize} tab${stopOnZeroBatchMatch ? "，0 明确符合则停" : ""}，stub 不 goto）...`
    );
    ...
    for (let i = 0; i < tabPoolSize; i += 1) {
      try {
        ...
        console.warn(
          `[resolveVideoPublishCountry] 9222 session ${i + 1} failed: ${e.message}`
        );
      }
    }
    if (!pool.length) {
    ...
    for (const session of pool) {
      try {
        ...
        console.warn(
          `[resolveVideoPublishCountry] 9222 session bootstrap: ${e.message}`
        );
      }
    }
    ...
    const searchLocByUser = new Map();
    for (const v of videos || []) {
      const u = normalizeUsername(v.username);
      if (!u) continue;
      if (!videoByUser.has(u)) videoByUser.set(u, v);
      if (v.locationCreated && !searchLocByUser.has(u)) {
        searchLocByUser.set(u, String(v.locationCreated));
      }
    ...
    async function probeLocation(record, sessionIdx) {
      ...
      try {
        return await resolveVideoLocationCreatedForInfluencer(page, {
          ...
          searchLocation:
            searchLocByUser.get(username) ||
            srcVideo?.locationCreated ||
            null,
        });
      } catch (e) {
        return {
    ...
    async function processOneRecord(record, globalIndex, probe) {
      ...
      // Lite：仅明确符合国家者进入 enrich（unknown / mismatch 均跳过）
      if (explicitMatch) {
    ...
    sendStep(
      "采集视频发布地",
      `Lite 9222 发布地完成：检查 ${checked.length} 人，进入 enrich ${passed.length} 人（仅明确符合国家），unknown ${unknown} mismatch ${mismatch}${stoppedEarly ? "（提前停止）" : ""}`
    );
