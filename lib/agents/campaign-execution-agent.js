...
- 当用户修改发布时的 Campaign / 产品信息（品牌、产品名、投放平台、投放地区、发布时间段、总预算、佣金、单位红人报价策略、交付结果）时，必须调用 modify_campaign，**params.scope 必须为 whole**，在 changes 中只填用户明确要改的字段：brandName、productName、platform、region、publishTimeRange、budget（数字，总预算）、commission（数字，如 0 表示 0%）、influencerPricing（{ mode, ecpmUsd, maxFlatFeeUsd }）或 pricingMode/pricingEcpmUsd/pricingMaxFlatFeeUsd、deliverables（交付结果，多行或单段文本，如「2条专属视频；Bio 保留30天」）。例如「品牌改为 Anker」「改成只要佣金」「ecpm 改成 5、上限 500」「交付结果改为 2 条视频，Bio 保留 30 天」。**budget / commission / ecpmUsd / maxFlatFeeUsd 必须填纯数字（如 1000、15450），勿用 "$1,000" 或带逗号字符串**。报价策略与交付结果变更**仅影响尚未发出首封邀约的红人**。
