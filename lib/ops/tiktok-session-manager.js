...
const QG_GET_URL =
  "https://overseas.proxy.qg.net/get?key=9OLWSHRE&area=US&num=1&keep_alive=3";
const QG_US_EGRESS_TRIES = 8;
// IPWEB 动态住宅代理：gate1/gate2/gate3.ipweb.cc:7778，帐号形如
// B_<用户编号>_<国家>[_<州>_<城市>]_<持续分钟>_<SID8位>，改 SID 即换 IP（自编规则）。
const IPWEB_USER_RE = /^B_\d+_(US|HK|SG|JP|000|\w{2})(_|$)/i;
const shortLivedRotateAt = new Map();
...
export function readTkIpSession(proxyPort) {
  ...
  try {
    ...
    if (IPWEB_USER_RE.test(username)) {
      return { ...info, username, password, provider: "ipweb" };
    }
...
/** 从 IPWEB 用户名取 IP 持续时间（分钟）：B_xxx_US___5_Ab000001 -> 5 */
function ipwebTtlMinutes(username) {
  const seg = String(username || "").split("_");
  const n = Number(seg[seg.length - 2]);
  return Number.isFinite(n) && n > 0 ? n : null;
}

function ipwebSid(username) {
  const seg = String(username || "").split("_");
  return seg.length ? seg[seg.length - 1] : "";
}

function randomIpwebSid() {
...
/**
 * 轮换 IPWEB 动态住宅代理：改 username 末段 SID（新 SID = 新 IP）→ 重启 mihomo →
 * 等端口 → 验证出口可达。server:port 固定（如 gate1.ipweb.cc:7778）。
 * 返回 { ok, skipped, sid, ip, proxyPort, error? }
 */
export async function rotateIpwebSession(proxyPort = resolveTkIpProxyPort()) {
  const ses = readTkIpSession(proxyPort);
  if (!ses || ses.provider !== "ipweb") {
    return { ok: false, skipped: true, proxyPort };
  }
  const oldSid = ipwebSid(ses.username);
  const newSid = randomIpwebSid();
  const newUser = ses.username.slice(0, ses.username.length - oldSid.length) + newSid;
  try {
    const text = fs.readFileSync(ses.configPath, "utf8");
    if (!text.includes(ses.username)) {
      return { ok: false, error: "username not in config", proxyPort };
    }
    fs.writeFileSync(ses.configPath, text.replace(ses.username, newUser), "utf8");
  } catch (e) {
    return { ok: false, error: `write config: ${String(e.message || e).slice(0, 100)}`, proxyPort };
  }
  await stopMihomoByConfig(ses.configPath);
  await sleep(2500);
  startMihomo(ses.configPath, ses.runtimeDir);
  ...
  await sleep(3000);
  let info = await getProxyInfo(ses.proxyPort);
  for (let i = 0; i < 3 && !info?.ip; i++) {
    await sleep(4000);
    info = await getProxyInfo(ses.proxyPort);
  }
  const ttlMin = ipwebTtlMinutes(newUser);
  console.log(
    `[tkip-session] ipweb rotate sid=${oldSid}->${newSid} ttl=${ttlMin || "?"}m ip=${info?.ip || "-"} country=${info?.country || "-"}`
  );
  return {
    ok: !!info?.ip,
    skipped: false,
...
/**
 * 短存活（keep_alive=3min）协作式轮换：
 * 每端口限频（默认 150s），qg provider 才生效。
 * 任务步骤间隙由 worker 定时调用；失败路径可 force。
 */
export async function maybeRotateForShortLivedProxy(
  proxyPort = resolveTkIpProxyPort(),
  { force = false, minIntervalMs = 150000 } = {}
) {
  const ses = readTkIpSession(proxyPort);
  if (!ses || (ses.provider !== "qg" && ses.provider !== "ipweb")) {
    return { ok: false, skipped: true, proxyPort };
  }
  const ttlMin = ipwebTtlMinutes(ses.username);
  if (ses.provider === "ipweb" && ttlMin) {
    // 短存活代理：按 IP 持续时间的 ~60% 周期轮换，避免到期/被风控
    minIntervalMs = Math.max(60_000, Math.floor(ttlMin * 60 * 1000 * 0.6));
  }
  const last = shortLivedRotateAt.get(proxyPort) || 0;
  if (!force && Date.now() - last < minIntervalMs) {
    return { ok: false, skipped: true, reason: "interval", proxyPort };
  }
  shortLivedRotateAt.set(proxyPort, Date.now());
  return ses.provider === "qg"
    ? rotateQgShortProxy(proxyPort)
    : rotateIpwebSession(proxyPort);
}

/**
 * 协作式轮换：由 pipeline 在步骤间隙（搜索后/每红人之间）调用。
 * 轮换成功即标记会话失效 + 清理多余 tab，下一次页面获取自动开新 tab 继续，
 * 任务不中断、不重跑。全程 best-effort 不抛错。
 */
export async function cooperativeRotateIfStale({
  proxyPort = resolveTkIpProxyPort(),
  minIntervalMs = 150000,
  label = "协作",
} = {}) {
  try {
    const rot = await maybeRotateForShortLivedProxy(proxyPort, { minIntervalMs });
    ...
    console.log(
      `[tkip-session] ${label}轮换 ok sid=${rot.sid || "-"} ip=${rot.ip || "-"}`
    );
    return rot;
  } catch (e) {
    console.warn(
      `[tkip-session] ${label}轮换异常: ${String(e?.message || e).slice(0, 100)}`
    );
    return null;
  }
}
...
export async function rotateTkIpSession(proxyPort = resolveTkIpProxyPort()) {
  ...
  if (ses.provider === "ipweb") {
    return rotateIpwebSession(proxyPort);
  }
