...
export function isProfileEnrichReady(profileData) {
  return profileData?.success === true && profileData?.userInfo != null;
}
...
export async function enrichInfluencerProfiles(influencerRecords, options = {}) {
  ...
  async function ensureIgLitePagePool() {
    ...
    while (igLitePagePool.length < poolSize) {
      try {
        ...
        console.log(
          `[enrichInfluencerProfiles] Instagram Lite: 新建 API 页 ${igLitePagePool.length}/${poolSize}（enrich 并发 ${effectiveConcurrency}）`
        );
      } catch (e) {
        console.warn(
  ...
  const processOneRecord = async (record, index, batchTotal) => {
    ...
    const run = async () => {
      ...
      const rawFinalFollowerCount = profileData?.userInfo?.followers?.count;
      const finalFollowerCount = Number(rawFinalFollowerCount);
      const hasKnownLowFollowerCount =
        rawFinalFollowerCount != null &&
        rawFinalFollowerCount !== "" &&
        Number.isFinite(finalFollowerCount) &&
        finalFollowerCount >= 0 &&
        finalFollowerCount < 500;
      const skipLiveMatchForLowFollowers =
        recordIsYouTube &&
        liteMode &&
        (profileData?.skippedReason === "followers_below_500" ||
          hasKnownLowFollowerCount);
      if (skipLiveMatchForLowFollowers) {
        profileData.skippedReason = "followers_below_500";
        mergedRecord.enrich_skipped_reason = "followers_below_500";
        sendStep(
          "粉丝过滤",
          `[${progress}] ⏭️ @${record.username} 粉丝 ${Number.isFinite(finalFollowerCount) ? finalFollowerCount : "低于500"}，跳过画像分析与候选池`
        );
      }
