...
const ACTION_BRIEF = {
  ...
  approveQuote_no_sample:
    "品牌方已确认合作并同意报价。请通知红人品牌已确认，并请红人**先提交脚本（script）供品牌审核**：请红人按创作方向（见 contentBrief）先给出脚本，说明脚本通过后才会进入拍摄，拍摄完成后才提交视频草稿（video draft）链接供品牌审核。不要请红人直接开始制作成片，也不要直接索要视频草稿链接。",
  approveQuote_need_sample_no_address:
    "品牌方已确认合作并同意报价。请通知红人品牌已确认，并请红人提供寄样收件信息（姓名、地址、电话等）。",
  approveQuote_need_sample_has_address:
    "品牌方已确认合作并同意报价；系统已有红人此前提供的完整寄样信息。请通知红人品牌已确认，并完整列出已有地址，请红人确认本次是否可以使用该地址。不要要求红人重新填写；不要说样品已经安排寄出。",
  confirmShip:
    "品牌方已确认样品已寄出。请通知红人样品已在路上（或已发出），并请红人收到后**先提交脚本（script）供品牌审核**；说明脚本通过后再按脚本拍摄并提交视频草稿（video draft）链接。不要请红人直接开始制作成片，也不要直接索要视频草稿链接。",
  approveDraft:
    "品牌方已通过草稿审核。请通知红人可以发布，并请红人在发布后回复最终上线视频链接。",
  approveScript:
    "品牌方已通过红人提交的脚本。请通知红人脚本方向已确认，请红人按脚本开始拍摄，并在完成后提交视频草稿（video draft）链接供品牌审核；在收到明确通过通知前不要发布。",
  rejectDraft:
...
- **不要**限定 TikTok 或某一固定平台，除非 conversationHistory 中该 campaign 明确只有单一平台且已自然提及。`;
  ...
  if (action === "approveScript") {
    return `
- 本条为脚本已通过通知：正文须说明脚本方向已确认。
- 请红人按脚本开始拍摄，并在完成后提交**视频草稿链接**（video draft link）供品牌审核。
- 必须提醒：在收到品牌对视频草稿的明确通过通知前，**不要发布**。`;
  }
...
/**
 * 生成「广告主在 Portal 操作后」发给红人的跟进邮件正文。
 * 默认英文；传入 outboundLanguage 时按该语言书写（红人沟通语言优先）。
 */
export async function generateAdvertiserExecutionFollowupEmailBody({
  ...
  influencer,
  outboundLanguage = "en",
  outboundLanguageName = "",
  outboundLanguageSource = null,
  extraInstruction = "",
}) {
  ...
  const languageCode = String(outboundLanguage || "en").trim() || "en";
  const languageLabel = String(outboundLanguageName || "").trim() || languageCode;
  const languageRule = `${buildReplyLanguageRule({
    language: languageCode,
    languageName: languageLabel,
    languageEn: languageCode,
    source: outboundLanguageSource || null,
    label: "跟进邮件正文",
...
- 若 askShippingConfirmation=true 且 hasShippingInfo=true 且 shippingInfo 非空：本次邮件必须完整展示 shippingInfo 中的收件人、国家、城市、地址、邮编、电话，并询问红人确认是否可用于本次寄样。
- 若 action === "confirmShip"：本动作只通知样品已寄出（on its way / has been shipped），并请红人收到后**先提交脚本（script）供品牌审核**（脚本通过后再拍摄、再提交视频草稿）；**禁止**展示收件地址，也**禁止**要求红人确认地址或任何寄样信息。
- 若 askShippingConfirmation=false 且 action !== "confirmShip"：本次动作与寄样/地址无关，正文**禁止**提及寄样、发货、收件地址，或要求红人确认地址；即使 conversationHistory 中有寄样相关历史，也不要重复询问或引用寄样地址。
- 若 sampleSentAt 非空（样品已寄出）：禁止使用 “about to go out / shipment is going out / preparing to ship / about to send” 等“即将寄出”的表述（发信语言不是英语时，同样禁止该含义的本地说法）。
${languageRule}${actionExtraRules}${extraInstruction ? `\n${extraInstruction}` : ""}`;
  ...
  const userContent = `请根据以下 JSON 上下文撰写跟进邮件正文。${buildReplyLanguageTail(
    { languageEn: languageCode, label: "follow-up email body" }
