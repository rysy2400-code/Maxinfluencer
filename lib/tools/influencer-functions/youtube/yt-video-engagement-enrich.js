...
import { formatYtDurationDisplay, formatYtNumber, parseYtViewCount } from "./youtube-json-utils.js";
 ...
 * @param {object} json
 * @returns {{ views: number|null, likes: number|null, comments: number|null, durationSeconds: number|null, publishDate: string|null }}
 */
export function extractEngagementFromInnertubeJson(json) {
  ...
  const walk = (obj, d = 0) => {
    ...
    if (obj.videoDetails) {
      ...
      if (vd.viewCount != null && out.views == null) out.views = rawToCount(vd.viewCount);
      if (durationSeconds == null && vd.lengthSeconds != null) {
        durationSeconds = rawToCount(vd.lengthSeconds);
      }
      if (publishDate == null) {
        publishDate = vd.publishDate || vd.uploadDate || null;
      }
    }
    if (publishDate == null && obj.playerMicroformatRenderer?.publishDate) {
      publishDate = obj.playerMicroformatRenderer.publishDate;
    }
...
function mergeEngagementCounts(base, patch) {
  ...
  if (patch.durationSeconds != null) out.durationSeconds = patch.durationSeconds;
  if (patch.publishDate != null) out.publishDate = patch.publishDate;
  return out;
}
...
function applyEngagementToVideo(video, counts) {
  ...
  if (counts.durationSeconds != null) {
    out.durationSeconds = counts.durationSeconds;
    out.duration = formatYtDurationDisplay(counts.durationSeconds) || out.duration;
  }
  if (counts.publishDate != null) {
    out.publishDate = counts.publishDate;
    out.publishedTimeText = out.publishedTimeText || null;
  }
