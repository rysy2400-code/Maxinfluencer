...
/** bootstrap 目标页（试点：切到非 feed 主页省后台流量；默认首页保持原行为） */
function resolveBootstrapUrl() {
  return (
    String(process.env.TT_LITE_BOOTSTRAP_URL || "").trim() ||
...
async function refreshTiktokHomepageContext(page) {
  ...
  const homeTimeoutMs = Math.max(
    5000,
    Number(process.env.TT_LITE_HOMEPAGE_TIMEOUT_MS || 20_000)
  );
  try {
    ...
    if (reusableHome) {
      await page.reload({
        ...
        timeout: homeTimeoutMs,
      });
    } else {
      await page.goto(resolveBootstrapUrl(), {
        waitUntil: "domcontentloaded",
        timeout: homeTimeoutMs,
      });
    }
...
async function fetchSignedJsonInBrowser(page, signedUrl, referer) {
  ...
  const evalTimeoutMs = Math.max(
    5000,
    Number(process.env.TT_LITE_API_EVAL_TIMEOUT_MS || 20_000)
  );
  ...
  try {
    ...
    logTikTokTraffic(
      classifyTikTokApiKind(targetPath),
      result.bytes,
      actualUrl || signedUrl,
      `status=${result.status} items=${itemListLength(result.json)}`
    );
...
export async function tiktokMakeRequest(page, apiUrl, extraParams = {}, opts = {}) {
  ...
  // 页面健康探测：卡死 renderer 上连同步 evaluate 都不响应，
  // 3s 内判定坏 tab，快速交给上层“新 tab / 换 IP + 新 tab”恢复，
  // 避免在坏 tab 上再花 45s×N 的会话刷新/参数探测。
  try {
    await page.evaluate(() => 1, null, 3000);
  } catch (e) {
    throw new Error(
      `tiktok_page_unresponsive: ${String(e?.message || e).slice(0, 120)}`
    );
  }
  ...
  for (let attempt = 0; attempt < retries; attempt += 1) {
    try {
      ...
      // 快速失败路径（item_list 等）：不在同一个 tab 上强制重载首页/冷却，
      // 由上层换新 tab 恢复（新 tab 自带新 bootstrap + 新签名）。
      if (opts.skipRefreshOnEmpty) {
        logApi(`SKIP_REFRESH_AFTER_EMPTY url=${apiUrl} attempt=${attempt + 1}/${retries}`);
        continue;
      }
