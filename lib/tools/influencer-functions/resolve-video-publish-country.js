...
export function passesCampaignCountry(videoPublishCountry, allowedCountries) {
  ...
  if (!videoPublishCountry) return false;
  return allowed.includes(String(videoPublishCountry).trim());
}
...
export async function resolveVideoPublishCountryForInfluencers({
  ...
  try {
    for (let i = 0; i < queue.length; i++) {
      ...
      const pass = passesCampaignCountry(locationCreated, allowedCountries);
      ...
      if (tid) {
        try {
          ...
          await appendSearchTaskCountryOutcome(tid, outcome, allowedCountries);
    ...
    // 仅断开 Playwright，不关闭用户手动启动的 Chrome（9222）
    try {
