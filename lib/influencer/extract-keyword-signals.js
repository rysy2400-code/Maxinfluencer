...
const ALLOWED_SHORT_HASHTAGS = new Set(["#ai", "#ui", "#ux", "#3d", "#ar", "#vr"]);
...
function classifyHashtagForSearch(value) {
  const tag = normalizeHashtag(value);
  if (!tag) return { allowed: false, reason: "invalid_hashtag_format" };

  const body = tag.slice(1);
  if (/^\d+$/.test(body)) return { allowed: false, reason: "numeric_hashtag" };
  if (body.length < 3 && !ALLOWED_SHORT_HASHTAGS.has(tag)) {
    return { allowed: false, reason: "too_short_hashtag" };
  }
  if (NOISE_HASHTAGS.has(tag)) return { allowed: false, reason: "generic_noise_hashtag" };
  if (/^(?:no|num|number|rank|ranking|top|part|pt|chapter|ch|episode|ep)_?\d+[a-z]?$/.test(body)) {
    return { allowed: false, reason: "ranking_or_chapter_hashtag" };
  }

  return { allowed: true, reason: "kept" };
}

function classifyMentionForSearch(value) {
  const mention = normalizeMention(value);
  if (!mention) return { allowed: false, reason: "invalid_mention_format" };

  const body = mention.slice(1);
  if (body.length < 3) return { allowed: false, reason: "too_short_mention" };
  if (/^\d+$/.test(body)) return { allowed: false, reason: "numeric_mention" };

  return { allowed: true, reason: "kept" };
}

export function classifyKeywordSignalForSearch(signal = {}) {
  const signalType = signal.signal_type || signal.signalType || "hashtag";
  const signalValue = String(signal.signal_value || signal.signalValue || "").trim();
  if (!signalValue) return { allowed: false, reason: "empty_signal" };

  if (signalType === "mention" || signalValue.startsWith("@")) {
    return classifyMentionForSearch(signalValue);
  }
  return classifyHashtagForSearch(signalValue);
}

export function filterKeywordSignalsForSearch(signals = []) {
  const kept = [];
  const dropped = [];

  for (const signal of signals || []) {
    const signalValue = String(signal?.signal_value || signal?.signalValue || "").trim();
    const verdict = classifyKeywordSignalForSearch(signal);
    if (verdict.allowed) {
      kept.push(signal);
    } else {
      dropped.push({
...
function collectVideosFromInfluencer(influencer = {}) {
  const profile = influencer.profile_data || {};
