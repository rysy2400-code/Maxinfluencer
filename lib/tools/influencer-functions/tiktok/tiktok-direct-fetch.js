...
export async function fetchLocationCreatedFromVideoHtmlRequest(page, username, videoId) {
  ...
  const maxAttempts = Math.min(
    Number(process.env.TT_LITE_COUNTRY_HTML_FETCH_TRIES || 2),
    6
  );
  const htmlTimeoutMs = Math.max(
    5000,
    Number(process.env.TT_LITE_COUNTRY_HTML_TIMEOUT_MS || 12000)
  );
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    try {
      const evalPromise = page.evaluate(async (url) => {
        ...
        const timer = setTimeout(() => ctrl.abort(), htmlTimeoutMs);
        try {
