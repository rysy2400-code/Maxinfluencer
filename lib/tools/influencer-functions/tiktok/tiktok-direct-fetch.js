 ...
 */
export async function fetchLocationCreatedFromVideoInfoDocument(page, username, videoId) {
  const handle = String(username || "").replace(/^@/, "").trim();
  const id = String(videoId || "").trim();
  if (!handle || !id || typeof page.goto !== "function") return null;
  if (!isVideoInfoDocumentAllowed()) return null;
  const videoUrl = `https://www.tiktok.com/@${handle}/video/${id}`;
