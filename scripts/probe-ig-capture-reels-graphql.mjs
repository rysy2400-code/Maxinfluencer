...
page.on("request", (req) => {
  const url = req.url();
  if (!url.includes("graphql") && !url.includes("/api/v1/clips")) return;
  ...
  gql.push({
    ...
    docId,
    friendly,
    varsPreview: typeof vars === "string" ? vars.slice(0, 300) : JSON.stringify(vars || "").slice(0, 300),
  });
});
...
const unique = gql.filter((g) => {
  const k = `${g.path}|${g.docId}|${g.friendly}`;
  if (seen.has(k)) return false;
  seen.add(k);
  return g.docId || g.path.includes("clips");
});
