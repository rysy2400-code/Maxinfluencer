...
function affiliateEnabled() {
  const v = String(process.env.AFFILIATE_GMV_ENRICH ?? "false").trim().toLowerCase();
  return v === "1" || v === "true" || v === "yes";
}

export function isAffiliateEnrichEnabled() {
 ...
 * @param {object} empty
 */
function buildAffiliateMetricsFromFindResponse(handle, json, empty) {
...
/**
 * 9222 partner 标签：page.evaluate + fetch（Lite enrich 9223 无 request 上下文时使用）
 * @param {{ evaluate: Function }} page
 * @param {string} username
 * @param {{ market?: number }} [options]
 */
export async function fetchAffiliateMetricsByCdpPage(page, username, options = {}) {
  ...
  const market = options.market ?? process.env.AFFILIATE_MARKET ?? 100;
  const url = buildFindUrl();

  try {
    const result = await page.evaluate(
      async ({ url, handle, market, timeoutMs }) => {
        ...
        try {
          const res = await fetch(url, {
            ...
            headers: {
              ...
              referer: `https://partner.us.tiktokshop.com/affiliate-cmp/creator?market=${market}`,
            },
            body: JSON.stringify({
              query: handle,
              query_type: 1,
    ...
    if (!result?.json) {
      return { ...empty, reason: result?.error ? "request_failed" : "invalid_json" };
    }

    return buildAffiliateMetricsFromFindResponse(handle, result.json, empty);
  } catch (err) {
    return {
      ...empty,
 ...
 */
export async function acquireAffiliatePartnerSession(options = {}) {
  const endpoint = process.env.CDP_ENDPOINT || "http://127.0.0.1:9222";
  const { acquirePartnerCdpPage, closeCdpTarget } = await import(
    "../../cdp/cdp-target-page.js"
  );
  const { page, target, created } = await acquirePartnerCdpPage(endpoint, {
...
/**
 * @param {import('playwright').BrowserContext | { evaluate: Function, request?: object }} contextOrPage
 * @param {string} username
 * @param {{ market?: number }} [options]
 */
export async function fetchAffiliateMetricsByUsername(contextOrPage, username, options = {}) {
  const handle = String(username || "").replace(/^@/, "").trim();
  const empty = {
    ok: false,
    gmv: null,
  ...
  if (contextOrPage?.evaluate && !contextOrPage?.request) {
    return fetchAffiliateMetricsByCdpPage(contextOrPage, username, options);
  }
  if (!contextOrPage?.request) {
    return { ...empty, reason: "no_request_context" };
  }

  const context = contextOrPage;
  ...
  try {
    ...
    return buildAffiliateMetricsFromFindResponse(handle, json, empty);
  } catch (err) {
    return {
      ...empty,
      reason: err?.message?.includes("Timeout")
