...
/**
 * 清空当前 CDP 页所在浏览器配置文件的 cookies（登出/重建匿名指纹）。
 * 注意：会同时影响同 profile 的所有标签页；仅应在匿名会话（无共享账号依赖）下使用。
 */
export async function clearTiktokSessionCookies(page) {
  if (!page) return false;
  try {
    const ctx = typeof page.context === "function" ? page.context() : null;
    if (ctx && typeof ctx.clearCookies === "function") {
      await Promise.race([
        ctx.clearCookies(),
        new Promise((r) =>
          setTimeout(
            r,
            Number(process.env.TT_LITE_SESSION_CLEAR_COOKIES_TIMEOUT_MS || 10000)
          )
        ),
      ]);
      return true;
    ...
    console.warn(`[tiktok-api] clear cookies failed: ${e.message}`);
  }
  return false;
}

/**
 * 会话刷新：清 cookies（重建设备指纹）→ 失效缓存 → 重新 bootstrap。
 * 用于周期刷新或国家 HTML fetch 反复被风控（1485B slardar stub）时自愈。
 */
const lastRefreshAtByEndpoint = new Map();

export async function refreshTiktokApiSession(page, options = {}) {
  if (!page) return false;
  const endpointKey = String(
    page._ttApiSessionKey || page._ttApiEndpoint || ""
  )
    .split("#")[0]
    .trim();
  const minIntervalMs = Math.max(
    0,
    Number(process.env.TT_LITE_SESSION_REFRESH_MIN_INTERVAL_MS || 20 * 60 * 1000)
  );
  const lastAt = lastRefreshAtByEndpoint.get(endpointKey || "default") || 0;
  if (!options.force && minIntervalMs > 0 && Date.now() - lastAt < minIntervalMs) {
    return false;
  }
  const bootstrapTimeoutMs = Math.max(
    3000,
    Number(process.env.TT_LITE_SESSION_REFRESH_TIMEOUT_MS || 12000)
  );
  const refreshTimeoutMs = Math.max(
    bootstrapTimeoutMs + 3000,
    Number(process.env.TT_LITE_SESSION_REFRESH_TOTAL_TIMEOUT_MS || 25000)
  );
  try {
    const refreshed = await Promise.race([
      (async () => {
        await clearTiktokSessionCookies(page);
        invalidateTiktokWebSession(page);
        await Promise.race([
          bootstrapTiktokWebSession(page, { forceRefresh: true }),
          new Promise((r) => setTimeout(r, bootstrapTimeoutMs)),
        ]);
        if (typeof page.waitForTimeout === "function") {
      ...
      new Promise((r) => setTimeout(() => r(false), refreshTimeoutMs)),
    ]);
    if (refreshed) lastRefreshAtByEndpoint.set(endpointKey || "default", Date.now());
    return refreshed;
  } catch (e) {
    console.warn(`[tiktok-api] refresh session failed: ${e.message}`);
    return false;
  }
}
...
async function fetchSignedJsonInBrowser(page, signedUrl, referer) {
  ...
  try {
    ...
    const callTimeoutMs = Math.max(
      5000,
      Number(process.env.TT_LITE_SIGNED_FETCH_TIMEOUT_MS || 25000)
    );
    ...
    const result = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`signed fetch CDP timeout ${callTimeoutMs}ms`)),
          callTimeoutMs
        )
      ),
    ]);
