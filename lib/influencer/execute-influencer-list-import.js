...
function buildImportAckSummary({ fileName, mergedValidCount, platformCounts, skippedUnsupported }) {
  ...
  const unsupportedText =
    Number(skippedUnsupported) > 0
      ...
      : "";
  if (name) {
    return `已收到 ${name}，共识别 ${n} 位红人，正在分析与筛选${platformText}${unsupportedText}；完成后会和您同步。`;
  }
  return `共识别 ${n} 位红人，正在分析与筛选${platformText}${unsupportedText}；完成后会和您同步。`;
}
...
export async function executeInfluencerListImport(input) {
  ...
  const contactMode = String(input.contactMode || "default").trim();
