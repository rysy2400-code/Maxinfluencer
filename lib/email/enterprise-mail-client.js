...
import { accountMatchesTemporaryOutboundPool } from "./temporary-outbound-pool.js";
...
export async function pickRandomOutboundAccount() {
  const accounts = await getAllOutboundAccounts();
  const filtered = accounts.filter(accountMatchesTemporaryOutboundPool);
  if (!filtered.length) {
    console.warn(
      "[EnterpriseMailClient] pickRandomOutboundAccount：临时白名单内邮箱在 op_contacts 中均无可用 SMTP 配置。"
    );
    return null;
  }
