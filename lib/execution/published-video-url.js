...
/** @typedef {'tiktok'|'instagram'|'youtube'|'x'|'unknown'} PublishedVideoPlatform */
...
export function parsePublishedVideoUrl(raw) {
  ...
  try {
    ...
    if (host.includes("tiktok.com")) {
      ...
      if (um) username = um[1];
      // 短链（vt.tiktok.com/XXXX、tiktok.com/t/XXXX）没有数字 videoId，
      // 用短码兜底，保证多平台去重与展示稳定。
      if (!videoId) {
        const sm = u.pathname.match(/^\/(?:t\/)?([A-Za-z0-9]{5,})\/?$/);
        if (sm) shortcode = sm[1];
      }
      ...
      platform = "x";
      const sm = u.pathname.match(/^\/(?:@)?([^/]+)\/status\/(\d+)/);
      if (sm) {
        username = sm[1];
        videoId = sm[2];
      }
