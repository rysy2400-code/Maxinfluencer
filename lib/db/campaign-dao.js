/**
 * Campaign 表 DAO（已发布 campaign）
 *
 * 目前实现基于：
 * - tiktok_campaign：Campaign 配置与 4 大板块快照 + 关键标量字段
 * - tiktok_campaign_execution：按红人维度的执行状态
 */
import { queryTikTok } from "./mysql-tiktok.js";
import {
  buildNormalizedInfluencerSnapshot,
  resolveTiktokUsername,
} from "./campaign-candidates-dao.js";
import {
  enrichCampaignInfoCountryFields,
  primaryRegionIsoFromCampaignInfo,
} from "../influencer/campaign-country-codes.js";
import {
  normalizeCampaignInfoPlatforms,
  primaryPlatformSlugFromCampaignInfo,
} from "../influencer/resolve-campaign-platforms.js";
import {
  SQL_EXECUTION_CREATOR_MATCH,
  paramsExecutionCreatorMatch,
} from "./campaign-execution-keys.js";
import { resolveNeedSample } from "../execution/need-sample.js";
import { parseQuoteNegotiation, resolveLatestInfluencerQuote } from "../execution/quote-resolution.js";
import {
  normalizeDeliverables,
  resolveLatestDeliverablesFromRow,
} from "../execution/deliverables-resolution.js";
import { normalizeDeliverablesText } from "../campaign/deliverables.js";
import { influencerEntryDelta } from "../execution/influencer-entry-seq.js";
import { loadSpecialRequestCountsByUsername } from "./campaign-communication-dao.js";
import { loadInfluencerLanguageMap } from "../influencer/influencer-language-store.js";
import {
  invalidateExecutionCounts,
  loadExecutionCounts,
} from "./execution-counts-cache.js";
import { normalizeProductLink } from "../campaign/product-link.js";
import {
  resolvePublishedVideos,
  aggregatePublishedStats,
  computeTotalCpm,
} from "../execution/published-videos.js";
/** 安全解析 JSON：若已是对象则直接返回，避免 mysql2 已解析的 JSON 列被二次 parse 报错 */
function parseJson(value) {
  if (value == null) return null;
  if (typeof value === "object") return value;
  if (typeof value !== "string") return null;
  try {
    return JSON.parse(value);
  } catch {
    return null;
  }
}

function normalizeExecutionCurrency(c) {
  const s = String(c || "")
    .trim()
    .toUpperCase()
    .slice(0, 8);
  return s || "USD";
}

function parseQuoteNegotiationColumn(raw) {
  return parseQuoteNegotiation(raw);
}

/** campaign_info JSON 落库前：平台 canonical + 国家 ISO 等 */
function prepareCampaignInfoForDb(campaignInfo) {
  if (campaignInfo == null) return null;
  return enrichCampaignInfoCountryFields(
    normalizeCampaignInfoPlatforms(campaignInfo)
  );
}

export async function createCampaign(data) {
  const id = data.id;
  const sessionId = data.sessionId;
  const productInfo = data.productInfo || null;
  const campaignInfo = data.campaignInfo || null;
  const influencerProfile = data.influencerProfile || null;
  const influencers = Array.isArray(data.influencers) ? data.influencers : [];
  const contentScript = data.contentScript || null;
  const influencersPerDay = data.influencersPerDay ?? 5;
  const keywordStrategy = typeof data.keywordStrategy === "string" ? data.keywordStrategy.trim() : null;

  const campaignInfoForDb = prepareCampaignInfoForDb(campaignInfo);
  const platform = primaryPlatformSlugFromCampaignInfo(campaignInfoForDb);
  const region = primaryRegionIsoFromCampaignInfo(campaignInfoForDb);
  const budget =
    campaignInfo && typeof campaignInfo.budget === "number"
      ? campaignInfo.budget
      : null;
  const commission =
    campaignInfo && typeof campaignInfo.commission === "number"
      ? campaignInfo.commission
      : null;

  const recommendedInfluencersPayload =
    influencers.length > 0
      ? influencers
          .map((inf) => {
            if (!inf || typeof inf !== "object") return null;
            const handle = resolveTiktokUsername(inf);
            if (!handle) return null;
            return {
              id: handle,
              ...buildNormalizedInfluencerSnapshot(inf, {}),
            };
          })
          .filter(Boolean)
      : [];

  const sql = `
    INSERT INTO tiktok_campaign (
      id, session_id,
      platform, region, start_date, end_date, budget, commission,
      product_info, campaign_info, influencer_profile, content_script,
      recommended_influencers,
      keyword_strategy, influencers_per_day, status
    ) VALUES (?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'running')
  `;
  await queryTikTok(sql, [
    id,
    sessionId,
    platform,
    region,
    budget,
    commission,
    productInfo ? JSON.stringify(productInfo) : null,
    campaignInfoForDb ? JSON.stringify(campaignInfoForDb) : null,
    influencerProfile ? JSON.stringify(influencerProfile) : null,
    contentScript ? JSON.stringify(contentScript) : null,
    recommendedInfluencersPayload.length > 0
      ? JSON.stringify(recommendedInfluencersPayload)
      : null,
    keywordStrategy || null,
    influencersPerDay,
  ]);

  return { id, sessionId };
}

/**
 * 同一会话仅保留一条活跃 campaign：已存在则更新快照，否则 INSERT。
 * @returns {Promise<{ id: string, sessionId: string, reused: boolean }>}
 */
export async function upsertCampaignForSession(data) {
  const sessionId = typeof data.sessionId === "string" ? data.sessionId.trim() : "";
  if (!sessionId) {
    throw new Error("upsertCampaignForSession 需要 sessionId");
  }

  const existing = await getCampaignBySessionId(sessionId);
  if (!existing) {
    const created = await createCampaign(data);
    return { ...created, reused: false };
  }

  const influencers = Array.isArray(data.influencers) ? data.influencers : [];
  const recommendedInfluencersPayload =
    influencers.length > 0
      ? influencers
          .map((inf) => {
            if (!inf || typeof inf !== "object") return null;
            const handle = resolveTiktokUsername(inf);
            if (!handle) return null;
            return {
              id: handle,
              ...buildNormalizedInfluencerSnapshot(inf, {}),
            };
          })
          .filter(Boolean)
      : undefined;

  await updateCampaign(existing.id, {
    productInfo: data.productInfo ?? undefined,
    campaignInfo: data.campaignInfo ?? undefined,
    influencerProfile: data.influencerProfile ?? undefined,
    contentScript: data.contentScript ?? undefined,
    influencersPerDay: data.influencersPerDay ?? undefined,
    keywordStrategy: data.keywordStrategy ?? undefined,
    ...(recommendedInfluencersPayload !== undefined
      ? { recommendedInfluencers: recommendedInfluencersPayload }
      : {}),
    status: "running",
  });

  return { id: existing.id, sessionId, reused: true };
}

/**
 * 按会话 id 查关联的已发布 Campaign（排除软删）
 * @param {string} sessionId - tiktok_campaign_sessions.id
 * @returns {Promise<{ id: string, sessionId: string, status: string, influencersPerDay: number|null, createdAt: * }|null>}
 */
export async function getCampaignBySessionId(sessionId) {
  const sid = typeof sessionId === "string" ? sessionId.trim() : "";
  if (!sid) return null;

  const rows = await queryTikTok(
    `SELECT id, session_id, status, influencers_per_day, created_at
     FROM tiktok_campaign
     WHERE session_id = ? AND status <> 'deleted'
     ORDER BY created_at DESC
     LIMIT 1`,
    [sid]
  );
  if (!rows?.length) return null;
  const r = rows[0];
  return {
    id: r.id,
    sessionId: r.session_id,
    status: r.status,
    influencersPerDay: r.influencers_per_day ?? null,
    createdAt: r.created_at,
  };
}

/**
 * 按归一化产品链接查找已发布 campaign（排除软删）。
 * 用于发布/修改时校验「该产品链接是否已被其他 campaign 使用」。
 * @param {string|null} normalizedLink - normalizeProductLink 的结果
 * @param {{ excludeCampaignId?: string|null, excludeSessionId?: string|null }} [opts]
 * @returns {Promise<Array<{ id: string, sessionId: string, status: string, productInfo: object|null }>>}
 */
export async function findCampaignsByNormalizedProductLink(
  normalizedLink,
  { excludeCampaignId = null, excludeSessionId = null } = {}
) {
  if (!normalizedLink) return [];
  // 归一化在应用层完成（避免生产库 DDL/索引变更）。
  // 当前数据量（百级 campaign）全量读取成本可忽略；如未来数据量增大可再改为
  // 落库归一化列 + 索引方案（参考 lib/db/migrations/add-product-link-norm.sql）。
  const rows = await queryTikTok(
    `SELECT id, session_id, status, product_info
     FROM tiktok_campaign
     WHERE status <> 'deleted'`
  );
  const matches = [];
  for (const r of rows) {
    if (excludeCampaignId && String(r.id) === String(excludeCampaignId)) continue;
    if (excludeSessionId && String(r.session_id) === String(excludeSessionId)) continue;
    const productInfo = parseJson(r.product_info);
    if (!productInfo?.productLink) continue;
    if (normalizeProductLink(productInfo.productLink) === normalizedLink) {
      matches.push({
        id: r.id,
        sessionId: r.session_id,
        status: r.status,
        productInfo,
      });
    }
  }
  return matches;
}

export async function getCampaignById(campaignId) {
  const sql = `SELECT * FROM tiktok_campaign WHERE id = ?`;
  const rows = await queryTikTok(sql, [campaignId]);
  if (!rows || rows.length === 0) return null;
  const r = rows[0];

  const execRows = await queryTikTok(
    `SELECT * FROM tiktok_campaign_execution WHERE campaign_id = ?`,
    [campaignId]
  );

  const influencers = execRows.map((row) => {
    const base = parseJson(row.influencer_snapshot) || {};
    return {
      id: row.tiktok_username,
      platformInfluencerId: row.influencer_id || null,
      stage: row.stage,
      ...base,
    };
  });

  return {
    id: r.id,
    sessionId: r.session_id,
    productInfo: parseJson(r.product_info),
    campaignInfo: parseJson(r.campaign_info),
    influencerProfile: parseJson(r.influencer_profile),
    influencers,
    recommendedInfluencers: parseJson(r.recommended_influencers) || [],
    contentScript: parseJson(r.content_script),
    keywordStrategy: r.keyword_strategy || null,
    status: r.status,
    statusBeforePause: r.status_before_pause ?? null,
    influencersPerDay: r.influencers_per_day ?? null,
    createdAt: r.created_at,
    updatedAt: r.updated_at,
  };
}

export async function getCampaignCoreById(campaignId) {
  const rows = await queryTikTok(
    `SELECT id, session_id, product_info, campaign_info, influencer_profile,
            recommended_influencers, content_script, keyword_strategy, status,
            status_before_pause, influencers_per_day, created_at, updated_at
       FROM tiktok_campaign WHERE id = ? LIMIT 1`,
    [campaignId]
  );
  if (!rows || rows.length === 0) return null;
  const r = rows[0];
  return {
    id: r.id,
    sessionId: r.session_id,
    productInfo: parseJson(r.product_info),
    campaignInfo: parseJson(r.campaign_info),
    influencerProfile: parseJson(r.influencer_profile),
    influencers: [],
    recommendedInfluencers: parseJson(r.recommended_influencers) || [],
    contentScript: parseJson(r.content_script),
    keywordStrategy: r.keyword_strategy || null,
    status: r.status,
    statusBeforePause: r.status_before_pause ?? null,
    influencersPerDay: r.influencers_per_day ?? null,
    createdAt: r.created_at,
    updatedAt: r.updated_at,
  };
}

const CAMPAIGN_UPDATE_KEYS = {
  status: "status",
  statusBeforePause: "status_before_pause",
  influencersPerDay: "influencers_per_day",
  recommendedInfluencers: "recommended_influencers",
  productInfo: "product_info",
  campaignInfo: "campaign_info",
  influencerProfile: "influencer_profile",
  contentScript: "content_script",
  keywordStrategy: "keyword_strategy",
  deletedAt: "deleted_at",
  deletedBy: "deleted_by",
  deleteReason: "delete_reason",
};

export async function updateCampaign(campaignId, updates) {
  const normalized = { ...updates };
  if (normalized.campaignInfo != null) {
    normalized.campaignInfo = prepareCampaignInfoForDb(normalized.campaignInfo);
  }

  const setClauses = [];
  const values = [];
  for (const [k, v] of Object.entries(normalized)) {
    const col = CAMPAIGN_UPDATE_KEYS[k];
    if (!col) continue;
    setClauses.push(`${col} = ?`);
    values.push(
      typeof v === "object" && v !== null && !(v instanceof Date)
        ? JSON.stringify(v)
        : v
    );
  }
  if (normalized.campaignInfo != null) {
    const ci = normalized.campaignInfo;
    setClauses.push("region = ?");
    values.push(primaryRegionIsoFromCampaignInfo(ci));
    setClauses.push("platform = ?");
    values.push(primaryPlatformSlugFromCampaignInfo(ci));
    if (ci.budget != null && typeof ci.budget === "number") {
      setClauses.push("budget = ?");
      values.push(ci.budget);
    }
    if (ci.commission != null && typeof ci.commission === "number") {
      setClauses.push("commission = ?");
      values.push(ci.commission);
    }
  }

  if (setClauses.length === 0) return;
  values.push(campaignId);
  const sql = `UPDATE tiktok_campaign SET ${setClauses.join(
    ", "
  )} WHERE id = ?`;
  await queryTikTok(sql, values);
}

/** 执行表 created_at → ISO，供排序与前端展示 */
function executionCreatedAtToIso(createdAt) {
  if (createdAt == null) return null;
  if (createdAt instanceof Date) return createdAt.toISOString();
  const d = new Date(createdAt);
  return Number.isNaN(d.getTime()) ? null : d.toISOString();
}

/**
 * 对话表 event_time / sent_at 是 TIMESTAMP，库里存的是真实瞬间，mysql2 读出来已是正确时刻，
 * 直接 toISOString() 即可（前端统一按 Asia/Shanghai 展示）。
 * 历史数据少 8 小时的偏移已由 scripts/backfill-conversation-timestamps-utc.mjs 回填，
 * 这里不能再做「UTC 墙钟还原」，否则会二次偏移、页面时间比北京时间晚 8 小时。
 * 仅对带时区后缀的字符串（如 quote_negotiation 里的 ISO）或无时区的 naive 字符串兜底解析。
 */
function toIsoInstant(at) {
  if (at == null) return null;
  if (at instanceof Date) {
    return Number.isNaN(at.getTime()) ? null : at.toISOString();
  }
  const raw = String(at).trim();
  if (!raw) return null;
  if (/Z$/i.test(raw) || /[+-]\d{2}:?\d{2}$/.test(raw)) {
    const parsed = new Date(raw);
    return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
  }
  const norm = raw.includes("T") ? raw : raw.replace(" ", "T");
  const parsed = new Date(`${norm}Z`);
  return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
}

/** 最新回复：邮件 inbound 与砍价记录中红人条目的较晚者 */
function resolveLastInboundReplyAt(emailAt, quoteNegotiation) {
  let bestIso = emailAt ? toIsoInstant(emailAt) : null;
  let bestMs = bestIso ? new Date(bestIso).getTime() : 0;
  if (!Array.isArray(quoteNegotiation)) return bestIso;
  for (const entry of quoteNegotiation) {
    if (entry?.role !== "influencer") continue;
    const iso = toIsoInstant(entry?.at);
    const ms = iso ? new Date(iso).getTime() : 0;
    if (ms > bestMs) {
      bestMs = ms;
      bestIso = iso;
    }
  }
  return bestMs > 0 ? bestIso : null;
}

/** 红人执行阶段分组用 */
const STAGE_PENDING_QUOTE = "pending_quote";
const STAGE_QUOTE_SUBMITTED = "quote_submitted";
const STAGE_QUOTE_REJECTED = "quote_rejected";
const STAGE_PENDING_SHIPPING_ADDRESS = "pending_shipping_address";
const STAGE_PENDING_SAMPLE = "pending_sample";
const STAGE_PENDING_SCRIPT = "pending_script";
const STAGE_SCRIPT_REVIEW = "script_review";
const STAGE_VIDEO_REVIEW = "video_review";
const STAGE_PENDING_VIDEO = "pending_video";
const STAGE_PUBLISHED = "published";

/** campaign 下各执行红人最近一封红人来信（email_inbound）时间，key = tiktok_username */
async function loadLastInboundReplyAtByUsername(campaignId, usernames = null) {
  const normalizedUsernames = Array.isArray(usernames)
    ? usernames
        .map((u) => (u == null ? "" : String(u).trim()))
        .filter(Boolean)
    : null;
  const usernameFilter =
    normalizedUsernames && normalizedUsernames.length > 0
      ? `AND tiktok_username IN (${normalizedUsernames.map(() => "?").join(",")})`
      : "";
  const rows = await queryTikTok(
    `
    WITH execs AS (
      SELECT
        tiktok_username,
        influencer_id AS platform_influencer_id
      FROM tiktok_campaign_execution
      WHERE campaign_id = ?
        ${usernameFilter}
    ),
    inbound AS (
      SELECT
        e.tiktok_username,
        COALESCE(m.event_time, m.sent_at, m.created_at) AS last_inbound_at,
        ROW_NUMBER() OVER (
          PARTITION BY e.tiktok_username
          ORDER BY COALESCE(m.event_time, m.sent_at, m.created_at) DESC, m.id DESC
        ) AS rn
      FROM execs e
      INNER JOIN tiktok_influencer_conversation_messages m
        ON (
          (e.platform_influencer_id IS NOT NULL AND TRIM(e.platform_influencer_id) <> '' AND m.influencer_id = e.platform_influencer_id)
          OR m.influencer_id = e.tiktok_username
        )
      WHERE m.event_type = 'email_inbound'
    )
    SELECT tiktok_username, last_inbound_at
    FROM inbound
    WHERE rn = 1
    `,
    normalizedUsernames && normalizedUsernames.length > 0
      ? [campaignId, ...normalizedUsernames]
      : [campaignId]
  );
  const map = {};
  for (const r of rows || []) {
    const u = r.tiktok_username;
    if (!u) continue;
    const at = r.last_inbound_at;
    if (at == null) continue;
    const iso = toIsoInstant(at);
    if (iso) map[u] = iso;
  }
  return map;
}

/**
 * 执行卡片「语言」列：优先用快照里的 bio 语言，快照缺失（老执行行）时回查语言旁路表。
 * key = influencer_id
 */
async function loadBioLanguageByInfluencerId(rows = []) {
  const ids = [
    ...new Set(
      (rows || [])
        .map((r) => (r?.influencer_id == null ? "" : String(r.influencer_id).trim()))
        .filter(Boolean)
    ),
  ];
  if (!ids.length) return {};
  try {
    const map = await loadInfluencerLanguageMap(ids);
    const out = {};
    for (const [id, value] of map) {
      if (value?.bioLanguage) out[id] = value.bioLanguage;
    }
    return out;
  } catch (err) {
    console.warn(
      "[campaign-dao] 读取红人画像语言失败:",
      err?.message || err
    );
    return {};
  }
}

const EXECUTION_STATUS_STAGE_MAP = {
  contacted: [STAGE_PENDING_QUOTE],
  pendingPrice: [STAGE_QUOTE_SUBMITTED, "pending_creator_confirmation", STAGE_QUOTE_REJECTED],
  pendingSample: [STAGE_PENDING_SHIPPING_ADDRESS, STAGE_PENDING_SAMPLE],
  pendingDraft: [
    STAGE_PENDING_SCRIPT,
    STAGE_SCRIPT_REVIEW,
    STAGE_VIDEO_REVIEW,
    STAGE_PENDING_VIDEO,
  ],
  published: [STAGE_PUBLISHED],
};

/** 待审核草稿子 Tab：script=待审核脚本（等脚本/脚本待审），video=待审核视频（视频待审/等视频） */
const PENDING_DRAFT_SUBTAB_STAGES = {
  script: [STAGE_PENDING_SCRIPT, STAGE_SCRIPT_REVIEW],
  video: [STAGE_VIDEO_REVIEW, STAGE_PENDING_VIDEO],
};

const EXECUTION_STATUS_COLUMN_KEYS = [
  "contacted",
  "pendingPrice",
  "pendingSample",
  "pendingDraft",
  "published",
];

function emptyExecutionColumns() {
  return {
    contacted: [],
    pendingPrice: [],
    pendingShippingAddress: [],
    pendingSample: [],
    pendingDraft: [],
    pendingDraftScript: [],
    pendingDraftVideo: [],
    published: [],
  };
}

/** 由 tiktok_campaign_execution 单行构建执行进度 item（status 列表与导出共用） */
function buildExecutionStatusItem(row, lastInboundReplyByUsername = {}) {
  const id = row.tiktok_username;
  const base = parseJson(row.influencer_snapshot) || {};
  const lastEvent = parseJson(row.last_event) || {};
  const name = base.name || id;
  const stage = row.stage || STAGE_PENDING_QUOTE;
  const executionShippingInfo = parseJson(row.shipping_info);
  const executionVideoDraft = parseJson(row.video_draft);
  const quoteNeg = parseJson(row.quote_negotiation);
  const quoteNegotiation = Array.isArray(quoteNeg) ? quoteNeg : [];
  const effectiveInfluencerQuote = resolveLatestInfluencerQuote({
    quoteNegotiation,
    fallbackAmount: row.flat_fee,
    fallbackCurrency: row.currency,
  });
  // 已发布视频：多平台数组（老数据由 deliverablesTimeline / 单值字段兜底合成）
  const publishedVideos = resolvePublishedVideos(lastEvent, {
    videoLink: row.video_link,
  });
  const publishedStats = aggregatePublishedStats(publishedVideos);
  const publishedCpm = computeTotalCpm(
    effectiveInfluencerQuote?.amount ?? row.flat_fee,
    publishedStats.views
  );
  return {
    id,
    platformInfluencerId: row.influencer_id || null,
    name,
    stage,
    source: row.source || "web_search",
    quoteOrigin: row.quote_origin || null,
    flatFeeUsd: effectiveInfluencerQuote?.amount ?? null,
    // 执行级佣金（NULL = 未谈定）；卡片与审批同口径
    commissionPercent:
      row.commission_percent == null ? null : Number(row.commission_percent),
    executionVideoLink: row.video_link || null,
    executionShippingInfo,
    executionVideoDraft,
    ...base,
    ...lastEvent,
    // 口径固定：卡片顶部「播放」与 eCPM 用搜索入库时的近 N 条视频均播（快照 views），
    // 已发布视频的播放量只走 publishedVideos / publishedStats，避免被 last_event.views 覆盖。
    ...(base.views !== undefined ? { views: base.views } : {}),
    ...(base.avg_views !== undefined ? { avg_views: base.avg_views } : {}),
    ...(base.avgViews !== undefined ? { avgViews: base.avgViews } : {}),
    publishedVideos,
    publishedStats,
    publishedCpm,
    currency:
      effectiveInfluencerQuote?.currency ||
      (row.currency ? String(row.currency).toUpperCase() : "USD"),
    quoteNegotiation,
    // 红人级「最新交付结果」：与最新报价同源（quote_negotiation 条目的 deliverables 字段）
    latestDeliverables: resolveLatestDeliverablesFromRow(row),
    executionCreatedAt: executionCreatedAtToIso(row.created_at),
    lastInboundReplyAt: resolveLastInboundReplyAt(
      lastInboundReplyByUsername[id] || null,
      quoteNegotiation
    ),
  };
}

/**
 * 导出执行进度 Excel 专用：只拉目标 stage 的行，且只 SELECT 导出所需的列，
 * 并对 influencer_snapshot 做 JSON 投影（不搬整份十几 MB 的快照）。
 * 返回的 item 结构与 getCampaignExecutionStatus 各 stage 数组一致。
 */
const SQL_EXECUTION_EXPORT_SNAPSHOT_PROJECTION = `JSON_OBJECT(
  'name', JSON_EXTRACT(influencer_snapshot, '$.name'),
  'displayName', JSON_EXTRACT(influencer_snapshot, '$.displayName'),
  'username', JSON_EXTRACT(influencer_snapshot, '$.username'),
  'platform', JSON_EXTRACT(influencer_snapshot, '$.platform'),
  'Platform', JSON_EXTRACT(influencer_snapshot, '$.Platform'),
  'profileUrl', JSON_EXTRACT(influencer_snapshot, '$.profileUrl'),
  'profile_url', JSON_EXTRACT(influencer_snapshot, '$.profile_url'),
  'followers', JSON_EXTRACT(influencer_snapshot, '$.followers'),
  'followerCount', JSON_EXTRACT(influencer_snapshot, '$.followerCount'),
  'follower_count', JSON_EXTRACT(influencer_snapshot, '$.follower_count'),
  'avg_views', JSON_EXTRACT(influencer_snapshot, '$.avg_views'),
  'avgViews', JSON_EXTRACT(influencer_snapshot, '$.avgViews'),
  'views', JSON_EXTRACT(influencer_snapshot, '$.views'),
  'gmv', JSON_EXTRACT(influencer_snapshot, '$.gmv'),
  'gmvDisplay', JSON_EXTRACT(influencer_snapshot, '$.gmvDisplay'),
  'gmv_display', JSON_EXTRACT(influencer_snapshot, '$.gmv_display'),
  'affiliateMetrics', JSON_EXTRACT(influencer_snapshot, '$.affiliateMetrics'),
  'videoPublishCountry', JSON_EXTRACT(influencer_snapshot, '$.videoPublishCountry'),
  'video_publish_country', JSON_EXTRACT(influencer_snapshot, '$.video_publish_country'),
  'residenceCountry', JSON_EXTRACT(influencer_snapshot, '$.residenceCountry'),
  'residence_country', JSON_EXTRACT(influencer_snapshot, '$.residence_country'),
  'accountCountry', JSON_EXTRACT(influencer_snapshot, '$.accountCountry'),
  'account_country', JSON_EXTRACT(influencer_snapshot, '$.account_country'),
  'bioLanguage', JSON_EXTRACT(influencer_snapshot, '$.bioLanguage'),
  'bio_language', JSON_EXTRACT(influencer_snapshot, '$.bio_language'),
  'bioLanguageConfidence', JSON_EXTRACT(influencer_snapshot, '$.bioLanguageConfidence'),
  'bioLanguageSource', JSON_EXTRACT(influencer_snapshot, '$.bioLanguageSource'),
  'analysisSummary', JSON_EXTRACT(influencer_snapshot, '$.analysisSummary'),
  'analysis_summary', JSON_EXTRACT(influencer_snapshot, '$.analysis_summary'),
  'match_analysis_summary', JSON_EXTRACT(influencer_snapshot, '$.match_analysis_summary'),
  'matchAnalysis', JSON_EXTRACT(influencer_snapshot, '$.matchAnalysis'),
  'analysis', JSON_EXTRACT(influencer_snapshot, '$.analysis'),
  'profile_analysis', JSON_EXTRACT(influencer_snapshot, '$.profile_analysis'),
  'reason', JSON_EXTRACT(influencer_snapshot, '$.reason'),
  'flatFeeUSD', JSON_EXTRACT(influencer_snapshot, '$.flatFeeUSD'),
  'campaignAgentDecision', JSON_EXTRACT(influencer_snapshot, '$.campaignAgentDecision'),
  'shippingAddress', JSON_EXTRACT(influencer_snapshot, '$.shippingAddress'),
  'shipping_info', JSON_EXTRACT(influencer_snapshot, '$.shipping_info'),
  'videoDraftLink', JSON_EXTRACT(influencer_snapshot, '$.videoDraftLink'),
  'draftLink', JSON_EXTRACT(influencer_snapshot, '$.draftLink'),
  'videoLink', JSON_EXTRACT(influencer_snapshot, '$.videoLink'),
  'video_link', JSON_EXTRACT(influencer_snapshot, '$.video_link'),
  'ecpm', JSON_EXTRACT(influencer_snapshot, '$.ecpm'),
  'cpm', JSON_EXTRACT(influencer_snapshot, '$.cpm'),
  'likes', JSON_EXTRACT(influencer_snapshot, '$.likes'),
  'comments', JSON_EXTRACT(influencer_snapshot, '$.comments'),
  'revisionHistory', JSON_EXTRACT(influencer_snapshot, '$.revisionHistory'),
  'draftFeedback', JSON_EXTRACT(influencer_snapshot, '$.draftFeedback'),
  'feedback', JSON_EXTRACT(influencer_snapshot, '$.feedback')
)`;

/** 导出专用：目标 stage 的执行行（含导出所需的全部字段，无分页） */
export async function getExecutionRowsForStageExport(campaignId, stageKey) {
  const stages = EXECUTION_STATUS_STAGE_MAP[stageKey];
  if (!stages) return [];

  const rowWhere = ["campaign_id = ?"];
  const rowParams = [campaignId];
  if (stageKey === "contacted") {
    rowWhere.push(
      `(stage IN (${stages.map(() => "?").join(",")}) OR stage IS NULL OR stage = '')`
    );
  } else {
    rowWhere.push(`stage IN (${stages.map(() => "?").join(",")})`);
  }
  rowParams.push(...stages);

  const rows = await queryTikTok(
    `
    SELECT
      tiktok_username,
      influencer_id,
      stage,
      created_at,
      updated_at,
      flat_fee,
      currency,
      video_link,
      source,
      quote_origin,
      last_event,
      quote_negotiation,
      shipping_info,
      video_draft,
      ${SQL_EXECUTION_EXPORT_SNAPSHOT_PROJECTION} AS influencer_snapshot
    FROM tiktok_campaign_execution
    WHERE ${rowWhere.join(" AND ")}
    ORDER BY created_at DESC, id DESC
    `,
    rowParams
  );

  // JSON_OBJECT 投影会把缺失字段输出为 null；剥掉顶层 null 键，
  // 与旧路径「快照里没有该键」的语义一致，避免 null 覆盖计算字段（如 name）。
  const exportRows = (rows || []).map((row) => {
    const snapshot = parseJson(row.influencer_snapshot) || {};
    const compact = {};
    for (const [k, v] of Object.entries(snapshot)) {
      if (v !== null && v !== undefined) compact[k] = v;
    }
    return { ...row, influencer_snapshot: compact };
  });

  const usernames = (exportRows)
    .map((row) => row?.tiktok_username)
    .filter(Boolean);
  const lastInboundReplyByUsername = await loadLastInboundReplyAtByUsername(
    campaignId,
    usernames
  );

  const bioLanguageByInfluencerId = await loadBioLanguageByInfluencerId(exportRows);
  const items = exportRows.map((row) => {
    const item = buildExecutionStatusItem(row, lastInboundReplyByUsername);
    if (!item.bioLanguage && item.platformInfluencerId) {
      item.bioLanguage =
        bioLanguageByInfluencerId[String(item.platformInfluencerId)] || null;
    }
    return item;
  });

  if (stageKey === "contacted") {
    items.sort((a, b) => {
      const ta = a.executionCreatedAt
        ? new Date(a.executionCreatedAt).getTime()
        : 0;
      const tb = b.executionCreatedAt
        ? new Date(b.executionCreatedAt).getTime()
        : 0;
      return tb - ta;
    });
  } else if (stageKey === "pendingPrice") {
    items.sort((a, b) => {
      const ta = a.lastInboundReplyAt
        ? new Date(a.lastInboundReplyAt).getTime()
        : 0;
      const tb = b.lastInboundReplyAt
        ? new Date(b.lastInboundReplyAt).getTime()
        : 0;
      return tb - ta;
    });
  }
  return items;
}

function normalizeExecutionStatusOptions(options = {}) {
  const stage =
    typeof options.stage === "string" &&
    EXECUTION_STATUS_STAGE_MAP[options.stage]
      ? options.stage
      : null;
  const rawSubTab =
    typeof options.subTab === "string" ? options.subTab.trim().toLowerCase() : "";
  const subTab =
    stage === "pendingDraft" && (rawSubTab === "script" || rawSubTab === "video")
      ? rawSubTab
      : null;
  const limitNum = Number(options.limit);
  return {
    stage,
    subTab,
    limit:
      Number.isFinite(limitNum) && limitNum > 0
        ? Math.min(Math.floor(limitNum), 100)
        : null,
    cursor: decodeExecutionCursor(options.cursor),
    username:
      typeof options.username === "string" && options.username.trim()
        ? options.username.trim().replace(/^@/, "").slice(0, 255)
        : null,
    usernameExact:
      options.usernameExact === true ||
      options.usernameExact === "1" ||
      options.usernameExact === "true",
  };
}

function encodeExecutionCursor(row, sortColumn) {
  if (!row || row.id == null || row[sortColumn] == null) return null;
  return Buffer.from(
    JSON.stringify({
      sortValue: executionCreatedAtToIso(row[sortColumn]),
      id: Number(row.id),
    })
  ).toString("base64url");
}

function decodeExecutionCursor(raw) {
  if (typeof raw !== "string" || !raw.trim()) return null;
  try {
    const parsed = JSON.parse(Buffer.from(raw, "base64url").toString("utf8"));
    const id = Number(parsed?.id);
    const sortValue = parsed?.sortValue ? new Date(parsed.sortValue) : null;
    if (!Number.isInteger(id) || id <= 0 || !sortValue || Number.isNaN(sortValue.getTime())) {
      return null;
    }
    return { id, sortValue };
  } catch {
    return null;
  }
}

function resolveExecutionColumnFromStage(stage) {
  if (stage === STAGE_PENDING_QUOTE) return "contacted";
  if (stage === STAGE_QUOTE_SUBMITTED || stage === "pending_creator_confirmation" || stage === STAGE_QUOTE_REJECTED) {
    return "pendingPrice";
  }
  if (stage === STAGE_PENDING_SAMPLE) return "pendingSample";
  if (stage === STAGE_PENDING_SHIPPING_ADDRESS) return "pendingShippingAddress";
  if (
    stage === STAGE_PENDING_SCRIPT ||
    stage === STAGE_SCRIPT_REVIEW ||
    stage === STAGE_VIDEO_REVIEW ||
    stage === STAGE_PENDING_VIDEO
  ) {
    return "pendingDraft";
  }
  if (stage === STAGE_PUBLISHED) return "published";
  return "contacted";
}

export async function getCampaignExecutionStatus(campaignId, options = {}) {
  // 这里只需要 campaign 自身的标量字段（id/status/influencersPerDay/productInfo），
  // 不需要 getCampaignById 全量读取执行表（几万行 + 大 JSON snapshot），避免每次请求搬十几 MB。
  const c = await getCampaignCoreById(campaignId);
  if (!c) return null;

  const needSample = resolveNeedSample(c.productInfo);
  const paging = normalizeExecutionStatusOptions(options);
  const columns = emptyExecutionColumns();

  const rowWhere = ["campaign_id = ?"];
  const rowParams = [campaignId];
  if (paging.stage) {
    const stages =
      paging.stage === "pendingDraft" && paging.subTab
        ? PENDING_DRAFT_SUBTAB_STAGES[paging.subTab]
        : EXECUTION_STATUS_STAGE_MAP[paging.stage];
    if (paging.stage === "contacted") {
      rowWhere.push(
        `(stage IN (${stages.map(() => "?").join(",")}) OR stage IS NULL OR stage = '')`
      );
    } else {
      rowWhere.push(`stage IN (${stages.map(() => "?").join(",")})`);
    }
    rowParams.push(...stages);
  }
  if (paging.username) {
    rowWhere.push(
      paging.usernameExact
        ? "tiktok_username = ?"
        : "tiktok_username LIKE ?"
    );
    rowParams.push(paging.usernameExact ? paging.username : `%${paging.username}%`);
  }
  const sortColumn = paging.stage === "pendingPrice" ? "updated_at" : "created_at";
  if (paging.cursor) {
    rowWhere.push(`(${sortColumn} < ? OR (${sortColumn} = ? AND id < ?))`);
    rowParams.push(paging.cursor.sortValue, paging.cursor.sortValue, paging.cursor.id);
  }
  const orderSql = `ORDER BY ${sortColumn} DESC, id DESC`;
  const queryLimit = paging.limit != null ? paging.limit + 1 : null;
  const limitSql = queryLimit != null ? `LIMIT ${queryLimit}` : "";

  const execRowsPromise = queryTikTok(
    `
    SELECT *
    FROM tiktok_campaign_execution
    WHERE ${rowWhere.join(" AND ")}
    ${orderSql}
    ${limitSql}
    `,
    rowParams
  );
  // 翻页请求（带 cursor）时跳过全表计数：计数在翻页期间不会变化，前端 merge 时会保留
  // 上一页拿到的 totalByStage，这里返回 null 即可，避免每次滚动重复扫表。
  const withCounts = !paging.cursor;
  const countsPromise = withCounts
    ? loadExecutionCounts(campaignId)
    : Promise.resolve(null);
  const [execRows, cachedCounts] = await Promise.all([
    execRowsPromise,
    countsPromise,
  ]);

  const analyzedCount = cachedCounts?.analyzedCount ?? 0;
  const totalByStage = cachedCounts?.totalByStage ?? null;
  const hasMore = paging.limit != null && execRows.length > paging.limit;
  const pageRows = hasMore ? execRows.slice(0, paging.limit) : execRows;
  const fetchedUsernames = (pageRows || [])
    .map((row) => row?.tiktok_username)
    .filter(Boolean);
  const lastInboundReplyByUsername = await loadLastInboundReplyAtByUsername(
    campaignId,
    paging.limit != null ? fetchedUsernames : null
  );
  const bioLanguageByInfluencerId = await loadBioLanguageByInfluencerId(pageRows);
  const specialRequestCounts =
    fetchedUsernames.length > 0
      ? await loadSpecialRequestCountsByUsername(campaignId, fetchedUsernames)
      : {};

  pageRows.forEach((row) => {
    const item = buildExecutionStatusItem(row, lastInboundReplyByUsername);
    if (!item.bioLanguage && item.platformInfluencerId) {
      item.bioLanguage =
        bioLanguageByInfluencerId[String(item.platformInfluencerId)] || null;
    }
    // 佣金没单独谈过的执行行，按 campaign 配置的佣金展示（与审批口径一致）
    if (item.commissionPercent == null) {
      const campaignCommission =
        c.campaignInfo?.commission ?? c.commission ?? null;
      item.commissionPercent =
        campaignCommission == null ? null : Number(campaignCommission);
    }
    // Campaign 默认交付结果：卡片在红人级最新交付结果缺失时展示为「继承 Campaign 默认」
    item.campaignDeliverables =
      normalizeDeliverablesText(c.campaignInfo?.deliverables) || null;
    const quoteCount = Array.isArray(item.quoteNegotiation)
      ? item.quoteNegotiation.length
      : 0;
    const deliverableCount = Array.isArray(item.deliverablesTimeline)
      ? item.deliverablesTimeline.length
      : 0;
    item.communicationCount =
      quoteCount +
      deliverableCount +
      (item.id && specialRequestCounts[item.id]
        ? Number(specialRequestCounts[item.id])
        : 0);
    if (paging.stage === "pendingDraft" && paging.subTab) {
      const columnKey =
        paging.subTab === "script" ? "pendingDraftScript" : "pendingDraftVideo";
      columns[columnKey].push(item);
    } else {
      columns[resolveExecutionColumnFromStage(item.stage)].push(item);
    }
  });

  columns.contacted.sort((a, b) => {
    const ta = a.executionCreatedAt
      ? new Date(a.executionCreatedAt).getTime()
      : 0;
    const tb = b.executionCreatedAt
      ? new Date(b.executionCreatedAt).getTime()
      : 0;
    return tb - ta;
  });

  columns.pendingPrice.sort((a, b) => {
    const ta = a.lastInboundReplyAt
      ? new Date(a.lastInboundReplyAt).getTime()
      : 0;
    const tb = b.lastInboundReplyAt
      ? new Date(b.lastInboundReplyAt).getTime()
      : 0;
    return tb - ta;
  });

  return {
    campaignId: c.id,
    status: c.status,
    influencersPerDay: c.influencersPerDay,
    needSample,
    columns,
    totalByStage,
    page: {
      stage: paging.stage,
      subTab: paging.subTab,
      limit: paging.limit,
      nextCursor:
        hasMore && pageRows.length > 0
          ? encodeExecutionCursor(pageRows[pageRows.length - 1], sortColumn)
          : null,
      nextCursorByStage: Object.fromEntries(
        EXECUTION_STATUS_COLUMN_KEYS.map((key) => [
          key,
          paging.stage === key && hasMore && pageRows.length > 0
            ? encodeExecutionCursor(pageRows[pageRows.length - 1], sortColumn)
            : null,
        ])
      ),
      hasMoreByStage: Object.fromEntries(
        EXECUTION_STATUS_COLUMN_KEYS.map((key) => [
          key,
          paging.stage === key ? hasMore : false,
        ])
      ),
    },
    contactedCount: totalByStage?.contacted ?? 0,
    analyzedCount: Number.isFinite(analyzedCount) ? analyzedCount : 0,
    repliedCount: 0,
    publishedCount: totalByStage?.published ?? 0,
    recentEvents: [],
    lastInboundReplyByUsername,
  };
}

/**
 * 获取单条执行记录（含 last_event）
 */
export async function getExecutionRow(campaignId, influencerId) {
  const rows = await queryTikTok(
    `SELECT * FROM tiktok_campaign_execution WHERE campaign_id = ? AND ${SQL_EXECUTION_CREATOR_MATCH}`,
    [campaignId, ...paramsExecutionCreatorMatch(influencerId)]
  );
  if (!rows || rows.length === 0) return null;
  const r = rows[0];
  const lastEvent = parseJson(r.last_event) || {};
  return { ...r, lastEvent };
}

/**
 * 更新红人执行阶段及 last_event，可选追加 quote_negotiation 并更新 flat_fee / currency
 * @param {string} campaignId
 * @param {string} influencerId
 * @param {{
 *   stage?: string,
 *   lastEvent?: object,
 *   quoteAppend?: {
 *     role?: string,
 *     amount?: number | null,
 *     currency?: string,
 *     reason?: string | null,
 *     type?: string | null,
 *     source?: string,
 *     at?: string,
 *     deliverables?: object | null,
 *     updateFlatFee?: boolean
 *   }
 * }} updates
 */
export async function updateExecutionStage(campaignId, influencerId, updates) {
  const { stage, lastEvent, quoteAppend } = updates;
  if (!stage && !quoteAppend && (lastEvent === undefined || lastEvent === null)) {
    return null;
  }

  const rows = await queryTikTok(
    `
    SELECT last_event, flat_fee, currency, quote_negotiation
    FROM tiktok_campaign_execution
    WHERE campaign_id = ? AND ${SQL_EXECUTION_CREATOR_MATCH}
  `,
    [campaignId, ...paramsExecutionCreatorMatch(influencerId)]
  );
  if (!rows || rows.length === 0) return null;

  const row = rows[0];
  let merged = parseJson(row.last_event) || {};
  if (lastEvent && typeof lastEvent === "object") {
    merged = { ...merged, ...lastEvent };
  }

  let nextFlat = row.flat_fee;
  let nextCurrency = normalizeExecutionCurrency(row.currency);
  let negotiation = parseQuoteNegotiationColumn(row.quote_negotiation);
  let appendedQuote = null;

  if (quoteAppend && typeof quoteAppend === "object") {
    const entryCurrency = normalizeExecutionCurrency(
      quoteAppend.currency ?? nextCurrency
    );
    const amt =
      quoteAppend.amount != null && Number.isFinite(Number(quoteAppend.amount))
        ? Number(quoteAppend.amount)
        : null;
    const entryDeliverables = normalizeDeliverables(quoteAppend.deliverables);
    const entry = {
      role: quoteAppend.role || "advertiser",
      amount: amt,
      currency: entryCurrency,
      ...(entryDeliverables ? { deliverables: entryDeliverables } : {}),
      reason:
        typeof quoteAppend.reason === "string"
          ? quoteAppend.reason.trim() || null
          : null,
      type: quoteAppend.type || null,
      at: quoteAppend.at || new Date().toISOString(),
      source: quoteAppend.source || "patch_execution",
    };
    appendedQuote = entry;
    negotiation = [...negotiation, entry];
    if (amt != null && quoteAppend.updateFlatFee !== false) {
      nextFlat = amt;
      nextCurrency = entryCurrency;
    }
  }

  const setClauses = [];
  const values = [];
  if (stage) {
    setClauses.push("stage = ?");
    values.push(stage);
  }
  if (lastEvent != null && typeof lastEvent === "object") {
    setClauses.push("last_event = ?");
    values.push(JSON.stringify(merged));
  }
  if (quoteAppend && quoteAppend.updateFlatFee !== false) {
    setClauses.push("flat_fee = ?");
    values.push(nextFlat);
    setClauses.push("currency = ?");
    values.push(nextCurrency);
  }
  if (quoteAppend) {
    setClauses.push("quote_negotiation = ?");
    values.push(JSON.stringify(negotiation));
  }

  // 红人侧沟通条目累计计数：未读提示基于它做纯 SQL 判定，避免每分钟解析 last_event JSON。
  const entryDelta = influencerEntryDelta(
    { quoteNegotiation: row.quote_negotiation, lastEvent: row.last_event },
    { quoteNegotiation: negotiation, lastEvent: merged }
  );
  if (entryDelta.eventDelta > 0) {
    setClauses.push("infl_event_seq = COALESCE(infl_event_seq, 0) + ?");
    values.push(entryDelta.eventDelta);
  }
  if (entryDelta.cardDelta > 0) {
    setClauses.push("infl_card_seq = COALESCE(infl_card_seq, 0) + ?");
    values.push(entryDelta.cardDelta);
  }

  if (setClauses.length === 0) return null;
  values.push(campaignId, ...paramsExecutionCreatorMatch(influencerId));
  await queryTikTok(
    `UPDATE tiktok_campaign_execution SET ${setClauses.join(", ")} WHERE campaign_id = ? AND ${SQL_EXECUTION_CREATOR_MATCH}`,
    values
  );
  // 阶段/报价变更会影响各 Tab 计数，清掉缓存让下一次请求立即拿到新数字。
  invalidateExecutionCounts(campaignId);
  return {
    quoteEntry: appendedQuote,
    flatFeeUsd:
      nextFlat != null && Number.isFinite(Number(nextFlat))
        ? Number(nextFlat)
        : null,
    currency: nextCurrency,
  };
}

/**
 * 通过 session_id 软删除已发布 Campaign（不可恢复）
 * 仅更新 tiktok_campaign，不物理删除明细数据。
 * 同一 session 若历史上有多条关联行，会一并标记为 deleted。
 */
/**
 * 从执行行读取与主档一致的平台 influencer_id（TikTok userId）。
 * @param {string} campaignId
 * @param {string} tiktokUsername handle，无 @
 * @returns {Promise<string|null>}
 */
export async function getExecutionPlatformInfluencerId(campaignId, tiktokUsername) {
  if (!campaignId || tiktokUsername == null) return null;
  const h = String(tiktokUsername).replace(/^@/, "").trim();
  if (!h) return null;
  const rows = await queryTikTok(
    `
    SELECT influencer_id
    FROM tiktok_campaign_execution
    WHERE campaign_id = ? AND tiktok_username = ?
    LIMIT 1
  `,
    [campaignId, h]
  );
  const v = rows?.[0]?.influencer_id;
  return v != null && String(v).trim() !== "" ? String(v).trim() : null;
}

export async function softDeleteCampaignBySessionId(
  sessionId,
  { deletedBy = "user", deleteReason = "用户删除已发布 campaign" } = {}
) {
  if (!sessionId) {
    return { success: false, message: "sessionId 不能为空" };
  }

  const rows = await queryTikTok(
    `SELECT id, status FROM tiktok_campaign WHERE session_id = ?`,
    [sessionId]
  );
  if (!rows || rows.length === 0) {
    return { success: false, message: "未找到关联的已发布 campaign" };
  }

  const active = rows.filter((r) => r.status !== "deleted");
  if (active.length === 0) {
    return { success: true, campaignId: rows[0].id, message: "campaign 已是删除状态" };
  }

  await queryTikTok(
    `UPDATE tiktok_campaign
     SET status = 'deleted',
         deleted_at = NOW(),
         deleted_by = ?,
         delete_reason = ?,
         updated_at = NOW()
     WHERE session_id = ? AND status <> 'deleted'`,
    [deletedBy, deleteReason, sessionId]
  );

  return { success: true, campaignId: active[0].id, message: "campaign 已软删除" };
}
