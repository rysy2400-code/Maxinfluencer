...
/**
 * 构建入库用的 match_analysis JSON（长文）；短摘要仍用 analysis_summary。
 * @param {object} inf
 * @param {{ taskId?: number|null, runId?: string|null, searchKeyword?: string|null }} taskMeta
 */
function buildMatchAnalysisJson(inf, taskMeta = {}) {
  const analysisLong =
...
/**
 * 批量插入候选红人（INSERT IGNORE：同一 campaign+influencer 已存在则整行不写入、不覆盖）
...
export async function upsertCandidatesForCampaign(campaignId, influencers, taskMeta = {}) {
  ...
  for (const inf of influencers) {
    ...
    const matchAnalysis = buildMatchAnalysisJson(inf, taskMeta);
