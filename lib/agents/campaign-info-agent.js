...
6. 单位红人报价策略（必填，有默认值）：默认「按 eCPM=$3 给红人报价，单人最高 $1000」；用户也可选「无固定费用，仅佣金」，或自定义 eCPM / 单人上限
...
6. influencerPricing（单位红人报价策略）：
   - mode: "ecpm_with_cap"（按 eCPM 报固定费+佣金，默认）或 "commission_only"（无固定费，仅佣金）
...
只返回 JSON 格式：
{
  ...
  formatCampaignInfo(info) {
    ...
    parts.push(
      `**单位红人报价策略**: ${formatInfluencerPricingLabel(info.influencerPricing)}`
    );
