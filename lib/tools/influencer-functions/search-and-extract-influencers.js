...
export async function searchInfluencersByKeyword(params = {}, options = {}) {
  ...
  if (USE_HEADLESS) {
    ...
    // 为了避免其它 Tab 干扰：先用现有页面做登录态快速检查，然后专门创建新页面进行采集
    const pages = context.pages();
    if (pages.length > 0) {
      const existingPage = pages[0];
      page = existingPage;
      console.log(`[searchInfluencersByKeyword] ✅ 使用现有页面（用于登录检测）`);
      try {
        const currentUrl = page.url();
        if (currentUrl && currentUrl !== 'about:blank') {
          const loggedIn = await checkTikTokLoginStatus(page);
          if (!loggedIn) {
            throw new Error('检测到 TikTok 未登录（请在手动启动的 Chrome 中先登录 TikTok）');
          }
        } else {
          console.warn(`[searchInfluencersByKeyword] ⚠️ 现有页面 URL 为 about:blank，跳过登录检测`);
        }
      } catch (e) {
        throw new Error(`TikTok 登录检测失败：${e.message}`);
      }
    } else {
      console.warn(`[searchInfluencersByKeyword] ⚠️ 当前 context 无现有页面，跳过登录检测`);
    }
