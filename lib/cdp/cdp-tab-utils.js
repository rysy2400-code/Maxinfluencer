...
export async function alignCdpPersistentPlatformTabs(context, meta = {}) {
  ...
  const disabled = String(process.env.CDP_ALIGN_DISABLE || "")
    .trim()
    .toLowerCase();
  if (disabled === "1" || disabled === "true" || disabled === "yes") {
    const kept = (await listCdpPageTargets(cdp9222Endpoint())).length;
    console.log(
      `${logPrefix} skipped kept=${kept}` +
        (meta.platform ? ` taskPlatform=${meta.platform}` : "") +
        (meta.phase ? ` phase=${meta.phase}` : "")
    );
    return { closed: 0, kept, aligned: [] };
  }
