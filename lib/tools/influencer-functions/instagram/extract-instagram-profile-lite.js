...
import {
  ...
  isIgPageUnhealthy,
  resetIgProfileCircuits,
  resolveIgUserPkByUsername,
...
/** 用 clips/其他响应里的 owner user 节点回填缺失的 bio/邮箱/头像/粉丝（只填空缺字段） */
function mergeIgUserBioBackfill(userInfo, userNode) {
  if (!userInfo || !userNode) return userInfo;
  const backfill = mapIgUserToUserInfo(userNode);
  if (!backfill) return userInfo;
  const merged = { ...userInfo };
  if (!merged.bio && backfill.bio) merged.bio = backfill.bio;
  if (!merged.email && backfill.email) merged.email = backfill.email;
  if (!merged.displayName && backfill.displayName) merged.displayName = backfill.displayName;
  if (!merged.avatarUrl && backfill.avatarUrl) merged.avatarUrl = backfill.avatarUrl;
  if (!merged.verified && backfill.verified) merged.verified = backfill.verified;
  if (!merged.followers?.count && backfill.followers?.count) merged.followers = backfill.followers;
  if (!merged.following?.count && backfill.following?.count) merged.following = backfill.following;
  if (!merged.postsCount?.count && backfill.postsCount?.count) merged.postsCount = backfill.postsCount;
  if (!merged.userId && backfill.userId) merged.userId = backfill.userId;
  return merged;
}

// 双资料端点（web_profile_info + users/info）全挂时的早探 clips 预算：
// 任务内限量（默认 10），避免对已封锁账户连击 clips 加重风控。
let igEarlyClipsGen = 0;
const igEarlyClipsBudget = new WeakMap();

export function resetInstagramLiteProfileCircuits() {
  igEarlyClipsGen += 1;
  resetIgProfileCircuits();
}

function tryTakeIgEarlyClipsBudget(page) {
  const max = Math.max(Number(process.env.IG_LITE_EARLY_CLIPS_MAX || 10), 1);
  let st = igEarlyClipsBudget.get(page);
  if (!st || st.gen !== igEarlyClipsGen) {
    st = { gen: igEarlyClipsGen, left: max };
    igEarlyClipsBudget.set(page, st);
  }
...
export async function extractInstagramProfileLite(page, username, options = {}) {
  ...
  // API-only username -> userId 兜底：web_profile_info 缺失且名单导入无 userId 时，
  // 用 topsearch（GET，不导航页面）解析精确 username 的 pk，再走 users/info/clips。
  if (!profileJson && !userId) {
    try {
      const lookup = await resolveIgUserPkByUsername(page, handle);
      if (lookup?.pk) {
        userId = lookup.pk;
        countryCtx = { ...countryCtx, userId };
        console.log(
          `[extractInstagramProfileLite] @${handle} web_profile_info 缺失，topsearch 解析 userId=${lookup.pk}（API-only）`
        );
      }
    } catch (resolveErr) {
      console.warn(
        `[extractInstagramProfileLite] @${handle} topsearch userId 解析失败: ${resolveErr?.message || resolveErr}`
      );
    }
  }
  ...
  // 兜底 2：web_profile_info 与 users/info 都没拿到资料时，
  // 用 clips 首屏 owner user 节点回填 bio/邮箱（任务内限量，避免连击已封锁端点）。
  let earlyClipBatches = [];
  if (
    !profileJson &&
    !hasInstagramProfileEmail(userInfo) &&
    userId &&
    tryTakeIgEarlyClipsBudget(page)
  ) {
    try {
      const one = await fetchUserClipsPage(page, String(userId), {
        maxPages: 1,
        pageSize: 24,
        username: handle,
      });
      if (one?.length) {
        earlyClipBatches = one;
        for (const batch of one) {
          const userNode = extractUserNodesFromJson(batch, handle);
          if (userNode) userInfo = mergeIgUserBioBackfill(userInfo, userNode);
        }
        if (hasInstagramProfileEmail(userInfo)) {
          console.log(
            `[extractInstagramProfileLite] @${handle} 通过 clips owner 回填到邮箱（email=true followers=${userInfo.followers?.count ?? "?"}）`
          );
        }
      }
    } catch (earlyErr) {
      console.warn(
        `[extractInstagramProfileLite] @${handle} clips 回填 bio 失败: ${earlyErr?.message || earlyErr}`
      );
    }
  }
  ...
  // 复用邮箱门槛前的早探 clips 批次，避免重复请求
  const clipBatches = [...earlyClipBatches];

  // 先把早探批次并入 videoMap，已够 maxReels 就不再重复拉取
  for (const json of earlyClipBatches) {
  ...
  if (userId && handle && videoMap.size < maxReels) {
