 ...
 * @returns {Object} 用户信息
 */
export function extractUserInfoFromAPI(apiData) {
  const userInfo = {
    username: null,
    displayName: null,
    avatarUrl: null,
  ...
  // 从 bio 中提取 email
  if (userInfo.bio) {
    const email = extractEmailFromBio(userInfo.bio);
...
export function extractUserInfoFromUserDetailAPI(apiData) {
  const user =
    apiData?.userInfo?.user ||
    apiData?.userInfo ||
    apiData?.user ||
    null;
  if (!user) return null;

  const stats = user.stats || user.statsV2 || apiData?.userInfo?.stats || {};
  const userInfo = {
    username: user.uniqueId || user.unique_id || null,
    displayName: user.nickname || user.nickName || null,
    avatarUrl: user.avatarMedium || user.avatarLarger || user.avatarThumb || null,
    bio: user.signature || user.bio || null,
    email: null,
    followers: stats.followerCount
      ? { count: parseInt(stats.followerCount) || 0, display: formatNumber(stats.followerCount) }
      : null,
    following: stats.followingCount
      ? { count: parseInt(stats.followingCount) || 0, display: formatNumber(stats.followingCount) }
      : null,
    ...
    postsCount: stats.videoCount
      ? { count: parseInt(stats.videoCount) || 0, display: formatNumber(stats.videoCount) }
      : null,
 ...
 * @returns {Array} 视频列表
 */
export function extractVideosFromAPI(apiData, username) {
  const videos = [];
