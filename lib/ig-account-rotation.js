...
export async function checkIgAccountLoginState(endpoint) {
  ...
  try {
    ...
    try {
      ...
      igUrl = pages.map((p) => p.url()).find((u) => /instagram\.com/i.test(u || "")) || null;
      if (!igUrl && pages.length) {
        const chromeErrorPage = pages.find((p) => /chrome-error/i.test(p.url() || ""));
        if (chromeErrorPage) igUrl = chromeErrorPage.url();
      }
    } catch {
      /* ignore */
    }
    result.pageUrl = igUrl;

    const u = String(igUrl || "");
    if (/accounts\/suspended/i.test(u)) {
      result.reason = "suspended";
    } else if (/chrome-error/i.test(u)) {
      result.reason = "chrome_error";
    } else if (/auth_platform|checkpoint|challenge/i.test(u)) {
      result.reason = "checkpoint";
    } else if (/accounts\/login/i.test(u)) {
