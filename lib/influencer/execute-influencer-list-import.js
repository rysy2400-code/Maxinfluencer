...
import { candidateHandleKey } from "../db/campaign-candidates-dao.js";
...
/**
 * 在删除 PENDING 上传附件前，把原始字节落一份 IMP-* 审计副本。
 * 聊天历史里的下载链接仍指向 PENDING storageKey；副本存在时，下载接口
 * 可按 source_file_storage_key 反查并读取该副本（或从 payload 重建）。
 * @returns {string|null} 审计副本 storageKey；保存失败返回 null（此时不删原文件）
 */
function persistDurableImportCopy({ sessionId, importBatchId, buffer, fileName }) {
  if (!sessionId || !importBatchId || !buffer?.length) return null;
  try {
    const saved = saveSessionImportFile(sessionId, importBatchId, buffer, fileName);
    return saved?.storageKey || null;
  } catch (e) {
    console.warn(
      "[execute-influencer-list-import] 保存审计副本失败，保留 PENDING 附件:",
      e?.message || e
    );
    return null;
  }
...
async function loadExistingCandidateKeys(campaignId) {
  ...
  // 去重键 = platform|handle：同一 handle 在别的平台不算重复，否则会被误跳过
  return new Set(
    (rows || [])
      .map((r) => {
...
export async function executeInfluencerListImport(input) {
  ...
  if (attachmentPlan?.storageKey) {
    ...
    const autoErrors = (autoParsed.parseErrors || []).map((e) => ({
      ...e,
      source: "attachment",
    }));
    // 自动解析更优时接管；若两套都解析不到，也要带回自动解析的错误，
    // 避免“未能识别任何有效红人”后面不显示任何具体原因。
    const shouldUseAutoErrors =
      autoParsed.rows.length === 0 &&
      parsed.rows.length === 0 &&
      autoErrors.length > 0;
    if (
      autoParsed.rows.length > parsed.rows.length ||
      shouldUseAutoErrors
    ) {
      parsed = {
        rows: autoParsed.rows,
        parseErrors: shouldUseAutoErrors
          ? [...(parsed.parseErrors || []), ...autoErrors]
          : autoErrors,
      };
    }
  ...
  for (const row of merged) {
    ...
    if (!handle) continue;
    const handleKey = candidateHandleKey(row.platformSlug, handle);
    if (!forceContact && !exclusionMode && handleKey && existing.has(handleKey)) {
      skippedDuplicate += 1;
      continue;
    }
    toProcess.push({
  ...
  if (exclusionMode) {
    ...
    if (attachmentPlan?.storageKey) {
      const durableStorageKey = persistDurableImportCopy({
        sessionId,
        importBatchId,
        buffer: attachmentBuffer,
        fileName,
      });
      // 只有审计副本落盘成功才清理 PENDING，避免聊天附件下载链接失效
      if (durableStorageKey) {
        try {
  ...
  // 先落 IMP-* 审计副本，仅副本成功后才删 PENDING（保证聊天下载可用）
  if (attachmentPlan?.storageKey) {
    const durableStorageKey = persistDurableImportCopy({
      sessionId,
      importBatchId,
