          ...
          return {
            ...
            toolCall: {
              ...
              params: {
                ...
                contactMode: importTurn.contactMode || "default",
              },
            },
          };
        }
...
10. 若用户**仅询问当前 Campaign 配置**（交付结果、红人要求、预算、平台、关键词策略等，含「是什么」「目前多少」「查看」且非修改意图）→ needTool=false；回答时引用【当前 Campaign 配置快照】，只答所问项；快照为「未设置」则答未设置，**禁止**声称库中为空若快照有值。
11. 寒暄、问汇报选项说明书、问「有哪些 Campaign 状态可以修改」等说明类问题 → needTool=false；说明当前状态时必须引用【当前 Campaign 数据库状态】与配置快照，不得声称已是名单模式除非 status=running_passive。
...
说明：用户仅问进度、或同时问配置+进度时，必须调用此工具；纯配置询问（如「交付结果是什么？」）则 needTool=false。
