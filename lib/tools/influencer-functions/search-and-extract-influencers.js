    ...
    const run = async () => {
      ...
      if (
        ...
        try {
          ...
          if (affiliateResult.gmv != null || affiliateResult.unitsSold != null) {
            console.log(
...
async function searchAndExtractInfluencers_v2(params = {}, options = {}) {
  ...
  if (enrichBatchPolicy) {
    ...
    while (poolCursor < recordsToEnrich.length) {
      ...
      sendStep(
        "enrich 批次",
        `第 ${batchIndex} 批：enrich+分析 ${batchRecords.length} 位红人（每批 ${enrichBatchSize} 人，≥1 符合则继续）`
      );
      ...
      if (enrichBatchStopOnZeroMatch && batchRecommended === 0) {
        sendStep(
          "批次停止",
          `本批 ${batchRecords.length} 人 analyze 后 0 人符合要求，结束本关键词 enrich`
        );
