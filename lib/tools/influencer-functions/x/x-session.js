...
async function recoverXAccountLogin(context, page) {
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (authToken && ct0) {
        console.log(`[x-session] @${rows[0]?.x_username || "?"} auth_token 丢失，已自动重注入恢复（port=${port} attempt=${attempt + 1}）`);
        return { authToken, ct0 };
      }
      console.log(`[x-session] 重注入后 ct0/auth_token 未就绪（attempt=${attempt + 1} auth=${!!authToken} ct0=${!!ct0}），重试`);
    }
  } catch (e) {
