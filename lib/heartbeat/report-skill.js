import { countPendingPriceReviewItems } from "../execution/pending-price-items.js";
 ...
 * @property {Array} columns.contacted
...
export function generateExecutionReport({
  ...
  const contacted = cols.contacted || [];
  const pendingPrice = cols.pendingPrice || [];
  const pendingPriceReviewCount = countPendingPriceReviewItems(pendingPrice);
  ...
  const metricValues = {
    ...
    pending_price_count: pendingPriceReviewCount,
  ...
  if (Array.isArray(includeMetrics) && includeMetrics.length) {
    ...
    if (executionStatus?.needSample !== false) {
      base.splice(2, 0, "pending_sample_count");
    }
    metricsToShow = base;
  }
  ...
  const metricLines = metricsToShow.map((key) => {
    ...
    if (key === "contacted_count") return `- 已联系：${value} 位红人`;
  ...
  lines.push("");
  lines.push("【阶段分布】");
  lines.push(`- 已联系：${contacted.length} 位红人`);
  lines.push(`- 待审核价格：${pendingPriceReviewCount} 位红人`);
  if (executionStatus?.needSample !== false) {
