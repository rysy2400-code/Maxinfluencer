...
async function reportSearchTaskCountrySkip(taskId, reason) {
  ...
  try {
    ...
    } else if (reason === "country_mismatch") {
    ...
    const run = async () => {
      ...
      if (recordIsInstagram && liteMode) {
        ...
        if (!igLitePage) {
          ...
          sendStep(
            "提取数据",
            `[${progress}] Instagram Lite Reels 单入口 @${record.username}...`
          );
          try {
            ...
            if (profileData.skippedReason === "country_unknown") {
              sendStep(
                "国家预筛",
                `[${progress}] ⏭️ @${record.username} 未获取国家，跳过 Reels 拉取`
              );
              await reportSearchTaskCountrySkip(taskId, "country_unknown");
              ...
              await reportSearchTaskCountrySkip(taskId, "country_mismatch");
            } else if (
