...
async function getOutreachAnchor(campaignId, influencerId) {
  if (!campaignId || !influencerId) return null;
  const rows = await queryTikTok(
    `
    SELECT
      from_email,
      ...
      subject,
      message_id,
      COALESCE(sent_at, created_at) AS anchor_time
    FROM tiktok_influencer_conversation_messages
    WHERE
      campaign_id = ?
      AND influencer_id = ?
...
export async function sendOutreach({ campaignId, influencerId, snapshot }) {
  ...
  const anchor = await getOutreachAnchor(campaignId, influencerId);

  let fromAccount = null;
  if (anchor?.fromEmail) {
    fromAccount = await getOutboundAccountByEmail(anchor.fromEmail);
  }
  if (!fromAccount) {
    fromAccount = await pickRandomOutboundAccount();
  }
  if (!fromAccount) {
    throw new Error("[InfluencerAgent] 没有可用企业邮箱账号可用于 sendOutreach");
  }

  const subject = anchor?.subject || buildOutreachSubject(campaign, influencer);
  ...
  if (anchor?.messageId) {
    headers["In-Reply-To"] = anchor.messageId;
    headers["References"] = anchor.messageId;
  }
