...
回复要专业、清晰，用中文与客户沟通。`;
  ...
  /**
   * 用户是否仅在「确认当前脚本 / 进入下一阶段」，没有修改脚本或分镜的诉求。
   * step_4 时每轮都会先进入本 Agent；若误判为「要改脚本」会整轮跑 DeepSeek×2 + 文生图，耗时数分钟且界面一直显示「生成内容脚本」。
   */
  looksLikePureContentStageConfirmation(message) {
    const t = (message || "").trim();
    if (!t || t.length > 200) return false;
    const editOrRefine =
      /场景\s*\d|场景[一二三四五六七八九十]|第\s*[一二三四五六七八九十\d]+\s*[个幕镜]|画面【|台词【|要点[:：]|分镜|改|改为|修改|调整|换成|重写|删除|不要|删掉|加上|补充|换成/i;
    if (editOrRefine.test(t)) return false;
    const advance =
      /下一阶段|下一步|确认无误|确认|继续|^行$|^好的$|没问题|可以|就这样|无误|OK|\bok\b/i;
    return advance.test(t);
  }

  normalizeHashtags(raw, fallback = "") {
    const src = (raw || "").trim();
    ...
    if (limited.length < 3 && fallback) {
      const fallbackTokens = fallback.split(/\s+/).filter(Boolean);
  ...
  extractScenesFromScript(scriptText) {
    return (scriptText || "")
      .split("\n")
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
  ...
  async generateUnifiedHashtags({
    hasPreviousScript,
    previousContentScript,
    lastUserMessage,
    productInfo,
    platformText,
    regionText,
    ...
    try {
      const previousHashtags = (previousContentScript?.hashtags || "").trim();
      const tagPrompt = hasPreviousScript
        ? `你是短视频标签编辑专家。请基于「上一版标签」和「用户最新修改建议」对标签做最小必要修改，并保持与 Campaign 目标一致。
...
上一版 hashtags：
${previousHashtags || "(none)"}

要求：
1. 输出 3-5 个标签；
2. 每个标签以 # 开头；
3. 标签之间用单个空格分隔；
4. 若用户没有明确要求改标签，尽量保留上一版标签，只做必要微调。

只输出一行标签字符串，不要任何解释。`
        : `基于下面的产品和 Campaign 信息，生成一行适合短视频脚本使用的「视频标签」，总共 3-5 个标签，用 # 开头，标签之间用空格分隔。
...
只输出这一行标签字符串，不要任何解释。`;
      ...
      return this.normalizeHashtags((tagResp || "").split("\n")[0].trim(), "");
    } catch (e) {
      console.warn("[ContentRequirementAgent] 生成视频标签失败，使用兜底规则:", e);
      const categoryTag = productInfo.productType || productInfo.category || "";
      const fallback = hasPreviousScript && previousContentScript?.hashtags
        ? previousContentScript.hashtags
        : [
            ...
            .filter(Boolean)
            .join(" ");
      return this.normalizeHashtags(fallback, "");
    }
  }
  ...
  async generateContent(messages, context = {}, onProgress = null) {
    ...
    try {
      ...
      // 已有成稿且用户仅表示确认/进入下一阶段：不要重新跑脚本 LLM + 文生图（否则 ToAPIs 轮询常需数分钟，体感「卡住」）
      if (
        previousContentScript &&
        previousContentScript.script &&
        this.looksLikePureContentStageConfirmation(lastUserMessage)
      ) {
        console.log("[ContentRequirementAgent] 纯确认进入下一阶段，跳过脚本与可视化重新生成", {
          lastUserPreview: lastUserMessage.slice(0, 80),
        });
        onProgress?.({ phase: "confirm_skip_regen" });
        return {
          reply:
            "内容脚本已确认。正在为你汇总 Campaign 信息，请稍候。",
          contentScript: { ...previousContentScript },
          video: null,
          isConfirmed: true,
        };
      }
...
时长建议：[时长建议]`;
      ...
      const hasPreviousScript = !!(previousContentScript && previousContentScript.script);
      const hashtags = await this.generateUnifiedHashtags({
        hasPreviousScript,
        previousContentScript,
        lastUserMessage,
        productInfo,
      ...
      // canonical contentScript：文本脚本与图片脚本都只读这份核心数据
      const contentScript = {
        brandName: productInfo.brandName || "",
        productName: productInfo.productName || "",
        title,
        script,
        scenes: this.extractScenesFromScript(script),
        keyPoints,
        duration,
        hashtags,
      ...
      console.log("[ContentRequirementAgent] canonical hashtags:", contentScript.hashtags);

      // 基于整段文字脚本生成「一张」可视化脚本示意图（包含多个场景）
      // 这张图用于给红人/创作者看，帮助理解全片结构和每个镜头要拍什么
      let visualImage = null;
      try {
        const previousVisualPrompt = (previousContentScript?.visualImage?.prompt || "").trim();
...
Product name: ${contentScript.productName || "unknown"}
Platform: ${platformText} (e.g. TikTok means vertical, fast-paced, authentic; Instagram means high-quality aesthetic reels)
Region: ${regionText}
Current hashtags (MUST be used exactly in header Tags line): ${contentScript.hashtags || ""}
User latest revision request: ${lastUserMessage || "none"}
Previous visual prompt for incremental update (if exists, keep style/layout continuity and only change what the user asked): ${previousVisualPrompt || "none"}
...
1) Header area:
   ...
   - Below that, a line labeled "Tags:" that MUST copy the hashtags VERBATIM from "Current hashtags" above.
     CRITICAL RULES: copy exactly / verbatim; do NOT modify, re-order, remove, translate, merge, or invent hashtags; do NOT use any example/default hashtags.
     The "Tags:" line in your returned prompt must be exactly: "Tags: ${contentScript.hashtags || ""}".
...
- Make sure the product ${contentScript.productName || "the product"} is clearly visible in all relevant frames where the script talks about the product or results, with appearance and colors consistent with the original product photos on the e-commerce page.
...
Constraints:
- The returned prompt MUST be at most 1000 characters (including spaces). If you are close to the limit, shorten less-critical wording first, but never drop the exact Tags line requirement.
...
Return ONLY the prompt text, no extra explanation.`;
        ...
        const finalImagePrompt =
          rawPrompt.length > 1000 ? rawPrompt.slice(0, 1000) : rawPrompt;
        const finalPromptTagsLine = (finalImagePrompt.match(/Tags:\s*[^\.\n]+/i)?.[0] || "").trim();
        ...
        console.log("[ContentRequirementAgent] final prompt tags line:", finalPromptTagsLine || "(not found)");
      ...
      contentScript.visualImage = visualImage;
      ...
      const brandLine = `${contentScript.brandName || ""}${contentScript.productName ? ` · ${contentScript.productName}` : ""}`.trim() || (contentScript.productName || "本次 Campaign");

      // 将脚本按行拆成「场景」方便红人逐段阅读（场景数量由脚本本身决定）
      const scriptScenes = contentScript.scenes;

      let reply = `我已经为你生成了内容脚本要求：

**品牌与产品**: ${brandLine}
**视频文案**: ${contentScript.title}
**视频标签**: ${contentScript.hashtags || ""}
