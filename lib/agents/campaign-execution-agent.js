...
- 当用户修改发布时的 Campaign / 产品信息（品牌、产品名、投放平台、投放地区、发布时间段、预算、佣金）时，必须调用 modify_campaign，**params.scope 必须为 whole**，在 changes 中只填用户明确要改的字段：brandName、productName、platform、region、publishTimeRange、budget（数字）、commission（数字，如 0 表示 0%）。例如「品牌改为 Anker」「投放平台改成 Instagram」「国家改为美国」。
