...
export function shouldRunImportIntentExtractor(text) {
  ...
  if (!t) return false;
  // 执行节奏 / 汇报 / Campaign 配置修改 — 不是红人名单导入
  if (/每天(?:只)?联系\s*\d+/.test(t)) return false;
  if (/每\s*\d+\s*天汇报|汇报时间|汇报形式|汇报频率|日报里/.test(t)) return false;
  if (
    /品牌改为|产品名改为|产品改为|投放平台|投放地区|总预算|佣金改为|关键词策略|粉丝量要求|播放量要求|帐号类型|ecpm/i.test(
      t
    )
  ) {
    return false;
  ...
  if (/名单|导入|上传|发邮件|邀约|reach out|contact|import|upload/i.test(t)) return true;
  // 「联系」单独出现易与「每天联系 N 位」混淆，需带名单/红人列表等语境
  if (/联系/.test(t) && /名单|列表|这些|以下|导入|上传|excel|链接/.test(t)) return true;
  return false;
}
...
export function hydrateImportPayload(decision, messages, userMessage) {
  ...
  if (out.phase === "import" && out.attachmentPlan?.storageKey) {
    // 文本优先：当前轮用户消息里含真实主页链接时保留 textItems（避免历史 PENDING 附件劫持文本名单）；
    // 仅当本轮无链接（纯确认/引用附件）时走附件解析。
    const currentHasLinks = PROFILE_URL_IN_TEXT.test(String(userMessage || ""));
    if (!currentHasLinks) out.textItems = [];
  }
