...
export async function generateOutreachBodyWithLLM({
  ...
  const pricingRules = (() => {
    ...
    if (mode === PRICING_MODE_ASK_CREATOR_QUOTE) {
      ...
      lines.push(
        `- 英文表述要求：在介绍产品与交付范围后，自然、礼貌地请创作者提供其合作报价（rate / fee for these deliverables），例如 "Could you share your rate for this collaboration?" 或 "Please let us know your fee based on the deliverables below."；不要主动给出任何建议价格。`
      );
      ...
      if (
        ...
        lines.push(
          `- 英文表述要求：说明 **${Number(pct)}% commission**（或 affiliate program），不要出现具体固定费美元数字。`
        );
      }
    } else {
      lines.push(
      ...
      lines.push(
        `- 若 quotedFlatFeeUsd 有数值：正文用英文说明该固定费与（若有）佣金；**禁止**自造其他美元数字。`
      );
      lines.push(
  ...
  const deliverablesRules = deliverablesEn
    ? (() => {
        ...
        return [
          isProductExchange
            ? `- **Deliverables（交付结果）**：正文必须用自然英文说明本次赠送产品（gifted product）是针对以下 Deliverables 的回报；**不要**写 fixed fee / commission 或任何金额。`
            : isAskQuote
              ? `- **Deliverables（交付结果）**：正文用自然英文列出以下交付要求，并请创作者**根据这些交付要求提供合作报价**；不要写 fixed fee / commission 或任何金额。`
              : `- **Deliverables（交付结果）**：正文必须用自然英文说明本次 fixed fee / commission 是针对以下 Deliverables 的报价。`,
          ...
          isDefaultEn
            ...
            : `- 交付内容：从上述全文筛选 influencer.platform 对应条目后，译成自然英文写出（不得遗漏该段条目，不得加入其他平台条目）。原文参考：${deliverablesEn}`,
  ...
  const countryQuestionRule = askCountry
    ? `- **国家补全（必需）**：当前红人国家未知。请在邮件结尾附近自然询问对方目前常驻哪个国家，例如 "Also, could you let us know which country you're currently based in? It helps us coordinate campaign details and shipping if needed." 不要把它写成表单，不要影响主体邀约。`
    : `- 当前已有国家/地区信息，不要额外询问对方所在国家。`;
