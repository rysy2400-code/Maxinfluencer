...
/**
 * 合同用的佣金口径：同意时刻的快照 → 执行行 → campaign 配置。
 * 与审批、卡片共用同一条款优先级，避免合同和扣款口径不一致。
 */
function resolveContractCommissionPercent({ approvedTerms, execution, campaign }) {
  const candidates = [
    approvedTerms?.commissionPercent,
    execution?.commission_percent,
    execution?.commissionPercent,
    campaign?.commissionPercent,
    campaign?.campaignInfo?.commission,
  ];
...
export async function generateContractForExecution({
  ...
  // 条款快照优先：品牌点「同意」那一刻写进 last_event.approvedTerms 的固定费与佣金
  const approvedTerms = execution.lastEvent?.approvedTerms || null;
  const feeAmountRaw =
    approvedSnapshot?.feeAmount ?? approvedTerms?.fixedFeeUsd ?? execution.flat_fee;
  const feeAmount = feeAmountRaw == null ? null : Number(feeAmountRaw);
  const commissionPercent = resolveContractCommissionPercent({
    approvedTerms,
    execution,
    campaign,
  });
  ...
  // 固定费为 0（纯佣金 / 纯置换）时必须能说清报酬：佣金口径缺失则阻断，避免合同写成「0 元买断」
  if (feeAmount === 0 && commissionPercent == null) {
    throw new Error(
      `固定费为 0 但佣金口径缺失，已阻断合同生成（campaign=${cid}，influencer=${
        creatorHandle || lookupKey
      }）。请先确认本单佣金比例（纯产品置换填 0%）后再重新触发。`
    );
  }
  const currency =
    approvedSnapshot?.currency ||
    approvedTerms?.currency ||
  ...
  const campaignPlatforms = parseCampaignPlatforms(
    campaignInfo.platform ?? campaignInfo.platforms
  );
  // 交付结果的权威来源：审批快照 → 执行级最新交付结果（quote_negotiation）→ Campaign 配置
  const confirmedDeliverables =
    normalizeDeliverables(approvedTerms?.deliverables) ||
    resolveLatestDeliverablesFromRow(execution);
  const confirmedPlatforms = confirmedDeliverables?.platforms?.length
    ? confirmedDeliverables.platforms
    : campaignPlatforms;
