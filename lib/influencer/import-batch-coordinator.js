...
function parseContactedFromSummary(resultSummary) {
  ...
  const m = text.match(/(?:已联系|将联系)[：:]\s*(\d+)/);
...
export function buildImportBatchSummary(rows = []) {
  ...
  for (const r of list) {
    ...
    const recommendedN = anyContactAll
      ? contactedN
      : Number(r.progress_recommended_count || 0) || 0;
    totalRows += n;
    analyzed += analyzedN;
    ...
    if (r.status === "succeeded") {
      lines.push(
        `- ${label}（${n} 位）：已分析 ${analyzedN} · 推荐 ${recommendedN} · ${anyContactAll ? "将联系" : "已联系"} ${contactedN}`
      );
  ...
  const modeNote = anyContactAll ? "（模式：联系符合campaign投放地区的所有红人）" : "";
  const contactLabel = anyContactAll ? "将联系" : "已联系";
  const head =
    failed.length || cancelled.length
      ? `【名单导入完成】共 ${totalRows} 位红人，其中 ${failed.length} 个平台执行失败${modeNote}`
      : `【名单导入完成】共 ${totalRows} 位红人，已分析 ${analyzed} · 推荐 ${recommended} · ${contactLabel} ${contacted}${modeNote}`;
