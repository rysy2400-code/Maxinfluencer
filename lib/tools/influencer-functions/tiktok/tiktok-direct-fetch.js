...
export async function resolveVideoLocationCreated(page, opts = {}) {
  ...
  const secUid = opts.secUid ? String(opts.secUid) : "";
  ...
  if (secUid) {
    try {
      ...
      if (hit?.locationCreated != null && hit.locationCreated !== "") {
        return {
          locationCreated: String(hit.locationCreated),
          source: videoId ? "post_item_list_api" : "post_item_list_api_first",
        };
      }
      for (const v of list) {
        if (v?.locationCreated != null && v.locationCreated !== "") {
          return { locationCreated: String(v.locationCreated), source: "post_item_list_api_scan" };
        }
      }
    } catch {
      /* ignore */
