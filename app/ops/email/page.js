...
export default function EmailOpsPage() {
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setExpanded((current) => current.size ? current : new Set((data.domains || []).map((item) => item.domain)));
  ...
  return <main style={{ maxWidth: 1680, margin: "0 auto", padding: 20, color: C.text }}>
    ...
    <nav style={{ display: "flex", gap: 18, borderBottom: `1px solid ${C.border}` }}><Link href="/ops" style={navLink}>虚拟机运维</Link><span style={activeNav}>邮箱运维</span></nav>
    ...
    <section style={{ padding: "14px 0 10px", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}><input aria-label="搜索邮箱或域名" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索邮箱或域名" style={input}/><select aria-label="邮箱池筛选" value={pool} onChange={(event) => setPool(event.target.value)} style={input}><option value="all">全部邮箱</option><option value="outreach">首邀池</option><option value="other">非首邀池</option></select><span style={{ color: C.muted, fontSize: 11 }}>{domains.reduce((sum, item) => sum + item.mailboxes.length, 0)} 个邮箱</span></section>
