...
export async function warmUpIgRelayTemplateIfNeeded(page) {
  ...
  const maxWait = Math.min(
    Math.max(Number(process.env.IG_LITE_RELAY_WARMUP_MS || 6000), 2000),
    20_000
  );
  const step = 400;

  for (let waited = 0; waited < 2000; waited += step) {
    if (getIgRelayTemplate(page)) return true;
...
export async function fetchKeywordSearchPage(page, keyword, options = {}) {
  ...
  return igApiFetch(page, "/api/v1/fbsearch/web/top_serp/", {
    method: "POST",
