...
import {
  fetchWithPoolRetry,
  requestTiktokPortRestart,
  resolveCdpPortFromPage,
} from "./tiktok-proxy-pool.js";
import { refreshTiktokApiSession } from "./tiktok-api-client.js";

const searchSessionResetAt = new Map(); // endpoint -> last reset ts
...
export async function extractTiktokSearchLite(context, keyword, options = {}) {
  ...
  // 任务级搜索会话重置：9222 匿名指纹用久了会被 TikTok 标记（任何节点都返回空）。
  // 按 endpoint 节流（默认每 1 小时重置一次），避免每次搜索都重建导致 CDP 附着抖动。
  {
    const endpointKey = String(
      page._ttApiSessionKey || page._ttApiEndpoint || "9222"
    ).split("#")[0];
    const resetIntervalMs = Math.max(
      10 * 60 * 1000,
      Number(process.env.TT_LITE_SEARCH_SESSION_RESET_INTERVAL_MS || 60 * 60 * 1000)
    );
    const lastResetAt = searchSessionResetAt.get(endpointKey) || 0;
    if (Date.now() - lastResetAt >= resetIntervalMs) {
      await refreshTiktokApiSession(page, { force: true }).catch(() => false);
      searchSessionResetAt.set(endpointKey, Date.now());
      console.log(`[tt-search] task-level session reset ${endpointKey}`);
    }
  }
  ...
  try {
    ...
    const batches = await fetchWithPoolRetry({
      ...
      maxNodeSwitches: 8,
      // 搜索空多为 9222 匿名会话指纹老化：页内刷新不可靠，切节点前先清会话并请求重启 9222 Chrome
      // （重启节流 10 分钟，给下一次任务一个全新指纹）。
      onBeforeSwitch: async () => {
        await refreshTiktokApiSession(page, { force: true }).catch(() => false);
        await requestTiktokPortRestart(
          resolveCdpPortFromPage(page),
