...
export async function extractTiktokSearchLite(context, keyword, options = {}) {
  ...
  const maxInfluencers = Math.min(
    Math.max(Number(options.maxInfluencers || process.env.TT_SEARCH_MAX_INFLUENCERS || 120), 1),
    2000
  );

  const session = await acquireTiktokApiSession(context, {
    endpointKey: process.env.CDP_ENDPOINT || "http://127.0.0.1:9222",
  });
  const { page } = session;
  ...
  try {
    const batches = await fetchSearchItemFullAll(page, keyword, {
      maxPages: Math.min(Math.max(Number(process.env.TT_LITE_SEARCH_MAX_PAGES || 6), 2), 20),
    });
    ...
    return {
      ...
      stats: {
        ...
        extractionSource: "tiktok_search_item_full_direct",
        extractMode: "lite",
        scrollRounds: 0,
