...
export const CAMPAIGN_EXECUTION_TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    description:
      "修改已发布 campaign 的内容。可针对整体或单个红人修改。整体修改支持：品牌/产品（brandName/productName）、投放与预算（platform/region/publishTimeRange/budget/commission）、单位红人报价策略（influencerPricing 或 pricingMode/pricingEcpmUsd/pricingMaxFlatFeeUsd）、交付结果 deliverables（红人需交付的内容，独立于内容脚本）、红人画像 screeningConditions（与 influencer_profile 对齐：followerRange 粉丝量、viewRange 播放量、videoDuration 视频时长、accountType 帐号类型/内容方向）、内容要求 contentRequirements、关键词策略 keywordStrategy（供搜索关键词生成参考）。报价策略与交付结果变更仅影响尚未发出首封邀约的红人。修改后需同步给红人经纪人 agent。",
    parameters: {
      type: "object",
      properties: {
        campaignId: {
        ...
        changes: {
          ...
          properties: {
            screeningConditions: {
              type: "object",
              description:
                "红人画像字段，写入 tiktok_campaign.influencer_profile：followerRange（粉丝量要求）、viewRange（播放量要求）、videoDuration（视频时长要求，如长视频教程 30-60 分钟）、accountType（帐号类型与内容方向，可为长文本）。可只改其中若干项。",
            },
            publishTimeRange: {
              type: "string",
