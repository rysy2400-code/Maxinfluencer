...
14. 名单导入两轮确认：
    - 若用户**仅提供名单/附件、尚未明确同意导入** → needTool=false，并在 reply 中用自然语言描述看到的名单/附件，同时询问导入方式：「是否导入？按哪种方式处理：1) 只联系符合campaign投放地区和红人画像要求的红人（默认）；2) 联系符合campaign投放地区的所有红人。」；**不要**调用工具。
    - 若用户已同意导入并明确选择模式（如回复「直接联系」「按分析结果来」），或**上一轮 Bin 刚问过导入方式、用户回复确认** → 必须 needTool=true，toolName=import_influencer_list，params.contactMode=recommended_only|contact_all；params.textItems / params.attachmentPlan **可留空**，系统会从最近对话历史与附件自动补全，**不要编造 profileUrl 或 attachmentPlan.storageKey**。
...
第一轮（仅名单，未同意）：
{ "needTool": false, "toolName": null, "params": null, "reply": "我看到你提供了 2 个红人主页链接，是否导入？按哪种方式处理：1) 只联系符合campaign投放地区和红人画像要求的红人（默认）；2) 联系符合campaign投放地区的所有红人。" }
第二轮（用户选择直接联系）：
...
export function hydrateImportParams(
  ...
  if (!["recommended_only", "contact_all"].includes(params.contactMode)) {
    const recentText = (messages || [])
      .slice(-6)
      .map((m) => String(m?.content || ""))
      .join("\n");
    params.contactMode = /直接联系|全部联系|都联系|不筛选|不用筛选|直接导入|跳过筛选/i.test(recentText)
      ? "contact_all"
      : "recommended_only";
  }
  return params;
...
8. 红人名单导入：粘贴红人主页链接、@用户名或 📎 Excel 时，由统一工具调度器按「先确认导入方式再导入」流程处理；**必须**先和用户确认「只联系符合投放地区和画像要求的红人（默认）」还是「联系符合投放地区的所有红人」，即使用户已说直接联系也要追问确认；确认后调用 import_influencer_list 并填 params.contactMode（textItems / attachmentPlan 由系统自动补全，勿编造）。用户只想解读表格/链接时直接解读，不导入。用语自然，勿固定说「已收到附件」。
