...
class CdpSession {
  ...
  async attachLiteResourceBlocker(blockedTypeNames = []) {
    ...
    // 视频/流媒体 URL 模式：resourceType 是 XHR/fetch 的视频分段（m4s/m3u8/mp4 等）
    // 不会被类型拦截命中，按 URL 中止（media 拦截开启时生效）
    const blockMediaUrl = blocked.has("Media");
    const MEDIA_URL_RE =
    ...
    const handler = async (params) => {
      ...
      try {
        ...
        if (
          blocked.has(params.resourceType) ||
          (blockMediaUrl && MEDIA_URL_RE.test(url))
        ) {
...
export async function acquireTiktokCdpPage(cdpEndpoint, options = {}) {
  ...
  if (!target) {
    target = await openCdpTab(endpoint, process.env.TT_LITE_BOOTSTRAP_URL || "https://www.tiktok.com/");
    for (let i = 0; i < 20; i++) {
      await new Promise((r) => setTimeout(r, 1500));
      const list = await listCdpPageTargets(endpoint);
      const t =
