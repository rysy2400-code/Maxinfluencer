...
import { runScrollUntilStuck } from "./scroll-until-stuck.js";

function pickLocationFromSearchItem(item) {
  ...
  if (poi?.regionCode != null && poi.regionCode !== "") {
    ...
    return /^[A-Za-z]{2}$/.test(code) ? code.toUpperCase() : code;
  }
 ...
 * @returns {Array} 视频列表
 */
export function extractVideosFromSearchAPI(apiData) {
  const videos = [];
  ...
  for (const item of itemList) {
    ...
    if (item.locationCreated != null && item.locationCreated !== "") {
      video.locationCreated = String(item.locationCreated);
    } else {
      const locFromItem = pickLocationFromSearchItem(item);
      if (locFromItem) video.locationCreated = locFromItem;
    }
    ...
    if (author) {
      video.creator = {
        ...
        secUid: author.secUid || author.sec_uid || null,
        userId: author.id || author.uid || author.userId || null,
...
/**
 * 从综合搜索 API（search/general/full）提取视频条目：
 * 只取 type===1（视频），type=4 等空卡片丢弃；作者信息进入 creator 供下游红人提取。
 */
export function extractVideosFromGeneralSearchAPI(apiData) {
  const videos = [];
  ...
  for (const entry of data) {
    ...
    const username = author.uniqueId || null;
    if (!username || !item.id) continue;

    const stats = item.stats || item.statsV2 || {};
    const authorStats = item.authorStats || {};
    const playCount = parseInt(stats.playCount || stats.viewCount || 0) || null;
    const likeCount = parseInt(stats.diggCount || stats.likeCount || 0) || null;
    const commentCount = parseInt(stats.commentCount || 0) || null;
    const shareCount = parseInt(stats.shareCount || 0) || null;
    const collectCount = parseInt(stats.collectCount || 0) || null;
    const followerCount = parseInt(authorStats.followerCount || 0) || null;
    ...
    const video = {
      ...
      hashtags: Array.isArray(item.challenges)
        ? item.challenges.map((c) => c.title || c.challengeName).filter(Boolean)
        : [],
      mentions: [],
      views: playCount ? { count: playCount, display: formatNumber(playCount) } : null,
      likes: likeCount ? { count: likeCount, display: formatNumber(likeCount) } : null,
      comments: commentCount ? { count: commentCount, display: formatNumber(commentCount) } : null,
      shares: shareCount ? { count: shareCount, display: formatNumber(shareCount) } : null,
      favorites: collectCount ? { count: collectCount, display: formatNumber(collectCount) } : null,
      thumbnail: item.video?.cover || item.video?.originCover || null,
      music: item.music
        ? { id: item.music.id, title: item.music.title, authorName: item.music.authorName }
        : null,
      creator: author.id
        ? {
            id: author.id,
            ...
            avatarUrl: author.avatarMedium || author.avatarLarger || author.avatarThumb || null,
            avatarThumb: author.avatarThumb || null,
            secUid: author.secUid || null,
            userId: author.id || null,
            signature: author.signature || null,
            verified: author.verified || false,
            followers: followerCount
              ? { count: followerCount, display: formatNumber(followerCount) }
              : null,
          }
        : null,
      postedTime: item.createTime
 ...
 * @returns {Array} 红人列表
 */
export function extractInfluencersFromVideos(videos) {
  ...
  for (const video of videos) {
    ...
    if (influencerMap.has(username)) {
      ...
      if (!existing.secUid && video.creator.secUid) {
        existing.secUid = video.creator.secUid;
        existing.tiktokSecUid = video.creator.secUid;
      }
      if (!existing.userId && video.creator.userId) {
        existing.userId = video.creator.userId;
        existing.tiktokUserId = video.creator.userId;
      }
    } else {
      // 创建新的红人记录
      influencerMap.set(username, {
        ...
        bio: video.creator.signature || null,
        verified: video.creator.verified || false,
        secUid: video.creator.secUid || null,
        tiktokSecUid: video.creator.secUid || null,
        userId: video.creator.userId || null,
        tiktokUserId: video.creator.userId || null,
        platform: 'TikTok'
      });
    }
  }
...
export async function extractSearchResultsFromPageCDP(page, keyword, options = {}) {
  const { 
    ...
    scrollRounds = null, // 可选：自定义滚动轮数；scrollUntilStuck=true 时忽略
    scrollUntilStuck = true, // 持续下滑直到滑不动
    // TikTok 页面经常会“迟迟不触发 domcontentloaded”，导致整流程直接失败。
    // 采集策略主要依赖 CDP 拦截的 API 响应，因此导航超时后也应尽量继续等待 API。
    gotoWaitUntil = 'domcontentloaded',
    gotoTimeoutMs = 60000,
    apiMinInterceptCount = 1,
    apiWaitTimeoutMs = null // 默认按 humanLikeBehavior 设定
  } = options;
  ...
  const apiWaitTimeoutMsResolved =
    typeof apiWaitTimeoutMs === 'number' && apiWaitTimeoutMs > 0
      ? apiWaitTimeoutMs
      : (humanLikeBehavior ? 30000 : 20000);

  const apiMinInterceptCountResolved =
    typeof apiMinInterceptCount === 'number' && apiMinInterceptCount >= 0
      ? apiMinInterceptCount
      : 1;

  async function waitForInterceptedApi(minCount = 1, timeoutMs = 20000) {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
      if (page.isClosed()) return 0;
      const total = interceptedData.searchApi.length + interceptedData.recommendApi.length;
      if (total >= minCount) return total;
      await page.waitForTimeout(500);
    }
    return interceptedData.searchApi.length + interceptedData.recommendApi.length;
  }
  ...
  const gotoAttempts = [
    { waitUntil: gotoWaitUntil || 'domcontentloaded', timeout: gotoTimeoutMs || 60000 },
    // fallback：不等 dom 事件，最大化保证能继续跑“API 拦截提取”主逻辑
    { waitUntil: 'commit', timeout: Math.max(gotoTimeoutMs || 60000, 30000) }
  ];
  ...
  for (let i = 0; i < gotoAttempts.length; i++) {
    ...
    try {
      ...
      page = await guardedGoto(page, searchUrl, {
        label: "tt_keyword_search",
        budgetMs: attempt.timeout,
        waitUntil: attempt.waitUntil,
        retries: 0,
        createRetryPage: async () => openCdpContextPage(page.context()),
      });
      ...
      const waitUntilLabel = attempt.waitUntil;
      const attemptStr = `${i + 1}/${gotoAttempts.length}`;

      if (isConnectionError && i < gotoAttempts.length - 1) {
        console.warn(`[extractSearchResultsFromPageCDP] ⚠️  页面导航失败（${attemptStr}）：${error.message}`);
        sendStep('重试导航', `连接中断，等待 2 秒后重试（${attemptStr}）...`);
      ...
      } else if (isTimeout && !page.isClosed()) {
        // TikTok 页面未触发 domcontentloaded 也不要直接失败
        console.warn(`[extractSearchResultsFromPageCDP] ⚠️ 导航等待(${waitUntilLabel})超时（${attemptStr}），继续等待 API：${error.message}`);
        sendStep('导航超时但继续', `waitUntil=${waitUntilLabel} 超时，将继续等待 API 响应...`);
        break;
      } else {
        throw error;
      }
  ...
  if (!gotoSuccess && page.isClosed()) {
    throw lastGotoError || new Error('页面导航失败且页面已关闭');
  }

  // 等待首批搜索/推荐 API 返回（比单纯等待 dom 事件更稳）
  sendStep(
    '等待API',
    `正在等待首批 API 响应（最多 ${(apiWaitTimeoutMsResolved / 1000).toFixed(1)} 秒）...`
  );
  const interceptedCount = await waitForInterceptedApi(apiMinInterceptCountResolved, apiWaitTimeoutMsResolved);

  // 如果还没等到足够数据，给页面一个短暂缓冲后继续（避免过早截图/滚动）
  if (interceptedCount < apiMinInterceptCountResolved) {
    const initialWaitTime = humanLikeBehavior
      ? 1000 + Math.floor(Math.random() * 1000) // 1-2秒随机
      : 1000; // 固定 1 秒
    console.warn(`[extractSearchResultsFromPageCDP] ⚠️ 首批 API 数不足（${interceptedCount}），额外等待 ${initialWaitTime}ms 后继续...`);
  ...
  function countInterceptedItems() {
    let n = 0;
    for (const api of interceptedData.searchApi) {
      n +=
        api.item_list?.length ||
        api.itemList?.length ||
        api.items?.length ||
        api.data?.length ||
        0;
    }
    for (const api of interceptedData.recommendApi) {
      n +=
        api.item_list?.length ||
        api.itemList?.length ||
        api.items?.length ||
  ...
  async function tiktokSearchScrollOnce(page, i) {
    if (humanLikeBehavior) {
    ...
    if (i > 0 && i % 2 === 0) {
      await reportScreenshot(
        ...
        `滚动进度 第 ${i + 1} 轮（已拦截 ${countInterceptedItems()} 条）`
      );
    }
  }

  const useScrollUntilStuck =
    scrollUntilStuck !== false &&
    !(typeof scrollRounds === "number" && scrollRounds > 0);

  let scrollRoundsDone = 0;
  if (useScrollUntilStuck) {
    sendStep(
      "滚动页面",
      "持续下滑搜索页，直到无法加载更多结果..."
    );
    await runScrollUntilStuck(page, {
      waitMs: 0,
      scrollOnce: tiktokSearchScrollOnce,
      getProgress: countInterceptedItems,
      onRound: async (round) => {
        scrollRoundsDone = round + 1;
      },
    });
  } else {
    const scrollCount =
    ...
    sendStep(
      "滚动页面",
      `正在滚动页面触发更多 API 请求（${scrollCount}次）...`
    );
    for (let i = 0; i < scrollCount; i++) {
      await tiktokSearchScrollOnce(page, i);
  ...
  await reportScreenshot(
    ...
    page,
    BROWSER_STEP_IDS.SEARCH_VIDEOS,
    `滚动完成（共 ${scrollRoundsDone} 轮，拦截 ${countInterceptedItems()} 条）`
  );
