  ...
  const deliverablesRules = deliverablesEn
    ? (() => {
        ...
        return [
          ...
          `- **平台优先级（极其重要）**：以 JSON 中 **influencer.platform**（当前为 ${influencerPlatformLabel}）为准，判断该创作者属于 YouTube / Instagram / TikTok / X 哪一个。**忽略** campaign.platformLabel（${platformLabel}）里列出的其他投放平台——那只是整案多平台投放范围，不代表本邮件要写所有平台的要求。`,
          `- 阅读 **campaignInfo.deliverables** 与 payload.deliverables（通常为同一全文）。若文本按平台分段（如「YouTube：…」「Instagram：…」），**Deliverables 段落只写与 influencer.platform 匹配的那段**；若无平台分段（通用条目），可对全文使用。`,
          ...
          `- **禁止**自行添加未出现在匹配平台交付内容中的项目；**禁止**在 Deliverables 中提及其他平台（如为 YouTube 创作者时不得写 Instagram Reel / TikTok 等未筛选出的要求）。`,
        ].join("\n");
      })()
