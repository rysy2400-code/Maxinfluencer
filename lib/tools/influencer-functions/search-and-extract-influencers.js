...
import { touchSearchTaskLastProgressAt } from '../../db/campaign-candidates-dao.js';
import { 
  BROWSER_STEP_IDS, 
  STEP_STATUS, 
  createStep, 
    ...
    try {
      ...
      if (tid) {
        try {
          ...
          console.warn(`[enrichInfluencerProfiles] touchSearchTaskLastProgressAt: ${e.message}`);
        }
      }
    }
  };
  ...
  try {
    ...
    if (campaignId && finalInfluencerRecords.length > 0) {
      try {
        ...
        await upsertCandidatesForCampaign(campaignId, finalInfluencerRecords, taskMeta);
        const taskId = Number(options?.taskId || 0);
        if (taskId) {
          // 以「本批分析完成人数」为准（含 INSERT IGNORE 跳过已存在红人）
...
export async function searchAndExtractInfluencers(params = {}, options = {}) {
  ...
  try {
    ...
    if (campaignId && finalInfluencerRecords.length > 0) {
      ...
      try {
        const { upsertCandidatesForCampaign, bumpSearchTaskProgress } = await import("../../db/campaign-candidates-dao.js");
        await upsertCandidatesForCampaign(campaignId, analyzedCandidates, taskMeta);
        const taskId = Number(options?.taskId || 0);
        if (taskId) {
          // 以「本批分析完成人数」为准（含 INSERT IGNORE 跳过已存在红人）
