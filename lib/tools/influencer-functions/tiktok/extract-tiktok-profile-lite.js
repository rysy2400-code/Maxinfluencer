...
export async function extractTiktokProfileLite(page, username, options = {}) {
  ...
  try {
    ...
    itemBatches = await fetchWithPoolRetry({
      fn: async () => {
        const b = await fetchPostItemListAll(page, {
          secUid,
          referer: `https://www.tiktok.com/@${handle}`,
        });
        const n = b.reduce(
          (sum, x) => sum + (x?.itemList?.length || x?.item_list?.length || 0),
          0
        );
        if (n === 0) throw new Error(`item_list empty @${handle}`);
        return b;
      },
      page,
      controlUrl: resolveEnrichControllerForCdp(cdpPort) || undefined,
      label: `item_list:${handle}`,
      maxNodeSwitches: 4,
