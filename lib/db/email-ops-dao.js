...
export async function getEmailOpsSnapshot({ now = new Date() } = {}) {
  ...
  const [contacts, stats, inboundStats, failedAgentEvents] = await Promise.all([
    ...
    queryTikTok(
      `SELECT event_type,
         SUM(updated_at >= ?) AS c24h,
         SUM(updated_at >= ?) AS c7d,
         COUNT(*) AS c_total
       FROM tiktok_influencer_agent_event
       WHERE status = 'failed'
       GROUP BY event_type
       ORDER BY c_total DESC`,
      [cutoff(24, now), cutoff(24 * 7, now)]
    ),
  ]);
  ...
  const failedTotal24h = (failedAgentEvents || []).reduce(
    (sum, row) => sum + Number(row.c24h || 0),
    0
  );
  const failedTotal7d = (failedAgentEvents || []).reduce(
    (sum, row) => sum + Number(row.c7d || 0),
    0
  );
  const recentFailed = await queryTikTok(
    `SELECT id, event_type, campaign_id, influencer_id,
            LEFT(COALESCE(error_message, ''), 200) AS error_message, updated_at
     FROM tiktok_influencer_agent_event
     WHERE status = 'failed'
  ...
  return {
    ...
    failedAgentEvents: {
      total24h: failedTotal24h,
      total7d: failedTotal7d,
      byType: failedAgentEvents || [],
      recent: recentFailed || [],
    },
  };
}
