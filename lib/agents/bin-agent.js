...
- 当产品信息确认后，主动询问客户 campaign 信息（平台、地区、发布时间段、总预算、佣金、单位红人报价策略）
- 当需要生成内容脚本时，会基于产品信息生成脚本和视频

工作流状态说明：
- step_1_product_info：正在确认产品信息
- step_2_campaign_info：正在收集 campaign 信息（平台、地区、发布时间段、总预算、佣金、单位红人报价策略）
- step_3_influencer_profile：正在确认红人画像
- step_4_content_requirement：兼容历史会话或用户主动要求生成内容，不是主创建流程必经阶段
- step_5_publish_confirm：正在确认发布（主创建流程第 4 步）

当工作流状态为 step_2_campaign_info 时，如果客户还没有提供 campaign 信息，你应该主动询问：
"好的，现在我需要了解你的 Campaign 信息：
1. 投放平台：TikTok、Instagram、YouTube 或 X（可多选；与客户沟通时可用全称，系统会规范化存储）
2. 投放地区：各平台支持的国家/地区（如美国、德国、法国、英国、日本等），可以选择一个或多个
3. 发布时间段：例如 2024年3月1日-3月31日
4. 总预算：整个 campaign 的总预算金额（美元）
5. 佣金：给红人的佣金比例（%）
6. 单位红人报价策略：默认不主动报价，询问红人提供合作价格；也可选按 eCPM=$3 给红人报价（单人最高 $1000，可自定义 eCPM/上限）、无固定费用（仅佣金，或 0% 佣金+无固定费=纯产品置换）"
...
{ "toolName": "product_info_agent" | "campaign_info_agent" | "influencer_profile_agent" | "content_requirement_agent" | null }`;
  ...
  async detectConfirmIntentForStage(messages, context = {}) {
    ...
    const hasData = {
      ...
      [WORKFLOW_STATES.STEP_2_CAMPAIGN_INFO]: isCampaignInfoComplete(context.campaignInfo),
...
{ "toolName": "product_info_agent" | "campaign_info_agent" | "influencer_profile_agent" | "content_requirement_agent" | "campaign_publish_agent" | null }`;
  ...
  async processWithTools(messages, context = {}) {
    try {
      ...
      if (!context.published && !context.campaignId) {
        ...
        const detectedCrossStepTool = await this.detectMessageTargetBlock(messages, context);
        const stateRank = {
          [WORKFLOW_STATES.IDLE]: 1,
          [WORKFLOW_STATES.STEP_1_PRODUCT_INFO]: 1,
          [WORKFLOW_STATES.STEP_2_CAMPAIGN_INFO]: 2,
          [WORKFLOW_STATES.STEP_3_INFLUENCER_PROFILE]: 3,
          [WORKFLOW_STATES.STEP_4_CONTENT_REQUIREMENT]: 4,
          [WORKFLOW_STATES.STEP_5_PUBLISH_CONFIRM]: 5,
        }[context.workflowState || WORKFLOW_STATES.IDLE] || 1;
        const toolRank = {
          product_info_agent: 1,
          campaign_info_agent: 2,
          influencer_profile_agent: 3,
          content_requirement_agent: 4,
        }[detectedCrossStepTool];
        // 提前提供后续信息只做预填，不允许跳过尚未确认的阶段。
        const crossStepTool = toolRank && toolRank <= stateRank ? detectedCrossStepTool : null;
  ...
  fallbackRuleBasedDecision(messages, context) {
    ...
    const campaignKeywords = {
      ...
      region: ["美国", "德国", "法国", "英国", "日本", "地区", "投放地区", "国家"],
