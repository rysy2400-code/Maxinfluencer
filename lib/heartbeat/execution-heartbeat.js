...
const DEFAULT_MAX_PARALLEL = Math.max(
  1,
  Number(process.env.EXECUTION_MAX_PARALLEL_WORKERS || 1) || 1
);
...
async function planKeywords(campaign, slots, existingRunKeywords, targetPlatform = null) {
...
async function fillExecutionFromCandidates(campaignId, needCount, now) {
  ...
  for (const cand of candidates) {
    ...
    const insertResult = await queryTikTok(
      `
      INSERT IGNORE INTO tiktok_campaign_execution (campaign_id, tiktok_username, influencer_id, influencer_snapshot, source, stage, last_event)
...
async function runExecutionForCampaign(campaign, now) {
  ...
  let platformsToDispatch = dispatchPlatformSlugs;
  if (ONE_TASK_PER_TICK && dispatchPlatformSlugs.length > 1) {
    const nextSlug = await pickNextDispatchPlatform(
      campaignId,
      campaignPlatforms,
      queryTikTok
    );
    platformsToDispatch = [nextSlug];
  }

  for (const platformSlug of platformsToDispatch) {
    const plans = await planKeywords(campaign, slots, existingKeywords, platformSlug);
    const plansToDispatch = ONE_TASK_PER_TICK ? plans.slice(0, 1) : plans;
    ...
    for (const plan of plansToDispatch) {
      ...
      console.log(
        `[ExecutionHeartbeat] Campaign ${campaignId} 派发任务: platform=${platformSlug} keyword=${plan.keyword} type=${plan.keywordType}`
      );
