...
export function buildNormalizedInfluencerSnapshot(inf = {}, taskMeta = {}) {
  ...
  return {
    ...
    // 账号国家：与 videoPublishCountry 同源赋值，避免新旧快照字段不一致
    // （前端 resolveAccountCountry 读 accountCountry / account_country）
    accountCountry: normalizeSnapshotVideoCountry(inf),
    account_country: normalizeSnapshotVideoCountry(inf),
    ...
    // 红人主页简介语言（卡片「语言」列；由 enrich 阶段按 bio 推断后透传）
    bioLanguage: inf.bioLanguage ?? inf.bio_language ?? null,
    bioLanguageConfidence:
      inf.bioLanguageConfidence ?? inf.bio_language_confidence ?? null,
...
export async function upsertCandidatesForCampaign(campaignId, influencers, taskMeta = {}) {
  ...
  // contact_all 模式：符合投放地区且有邮箱的红人一律联系（不再按 isRecommended 决定）
  const contactAllEmailOnly = taskMeta.contactAllEmailOnly === true;
  ...
  for (const inf of influencers) {
    ...
    const email = snapshot.email;
    const hasEmail = email ? 1 : 0;
    const shouldContact = contactAllEmailOnly
      ? hasEmail
        ? 1
        : 0
      : typeof infForSnapshot.isRecommended === "boolean"
...
/**
 * contact_all 模式：已存在候选「有邮箱且未联系」的直接翻为联系（should_contact 0→1）。
 * 不动 match_score / analysis（推荐展示不变）；已有执行行的跳过（幂等）；无邮箱不翻。
 * @returns {Promise<{ candidates: number, flipped: number }>}
 */
export async function flipShouldContactForExistingCampaignCandidates(campaignId, usernames) {
  const handles = [
    ...new Set(
      (usernames || [])
        .map((u) => String(u || "").replace(/^@/, "").trim().toLowerCase())
  ...
  if (!campaignId || handles.length === 0) return { candidates: 0, flipped: 0 };
  ...
  const flippable = (rows || []).filter(
    (r) => Number(r.has_email) === 1 && Number(r.should_contact) === 0
  );
  let flipped = 0;
  if (flippable.length > 0) {
    const fHandles = flippable.map((r) => String(r.u));
    const fPlaceholders = fHandles.map(() => "?").join(", ");
    const result = await queryTikTok(
      `UPDATE tiktok_campaign_influencer_candidates
       SET should_contact = 1, updated_at = CURRENT_TIMESTAMP
       WHERE campaign_id = ?
         AND LOWER(tiktok_username) IN (${fPlaceholders})
         AND has_email = 1
         AND should_contact = 0
         AND NOT EXISTS (
           SELECT 1 FROM tiktok_campaign_execution e
           WHERE e.campaign_id = tiktok_campaign_influencer_candidates.campaign_id
             AND e.tiktok_username = tiktok_campaign_influencer_candidates.tiktok_username
         )`,
      [campaignId, ...fHandles]
    );
    flipped = Number(result?.affectedRows || 0);
  }
  return { candidates: (rows || []).length, flipped };
}

/**
 * contact_all 全链路后：把有邮箱的候选强制置为联系（覆盖 enrich 阶段按 isRecommended=0 写入的行）。
 */
export async function setShouldContactForCampaignCandidates(campaignId, usernames, shouldContact = 1) {
  const handles = [
    ...new Set(
      (usernames || [])
        .map((u) => String(u || "").replace(/^@/, "").trim().toLowerCase())
  ...
  if (!campaignId || handles.length === 0) return { updated: 0 };
  const placeholders = handles.map(() => "?").join(", ");
  const result = await queryTikTok(
    `UPDATE tiktok_campaign_influencer_candidates
     SET should_contact = ?, updated_at = CURRENT_TIMESTAMP
     WHERE campaign_id = ? AND LOWER(tiktok_username) IN (${placeholders}) AND has_email = 1`,
    [shouldContact ? 1 : 0, campaignId, ...handles]
  );
