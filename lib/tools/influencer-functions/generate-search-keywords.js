...
function buildKeywordSignalsSection(keywordSignals = [], platformDisplayName = "TikTok") {
  ...
  return `
【红人视频挖掘线索 — ${platformDisplayName}（严格模式，补充探索）】
以下线索来自已匹配红人视频，按红人覆盖数降序排列。
你必须将下列每一条原样作为 search_queries 中的 keyword 输出（保留 # 或 @，不得改写）。
这些 keyword 可占用本轮输出配额。

${lines.join("\n")}

【严格模式约束】
- 上方列表中的每一条必须出现在 search_queries 中，且 keyword 字段与列表完全一致。
- 除上述条目外，可再生成其他 keyword 以满足 bucket 配比；但不得遗漏列表中任何一条。
...
export async function generateSearchKeywords(params = {}) {
  ...
  try {
    ...
    const brandTerms = Array.isArray(forbiddenBrandTerms)
      ...
      : [];
    const activeSignals = Array.isArray(keywordSignals)
      ? keywordSignals
          .map((s) => ({
