...
function filterPromptSignals(signals = [], excludeSet) {
  return (signals || []).filter((s) => {
    ...
    return !isKeywordExcluded(value, excludeSet);
  });
}
...
async function runExecutionForCampaign(campaign, now) {
  ...
  if (runningTasks > 0) {
    console.log(
      `[ExecutionHeartbeat] Campaign ${campaignId} 仍有 ${runningTasks} 个搜索任务 pending/processing，跳过本轮关键词规划。`
    );
    return;
  }
  ...
  for (const platformSlug of platformsToDispatch) {
    const plans = await planKeywords(campaign, existingKeywords, platformSlug);
