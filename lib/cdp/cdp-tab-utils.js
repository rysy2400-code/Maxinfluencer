...
export async function pruneCdpContextTabs(context, meta = {}) {
  ...
  try {
    ...
    console.warn(`${logPrefix} newPage after prune failed: ${e?.message || e}`);
  }
  ...
  console.log(
    `${logPrefix} pruned closed=${closed} kept_blank=${kept} remain=${remain}` +
      (meta.platform ? ` platform=${meta.platform}` : "") +
      (meta.phase ? ` phase=${meta.phase}` : "")
  );

  return { closed, kept: remain || kept };
}
