...
注意：红人画像只有 followerRange（粉丝量）/ viewRange（播放量）/ accountType（账户类型与内容方向）三个字段。用户提出的互动率、点赞率、评论率、完播率等其它筛选要求，没有独立字段，必须原样并入 accountType 保留，禁止丢弃，也禁止新增自定义字段。
...
3. 不在此阶段生成或展示红人名单。
 ...
 回复要专业、清晰，用中文与客户沟通。`;
   ...
   */
  async recommendInfluencers(messages, context = {}, onStepUpdate = null, options = {}) {
    try {
      ...
      if (existingProfile && options.enteringStage) {
        const complete = this.isProfileMetricsComplete(existingProfile);
        const ambiguityNotes = (context.briefIntake?.ambiguities || [])
          .filter((item) => item.block === "influencerProfile")
          .slice(-3)
          .map((item) => `- “${item.quote}”：${item.reason}`);
        const suffix = complete
          ? "请确认红人画像是否正确；如需调整，请直接说明。"
          : "请补充尚未明确的粉丝量或播放量要求；不设限制可直接说“无要求”。";
        return {
          reply: `你之前已经提供过红人画像信息：\n\n${this.formatProfileSummary(existingProfile)}\n\n${suffix}${
...
{ "intent": "confirm" | "provide_or_update" | "unclear" }`;
    ...
    try {
      ...
      const parsed = parseFirstJsonObject(llmResponse);
...
决策规则（必须遵守）：
1) 三个字段都必须输出；用户未明确提供且当前画像也没有的字段返回 null。
...
4b) 用户提到的互动率、点赞率、评论率、完播率等其它筛选要求**没有独立字段**，必须原样并入 accountType：与同句中的基地/城市/垂类要求合并成一段，不要拆句、不要丢字。例如「互动率要求5%以上，base在巴西圣保罗的美食探店类或母婴类红人都可以」整句写入 accountType。**禁止**输出 engagementRate / interactionRate 等额外键，**禁止**只保留账户类型部分而丢掉互动率要求。若用户只单独补充互动率等要求，必须与上面「当前已有画像」里的 accountType 合并后整段输出，**禁止覆盖原有内容**。
5) 用户同时说粉丝/播放无要求并指定账户类型时，followerRange 与 viewRange 均为 "无要求"，accountType 为用户指定类型。
6) 禁止基于 Campaign 猜测粉丝量或播放量；用户没有明确提供时保留当前值，否则返回 null。

只返回 JSON：
{
  "followerRange": "无要求" | "大于1万" | "1万-5万" 等,
  "viewRange": "无要求" | "大于1000" 等,
  "accountType": "账户类型/内容方向描述；用户提到的互动率、点赞率等其它筛选要求也原样并入此字段"
}`;
    ...
    try {
      ...
      try {
        ...
        parsed = parseFirstJsonObject(llmResponse);
      }
      return {
      ...
      return {
        followerRange: existingProfile?.followerRange ?? null,
        viewRange: existingProfile?.viewRange ?? null,
