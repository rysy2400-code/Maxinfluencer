...
async function loadLastInboundReplyAtByUsername(campaignId) {
  const rows = await queryTikTok(
    `
    WITH execs AS (
      SELECT
...
export async function getCampaignExecutionStatus(campaignId) {
  ...
  const [execRows, lastInboundReplyByUsername] = await Promise.all([
    queryTikTok(
      `SELECT * FROM tiktok_campaign_execution WHERE campaign_id = ?`,
      [campaignId]
    ),
    loadLastInboundReplyAtByUsername(campaignId),
  ]);
  ...
  execRows.forEach((row) => {
    ...
    const item = {
      ...
      lastInboundReplyAt: lastInboundReplyByUsername[id] || null,
    };
